...
#define FT_FILENAME( x )  ( (x) ? (x) : "unknown file" )
  ...
  ft_mem_table_new( FT_Memory  memory )
  {
    ...
    table->_realloc = memory->_realloc;
    table->_free    = memory->_free;
  ...
  ft_mem_debug_init( FT_Memory  memory )
  {
    ...
    if ( ft_getenv( "FT2_DEBUG_MEMORY" ) )
    {
      ...
      if ( table )
      {
        ...
        memory->_realloc = ft_mem_debug_realloc;
        memory->_free    = ft_mem_debug_free;
  ...
  ft_mem_debug_done( FT_Memory  memory )
  {
    ...
    if ( table )
    {
      memory->_free    = table->_free;
      memory->_realloc = table->_realloc;

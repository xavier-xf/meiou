...
lv_obj_t *lv_textarea_create(lv_obj_t *par, const lv_obj_t *copy)
{
	...
	if (ancestor_signal == NULL)
		ancestor_signal = lv_obj_get_signal_cb(ta);
	if (ancestor_design == NULL)
		ancestor_design = lv_obj_get_design_cb(ta);
	if (scrl_signal == NULL)
		scrl_signal = lv_obj_get_signal_cb(lv_page_get_scrollable(ta));
	if (scrl_design == NULL)
		scrl_design = lv_obj_get_design_cb(lv_page_get_scrollable(ta));
...
void lv_textarea_add_char(lv_obj_t *ta, uint32_t c)
{
	...
	if (ext->pwd_mode != 0)
		pwd_char_hider(ta); /*Make sure all the current text contains only '*'*/
...
void lv_textarea_del_char_forward(lv_obj_t *ta)
{
	...
	if (cp != lv_textarea_get_cursor_pos(ta))
...
void lv_textarea_set_cursor_pos(lv_obj_t *ta, int32_t pos)
{
	...
	lv_textarea_ext_t *ext = lv_obj_get_ext_attr(ta);
	if ((uint32_t)ext->cursor.pos == (uint32_t)pos)
		return;
						     ...
						     lv_design_mode_t mode)
{
	...
	else if (mode == LV_DESIGN_DRAW_POST)
	{
		...
		if (txt[0] == '\0' && ext->placeholder_txt && ext->placeholder_txt[0] != 0)
		{
			...
			if (ext->one_line)
				ph_dsc.flag |= LV_TXT_FLAG_EXPAND;
...
static bool char_is_accepted(lv_obj_t *ta, uint32_t c)
{
	...
	if (ext->accepted_chars == NULL && ext->max_length == 0)
		return true;
...
static void update_cursor_position_on_click(lv_obj_t *ta, lv_signal_t sign, lv_indev_t *click_source)
{
	...
	lv_textarea_ext_t *ext = lv_obj_get_ext_attr(ta);
	if (ext->cursor.click_pos == 0)
		return;
	if (ext->cursor.hidden)
		return;
	...
	if (ext->text_sel_in_prog || sign == LV_SIGNAL_PRESSED)
		lv_textarea_set_cursor_pos(ta, char_id_at_click);

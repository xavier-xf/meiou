...
lv_obj_t *lv_label_create(lv_obj_t *par, const lv_obj_t *copy)
{
	...
	if (ancestor_signal == NULL)
		ancestor_signal = lv_obj_get_signal_cb(new_label);
...
#if LV_USE_BIDI
	if (mutable_bidi_txt)
		_lv_mem_buf_release(mutable_bidi_txt);
#endif
}
...
void lv_label_refr_text(lv_obj_t *label)
{
	...
	else if (ext->long_mode == LV_LABEL_LONG_DOT)
	{
		...
		{ /*No dots are required, the text is short enough*/
			ext->dot_end = LV_LABEL_DOT_END_INV;
		}
		else if (_lv_txt_get_encoded_length(ext->text) <= LV_LABEL_DOT_NUM)
...
static lv_design_res_t lv_label_design(lv_obj_t *label, const lv_area_t *clip_area, lv_design_mode_t mode)
{
	...
	else if (mode == LV_DESIGN_DRAW_MAIN)
	{
		...
		{
			lv_point_t size;
			_lv_txt_get_size(&size, ext->text, label_draw_dsc.font, label_draw_dsc.letter_space, label_draw_dsc.line_space, LV_COORD_MAX, flag);
...
static lv_res_t lv_label_signal(lv_obj_t *label, lv_signal_t sign, void *param)
{
	...
	if (sign == LV_SIGNAL_GET_STYLE)
	{
		...
		if (info->result != NULL)
			...
			return ancestor_signal(label, sign, param);
	}
...
{
	lv_label_ext_t *ext = lv_obj_get_ext_attr(label);
	if (ext->long_mode != LV_LABEL_LONG_DOT)
		return;
	if (ext->dot_end == LV_LABEL_DOT_END_INV)
		return;

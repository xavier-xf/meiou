...
static const lv_btnmatrix_ctrl_t default_kb_offset_lc_map[] = {
    0, 
    0, 
    0,
    0,
};
static const lv_btnmatrix_ctrl_t default_kb_offset_lc_num_map[] = {
    0, 
    0, 
    0,
    52,
...
static const lv_btnmatrix_ctrl_t *kb_offset[] = {
    default_kb_offset_lc_map,
    default_kb_offset_lc_map,
    default_kb_offset_lc_map,
    default_kb_offset_lc_num_map};
/**********************
 *      MACROS
 **********************/
...
lv_obj_t *lv_keyboard_create(lv_obj_t *par, const lv_obj_t *copy)
{
	...
	if (copy == NULL)
	{
		...
		lv_btnmatrix_set_lines_offset_map(kb, kb_offset[ext->mode]);
...
void lv_keyboard_set_mode(lv_obj_t *kb, lv_keyboard_mode_t mode)
{
	...
	lv_btnmatrix_set_lines_offset_map(kb, kb_offset[mode]);
}
...
void lv_keyboard_def_event_cb(lv_obj_t *kb, lv_event_t event)
{
	...
	if (strcmp(txt, "Aa") == 0)
	{
		ext->mode = ext->mode == LV_KEYBOARD_MODE_TEXT_LOWER ? LV_KEYBOARD_MODE_TEXT_UPPER : ext->mode == LV_KEYBOARD_MODE_SPECIAL ? LV_KEYBOARD_MODE_NUM
												 : ext->mode == LV_KEYBOARD_MODE_NUM	   ? LV_KEYBOARD_MODE_SPECIAL
																	   : LV_KEYBOARD_MODE_TEXT_LOWER;
		lv_btnmatrix_set_map(kb, kb_map[ext->mode]);
		lv_btnmatrix_set_ctrl_map(kb, kb_ctrl[ext->mode]);
		return;
	}
#if 0 // LV_USE_ARABIC_PERSIAN_CHARS == 1
	...
	{
		ext->mode = ext->mode == LV_KEYBOARD_MODE_SPECIAL ? LV_KEYBOARD_MODE_TEXT_LOWER : LV_KEYBOARD_MODE_SPECIAL;
		lv_btnmatrix_set_map(kb, kb_map[ext->mode]);
		lv_btnmatrix_set_ctrl_map(kb, kb_ctrl[ext->mode]);
		return;
	}
	else if (btn_id == 27) //(strcmp(txt, "ERROR" /*LV_SYMBOL_CLOSE*/) == 0)
	{
	...
	else if (strcmp(txt, "OK" /*LV_SYMBOL_OK*/) == 0)
	{
#
		if (kb->event_cb == lv_keyboard_def_event_cb)
		{
	...
	if (ext->ta == NULL)
		return;
#if 0
    if (strcmp(txt, "Enter") == 0 || strcmp(txt, "ERROR" /* LV_SYMBOL_NEW_LINE*/) == 0)
        lv_textarea_add_char(ext->ta, '\n');
    else if (strcmp(txt, "ERROR" /*LV_SYMBOL_LEFT*/) == 0)
        lv_textarea_cursor_left(ext->ta);
    else if (strcmp(txt, "ERROR" /*LV_SYMBOL_RIGHT*/) == 0)
        lv_textarea_cursor_right(ext->ta);
    else if (strcmp(txt, "ERROR" /*LV_SYMBOL_BACKSPACE*/) == 0)
        lv_textarea_del_char(ext->ta);

...
lv_res_t lv_indev_finish_drag(lv_indev_t *indev)
{
    ...
    if (indev->driver.type != LV_INDEV_TYPE_POINTER)
        return LV_RES_OK;
    if (indev->proc.types.pointer.drag_in_prog == 0)
        return LV_RES_OK;
...
{
#if LV_USE_GROUP
    if (data->state == LV_INDEV_STATE_PR && i->proc.wait_until_release)
        return;
...
static void indev_proc_release(lv_indev_proc_t *proc)
{
    ...
    if (indev_obj_act)
    {
        ...
        if (drag_obj)
        {
            ...
            {
                if (drag_obj->signal_cb)
                    drag_obj->signal_cb(drag_obj, LV_SIGNAL_DRAG_THROW_BEGIN, NULL);
                if (indev_reset_check(proc))
                    return;
...
lv_obj_t *lv_indev_search_obj(lv_obj_t *obj, lv_point_t *point)
{
    ...
    if (lv_obj_hittest(obj, point))
    {
        ...
        if (found_p == NULL && lv_obj_get_click(obj) != false)
        {
            ...
            while (hidden_i != NULL)
            {
                if (lv_obj_get_hidden(hidden_i) == true)
                    break;
            ...
            {
                if (hidden_i == NULL && (lv_obj_get_state(obj, LV_OBJ_PART_MAIN) & LV_STATE_DISABLED) == false)
                    found_p = obj;
            }
            else
            {
...
static void indev_drag_throw(lv_indev_proc_t *proc)
{
    ...
    if (proc->types.pointer.drag_throw_vect.x != 0 || proc->types.pointer.drag_throw_vect.y != 0)
    {
        ...
        {
            if (proc->types.pointer.drag_sum.x)
                lv_obj_set_x(drag_obj, act_x);
            else
                lv_obj_set_y(drag_obj, act_y);
        }

...
lv_font_t *LV_FONT_SIZE_40 = NULL;
lv_font_t *LV_FONT_SIZE_150 = NULL;
...
const unsigned char *get_rom_bin_base(void)
{
	...
	{
		struct stat statbuf;
		int ret = stat(ROM_BIN_PATH, &statbuf); //调用stat函数 成功返回0，失败返回-1
		if (ret != 0)
		{
...
void lv_init(void)
{
	...
	lv_ft_font_init(&LV_FONT_SIZE_31, 25);
	lv_ft_font_init(&LV_FONT_SIZE_40, 35);
	lv_ft_font_init(&LV_FONT_SIZE_150, 120);
...
lv_obj_t *lv_obj_create(lv_obj_t *parent, const lv_obj_t *copy)
{
	...
	{
		if (parent != NULL)
			lv_theme_apply(new_obj, LV_THEME_OBJ);
		else
			lv_theme_apply(new_obj, LV_THEME_SCR);
	}
	else
	{
...
lv_res_t lv_obj_del(lv_obj_t *obj)
{
	...
	if (par == NULL)
	{
		...
		if (!disp)
			return LV_RES_INV; /*Shouldn't happen*/
		if (disp->act_scr == obj)
			act_scr_del = true;
	}
...
bool lv_obj_area_is_visible(const lv_obj_t *obj, lv_area_t *area)
{
	    ...
	    obj_scr == lv_disp_get_layer_sys(disp))
	{
		...
		if (is_common == false)
			return false; /*The area is not on the object*/
		...
		while (par != NULL)
		{
			...
			if (is_common == false)
				return false; /*If no common parts with parent break;*/
			if (lv_obj_get_hidden(par))
				return false; /*If the parent is hidden then the child is hidden and won't be drawn*/
...
void lv_obj_move_foreground(lv_obj_t *obj)
{
	...
	if (_lv_ll_get_head(&parent->child_ll) == obj)
		return;
...
void lv_obj_move_background(lv_obj_t *obj)
{
	...
	if (_lv_ll_get_tail(&parent->child_ll) == obj)
		return;
...
void lv_obj_refresh_style(lv_obj_t *obj, uint8_t part, lv_style_property_t prop)
{
	...
	if (real_refr)
	{
		...
		case LV_STYLE_MARGIN_RIGHT:
			if (obj->parent)
				obj->parent->signal_cb(obj->parent, LV_SIGNAL_CHILD_CHG, NULL);
			break;
		}
...
void lv_obj_set_drag(lv_obj_t *obj, bool en)
{
	...
	if (en == true)
		lv_obj_set_click(obj, true); /*Drag is useless without enabled clicking*/
...
void lv_obj_set_state(lv_obj_t *obj, lv_state_t new_state)
{
	...
	if (cmp_res == STYLE_COMPARE_DIFF)
		lv_obj_refresh_style(obj, part, LV_STYLE_PROP_ALL);
	else if (cmp_res == STYLE_COMPARE_VISUAL_DIFF)
...
void lv_event_send_refresh_recursive(lv_obj_t *obj)
{
	...
	else
	{
		...
		if (res != LV_RES_OK)
			return; /*If invalid returned do not check the children*/
...
lv_res_t lv_signal_send(lv_obj_t *obj, lv_signal_t signal, void *param)
{
	...
	if (obj->signal_cb)
		res = obj->signal_cb(obj, signal, param);
...
void lv_obj_set_id(lv_obj_t *obj, unsigned int id)
{
	...
	if (parent != NULL)
	{
		...
		{
			LV_LOG_ERROR("\033[1;31m"  "lv_obj_set_id: Duplicate ID(%d) setting "  "\033[0m", id);
			return;
		}
		obj->obj_id = id;
	}
...
void lv_obj_get_inner_coords(const lv_obj_t *obj, lv_area_t *coords_p)
{
	...
	if (part & LV_BORDER_SIDE_LEFT)
		coords_p->x1 += w;

	if (part & LV_BORDER_SIDE_RIGHT)
		coords_p->x2 -= w;

	if (part & LV_BORDER_SIDE_TOP)
		coords_p->y1 += w;

	if (part & LV_BORDER_SIDE_BOTTOM)
		coords_p->y2 -= w;
}
...
lv_style_list_t *lv_obj_get_style_list(const lv_obj_t *obj, uint8_t part)
{
	if (part == LV_OBJ_PART_MAIN)
		return &((lv_obj_t *)obj)->style_list;
...
lv_style_int_t _lv_obj_get_style_int(const lv_obj_t *obj, uint8_t part, lv_style_property_t prop)
{
	...
	while (parent)
	{
		...
		{
			if (!list->valid_cache)
				update_style_cache((lv_obj_t *)parent, part, prop & (~LV_STYLE_STATE_MASK));

			bool def = false;
			switch (prop & (~LV_STYLE_STATE_MASK))
			{
			case LV_STYLE_CLIP_CORNER:
				if (list->clip_corner_off)
					def = true;
				break;
			case LV_STYLE_TEXT_LETTER_SPACE:
			case LV_STYLE_TEXT_LINE_SPACE:
				if (list->text_space_zero)
					def = true;
				break;
			case LV_STYLE_TRANSFORM_ANGLE:
			case LV_STYLE_TRANSFORM_WIDTH:
			case LV_STYLE_TRANSFORM_HEIGHT:
			case LV_STYLE_TRANSFORM_ZOOM:
				if (list->transform_all_zero)
					def = true;
				break;
			case LV_STYLE_BORDER_WIDTH:
				if (list->border_width_zero)
					def = true;
				break;
			case LV_STYLE_BORDER_SIDE:
				if (list->border_side_full)
					def = true;
				break;
			case LV_STYLE_BORDER_POST:
				if (list->border_post_off)
					def = true;
				break;
			case LV_STYLE_OUTLINE_WIDTH:
				if (list->outline_width_zero)
					def = true;
				break;
			...
			case LV_STYLE_VALUE_BLEND_MODE:
				if (list->blend_mode_all_normal)
					def = true;
				break;
			case LV_STYLE_TEXT_DECOR:
				if (list->text_decor_none)
					def = true;
				break;
			}
		...
		if (LV_STYLE_ATTR_GET_INHERIT(attr) == 0)
			break;
	...
	while (parent)
	{
		...
		if (!list->ignore_cache && list->style_cnt > 0)
		{
			...
			case LV_STYLE_OPA_SCALE:
				if (list->opa_scale_cover)
					def = true;
				break;
			case LV_STYLE_BG_OPA:
				if (list->bg_opa_cover)
					return LV_OPA_COVER; /*Special case, not the default value is used*/
				if (list->bg_opa_transp)
					def = true;
				break;
			case LV_STYLE_IMAGE_RECOLOR_OPA:
				if (list->img_recolor_opa_transp)
					def = true;
				break;
			}
...
const void *_lv_obj_get_style_ptr(const lv_obj_t *obj, uint8_t part, lv_style_property_t prop)
{
	...
	while (parent)
	{
		...
		if (!list->ignore_cache && list->style_cnt > 0)
		{
			...
			{
			case LV_STYLE_VALUE_STR:
				if (list->value_txt_str)
					def = true;
				break;
			case LV_STYLE_PATTERN_IMAGE:
				if (list->pattern_img_null)
					def = true;
				break;
			case LV_STYLE_TEXT_FONT:
				if (list->text_font_normal)
					def = true;
				break;
			}
...
lv_bidi_dir_t lv_obj_get_base_dir(const lv_obj_t *obj)
{
	...
	while (parent)
	{
		if (parent->base_dir != LV_BIDI_DIR_INHERIT)
...
lv_state_t lv_obj_get_state(const lv_obj_t *obj, uint8_t part)
{
	...
	if (part < _LV_OBJ_PART_REAL_LAST)
	...
	if (draw_dsc->pattern_image)
	{
		if (draw_dsc->pattern_opa != LV_OPA_TRANSP)
		{
			...
			if (draw_dsc->pattern_opa > LV_OPA_MIN)
			{
				...
				draw_dsc->pattern_align = lv_obj_get_style_pattern_align(obj, part);
...
bool lv_debug_check_obj_type(const lv_obj_t *obj, const char *obj_type)
{
	...
	{
		if (types.type[i] == NULL)
			break;
		if (strcmp(types.type[i], obj_type) == 0)
			return true;
	}
...
static lv_design_res_t lv_obj_design(lv_obj_t *obj, const lv_area_t *clip_area, lv_design_mode_t mode)
{
	if (mode == LV_DESIGN_COVER_CHK)
	{
		...
		if (_lv_area_is_in(clip_area, &coords, r) == false)
			return LV_DESIGN_RES_NOT_COVER;

		if (lv_obj_get_style_bg_opa(obj, LV_OBJ_PART_MAIN) < LV_OPA_MAX)
			return LV_DESIGN_RES_NOT_COVER;

		if (lv_obj_get_style_bg_blend_mode(obj, LV_OBJ_PART_MAIN) != LV_BLEND_MODE_NORMAL)
			return LV_DESIGN_RES_NOT_COVER;
		if (lv_obj_get_style_border_blend_mode(obj, LV_OBJ_PART_MAIN) != LV_BLEND_MODE_NORMAL)
			return LV_DESIGN_RES_NOT_COVER;
		if (lv_obj_get_style_opa_scale(obj, LV_OBJ_PART_MAIN) < LV_OPA_MAX)
			return LV_DESIGN_RES_NOT_COVER;
...
static lv_res_t lv_obj_signal(lv_obj_t *obj, lv_signal_t sign, void *param)
{
	if (sign == LV_SIGNAL_GET_STYLE)
	{
		...
		if (info->part == LV_OBJ_PART_MAIN)
			info->result = &obj->style_list;
		else
			info->result = NULL;
		return LV_RES_OK;
	}
	...
	if (sign == LV_SIGNAL_CHILD_CHG)
	{
		...
		if (lv_obj_is_protected(obj, LV_PROTECT_CHILD_CHG) != false)
			res = LV_RES_INV;
	}
			   ...
			   lv_coord_t x_ofs, lv_coord_t y_ofs)
{
	...
	if (x_set && y_set)
		...
		lv_obj_set_x(obj, new_pos.x);
	else if (y_set)
		lv_obj_set_y(obj, new_pos.y);
}
				      ...
				      lv_state_t new_state)
{
	...
	{ /*Int*/
		...
		if (prop == LV_STYLE_RADIUS)
		{
			if (int1 == LV_RADIUS_CIRCLE || int2 == LV_RADIUS_CIRCLE)
			{
				...
				if (int1 == LV_RADIUS_CIRCLE)
					int1 = LV_MATH_MIN(whalf + 1, hhalf + 1);
				if (int2 == LV_RADIUS_CIRCLE)
					int2 = LV_MATH_MIN(whalf + 1, hhalf + 1);
			}
		}
	...
	{ /*Ptr*/
		...
		if (memcmp(&p1, &p2, sizeof(const void *)) == 0)
			return NULL;
		obj->state = prev_state;
...
static void trans_anim_cb(lv_style_trans_t *tr, lv_anim_value_t v)
{
	...
	if ((tr->prop & 0xF) < LV_STYLE_ID_COLOR)
	{ /*Value*/
		lv_style_int_t x;
		if (v == 0)
			x = tr->start_value._int;
		else if (v == 255)
			x = tr->end_value._int;
		else
			x = tr->start_value._int + ((int32_t)((int32_t)(tr->end_value._int - tr->start_value._int) * v) >> 8);
		_lv_style_set_int(style, tr->prop, x);
	}
	else if ((tr->prop & 0xF) < LV_STYLE_ID_OPA)
	{ /*Color*/
		lv_color_t x;
		if (v <= 0)
			x = tr->start_value._color;
		else if (v >= 255)
			x = tr->end_value._color;
		else
			x = lv_color_mix(tr->end_value._color, tr->start_value._color, v);
		_lv_style_set_color(style, tr->prop, x);
	}
	else if ((tr->prop & 0xF) < LV_STYLE_ID_PTR)
	{ /*Opa*/
		lv_opa_t x;
		if (v <= 0)
			x = tr->start_value._opa;
		else if (v >= 255)
			x = tr->end_value._opa;
		else
			x = tr->start_value._opa + (((tr->end_value._opa - tr->start_value._opa) * v) >> 8);
...
static void update_style_cache(lv_obj_t *obj, uint8_t part, uint16_t prop)
{
	if (style_prop_is_cacheble(prop) == false)
		return;
...
static style_snapshot_res_t style_snapshot_compare(style_snapshot_t *shot1, style_snapshot_t *shot2)
{
	if (memcmp(shot1, shot2, sizeof(style_snapshot_t)) == 0)
		return STYLE_COMPARE_SAME;

	if (shot1->pad_top != shot2->pad_top)
		return STYLE_COMPARE_DIFF;
	if (shot1->pad_bottom != shot2->pad_bottom)
		return STYLE_COMPARE_DIFF;
	if (shot1->pad_left != shot2->pad_right)
		return STYLE_COMPARE_DIFF;
	if (shot1->pad_right != shot2->pad_right)
		return STYLE_COMPARE_DIFF;
	if (shot1->pad_top != shot2->pad_top)
		return STYLE_COMPARE_DIFF;
	if (shot1->pad_inner != shot2->pad_inner)
		return STYLE_COMPARE_DIFF;
	if (shot1->margin_top != shot2->margin_top)
		return STYLE_COMPARE_DIFF;
	if (shot1->margin_bottom != shot2->margin_bottom)
		return STYLE_COMPARE_DIFF;
	if (shot1->margin_left != shot2->margin_right)
		return STYLE_COMPARE_DIFF;
	if (shot1->margin_right != shot2->margin_right)
		return STYLE_COMPARE_DIFF;
	if (shot1->margin_top != shot2->margin_top)
		return STYLE_COMPARE_DIFF;
	if (shot1->transform_width != shot2->transform_width)
		return STYLE_COMPARE_DIFF;
	if (shot1->transform_height != shot2->transform_height)
		return STYLE_COMPARE_DIFF;
	if (shot1->transform_angle != shot2->transform_angle)
		return STYLE_COMPARE_DIFF;
	if (shot1->transform_zoom != shot2->transform_zoom)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.outline_width != shot2->rect.outline_width)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.outline_pad != shot2->rect.outline_pad)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.value_font != shot2->rect.value_font)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.value_align != shot2->rect.value_align)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.value_font != shot2->rect.value_font)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.shadow_spread != shot2->rect.shadow_spread)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.shadow_width != shot2->rect.shadow_width)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.shadow_ofs_x != shot2->rect.shadow_ofs_x)
		return STYLE_COMPARE_DIFF;
	if (shot1->rect.shadow_ofs_y != shot2->rect.shadow_ofs_y)
		return STYLE_COMPARE_DIFF;

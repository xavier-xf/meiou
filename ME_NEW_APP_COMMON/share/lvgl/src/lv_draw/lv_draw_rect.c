...
LV_ATTRIBUTE_FAST_MEM void lv_draw_rect_dsc_init(lv_draw_rect_dsc_t *dsc)
{
	...
	dsc->bg_color = LV_COLOR_BLACK; // LV_COLOR_WHITE;
...
static void draw_pattern(const lv_area_t *coords, const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc)
{
	...
	else if (src_type == LV_IMG_SRC_SYMBOL)
	{
		...
		lv_point_t s;
		_lv_txt_get_size(&s, dsc->pattern_image, label_dsc.font, label_dsc.letter_space, label_dsc.line_space, LV_COORD_MAX, LV_TXT_FLAG_NONE);
		img_w = s.x;
		img_h = s.y;
	}
	else
	...
	else
	{
		...
		if (dsc->pattern_align == LV_ALIGN_CENTER)
		{
			coords_tmp.x1 = coords->x1 + (obj_w - img_w) / 2;
			coords_tmp.y1 = coords->y1 + (obj_h - img_h) / 2;
		}
		else if (dsc->pattern_align == LV_ALIGN_IN_LEFT_MID)
		{
			coords_tmp.x1 = coords->x1;
			coords_tmp.y1 = coords->y1 + (obj_h - img_h) / 2;
		}
		else if (dsc->pattern_align == LV_ALIGN_IN_TOP_MID)
		{
			coords_tmp.x1 = coords->x1 + (obj_w - img_w) / 2;
			coords_tmp.y1 = coords->y1;
		}
		else if (dsc->pattern_align == LV_ALIGN_IN_RIGHT_MID)
		{
			coords_tmp.x1 = coords->x1 + obj_w - img_w;
			coords_tmp.y1 = coords->y1 + (obj_h - img_h) / 2;
		}
		coords_tmp.x2 = coords_tmp.x1 + img_w - 1;
		coords_tmp.y2 = coords_tmp.y1 + img_h - 1;
...
static void draw_value_str(const lv_area_t *coords, const lv_area_t *clip, const lv_draw_rect_dsc_t *dsc)
{
	...
	lv_point_t s;
	_lv_txt_get_size(&s, str, dsc->value_font, dsc->value_letter_space, dsc->value_line_space, LV_COORD_MAX, LV_TXT_FLAG_NONE);

...
#define LV_STYLE_ID_PTR 0xE   /*max 2 pcs*/
    ...
    enum
    {
        ...
        LV_STYLE_PROP_INIT(LV_STYLE_PATTERN_ALIGN, 0x6, LV_STYLE_ID_VALUE + 2, LV_STYLE_ATTR_NONE),
...
#define LV_STYLE_INHERIT_MASK 0x8000
    ...
    {
        if (list->has_trans && list->skip_trans)
            id++;
        if (list->style_cnt == 0 || id >= list->style_cnt)
            return NULL;
 ...
 */
#define LV_STYLE_CREATE(name, copy_p) \
    static lv_style_t name;           \
    lv_style_init(&name);             \
    lv_style_copy(&name, copy_p)

...
static int ak_serial_startup(struct uart_port *port)
{
	...
	}else
	{
		pr_info("%s: Not support Hardware flow control",__func__);	
			...
			/*
			 * if not use RTS/CTS, AK37E uart1 should set bit 18 and bit 19 to 1
			 */
			if(port->line == 1)
			    uart_reg |= (1<<18|1<<19);
			else
...
static void ak_serial_config_port(struct uart_port *port, int flags)
{
    ...
    /* 
	 *  when setting cap_hw_flow_control property, user should also set CTS/RTS sharepin in dts pinctrl.
	 */

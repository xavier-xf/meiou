...
#define ACTUAL_SENSOR_ID		0x2008
#define SENSOR_ID				0x2008
 ...
 */
struct mis2008_aec_parms {
	int curr_again_level;
	int curr_again_10x;
	int exp_time_msb_value;
	int exp_time_lsb_value;
	int target_exp_ctrl;
	int default_hts;
	int calc_vts_tmp;
	int curr_2x_dgain;
...
static const struct v4l2_ctrl_ops mis2008_ctrl_ops;
static const struct v4l2_ctrl_config config_sensor_get_id = {
	.ops	= &mis2008_ctrl_ops,
	.id		= SENSOR_GET_ID,
	.name	= "get sensor id",
...
struct mis2008_priv {
	...
	u32	cfmt_code;
	const struct mis2008_win_size	*win;
	...
	struct mis2008_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB mis2008_cb;
...
static void mis2008_set_fps_async(struct mis2008_priv *priv);
static int __mis2008_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int mis2008_sensor_read_id_func(void *arg);   //no use IIC bus
static int mis2008_sensor_get_resolution_func(void *arg, int *width, int *height);
static int mis2008_sensor_set_fps_func(void *arg, const int fps);
static int mis2008_fps_to_vts(struct mis2008_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 mis2008_codes[] = {
 ...
 * General functions
 */
static struct mis2008_priv *to_mis2008(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct mis2008_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct mis2008_priv, hdl)->subdev;
}
/*
 * XXXX_s_stream -
 ...
 * @enable:			enable flags
 */
static int mis2008_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int mis2008_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int mis2008_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int mis2008_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2008_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = mis2008_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2008_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2008_priv *priv = to_mis2008(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int mis2008_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2008_priv *priv = to_mis2008(client);
	int fps = ctrl->value;
	int vts = mis2008_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2008_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = mis2008_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = mis2008_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = mis2008_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2008_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2008_priv *priv = to_mis2008(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2008_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2008_priv *priv = to_mis2008(client);
	int fps = ctrl->value;

	mis2008_sensor_set_fps_func(priv, fps);
	mis2008_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2008_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = mis2008_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = mis2008_set_fps_direct(sd, ctrl);
			break;
...
static int mis2008_store_initial_regs(struct mis2008_priv *priv, void *arg)
{
...
static long mis2008_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2008_priv *priv = to_mis2008(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			mis2008_store_initial_regs(priv, arg);
			break;
 ...
 */
static int mis2008_get_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct mis2008_priv *priv = to_mis2008(client);
	static struct mis2008_win_size win = {
		.name = "default",
 ...
 */
static int mis2008_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
 ...
 */
static int mis2008_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
 ...
 */
static int mis2008_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(mis2008_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int mis2008_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int mis2008_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	mis2008_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int mis2008_video_probe(struct i2c_client *client)
{
	struct mis2008_priv *priv = to_mis2008(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops mis2008_ctrl_ops = {
	.g_volatile_ctrl	= mis2008_g_volatile_ctrl,
	.s_ctrl				= mis2008_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops mis2008_subdev_core_ops = {
	.s_power	= mis2008_core_s_power,
	.g_ctrl		= mis2008_core_g_ctrl,
	.s_ctrl		= mis2008_core_s_ctrl,
	.ioctl		= mis2008_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int mis2008_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int mis2008_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops mis2008_subdev_video_ops = {
	.s_stream	= mis2008_s_stream,
	.cropcap	= mis2008_cropcap,
	.g_crop		= mis2008_g_crop,
	.g_mbus_config	= mis2008_g_mbus_config,
	.s_crop		= mis2008_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops mis2008_subdev_pad_ops = {
	.enum_mbus_code = mis2008_enum_mbus_code,
	.get_fmt	= mis2008_get_fmt,
	.set_fmt	= mis2008_set_fmt,
	.get_selection	= mis2008_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops mis2008_subdev_ops = {
	.core	= &mis2008_subdev_core_ops,
	.video	= &mis2008_subdev_video_ops,
	.pad	= &mis2008_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct mis2008_priv *priv)
{
 ...
 */
static int mis2008_match(struct mis2008_priv *priv)
{
	int pid;
 ...
 * @did:			driver ids
 */
static int mis2008_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	struct mis2008_priv	*priv;
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"mis2008: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct mis2008_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!mis2008_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &mis2008_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &mis2008_ctrl_ops,
	...
	ret = mis2008_video_probe(client);
	if (ret < 0) {
	...
	dev_err(&adapter->dev, "mis2008 Probed success, subdev:%p\n", &priv->subdev);
...
static const struct of_device_id mis2008_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, mis2008_of_match);

static struct i2c_driver mis2008_i2c_driver = {
	.driver = {
		.name = "mis2008",
		.of_match_table = of_match_ptr(mis2008_of_match),
	},
	.probe    = mis2008_probe,
...
module_i2c_driver(mis2008_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for mis2008 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.2");

/*sensor i2c write*/
static int mis2008_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int mis2008_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
 ...
 *
 * */
static int mis2008_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct mis2008_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = mis2008_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		mis2008_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
	...
	// * VTS=(0x3200,0x3201)=0x0465=‭1125
	priv->fps_info.to_fps_value = ((mis2008_read(client, 0x3200)&0xff)<<8) | ((mis2008_read(client, 0x3201)&0xff)<<0); //(0x3200, 0x3201)
	// pr_err("0x3200:%d(0x%04x)", (mis2008_read(client, 0x3200)&0xff), (mis2008_read(client, 0x3200)&0xff));
	// pr_err("0x3201:%d(0x%04x)", (mis2008_read(client, 0x3201)&0xff), (mis2008_read(client, 0x3201)&0xff));

	// * HTS=(0x3202,0x3203)=0x0898=‭2200‬‬
	priv->aec_parms.default_hts = ((mis2008_read(client, 0x3202)&0xff)<<8) | ((mis2008_read(client, 0x3203)&0xff)<<0); //(0x320c,0x320d)
	// pr_err("0x3200:%02d(0x%04x)", (mis2008_read(client, 0x3202)&0xff), (mis2008_read(client, 0x3202)&0xff));
	// pr_err("0x3203:%02d(0x%04x)", (mis2008_read(client, 0x3203)&0xff), (mis2008_read(client, 0x3203)&0xff));
...
static int mis2008_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct mis2008_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return mis2008_read(client, reg_addr);
}
/*write sensor register*/
static int mis2008_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct mis2008_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return mis2008_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct mis2008_priv *priv)
{
	priv->aec_parms.curr_again_level = 0;
	priv->aec_parms.curr_again_10x = -1;
	priv->aec_parms.exp_time_msb_value = 0;
	priv->aec_parms.exp_time_lsb_value = 0;
...
/*set sensor again*/
static int mis2008_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	...
	mis2008_write(client, 0x3102, (corse_gain<<5)|(fine_gain&0x1f));
	mis2008_write(client, 0x3a02, blooming_dc);

	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int mis2008_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	return D_GAIN_EFFECT_FRAMES;
}

static int __set_reg_exp_time(struct mis2008_priv *priv, int exp_time)
{
	int ret = 0;
	struct i2c_client *client = priv->client;
	...
	mis2008_write(client, 0x3100, exposure_time_msb);
	mis2008_write(client, 0x3101, exposure_time_lsb);

	priv->aec_parms.exp_time_lsb_value = exposure_time_lsb;
	priv->aec_parms.exp_time_msb_value = exposure_time_msb;
...
static int __set_reg_frame_vts(struct mis2008_priv *priv, int vts)
{
	int ret = 0;
	struct i2c_client *client = priv->client;

	mis2008_write(client, 0x3200, (vts >> 8) & 0xff);
	mis2008_write(client, 0x3201, vts & 0xff);
...
static int __get_reg_frame_vts(struct mis2008_priv *priv)
{
	struct i2c_client *client = priv->client;

	return mis2008_read(client, 0x3200) << 8 | mis2008_read(client, 0x3201);
}

static void update_frame_vts_and_exp_ctrl(struct mis2008_priv *priv)
{
	struct i2c_client *client = priv->client;

    /*
     * min exp_time = 1
     * max exp_time = {R0x3200,R0x3201} - 2
     *
     * now:
     * 	{R0x3200,R0x3201} - 2
     * 	= 0x4b0 - 2
    ...
    int const max_exp_ctrl      = priv->fps_info.to_fps_value - 2;   //Vts - 2
...
/*set sensor exp time*/
static int mis2008_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	struct mis2008_priv *priv = arg;
	struct i2c_client *client = priv->client;
...
/*sensor timer*/
static int mis2008_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int mis2008_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int mis2008_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int mis2008_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct mis2008_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = mis2008_read(client, 0x3000);
	id = value << 8;

	value = mis2008_read(client, 0x3001);
	id |= value;
...
/*get mclk*/
static int mis2008_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int mis2008_sensor_get_fps_func(void *arg)
{
	struct mis2008_priv *priv = arg;
...
static int mis2008_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type mis2008_sensor_get_bus_type_func(void *arg)
{
...
static int mis2008_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct mis2008_priv *priv = arg;
...
/*sensor power on*/
static int mis2008_sensor_set_power_on_func(void *arg)
{
	struct mis2008_priv *priv = arg;
...
/*sensor power off*/
static int mis2008_sensor_set_power_off_func(void *arg)
{
	struct mis2008_priv *priv = arg;
...
static int mis2008_sensor_set_fps_func(void *arg, const int fps)
{
	...
	tmp = mis2008_fps_to_vts(priv, fps);
...
static AK_ISP_SENSOR_CB mis2008_cb = {
	.sensor_init_func					= mis2008_sensor_init_func,
	.sensor_read_reg_func				= mis2008_sensor_read_reg_func,
	.sensor_write_reg_func				= mis2008_sensor_write_reg_func,
	.sensor_read_id_func				= mis2008_sensor_read_id_func,
	.sensor_update_a_gain_func			= mis2008_sensor_update_a_gain_func,
	.sensor_update_d_gain_func			= mis2008_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func		= mis2008_sensor_updata_exp_time_func,
	.sensor_timer_func					= mis2008_sensor_timer_func,
	.sensor_set_standby_in_func			= mis2008_sensor_set_standby_in_func,
	.sensor_set_standby_out_func		= mis2008_sensor_set_standby_out_func,
	.sensor_probe_id_func				= mis2008_sensor_probe_id_func,
	.sensor_get_resolution_func			= mis2008_sensor_get_resolution_func,
	.sensor_get_mclk_func				= mis2008_sensor_get_mclk_func,
	.sensor_get_fps_func				= mis2008_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func	= mis2008_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func			= mis2008_sensor_get_bus_type_func,
	.sensor_get_parameter_func			= mis2008_sensor_get_parameter_func,
	.sensor_set_power_on_func			= mis2008_sensor_set_power_on_func,
	.sensor_set_power_off_func			= mis2008_sensor_set_power_off_func,
	.sensor_set_fps_func				= mis2008_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return mis2008_sensor_read_id_func(NULL);
}

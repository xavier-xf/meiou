...
static const struct v4l2_ctrl_ops sc2145_ctrl_ops;
...
struct sc2145_priv {
	...
	u32	cfmt_code;
	const struct sc2145_win_size	*win;
	...
	struct sc2145_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB sc2145_cb;
...
static void sc2145_set_fps_async(struct sc2145_priv *priv);
static int __sc2145_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int sc2145_sensor_read_id_func(void *arg);   //no use IIC bus
static int sc2145_sensor_get_resolution_func(void *arg, int *width, int *height);
static int sc2145_sensor_set_fps_func(void *arg, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);
static int sc2145_sensor_probe_id_func(void *arg);  //use IIC bus

static u32 sc2145_codes[] = {
 ...
 * General functions
 */
static struct sc2145_priv *to_sc2145(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct sc2145_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct sc2145_priv, hdl)->subdev;
}
...
static int sc2145_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int sc2145_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int sc2145_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2145_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = sc2145_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2145_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2145_priv *priv = to_sc2145(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int sc2145_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2145_priv *priv = to_sc2145(client);
	int fps = ctrl->value;
	int vts = 2250 * MAX_FPS / fps;//TODO
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2145_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = sc2145_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = sc2145_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = sc2145_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2145_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2145_priv *priv = to_sc2145(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2145_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2145_priv *priv = to_sc2145(client);
	int fps = ctrl->value;

	sc2145_sensor_set_fps_func(priv, fps);
	sc2145_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2145_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = sc2145_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = sc2145_set_fps_direct(sd, ctrl);
			break;
...
static int sc2145_store_initial_regs(struct sc2145_priv *priv, void *arg)
{
...
static long sc2145_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2145_priv *priv = to_sc2145(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			sc2145_store_initial_regs(priv, arg);
			break;
 ...
 */
static int sc2145_get_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct sc2145_priv *priv = to_sc2145(client);
	static struct sc2145_win_size win = {
		.name = "default",
 ...
 */
static int sc2145_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
 ...
 */
static int sc2145_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
 ...
 */
static int sc2145_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(sc2145_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int sc2145_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int sc2145_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	sc2145_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int sc2145_video_probe(struct i2c_client *client)
{
	struct sc2145_priv *priv = to_sc2145(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops sc2145_ctrl_ops = {
	.g_volatile_ctrl	= sc2145_g_volatile_ctrl,
	.s_ctrl				= sc2145_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops sc2145_subdev_core_ops = {
	.s_power	= sc2145_core_s_power,
	.g_ctrl		= sc2145_core_g_ctrl,
	.s_ctrl		= sc2145_core_s_ctrl,
	.ioctl		= sc2145_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int sc2145_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int sc2145_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops sc2145_subdev_video_ops = {
	.s_stream	= sc2145_s_stream,
	.cropcap	= sc2145_cropcap,
	.g_crop		= sc2145_g_crop,
	.g_mbus_config	= sc2145_g_mbus_config,
	.s_crop		= sc2145_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops sc2145_subdev_pad_ops = {
	.enum_mbus_code = sc2145_enum_mbus_code,
	.get_fmt	= sc2145_get_fmt,
	.set_fmt	= sc2145_set_fmt,
	.get_selection	= sc2145_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops sc2145_subdev_ops = {
	.core	= &sc2145_subdev_core_ops,
	.video	= &sc2145_subdev_video_ops,
	.pad	= &sc2145_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct sc2145_priv *priv)
{
 ...
 */
static int sc2145_match(struct sc2145_priv *priv)
{
	int pid;
			...
			const struct i2c_device_id *did)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"sc2145: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct sc2145_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!sc2145_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &sc2145_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &sc2145_ctrl_ops,
	...
	dev_err(&adapter->dev, "sc2145 Probed success, subdev:%p\n", &priv->subdev);
...
static const struct of_device_id sc2145_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, sc2145_of_match);

static struct i2c_driver sc2145_i2c_driver = {
	.driver = {
		.name = "sc2145",
		.of_match_table = of_match_ptr(sc2145_of_match),
	},
	.probe    = sc2145_probe,
...
module_i2c_driver(sc2145_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc2145 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.4");

/*sensor i2c write*/
static int sc2145_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int sc2145_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
 ...
 *
 * */
static int sc2145_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct sc2145_priv *priv = arg;
	struct i2c_client *client = priv->client;
	AK_ISP_SENSOR_INIT_PARA *para = &priv->para;

	sc2145_sensor_probe_id_func(arg);  //use IIC bus
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = sc2145_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		sc2145_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
...
static int sc2145_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct sc2145_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc2145_read(client, reg_addr);
}
/*write sensor register*/
static int sc2145_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct sc2145_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc2145_write(client, reg_addr, value);
}
...
/*set sensor again*/
static int sc2145_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int sc2145_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	return D_GAIN_EFFECT_FRAMES;
}
/*set sensor exp time*/
static int sc2145_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	return EXP_EFFECT_FRAMES;
}
/*sensor timer*/
static int sc2145_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int sc2145_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int sc2145_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
...
static int sc2145_sensor_probe_id_func(void *arg)  //use IIC bus
{
	...
	value = sc2145_read(client, 0x3107);
	id = value << 8;

	value = sc2145_read(client, 0x3108);
	id |= value;
...
/*get mclk*/
static int sc2145_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int sc2145_sensor_get_fps_func(void *arg)
{
	struct sc2145_priv *priv = arg;
...
static int sc2145_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type sc2145_sensor_get_bus_type_func(void *arg)
{
	return SENSOR_BUS_TYPE;
}
/*get self definded params*/
static int sc2145_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct sc2145_priv *priv = arg;
...
/*sensor power on*/
static int sc2145_sensor_set_power_on_func(void *arg)
{
	struct sc2145_priv *priv = arg;
...
/*sensor power off*/
static int sc2145_sensor_set_power_off_func(void *arg)
{
	struct sc2145_priv *priv = arg;
...
static AK_ISP_SENSOR_CB sc2145_cb = {
	.sensor_init_func = sc2145_sensor_init_func,
	.sensor_read_reg_func = sc2145_sensor_read_reg_func,
	.sensor_write_reg_func = sc2145_sensor_write_reg_func,
	.sensor_read_id_func = sc2145_sensor_read_id_func,
	.sensor_update_a_gain_func = sc2145_sensor_update_a_gain_func,
	.sensor_update_d_gain_func = sc2145_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func = sc2145_sensor_updata_exp_time_func,
	.sensor_timer_func = sc2145_sensor_timer_func,
	.sensor_set_standby_in_func = sc2145_sensor_set_standby_in_func,
	.sensor_set_standby_out_func = sc2145_sensor_set_standby_out_func,
	.sensor_probe_id_func = sc2145_sensor_probe_id_func,
	.sensor_get_resolution_func = sc2145_sensor_get_resolution_func,
	.sensor_get_mclk_func = sc2145_sensor_get_mclk_func,
	.sensor_get_fps_func = sc2145_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func = sc2145_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func = sc2145_sensor_get_bus_type_func,
	.sensor_get_parameter_func = sc2145_sensor_get_parameter_func,
	.sensor_set_power_on_func = sc2145_sensor_set_power_on_func,
	.sensor_set_power_off_func = sc2145_sensor_set_power_off_func,
	.sensor_set_fps_func = sc2145_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return sc2145_sensor_read_id_func(NULL);
}

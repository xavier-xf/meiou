...
#define ACTUAL_SENSOR_ID		0x020b
#define SENSOR_ID				0x2399
...
static const struct v4l2_ctrl_ops c2399_ctrl_ops;
...
struct c2399_priv {
	...
	u32	cfmt_code;
	const struct c2399_win_size	*win;
	...
	struct c2399_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB c2399_cb;
...
static void c2399_set_fps_async(struct c2399_priv *priv);
static int __c2399_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int c2399_sensor_read_id_func(void *arg);   //no use IIC bus
static int c2399_sensor_get_resolution_func(void *arg, int *width, int *height);
static int c2399_sensor_set_fps_func(void *arg, const int fps);
static int c2399_fps_to_vts(struct c2399_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 c2399_codes[] = {
 ...
 * General functions
 */
static struct c2399_priv *to_c2399(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct c2399_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct c2399_priv, hdl)->subdev;
}
 ...
 * @enable:			enable flags
 */
static int c2399_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int c2399_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int c2399_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int c2399_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int c2399_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = c2399_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int c2399_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct c2399_priv *priv = to_c2399(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int c2399_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct c2399_priv *priv = to_c2399(client);
	int fps = ctrl->value;
	int vts = c2399_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int c2399_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = c2399_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = c2399_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = c2399_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int c2399_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct c2399_priv *priv = to_c2399(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int c2399_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct c2399_priv *priv = to_c2399(client);
	int fps = ctrl->value;

	c2399_sensor_set_fps_func(priv, fps);
	c2399_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int c2399_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = c2399_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = c2399_set_fps_direct(sd, ctrl);
			break;
...
static int c2399_store_initial_regs(struct c2399_priv *priv, void *arg)
{
...
static long c2399_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct c2399_priv *priv = to_c2399(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			c2399_store_initial_regs(priv, arg);
			break;
 ...
 */
static int c2399_get_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct c2399_priv *priv = to_c2399(client);
	static struct c2399_win_size win = {
		.name = "default",
 ...
 */
static int c2399_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
 ...
 */
static int c2399_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
 ...
 */
static int c2399_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(c2399_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int c2399_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int c2399_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	c2399_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int c2399_video_probe(struct i2c_client *client)
{
	struct c2399_priv *priv = to_c2399(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops c2399_ctrl_ops = {
	.g_volatile_ctrl	= c2399_g_volatile_ctrl,
	.s_ctrl				= c2399_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops c2399_subdev_core_ops = {
	.s_power	= c2399_core_s_power,
	.g_ctrl		= c2399_core_g_ctrl,
	.s_ctrl		= c2399_core_s_ctrl,
	.ioctl		= c2399_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int c2399_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int c2399_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops c2399_subdev_video_ops = {
	.s_stream	= c2399_s_stream,
	.cropcap	= c2399_cropcap,
	.g_crop		= c2399_g_crop,
	.g_mbus_config	= c2399_g_mbus_config,
	.s_crop		= c2399_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops c2399_subdev_pad_ops = {
	.enum_mbus_code = c2399_enum_mbus_code,
	.get_fmt	= c2399_get_fmt,
	.set_fmt	= c2399_set_fmt,
	.get_selection	= c2399_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops c2399_subdev_ops = {
	.core	= &c2399_subdev_core_ops,
	.video	= &c2399_subdev_video_ops,
	.pad	= &c2399_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct c2399_priv *priv)
{
 ...
 */
static int c2399_match(struct c2399_priv *priv)
{
	int pid;
 ...
 * @did:			driver ids
 */
static int c2399_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	struct c2399_priv	*priv;
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"c2399: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct c2399_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &c2399_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &c2399_ctrl_ops,
	...
	ret = c2399_video_probe(client);
	if (ret < 0) {
		dev_err(&client->dev, "c2399 probe fail\n");
		goto err_videoprobe;
	}
	...
	dev_err(&adapter->dev, "c2399 Probed success, subdev:%p\n", &priv->subdev);
...
static const struct of_device_id c2399_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, c2399_of_match);

static struct i2c_driver c2399_i2c_driver = {
	.driver = {
		.name = "c2399",
		.of_match_table = of_match_ptr(c2399_of_match),
	},
	.probe    = c2399_probe,
	.remove   = c2399_remove,
	.id_table = c2399_id,
...
module_i2c_driver(c2399_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for c2399 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.3");

/*sensor i2c write*/
static int c2399_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int c2399_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
...
static void c2399_agc_mode(struct c2399_priv *priv)
{
	int value;
	struct i2c_client *client = priv->client;

	value = c2399_read(client, 0x3e03) & 0xf0;
	value |= 0xb;
	c2399_write(client, 0x3e03, value);
}
 ...
 *
 * */
static int c2399_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct c2399_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = c2399_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		c2399_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
	...
	//VTS=(0x0340,0x0341)=0x5ae=‭1454‬ at 20fps
	priv->fps_info.to_fps_value = ((c2399_read(client, 0x340)&0xff)<<8) | ((c2399_read(client, 0x341)&0xff)<<0); //(0x0340,0x0341)
	// pr_err("0x340:%d(0x%04x)", (c2399_read(client, 0x340)&0xff), (c2399_read(client, 0x340)&0xff));
	// pr_err("0x341:%d(0x%04x)", (c2399_read(client, 0x341)&0xff), (c2399_read(client, 0x341)&0xff));

	//HTS=(0x0342,0x0343)=0x7bc=1980
	priv->aec_parms.default_hts = ((c2399_read(client, 0x342)&0xff)<<8) | ((c2399_read(client, 0x343)&0xff)<<0); //(0x0340,0x0341)
	// pr_err("0x342:%d(0x%04x)", (c2399_read(client, 0x342)&0xff), (c2399_read(client, 0x342)&0xff));
	// pr_err("0x343:%d(0x%04x)", (c2399_read(client, 0x343)&0xff), (c2399_read(client, 0x343)&0xff));
...
static int c2399_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct c2399_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return c2399_read(client, reg_addr);
}
/*write sensor register*/
static int c2399_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct c2399_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return c2399_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct c2399_priv *priv)
{
...
// Gain: 1x = 1024
static unsigned int Tab_C2399RegToGain[240] = 
{ 
...
/*set sensor again*/
static int c2399_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	...
	if (igain >= Tab_C2399RegToGain[AGIAN_NUM - 1])
	{
		A_value = AGIAN_NUM - 1;
	...
	a_gain_out = A_value;
	// pr_err("a_gain_out=%d\n", a_gain_out);
	c2399_write(client, 0x0205, a_gain_out & 0xff);
	...
	// c2399_write(client, 0x3e08, corse_gain);
	// c2399_write(client, 0x3e09, fine_gain);

	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int c2399_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	return D_GAIN_EFFECT_FRAMES;
}
static int __set_reg_exp_time(struct c2399_priv *priv, int exp_time)
{
	int ret = 0;
	struct i2c_client *client = priv->client;
	...
	c2399_write(client, 0x202, exposure_time_msb);
	c2399_write(client, 0x203, exposure_time_lsb);
...
static int __set_reg_frame_vts(struct c2399_priv *priv, int vts)
{
	int ret = 0;
	struct i2c_client *client = priv->client;

	c2399_write(client, 0x340, (vts >> 8) & 0xff);
	c2399_write(client, 0x341, vts & 0xff);
...
static int __get_reg_frame_vts(struct c2399_priv *priv)
{
	struct i2c_client *client = priv->client;

	return c2399_read(client, 0x340) << 8 | c2399_read(client, 0x341);
}

static void update_frame_vts_and_exp_ctrl(struct c2399_priv *priv)
{
	struct i2c_client *client = priv->client;
...
/*set sensor exp time*/
static int c2399_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	struct c2399_priv *priv = arg;
	struct i2c_client *client = priv->client;
...
/*sensor timer*/
static int c2399_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int c2399_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int c2399_sensor_set_standby_out_func(void *arg)
{
	return 0;
}

/*low level read sensor ID, user i2c ops*/
static int c2399_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct c2399_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = c2399_read(client, 0x0000);
	id = value << 8;

	value = c2399_read(client, 0x0001);
	id |= value;
...
/*get mclk*/
static int c2399_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int c2399_sensor_get_fps_func(void *arg)
{
	struct c2399_priv *priv = arg;
...
static int c2399_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type c2399_sensor_get_bus_type_func(void *arg)
{
...
static int c2399_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct c2399_priv *priv = arg;
...
/*sensor power on*/
static int c2399_sensor_set_power_on_func(void *arg)
{
	struct c2399_priv *priv = arg;
...
/*sensor power off*/
static int c2399_sensor_set_power_off_func(void *arg)
{
	struct c2399_priv *priv = arg;
...
static int c2399_fps_to_vts(struct c2399_priv *priv, const int fps)
{
	/*
	 * c2399:
	 * C2399_1080p_mipi1lane_20fps_mclk24M_pclk57.6M_mipirat576Mbps_1454V_1980H_V3P0.txt
	 *
	 * mipi:
	 * HTS=(0x0342,0x0343)=0x7bc=1980
	 * VTS=(0x0340,0x0341)=0x5ae=‭1454‬ at 20fps
	 * fps = VTS1 * MAX_FPS / VTS
	 *
	 * so :
	 * vts = VTS1 * MAX_FPS / fps = ‭1454‬*20 / fps
	 *     = (29080*100) / (fps*100)
	 *     = (2908000UL) / vts;   //vts=fps*100
	 *
	 */
...
static AK_ISP_SENSOR_CB c2399_cb = {
	.sensor_init_func					= c2399_sensor_init_func,
	.sensor_read_reg_func				= c2399_sensor_read_reg_func,
	.sensor_write_reg_func				= c2399_sensor_write_reg_func,
	.sensor_read_id_func				= c2399_sensor_read_id_func,
	.sensor_update_a_gain_func			= c2399_sensor_update_a_gain_func,
	.sensor_update_d_gain_func			= c2399_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func		= c2399_sensor_updata_exp_time_func,
	.sensor_timer_func					= c2399_sensor_timer_func,
	.sensor_set_standby_in_func			= c2399_sensor_set_standby_in_func,
	.sensor_set_standby_out_func		= c2399_sensor_set_standby_out_func,
	.sensor_probe_id_func				= c2399_sensor_probe_id_func,
	.sensor_get_resolution_func			= c2399_sensor_get_resolution_func,
	.sensor_get_mclk_func				= c2399_sensor_get_mclk_func,
	.sensor_get_fps_func				= c2399_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func	= c2399_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func			= c2399_sensor_get_bus_type_func,
	.sensor_get_parameter_func			= c2399_sensor_get_parameter_func,
	.sensor_set_power_on_func			= c2399_sensor_set_power_on_func,
	.sensor_set_power_off_func			= c2399_sensor_set_power_off_func,
	.sensor_set_fps_func				= c2399_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return c2399_sensor_read_id_func(NULL);
}

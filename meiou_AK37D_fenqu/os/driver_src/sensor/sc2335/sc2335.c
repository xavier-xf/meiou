...
struct sc2335_priv {
	...
	u32	cfmt_code;
	const struct sc2335_win_size	*win;
	...
	struct sc2335_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB sc2335_cb;
...
static void sc2335_set_fps_async(struct sc2335_priv *priv);
static int __sc2335_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int sc2335_sensor_read_id_func(void *arg);   //no use IIC bus
static int sc2335_sensor_get_resolution_func(void *arg, int *width, int *height);
static int sc2335_sensor_set_fps_func(void *arg, const int fps);
static int sc2335_fps_to_vts(struct sc2335_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 sc2335_codes[] = {
 ...
 * General functions
 */
static struct sc2335_priv *to_sc2335(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct sc2335_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct sc2335_priv, hdl)->subdev;
}
 ...
 * @enable:			enable flags
 */
static int sc2335_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int sc2335_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int sc2335_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int sc2335_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2335_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = sc2335_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2335_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2335_priv *priv = to_sc2335(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int sc2335_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2335_priv *priv = to_sc2335(client);
	int fps = ctrl->value;
	int vts = sc2335_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2335_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = sc2335_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = sc2335_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = sc2335_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2335_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2335_priv *priv = to_sc2335(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2335_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2335_priv *priv = to_sc2335(client);
	int fps = ctrl->value;

	sc2335_sensor_set_fps_func(priv, fps);
	sc2335_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc2335_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = sc2335_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = sc2335_set_fps_direct(sd, ctrl);
			break;
...
static int sc2335_store_initial_regs(struct sc2335_priv *priv, void *arg)
{
...
static long sc2335_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc2335_priv *priv = to_sc2335(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			sc2335_store_initial_regs(priv, arg);
			break;
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct sc2335_priv *priv = to_sc2335(client);
	static struct sc2335_win_size win = {
		.name = "default",
 ...
 */
static int sc2335_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
...
{
	if (code->pad || code->index >= ARRAY_SIZE(sc2335_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int sc2335_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int sc2335_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	sc2335_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int sc2335_video_probe(struct i2c_client *client)
{
	struct sc2335_priv *priv = to_sc2335(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops sc2335_ctrl_ops = {
	.g_volatile_ctrl	= sc2335_g_volatile_ctrl,
	.s_ctrl				= sc2335_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops sc2335_subdev_core_ops = {
	.s_power	= sc2335_core_s_power,
	.g_ctrl		= sc2335_core_g_ctrl,
	.s_ctrl		= sc2335_core_s_ctrl,
	.ioctl		= sc2335_core_ioctl,
};
 ...
 * @crop:			crop to set
 */
static int sc2335_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops sc2335_subdev_video_ops = {
	.s_stream	= sc2335_s_stream,
	.cropcap	= sc2335_cropcap,
	.g_crop		= sc2335_g_crop,
	.g_mbus_config	= sc2335_g_mbus_config,
	.s_crop		= sc2335_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops sc2335_subdev_pad_ops = {
	.enum_mbus_code = sc2335_enum_mbus_code,
	.get_fmt	= sc2335_get_fmt,
	.set_fmt	= sc2335_set_fmt,
	.get_selection	= sc2335_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops sc2335_subdev_ops = {
	.core	= &sc2335_subdev_core_ops,
	.video	= &sc2335_subdev_video_ops,
	.pad	= &sc2335_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct sc2335_priv *priv)
{
 ...
 */
static int sc2335_match(struct sc2335_priv *priv)
{
	int pid;
			...
			const struct i2c_device_id *did)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"sc2335: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct sc2335_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!sc2335_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &sc2335_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &sc2335_ctrl_ops,
	...
	dev_err(&adapter->dev, "sc2335 Probed success, subdev:%p\n", &priv->subdev);
...
module_i2c_driver(sc2335_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc2335 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.5");

/*sensor i2c write*/
static int sc2335_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int sc2335_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
...
static void sc2335_agc_mode(struct sc2335_priv *priv)
{
	int value;
	struct i2c_client *client = priv->client;

	value = sc2335_read(client, 0x3e03) & 0xf0;
	value |= 0xb;
	sc2335_write(client, 0x3e03, value);
}
 ...
 *
 * */
static int sc2335_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct sc2335_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = sc2335_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		sc2335_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
...
static int sc2335_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct sc2335_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc2335_read(client, reg_addr);
}
/*write sensor register*/
static int sc2335_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct sc2335_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc2335_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct sc2335_priv *priv)
{
...
/*set sensor again*/
static int sc2335_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	 ...
	 * 有风险的，此处建议：每隔几帧执行一次逻辑操作（会出现增益、逻辑不匹配情况，但不会出现异常
	 * 帧）；或者不使用 group hold 操作，直接写增益逻辑寄存器（需要客户端实测增益逻辑切换点的亮度
	 * 变动风险）。 具体需要和 FAE 沟通
	 ...
	 *
	 *
	 * SC2335 逻辑汇总及注意事项: 没有描述0x5799，梁工给的示例代码是有的。
	 * }
	 */
	...
	sc2335_write(client, 0x3e08, corse_gain);
	sc2335_write(client, 0x3e09, fine_gain);

	sc2335_write(client, 0x3812, 0x00);
	sc2335_write(client, 0x363c, blooming_dc);
	if (a_gain >= 1536)/*6x*/
		sc2335_write(client, 0x5799, 0x07);
	else if (a_gain <= 1024)/*4x*/
		sc2335_write(client, 0x5799, 0x00);
	sc2335_write(client, 0x3812, 0x30);

	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int sc2335_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	/*
	...
	if(d_gain<512) {
		 ...
		 * */
		corse_gain = 0xf;
		fine_gain = 0x80 + (d_gain - 4096) / 32;/*4096/64=64*/
		fine_gain &= 0xfe;
	} else {
		corse_gain = 0xf;
		fine_gain = 0xfe;
	...
	sc2335_write(client, 0x3e06, corse_gain);
	sc2335_write(client, 0x3e07, fine_gain);

	return D_GAIN_EFFECT_FRAMES;
}
/*set sensor exp time*/
static int sc2335_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	/*
	 * exp_time=1 mean half line exptime
	 * max exp_time = {R0x320,R0x320f} * 2 - 8
	 *
	 * now:
	 * 	{R0x320,R0x320f} * 2 - 8
	 * 	= 0x465 * 2 - 8 = 2242
	...
	//tem=sc2335_read(client, 0x3e02)&0x0f;
	//exposure_time_lsb =exposure_time_lsb|tem;
	sc2335_write(client, 0x3e01,exposure_time_msb);
	sc2335_write(client, 0x3e02,exposure_time_lsb);

	//tem = sc2335_read(client, 0x3e00) & 0xf0;
	//tem |= exposure_time_ext;
	sc2335_write(client, 0x3e00, exposure_time_ext);
...
/*sensor timer*/
static int sc2335_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int sc2335_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int sc2335_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int sc2335_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct sc2335_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = sc2335_read(client, 0x3107);
	id = value << 8;

	value = sc2335_read(client, 0x3108);
	id |= value;
...
/*get mclk*/
static int sc2335_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int sc2335_sensor_get_fps_func(void *arg)
{
	struct sc2335_priv *priv = arg;
...
static int sc2335_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type sc2335_sensor_get_bus_type_func(void *arg)
{
	return SENSOR_BUS_TYPE;
}
/*get self definded params*/
static int sc2335_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct sc2335_priv *priv = arg;
...
/*sensor power on*/
static int sc2335_sensor_set_power_on_func(void *arg)
{
	struct sc2335_priv *priv = arg;
...
/*sensor power off*/
static int sc2335_sensor_set_power_off_func(void *arg)
{
	struct sc2335_priv *priv = arg;
...
static int sc2335_fps_to_vts(struct sc2335_priv *priv, const int fps)
{
	 ...
	 * fps = pclk / VTS / HTS
	 *
	 * now pclk=74.25MHz, HTS=0x44c=1100
	 *
	 * */
	int vts;
...
static int sc2335_sensor_set_fps_func(void *arg, const int fps)
{
	...
	tmp = sc2335_fps_to_vts(priv, fps);
	if (tmp > 0) {
		priv->fps_info.to_fps_value = tmp / 2;
...
static void sc2335_set_fps_async(struct sc2335_priv *priv)
{
	...
	if (priv->fps_info.to_fps != priv->fps_info.current_fps) {
		...
		sc2335_write(client, 0x320e, (value >> 8) & 0xff);
		sc2335_write(client, 0x320f, value & 0xff);
		priv->fps_info.current_fps = priv->fps_info.to_fps;
	}
}

static AK_ISP_SENSOR_CB sc2335_cb = {
	.sensor_init_func = sc2335_sensor_init_func,
	.sensor_read_reg_func = sc2335_sensor_read_reg_func,
	.sensor_write_reg_func = sc2335_sensor_write_reg_func,
	.sensor_read_id_func = sc2335_sensor_read_id_func,
	.sensor_update_a_gain_func = sc2335_sensor_update_a_gain_func,
	.sensor_update_d_gain_func = sc2335_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func = sc2335_sensor_updata_exp_time_func,
	.sensor_timer_func = sc2335_sensor_timer_func,
	.sensor_set_standby_in_func = sc2335_sensor_set_standby_in_func,
	.sensor_set_standby_out_func = sc2335_sensor_set_standby_out_func,
	.sensor_probe_id_func = sc2335_sensor_probe_id_func,
	.sensor_get_resolution_func = sc2335_sensor_get_resolution_func,
	.sensor_get_mclk_func = sc2335_sensor_get_mclk_func,
	.sensor_get_fps_func = sc2335_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func = sc2335_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func = sc2335_sensor_get_bus_type_func,
	.sensor_get_parameter_func = sc2335_sensor_get_parameter_func,
	.sensor_set_power_on_func = sc2335_sensor_set_power_on_func,
	.sensor_set_power_off_func = sc2335_sensor_set_power_off_func,
	.sensor_set_fps_func = sc2335_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return sc2335_sensor_read_id_func(NULL);
}

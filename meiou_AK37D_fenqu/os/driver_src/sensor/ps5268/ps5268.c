/*
 * ps5268 Camera Driver
 *
 * Copyright anyka
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 */

#include <linux/init.h>
#include <linux/module.h>
#include <linux/i2c.h>
#include <linux/slab.h>
#include <linux/delay.h>
#include <linux/gpio.h>
#include <linux/gpio/consumer.h>
#include <linux/of_gpio.h>
#include <linux/v4l2-mediabus.h>
#include <linux/videodev2.h>

#include <media/v4l2-ctrls.h>
#include <media/v4l2-subdev.h>
#include <media/v4l2-device.h>
#include <media/v4l2-image-sizes.h>
#include "../include/ak_video_priv_cmd.h"
#include "../include_internal/ak_video_priv_cmd_internal.h"
#include "../include/ak_isp_drv.h"

#include "../include/ak_sensor.h"
#include "../include/sensor_cmd.h"
#include "../common/sensor_sys.h"
#include "../common/sensor_i2c.h"

/*
 *
SENSOR_PWDN_LEVEL:	the io level when sensor power down
SENSOR_RESET_LEVEL:	the io level when sensor reset
SENSOR_I2C_ADDR:	the sensor slave address, 7 bits width.
					the i2c bus left shift 1bits.
SENSOR_ID:	the sensor ID.
SENSOR_MCLK:	mclk, unit：MHz。24MHz or 27MHz mclk for most sensors.
				NOTE: 27MHz mclk is inaccurate.
SENSOR_REGADDR_BYTE:	sensor register address bytes width, 1 byte or 2 bytes.
SENSOR_DATA_BYTE:		register's value bytes width, 1 byte or 2bytes.
SENSOR_OUTPUT_WIDTH:	use for isp cut window MAX width.
SENSOR_OUTPUT_HEIGHT:	use for isp cut window MAX height.
SENSOR_VALID_OFFSET_X:	use for isp cut window left offset.
SENSOR_VALID_OFFSET_Y:	use for isp cut window top offset.
SENSOR_BUS_TYPE:		sensor output data type.
SENSOR_IO_INTERFACE:	sensor output data bus type.
SENSOR_IO_LEVEL:		sensor output data io level.
MAX_FPS:				current sensor configurtion support max fps.
EXP_EFFECT_FRAMES:		!!IMPORT!! frequency for update exp_time
		0 - adjust exp_time for every frames
		1 - adjust exp_time for 1/2 frames
		2 - adjust exp_time for 1/3 frames
		and so on.
A_GAIN_EFFECT_FRAMES:	!!IMPORT!! frequency for update a_gain
		0 - adjust a_gain for every frames
		1 - adjust a_gain for 1/2 frames
		2 - adjust a_gain for 1/3 frames
		and so on.
D_GAIN_EFFECT_FRAMES:	!!IMPORT!! frequency for update d_gain
		0 - adjust d_gain for every frames
		1 - adjust d_gain for 1/2 frames
		2 - adjust d_gain for 1/3 frames
		and so on.
DELAY_FLAG
 *
 * */
#define SENSOR_PWDN_LEVEL		0
#define SENSOR_RESET_LEVEL		0
#define SENSOR_I2C_ADDR0		0x48
#define SENSOR_I2C_ADDR1		0x4c
static int SENSOR_I2C_ADDR		= SENSOR_I2C_ADDR0;
#define SENSOR_ID				0x5268
#define SENSOR_MCLK				24
#define SENSOR_REGADDR_BYTE		2
#define SENSOR_DATA_BYTE		1
#define SENSOR_OUTPUT_WIDTH		1920
#define SENSOR_OUTPUT_HEIGHT	1080
#define SENSOR_VALID_OFFSET_X	0
#define SENSOR_VALID_OFFSET_Y	0
#define SENSOR_BUS_TYPE			BUS_TYPE_RAW
//#define SENSOR_IO_INTERFACE		DVP_INTERFACE
#define SENSOR_IO_INTERFACE		MIPI_INTERFACE
#define SENSOR_IO_LEVEL			IO_LEVEL_1V8
#define MIPI_MBPS				460
#define MIPI_LANES				2
#define MAX_FPS					30
#define EXP_EFFECT_FRAMES		1	/*adjust exp_time for 1/2 frames*/
#define A_GAIN_EFFECT_FRAMES	0	/*adjust a_gain for every frames*/
#define D_GAIN_EFFECT_FRAMES	0	/*adjust d_gain for every frames*/

#define DELAY_FLAG		(0xffff)

#define AG_HS_NODE (16*1024)
#define AG_LS_NODE (24*1024)
#define INITIAL_TOTAL_LINES 1124/*Total exposure lines,
								  xxx = Bank1 (0xA<<8|0xB), based on initial*/

/*
 * Struct
 */
struct regval_list {
	u8 reg_num;
	u8 value;
};

struct ps5268_win_size {
	char				*name;
	u32				width;
	u32				height;
	const struct regval_list	*regs;
};

/*
 * store sensor fps informations
 * @current_fps: current fps
 * @to_fps: should changed to the fps
 * @to_fps_value: the fps paramter
 */
struct sensor_fps_info {
	int current_fps;
	int to_fps;
	int to_fps_value;
};

/*
 * the host callbacks for sensor
 * @isp_timing_cb_info: isp timing callback.
 * 	a few sensor need the callback when fps had changed
 */
struct host_callbacks {
	struct isp_timing_cb_info isp_timing_cb_info;
};

/*
 * aec paramters. some sensors need record paramters.
 */
struct ps5268_aec_parms {
	int sghd_flg;
	int total_lines;
};

static const struct v4l2_ctrl_ops ps5268_ctrl_ops;
static const struct v4l2_ctrl_config config_sensor_get_id = {
	.ops	= &ps5268_ctrl_ops,
	.id		= SENSOR_GET_ID,
	.name	= "get sensor id",
	.type	= V4L2_CTRL_TYPE_INTEGER,
	.min	= 0,
	.max	= 0xfffffff,
	.step	= 1,
	.def	= 0,
	.flags	= V4L2_CTRL_FLAG_VOLATILE,/*set V4L2_CTRL_FLAG_VOLATILE avoid get cached*/
};

struct dual_sensor_attr {
	int init_count;
};

/*
 * the sensor struct
 * @client: point to i2c client
 * @subdev: v4l2 subdev struct
 * @hdl: 	v4l2 control handler
 * @cfmt_code:
 * @win:	default window
 * @gpio_reset:	gpio number for reset pin
 * @gpio_pwdn:	gpio number for power down pin
 * @cb_info:	sensor callbacks
 * @fps_info:	fps informations
 * @hcb:		host callbacks
 * @aec_parms:	aec paramters
 */
struct ps5268_priv {
	struct i2c_client *client;
	struct v4l2_subdev		subdev;
	struct v4l2_ctrl_handler	hdl;
	u32	cfmt_code;
	const struct ps5268_win_size	*win;

	int gpio_reset;
	int gpio_pwdn;
	struct sensor_cb_info cb_info;
	struct sensor_fps_info fps_info;

	struct host_callbacks hcb;
	struct ps5268_aec_parms aec_parms;

	AK_ISP_SENSOR_INIT_PARA para;

	struct v4l2_ctrl *ctrl_sensor_get_id;

	enum sensor_master_or_slave_mode master_or_slave;
	struct dual_sensor_attr dual;
};

static AK_ISP_SENSOR_CB ps5268_cb;

static int dvp = 0;

module_param(dvp, int, 0644);

/*
 * check_id - check hardware sensor ID whether it meets this driver
 * 0-no check, force to meet
 * others-check, if not meet return fail
 */
static int check_id = 0;
module_param(check_id, int, 0644);

static void ps5268_set_fps_async(struct ps5268_priv *priv);
static int __ps5268_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int ps5268_sensor_read_id_func(void *arg);   //no use IIC bus
static int ps5268_sensor_get_resolution_func(void *arg, int *width, int *height);
static int ps5268_sensor_set_fps_func(void *arg, const int fps);
static int ps5268_fps_to_vts(struct ps5268_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 ps5268_codes[] = {
	MEDIA_BUS_FMT_YUYV8_2X8,
	MEDIA_BUS_FMT_UYVY8_2X8,
	MEDIA_BUS_FMT_RGB565_2X8_BE,
	MEDIA_BUS_FMT_RGB565_2X8_LE,
};

static const unsigned int ps5268_gain_table[97] =
{
	1024, 1088, 1152, 1216, 1280, 1344, 1408, 1472, 1536, 1600, 1664, 1728,
	1792, 1856, 1920, 1984, 2048, 2176, 2304, 2432, 2560, 2688, 2816, 2944,
	3072, 3200, 3328, 3456, 3584, 3712, 3840, 3968, 4096, 4352, 4608, 4864,
	5120, 5376, 5632, 5888, 6144, 6400, 6656, 6912, 7168, 7424, 7680, 7936,
	8192, 8704, 9216, 9728, 10240, 10752, 11264, 11776, 12288, 12800, 13312,
	13824, 14336, 14848, 15360, 15872, 16384, 17408, 18432, 19456, 20480, 21504,
	22528, 23552, 24576, 25600, 26624, 27648, 28672, 29696, 30720, 31744, 32768,
	34816, 36864, 38912, 40960, 43008, 45056, 47104, 49152, 51200, 53248, 55296,
	57344, 59392, 61440, 63488, 65536,
};

/*
 * General functions
 */
static struct ps5268_priv *to_ps5268(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct ps5268_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct ps5268_priv, hdl)->subdev;
}

/*
 * XXXX_s_stream -
 * set steaming enable/disable
 * soc_camera_ops functions
 *
 * @sd:				subdev
 * @enable:			enable flags
 */
static int ps5268_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int ps5268_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	struct v4l2_subdev *sd = ctrl_to_sd(ctrl);
	int ret = 0;

	switch (ctrl->id) {
		case SENSOR_GET_ID:
			ctrl->val = SENSOR_ID;
			break;

		default:
			ret = -EINVAL;
			pr_err("%s id:%d no support\n", __func__, ctrl->id);
			break;
	}

	return ret;
}

/*
 * XXXX_s_stream -
 * set ctrls
 * soc_camera_ops functions
 *
 * @ctrl:			pointer to ctrl
 */
static int ps5268_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}

/*
 * XXXX_s_power -
 * set power operation
 * soc_camera_ops functions
 *
 * @sd:			subdev
 * @on:			power flags
 */
static int ps5268_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}

/*
 * XXXX_get_sensor_id -
 * get sensor ID
 * private callback functions
 *
 * @ctrl:			pointer to ctrl
 */
static int ps5268_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = ps5268_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}

/*
 * XXXX_get_sensor_id -
 * get sensor callback
 * private callback functions
 *
 * @sd:				subdev
 * @ctrl:			pointer to ctrl
 */
static int ps5268_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ps5268_priv *priv = to_ps5268(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int ps5268_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ps5268_priv *priv = to_ps5268(client);
	int fps = ctrl->value;
	int vts = ps5268_fps_to_vts(priv, fps);

	/* 4 <= Ny <= LPF-3*/
	ctrl->value = vts - 3;
	return 0;
}

/*
 * XXXX_core_g_ctrl -
 * get control
 * core functions
 *
 * @sd:				subdev
 * @ctrl:			pointer to ctrl
 */
static int ps5268_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = ps5268_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = ps5268_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = ps5268_get_max_exp_for_fps(sd, ctrl);
			break;

		default:
			pr_err("%s cmd:%d not support\n", __func__, ctrl->id);
			ret = -1;
			break;
	}

	return ret;
}

/*
 * XXXX_set_isp_timing_cb -
 * set isp timing callback function
 * private callback functions
 *
 * @sd:				subdev
 * @ctrl:			pointer to ctrl
 */
static int ps5268_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ps5268_priv *priv = to_ps5268(client);
	struct isp_timing_cb_info *isp_timing_cb_info = (void *)ctrl->value;

	memcpy(&priv->hcb.isp_timing_cb_info,
			isp_timing_cb_info,
			sizeof(struct isp_timing_cb_info));
	return 0;
}

/*
 * XXXX_set_fps_direct -
 * set sensor fps directly, synchronously.
 * private callback functions
 *
 * @sd:				subdev
 * @ctrl:			pointer to ctrl
 */
static int ps5268_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ps5268_priv *priv = to_ps5268(client);
	int fps = ctrl->value;

	ps5268_sensor_set_fps_func(priv, fps);
	ps5268_set_fps_async(priv);
	return 0;
}

static int ps5268_set_master_or_slave(struct v4l2_subdev *sd, int is_master)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ps5268_priv *priv = to_ps5268(client);

	pr_debug("%s is_master:%d\n",__func__,is_master);

	if (is_master)
		priv->master_or_slave = MASTER_MODE;
	else
		priv->master_or_slave = SLAVER_MODE;

	return 0;
}

/*
 * XXXX_core_s_ctrl -
 * set core s_ctrl
 * core functions
 *
 * @sd:				subdev
 * @ctrl:			pointer to ctrl
 */
static int ps5268_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = ps5268_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = ps5268_set_fps_direct(sd, ctrl);
			break;

		case SET_MASTER:/*for dual sensor*/
			ret = ps5268_set_master_or_slave(sd, 1);
			break;

		case SET_SLAVER:/*for dual sensor*/
			ret = ps5268_set_master_or_slave(sd, 0);
			break;

		default:
			pr_err("%s cmd:%d not support\n", __func__, ctrl->id);
			ret = -1;
			break;
	}

	return ret;
}

static int ps5268_store_initial_regs(struct ps5268_priv *priv, void *arg)
{
	struct i2c_client *client = priv->client;
	AK_ISP_SENSOR_INIT_PARA *sensor_para = arg;
	void *p_reg_info;
	int ret;

	p_reg_info = sensor_para->reg_info;
	sensor_para->reg_info =
		kmalloc(sizeof(AK_ISP_SENSOR_REG_INFO) * sensor_para->num, GFP_KERNEL);
	if (!sensor_para->reg_info) {
		pr_err("%s alloc1 fail\n", __func__);
		ret = -ENOMEM;
		goto alloc1_fail;
	} else {
		if (copy_from_user(sensor_para->reg_info, p_reg_info,
					sizeof(AK_ISP_SENSOR_REG_INFO) * sensor_para->num)) {
			pr_err("%s cp2 fail\n", __func__);
			ret = -EFAULT;
			goto cp2_fail;
		}

		if (priv->para.reg_info) {
			devm_kfree(&client->dev, priv->para.reg_info);
			priv->para.reg_info = NULL;
			priv->para.num = 0;
		}

		/*store sensor configurtion*/
		priv->para.num = sensor_para->num;
		priv->para.reg_info = devm_kzalloc(&client->dev,
				sizeof(AK_ISP_SENSOR_REG_INFO)*sensor_para->num, GFP_KERNEL);
		if (!priv->para.reg_info) {
			pr_err("%s alloc2 fail\n", __func__);
			goto alloc2_fail;
		}
		memcpy(priv->para.reg_info, sensor_para->reg_info,
				sizeof(AK_ISP_SENSOR_REG_INFO)*sensor_para->num);

		pr_debug("%s para.num;%d\n", __func__, priv->para.num);
	}

	kfree(sensor_para->reg_info);
	return 0;

alloc2_fail:
cp2_fail:
	kfree(sensor_para->reg_info);
alloc1_fail:
	return ret;
}

static long ps5268_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ps5268_priv *priv = to_ps5268(client);
	int ret = 0;

	pr_debug("%s\n", __func__);

	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			ps5268_store_initial_regs(priv, arg);
			break;

		case AK_SENSOR_GET_MAX_EXP_FOR_FPS: {
			struct sensor_max_exp_for_fps *exp_for_fps = arg;
			struct v4l2_control ctrl;

			ctrl.value = exp_for_fps->fps;
			ret = ps5268_get_max_exp_for_fps(sd, &ctrl);
			if (!ret)
				exp_for_fps->max_exp = ctrl.value;
			break;
		}

		default:
			ret = -EINVAL;
			break;
	}

	return ret;
}

/*
 * XXXX_get_fmt -
 * get format
 * pad functions
 *
 * @sd:				subdev
 * @cfg:			pointer to pad config
 * @format:			return format
 */
static int ps5268_get_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct ps5268_priv *priv = to_ps5268(client);
	static struct ps5268_win_size win = {
		.name = "default",
		.width = SENSOR_OUTPUT_WIDTH,
		.height = SENSOR_OUTPUT_HEIGHT,
		.regs = NULL,
	};

	if (format->pad)
		return -EINVAL;

	if (!priv->win) {
		priv->win = &win;
		priv->cfmt_code = MEDIA_BUS_FMT_UYVY8_2X8;
	}

	mf->width	= priv->win->width;
	mf->height	= priv->win->height;
	mf->code	= priv->cfmt_code;

	switch (mf->code) {
	case MEDIA_BUS_FMT_RGB565_2X8_BE:
	case MEDIA_BUS_FMT_RGB565_2X8_LE:
		mf->colorspace = V4L2_COLORSPACE_SRGB;
		break;
	default:
	case MEDIA_BUS_FMT_YUYV8_2X8:
	case MEDIA_BUS_FMT_UYVY8_2X8:
		mf->colorspace = V4L2_COLORSPACE_JPEG;
	}
	mf->field	= V4L2_FIELD_NONE;

	return 0;
}

/*
 * XXXX_set_fmt -
 * set format
 * pad functions
 *
 * @sd:				subdev
 * @cfg:			pointer to pad config
 * @format:			format to set
 */
static int ps5268_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;

	if (format->pad)
		return -EINVAL;

	mf->field	= V4L2_FIELD_NONE;

	switch (mf->code) {
	case MEDIA_BUS_FMT_RGB565_2X8_BE:
	case MEDIA_BUS_FMT_RGB565_2X8_LE:
		mf->colorspace = V4L2_COLORSPACE_SRGB;
		break;
	default:
		mf->code = MEDIA_BUS_FMT_UYVY8_2X8;
	case MEDIA_BUS_FMT_YUYV8_2X8:
	case MEDIA_BUS_FMT_UYVY8_2X8:
		mf->colorspace = V4L2_COLORSPACE_JPEG;
	}

	if (cfg)
		cfg->try_fmt = *mf;
	return 0;
}

/*
 * XXXX_set_fmt -
 * set format
 * pad functions
 *
 * @sd:				subdev
 * @cfg:			pointer to pad config
 * @sel:			return selection
 */
static int ps5268_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_selection *sel)
{
	sel->r.left = SENSOR_VALID_OFFSET_X;
	sel->r.top = SENSOR_VALID_OFFSET_Y;
	sel->r.width = SENSOR_OUTPUT_WIDTH;
	sel->r.height = SENSOR_OUTPUT_HEIGHT;

	return 0;
}

/*
 * XXXX_enum_bus -
 * enum bus
 * pad functions
 *
 * @sd:				subdev
 * @cfg:			pointer to pad config
 * @code:			return code of bus type
 */
static int ps5268_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(ps5268_codes))
		return -EINVAL;

	code->code = MEDIA_BUS_FMT_YUYV8_2X8;

	return 0;
}

/*
 * XXXX_g_crop -
 * get crop
 * video functions
 *
 * @sd:				subdev
 * @a:				return crop
 */
static int ps5268_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
	a->c.left	= SENSOR_VALID_OFFSET_X;
	a->c.top	= SENSOR_VALID_OFFSET_Y;
	a->c.width	= SENSOR_OUTPUT_WIDTH;
	a->c.height	= SENSOR_OUTPUT_HEIGHT;
	a->type		= V4L2_BUF_TYPE_VIDEO_CAPTURE;

	return 0;
}

/*
 * XXXX_cropcap -
 * get crop capbilities
 * video functions
 *
 * @sd:				subdev
 * @a:				return crop capbilities
 */
static int ps5268_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	ps5268_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
	a->bounds.left			= SENSOR_VALID_OFFSET_X;
	a->bounds.top			= SENSOR_VALID_OFFSET_Y;
	a->defrect			= a->bounds;
	a->type				= V4L2_BUF_TYPE_VIDEO_CAPTURE;
	a->pixelaspect.numerator	= 1;
	a->pixelaspect.denominator	= 1;

	pr_err("%s bounds.width:%d, bounds.height:%d\n", __func__, a->bounds.width, a->bounds.height);

	return 0;
}

/*
 * XXXX_video_probe -
 * do some process in subdev probe
 * locale functions
 *
 * @sd:				subdev
 * @a:				return crop capbilities
 */
static int ps5268_video_probe(struct i2c_client *client)
{
	struct ps5268_priv *priv = to_ps5268(client);
	int ret;

	ret = v4l2_ctrl_handler_setup(&priv->hdl);

	return ret;
}

/*ctrl ops*/
static const struct v4l2_ctrl_ops ps5268_ctrl_ops = {
	.g_volatile_ctrl	= ps5268_g_volatile_ctrl,
	.s_ctrl				= ps5268_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops ps5268_subdev_core_ops = {
	.s_power	= ps5268_core_s_power,
	.g_ctrl		= ps5268_core_g_ctrl,
	.s_ctrl		= ps5268_core_s_ctrl,
	.ioctl		= ps5268_core_ioctl,
};

/*
 * XXXX_g_mbus_config -
 * get buf config
 * video functions
 *
 * @sd:				subdev
 * @cfg:			return config
 */
static int ps5268_g_mbus_config(struct v4l2_subdev *sd,
				struct v4l2_mbus_config *cfg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	cfg->flags = V4L2_MBUS_PCLK_SAMPLE_RISING | V4L2_MBUS_MASTER |
		V4L2_MBUS_VSYNC_ACTIVE_HIGH | V4L2_MBUS_HSYNC_ACTIVE_HIGH |
		V4L2_MBUS_DATA_ACTIVE_HIGH;
	cfg->type = V4L2_MBUS_PARALLEL;
	cfg->flags = 0;

	return 0;
}

/*
 * XXXX_s_crop -
 * set crop
 * video functions
 *
 * @sd:				subdev
 * @crop:			crop to set
 */
static int ps5268_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
	printk(KERN_ERR "%s %d, left:%d, top:%d, width:%d, height:%d\n",
			__func__, __LINE__, crop->c.left, crop->c.top, crop->c.width, crop->c.height);
	return 0;
}

/*video ops*/
static struct v4l2_subdev_video_ops ps5268_subdev_video_ops = {
	.s_stream	= ps5268_s_stream,
	.cropcap	= ps5268_cropcap,
	.g_crop		= ps5268_g_crop,
	.g_mbus_config	= ps5268_g_mbus_config,
	.s_crop		= ps5268_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops ps5268_subdev_pad_ops = {
	.enum_mbus_code = ps5268_enum_mbus_code,
	.get_fmt	= ps5268_get_fmt,
	.set_fmt	= ps5268_set_fmt,
	.get_selection	= ps5268_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops ps5268_subdev_ops = {
	.core	= &ps5268_subdev_core_ops,
	.video	= &ps5268_subdev_video_ops,
	.pad	= &ps5268_subdev_pad_ops,
};

/*
 * sensor_of_parse -
 * parse node of device
 *
 * @client:			pointor to i2c client
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct ps5268_priv *priv)
{
	struct device_node *np = client->dev.of_node;

	/*it is exist reset but lack of pwdn gpio in most case*/
	priv->gpio_reset = of_get_named_gpio(np, "reset-gpio", 0);
	priv->gpio_pwdn = of_get_named_gpio(np, "pwdn-gpio", 0);

	if (priv->gpio_reset >= 0) {
		devm_gpio_request(&client->dev, priv->gpio_reset, "sensor-reset");
	}

	if (priv->gpio_pwdn >= 0) {
		devm_gpio_request(&client->dev, priv->gpio_pwdn, "sensor-pwdn");
	}

	return 0;
}

static int ps5268_probe_addr(struct ps5268_priv *priv)
{
	struct i2c_client *client = priv->client;
	int pid;

	priv->cb_info.cb->sensor_set_power_on_func(priv);

	SENSOR_I2C_ADDR = SENSOR_I2C_ADDR0;
	client->addr = SENSOR_I2C_ADDR;
	pid = priv->cb_info.cb->sensor_probe_id_func(priv);
	if (pid > 0) {
		pr_info("%s get addr:0x%x\n", __func__, client->addr);
		return 0;
	}

	SENSOR_I2C_ADDR = SENSOR_I2C_ADDR1;
	client->addr = SENSOR_I2C_ADDR;
	pid = priv->cb_info.cb->sensor_probe_id_func(priv);
	if (pid > 0) {
		pr_info("%s get addr:0x%x\n", __func__, client->addr);
		return 0;
	}

	pr_err("%s get addr fail\n", __func__);
	return -EINVAL;
}

/*
 * XXXX_match - check sensor id for match driver
 * @priv:			pointer to pirvate structure
 *
 * @RETURN:	0-match fail; 1-match success
 */
static int ps5268_match(struct ps5268_priv *priv)
{
	int pid;

	if (!check_id)
		return 1;

	priv->cb_info.cb->sensor_set_power_on_func(priv);
	pid = priv->cb_info.cb->sensor_probe_id_func(priv);
	if (pid <= 0) {
		pr_err("%s fail\n", __func__);
		return 0;
	}

	return 1;
}

/*
 * XXXX_probe -
 * driver probe after platform probe ok
 * i2c_driver functions
 *
 * @client:			pointor to i2c client
 * @did:			driver ids
 */
static int ps5268_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	struct ps5268_priv	*priv;
	struct i2c_adapter	*adapter = to_i2c_adapter(client->dev.parent);
	int			ret;

	pr_err("%s %s %s\n", __func__, __DATE__, __TIME__);

	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"ps5268: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct ps5268_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
			"Failed to allocate memory for private data!\n");
		return -ENOMEM;
	}

	/*parse of node*/
	sensor_of_parse(client, priv);

	/*
	 * the current sensor slave address to client.
	 * the address from dts may incorrect
	 * */
	client->addr = SENSOR_I2C_ADDR;
	priv->cb_info.cb = &ps5268_cb;
	priv->cb_info.arg = priv;
	priv->client = client;

	if (!client->dev.of_node) {
		dev_err(&client->dev, "Missing platform_data for driver\n");
		ret = -EINVAL;
		goto err_clk;
	}

	ps5268_probe_addr(priv);

	if (!ps5268_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &ps5268_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &ps5268_ctrl_ops,
			V4L2_CID_VFLIP, 0, 1, 1, 0);
	v4l2_ctrl_new_std(&priv->hdl, &ps5268_ctrl_ops,
			V4L2_CID_HFLIP, 0, 1, 1, 0);

	/*private ctrl*/
	priv->ctrl_sensor_get_id = v4l2_ctrl_new_custom(&priv->hdl, &config_sensor_get_id, NULL);
	if (!priv->ctrl_sensor_get_id)
		pr_err("creat ctrl_sensor_get_id fail\n");

	priv->subdev.ctrl_handler = &priv->hdl;
	if (priv->hdl.error) {
		dev_err(&client->dev, "Hdl error\n");
		ret = priv->hdl.error;
		goto err_clk;
	}

	ret = ps5268_video_probe(client);
	if (ret < 0) {
		dev_err(&client->dev, "OV2640 probe fail\n");
		goto err_videoprobe;
	}

	ret = v4l2_async_register_subdev(&priv->subdev);
	if (ret < 0) {
		dev_err(&client->dev, "v4l2 async register subdev fail, ret:%d\n", ret);
		goto err_videoprobe;
	}

	priv->master_or_slave = SINGLE_MODE;

	dev_err(&adapter->dev, "ps5268 Probed success, subdev:%p\n", &priv->subdev);

	/*ak platform need export some informations from sensor*/
	call_sensor_sys_init();

	return 0;

err_videoprobe:
	v4l2_ctrl_handler_free(&priv->hdl);
err_clk:
	return ret;
}

/*
 * XXXX_remove -
 * driver probe after platform remove ok
 * i2c_driver functions
 *
 * @client:			pointor to i2c client
 */
static int ps5268_remove(struct i2c_client *client)
{
	struct ps5268_priv       *priv = to_ps5268(client);

	pr_err("%s %d\n", __func__, __LINE__);

	if (!priv) {
		pr_err("%s had remove\n", __func__);
		return 0;
	}

	/*unregister async subdev*/
	v4l2_async_unregister_subdev(&priv->subdev);
	/*unregister subdev*/
	v4l2_device_unregister_subdev(&priv->subdev);
	/*unregister handler*/
	v4l2_ctrl_handler_free(&priv->hdl);
	/*unregister sys node*/
	call_sensor_sys_deinit();
	return 0;
}

static const struct i2c_device_id ps5268_id[] = {
	{ "ps5268", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, ps5268_id);

static const struct of_device_id ps5268_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, ps5268_of_match);

static struct i2c_driver ps5268_i2c_driver = {
	.driver = {
		.name = "ps5268",
		.of_match_table = of_match_ptr(ps5268_of_match),
	},
	.probe    = ps5268_probe,
	.remove   = ps5268_remove,
	.id_table = ps5268_id,
};

module_i2c_driver(ps5268_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for ps5268 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.2");

/*sensor i2c write*/
static int ps5268_write(const struct i2c_client *client, int reg, int value)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
		.value_bytes = SENSOR_DATA_BYTE,
		.reg = reg,
		.value = value,
	};

	return i2c_write(&trans);
}

/*sensor i2c read*/
static int ps5268_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
		.value_bytes = SENSOR_DATA_BYTE,
		.reg = reg,
		.value = 0,
	};

	pr_err("%s client:%p, addr:%x\n", __func__, client, client->addr);
	return i2c_read(&trans);
}

static void ps5268_dual_init_func(struct ps5268_priv *priv,
		AK_ISP_SENSOR_INIT_PARA *para)
{
	/*
	 * PS5268_Sensor_Control_Guide_v1.4_Anyka.pdf
	 *
	 * 1) Slaver :
	 * (1) 設R_FrameSyncMaster = 0，R_FrameSyncMode = 1。
	 * (2) 設R_FameSyncWait = 0 (單一觸發連續輸出) 或是R_FameSyncWait = 1 (單一觸發單一
	 * 輸出)。
	 * (3) 設定PNS5268 其它參數(解析度、曝光值、R_CSI2_enable=1….等) ＂ with
	 * R_LTM_Bypass=1＂ 。
	 * (4) 設Update flag (Address : 16’h0111 bit_0)值為1。
	 * (5) 設 Sensor_EnH值為1。(啓動Sensor)
	 * (6) 等待外部Fsync觸發訊號。(由低電位轉高電位)
	 * (7) 確認外部Fsync觸發訊號已由低電位轉高電位或者畫面啟動後，若需要開啟LTM時，請
	 * 在此處啟動LTM，即R_LTM_Bypass=0
	 * 2) Master :
	 * (1) 設R_FrameSyncMaster = 1，R_FrameSyncMode = 1。
	 * (2) 設R_FameSyncWait = 0 (單一觸發連續輸出) 或是R_FameSyncWait = 1 (單一觸發單一
	 * 輸出)。
	 * (3) 設定PNS5268 其它參數(解析度、曝光值、R_CSI2_enable=1….等) ＂ with
	 * R_LTM_Bypass=1＂ 。
	 * (4) 設Update flag (Address : 16’h0111 bit_0)值為1。
	 * (5) 設 Sensor_EnH值為1。(啓動Sensor)
	 * (6) 設Cmd_FrameSyncTrg值為1，輸出Fsync觸發訊號。(由低電位轉高電位)。
	 * (7) 若需要開啟LTM時，請在此處啟動LTM，即R_LTM_Bypass=0
	 * (8) 設Cmd_FrameSyncTrg值為0，清除Fsync觸發訊號。(由高電位轉低電位)。
	 * (9) 為單一觸發單一輸出時，若需要重新觸發，請重覆步驟 (6)、(8)。
	 */

	struct i2c_client *client = priv->client;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	int value;
	int i;

	priv->dual.init_count++;

	if (priv->master_or_slave == SLAVER_MODE) {
		pr_debug("%s slaver init_count:%d\n",__func__,priv->dual.init_count);

		if (priv->dual.init_count == 1) {
			value = 0;
			value &= ~(1<<2);//R_FrameSyncMaster
			value |= 1;//R_FrameSyncMode
			value &= ~(1<<1);//R_FrameSyncWait:单一触发连续输出
			ps5268_write(client, 0x0177, value);

			preg_info = para->reg_info;
			for (i = 0; i < para->num; i++) {
				if (preg_info->reg_addr == 0x010f)//sensor_enh
					;
				else if (preg_info->reg_addr == 0x0111)//update flag
					;
				else if (preg_info->reg_addr == 0x1300)//R_LTM_Bypass
					ps5268_write(client, preg_info->reg_addr, 0x01);
				else
					ps5268_write(client, preg_info->reg_addr, preg_info->value);

				preg_info++;
			}

			/*update flag*/
			value = ps5268_read(client, 0x0111);
			value |= 1;
			ps5268_write(client, 0x0111, value);

			/*sensor_enh*/
			ps5268_write(client, 0x010f, 0x01);
		} else if (priv->dual.init_count == 2) {
			/*master had output frame & sync signal, so now to set R_LTM_Bypass*/
			preg_info = para->reg_info;
			for (i = 0; i < para->num; i++) {
				if (preg_info->reg_addr == 0x1300)//R_LTM_Bypass
					ps5268_write(client, preg_info->reg_addr, preg_info->value);
				else
					;

				preg_info++;
			}
		}
	} else {//MASTER_MODE
		/*
		 * when
		 * priv->dual.init_count == 1
		 * do nothing.
		 *
		 * wait slaver ready first then master
		 */
		pr_debug("%s master init_count:%d\n",__func__,priv->dual.init_count);
		if (priv->dual.init_count == 2) {
			AK_ISP_SENSOR_REG_INFO *r_ltm_bypass = NULL;

			value = 0;
			value |= 1<<2;//R_FrameSyncMaster
			value |= 1;//R_FrameSyncMode
			value &= ~(1<<1);//R_FrameSyncWait:单一触发连续输出
			ps5268_write(client, 0x0177, value);

			preg_info = para->reg_info;
			for (i = 0; i < para->num; i++) {
				if (preg_info->reg_addr == 0x010f)//sensor_enh
					;
				else if (preg_info->reg_addr == 0x0111)//update flag
					;
				else if (preg_info->reg_addr == 0x1300) {//R_LTM_Bypass
					ps5268_write(client, preg_info->reg_addr, 0x01);
					r_ltm_bypass = preg_info;
				} else
					ps5268_write(client, preg_info->reg_addr, preg_info->value);

				preg_info++;
			}

			/*update flag*/
			value = ps5268_read(client, 0x0111);
			value |= 1;
			ps5268_write(client, 0x0111, value);

			/*sensor_enh*/
			ps5268_write(client, 0x010f, 0x01);

			/*Cmd_FrameSyncTrg*/
			value = ps5268_read(client, 0x0176);
			value |= 1;
			ps5268_write(client, 0x0176, value);

			/*R_LTM_Bypass*/
			ps5268_write(client, r_ltm_bypass->reg_addr, r_ltm_bypass->value);

			/*Cmd_FrameSyncTrg*/
			value = ps5268_read(client, 0x0176);
			value &= ~(1);
			ps5268_write(client, 0x0176, value);
		}
	}
}

/*
 * The sensor must set callbacks
 *
 * */
static int ps5268_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct ps5268_priv *priv = arg;
	struct i2c_client *client = priv->client;
	AK_ISP_SENSOR_INIT_PARA *para = &priv->para;

	pr_debug("%s\n", __func__);

	if (para->num <= 0 && npara)
		para = (void *)npara;

	if (priv->master_or_slave == SINGLE_MODE) {
		preg_info = para->reg_info;
		for (i = 0; i < para->num; i++) {
#if 0
			{
				int value;

				value = ps5268_read(client, preg_info->reg_addr);
				pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
			}
#endif
			ps5268_write(client, preg_info->reg_addr, preg_info->value);
#if 0
			{
				int value;

				value = ps5268_read(client, preg_info->reg_addr);
				pr_err("reg:%x write value:%x, read back value:%x\n",
						preg_info->reg_addr, preg_info->value, value);
			}
#endif

			preg_info++;
		}
	} else {
		ps5268_dual_init_func(priv, para);
	}

	priv->fps_info.current_fps = MAX_FPS;
	priv->fps_info.to_fps = priv->fps_info.current_fps;
	priv->fps_info.to_fps_value = 0;

	return 0;
}

/*read sensor register*/
static int ps5268_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct ps5268_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return ps5268_read(client, reg_addr);
}
/*write sensor register*/
static int ps5268_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct ps5268_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return ps5268_write(client, reg_addr, value);
}
/*read sensor register, NO i2c ops*/
static int ps5268_sensor_read_id_func(void *arg)   //no use IIC bus
{
	return SENSOR_ID;
}

static int aec_parms_init(struct ps5268_priv *priv)
{
	priv->aec_parms.sghd_flg = 0;
	priv->aec_parms.total_lines = INITIAL_TOTAL_LINES;

	return 0;
}

/*set sensor again*/
static int ps5268_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	struct ps5268_priv *priv = arg;
	struct i2c_client *client = priv->client;
	unsigned int i;
	unsigned int gain_idx/*, sghd_patch = 1*/;
	int gain = a_gain;//1x gain, gain=1024.

	gain = gain<<2;

	/*Check gain*/
	if (gain > ps5268_gain_table[96])
		gain = ps5268_gain_table[96];
	else if(gain < ps5268_gain_table[0])
		gain = ps5268_gain_table[0];

	/*search most suitable gain into gain table*/
	for (i=0; i<96; i++) {
		if (ps5268_gain_table[i]> gain)
			break;
	}

	/*Check HS/LS mode*/
	if(ps5268_gain_table[i] > AG_HS_NODE)
	{
		priv->aec_parms.sghd_flg = 0;	// HS Mode
	}
	else if(ps5268_gain_table[i] < AG_LS_NODE)
	{
		priv->aec_parms.sghd_flg = 1;	// LS Mode
	}

	/*Calculate gain index*/
	gain_idx = (priv->aec_parms.sghd_flg == 0)? (i-16):i;

	ps5268_write(client, 0x012B, gain_idx);
	ps5268_write(client, 0x0128, priv->aec_parms.sghd_flg);
	ps5268_write(client, 0x0111, 0x01);//update flag

	return A_GAIN_EFFECT_FRAMES;
}

/*set sensor dgain*/
static int ps5268_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	return D_GAIN_EFFECT_FRAMES;
}
/*set sensor exp time*/
static int ps5268_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	struct ps5268_priv *priv = arg;
	struct i2c_client *client = priv->client;
	unsigned int ny, g_lines;
	int lines = exp_time;
	int total_lines = priv->aec_parms.total_lines;

	g_lines = ((total_lines - 4) > lines) ?
		((4 < lines)?lines:4):(total_lines - 4);	// 4 <= Ny <= LPF-3
	ny = total_lines - g_lines;		// ne_patch update.

	ps5268_write(client, 0x0118, (ny>>8)&0xFF);	//Cmd_OffNy1[15:8]
	ps5268_write(client, 0x0119, ny&0xFF);	//Cmd_OffNy1[7:0]
	ps5268_write(client, 0x0111, 0x01);//update flag

	return EXP_EFFECT_FRAMES;
}
/*sensor timer*/
static int ps5268_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int ps5268_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int ps5268_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int ps5268_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct ps5268_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	int value;

	value = ps5268_read(client, 0x0100);
	id = value << 8;

	value = ps5268_read(client, 0x0101);
	id |= value;

	printk(KERN_ERR "id:%x\n", id);

	if (id == SENSOR_ID)
		return SENSOR_ID;

	return 0;
}
/*get resolution*/
static int ps5268_sensor_get_resolution_func(void *arg, int *width, int *height)
{
	*width = SENSOR_OUTPUT_WIDTH;
	*height = SENSOR_OUTPUT_HEIGHT;
	return 0;
}
/*get mclk*/
static int ps5268_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int ps5268_sensor_get_fps_func(void *arg)
{
	struct ps5268_priv *priv = arg;

	return priv->fps_info.current_fps;
}
/*get valid coordinate*/
static int ps5268_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
	*x = SENSOR_VALID_OFFSET_X;
	*y = SENSOR_VALID_OFFSET_Y;
	return 0;
}
/*get bus type*/
static enum sensor_bus_type ps5268_sensor_get_bus_type_func(void *arg)
{
	return SENSOR_BUS_TYPE;
}
/*get self definded params*/
static int ps5268_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct ps5268_priv *priv = arg;

	switch (param) {
		case GET_INTERFACE:
			*((int *)value) = dvp ? DVP_INTERFACE:MIPI_INTERFACE;
			break;

		case GET_IO_LEVEL:
			*((int *)value) = SENSOR_IO_LEVEL;
			break;

		case GET_MIPI_MHZ:
			*((int *)value) = MIPI_MBPS;
			break;

		case GET_MIPI_LANES:
			*((int *)value) = MIPI_LANES;
			break;

		case GET_RESET_GPIO:
			*((int *)value) = priv->gpio_reset;
			break;

		case GET_PWDN_GPIO:
			*((int *)value) = priv->gpio_pwdn;
			break;

		default:
			pr_err("%s param:%d not support\n", __func__, param);
			ret = -1;
			break;
	}

	return ret;
}
/*sensor power on*/
static int ps5268_sensor_set_power_on_func(void *arg)
{
	struct ps5268_priv *priv = arg;

	if (priv->gpio_pwdn >= 0) {
		gpio_direction_output(priv->gpio_pwdn, !SENSOR_PWDN_LEVEL);
	}

	if (priv->gpio_reset >= 0) {
		gpio_direction_output(priv->gpio_reset, !SENSOR_RESET_LEVEL);
		msleep(50);
		gpio_direction_output(priv->gpio_reset, SENSOR_RESET_LEVEL);
		msleep(50);
		gpio_direction_output(priv->gpio_reset, !SENSOR_RESET_LEVEL);
		msleep(100);
	}

	aec_parms_init(priv);

	priv->dual.init_count = 0;

	return 0;
}
/*sensor power off*/
static int ps5268_sensor_set_power_off_func(void *arg)
{
	struct ps5268_priv *priv = arg;

	if (priv->gpio_pwdn >= 0) {
		gpio_direction_input(priv->gpio_pwdn);
	}

	if (priv->gpio_reset >= 0) {
		gpio_direction_input(priv->gpio_reset);
	}

	return 0;
}

static int ps5268_fps_to_vts(struct ps5268_priv *priv, const int fps)
{
	int vts;

	/* vts = 80M / (bank1,(reg0x27~28)/2=2110) / fps */
	switch (fps) {
		case 30:
			vts = 0x0464;
			break;
		case 25:
			vts = 0x0545;
			break;
		case 20:
			vts = 0x0696;
			break;
		case 15:
			vts = 0x08c9;
			break;
		case 14://14.28
			vts = 0x0939;//2361.3445378151260504201680672269
			break;
		case 12:
			vts = 0x0afb;
			break;
		case 10:
			vts = 0x0d2e;
			break;
		case 8:
			vts = 0x1079;
			break;
		default:
			pr_err("%s set fps=%d fail\n", __func__, fps);
			vts = 0;
			break;
	}

	return vts;
}

/*set sensor fps*/
static int ps5268_sensor_set_fps_func(void *arg, const int fps)
{
	struct ps5268_priv *priv = arg;
	int tmp;


	tmp = ps5268_fps_to_vts(priv, fps);

	if (tmp > 0) {
		priv->fps_info.to_fps = fps;
		priv->fps_info.to_fps_value = tmp;
	} else {
		return -EINVAL;
	}

	return 0;
}

static void ps5268_set_fps_async(struct ps5268_priv *priv)
{
	struct i2c_client *client = priv->client;
	int to_fps_value = priv->fps_info.to_fps_value;

	if (priv->fps_info.to_fps != priv->fps_info.current_fps) {
		ps5268_write(client, 0x0116, to_fps_value>>8);
		ps5268_write(client, 0x0117, to_fps_value & 0xff);
		ps5268_write(client, 0x0111, 0x01);

		priv->aec_parms.total_lines = to_fps_value;	// LPF

		priv->fps_info.current_fps = priv->fps_info.to_fps;
	}
}

static AK_ISP_SENSOR_CB ps5268_cb = {
	.sensor_init_func = ps5268_sensor_init_func,
	.sensor_read_reg_func = ps5268_sensor_read_reg_func,
	.sensor_write_reg_func = ps5268_sensor_write_reg_func,
	.sensor_read_id_func = ps5268_sensor_read_id_func,
	.sensor_update_a_gain_func = ps5268_sensor_update_a_gain_func,
	.sensor_update_d_gain_func = ps5268_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func = ps5268_sensor_updata_exp_time_func,
	.sensor_timer_func = ps5268_sensor_timer_func,
	.sensor_set_standby_in_func = ps5268_sensor_set_standby_in_func,
	.sensor_set_standby_out_func = ps5268_sensor_set_standby_out_func,
	.sensor_probe_id_func = ps5268_sensor_probe_id_func,
	.sensor_get_resolution_func = ps5268_sensor_get_resolution_func,
	.sensor_get_mclk_func = ps5268_sensor_get_mclk_func,
	.sensor_get_fps_func = ps5268_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func = ps5268_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func = ps5268_sensor_get_bus_type_func,
	.sensor_get_parameter_func = ps5268_sensor_get_parameter_func,
	.sensor_set_power_on_func = ps5268_sensor_set_power_on_func,
	.sensor_set_power_off_func = ps5268_sensor_set_power_off_func,
	.sensor_set_fps_func = ps5268_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return ps5268_sensor_read_id_func(NULL);
}

static char *sensor_if_func(void)
{
	static char ifstr[16] = "dvp";

	if (!dvp)
		sprintf(ifstr, "mipi%d", MIPI_LANES);

	return ifstr;
}

static int call_sensor_sys_init(void)
{
	return sensor_sys_init(sensor_id_func, sensor_if_func);
}

static int call_sensor_sys_deinit(void)
{
	sensor_sys_exit();
	return 0;
}

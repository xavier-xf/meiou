...
#define MIPI_MBPS				312
#define MIPI_LANES				2
#define MAX_FPS_MIPI			25
#define MAX_FPS_DVP				25
static int MAX_FPS =			MAX_FPS_MIPI;
...
static const struct v4l2_ctrl_ops mis2006_ctrl_ops;
static const struct v4l2_ctrl_config config_sensor_get_id = {
	.ops	= &mis2006_ctrl_ops,
	.id		= SENSOR_GET_ID,
	.name	= "get sensor id",
...
struct mis2006_priv {
	...
	u32	cfmt_code;
	const struct mis2006_win_size	*win;
	...
	struct mis2006_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB mis2006_cb;
...
static void mis2006_set_fps_async(struct mis2006_priv *priv);
static int __mis2006_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int mis2006_sensor_read_id_func(void *arg);   //no use IIC bus
static int mis2006_sensor_get_resolution_func(void *arg, int *width, int *height);
static int mis2006_sensor_set_fps_func(void *arg, const int fps);
static int mis2006_fps_to_vts(struct mis2006_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 mis2006_codes[] = {
 ...
 * General functions
 */
static struct mis2006_priv *to_mis2006(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct mis2006_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct mis2006_priv, hdl)->subdev;
}
 ...
 * @enable:			enable flags
 */
static int mis2006_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int mis2006_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int mis2006_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int mis2006_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2006_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = mis2006_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2006_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2006_priv *priv = to_mis2006(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int mis2006_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2006_priv *priv = to_mis2006(client);
	int fps = ctrl->value;
	int vts = mis2006_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2006_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = mis2006_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = mis2006_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = mis2006_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2006_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2006_priv *priv = to_mis2006(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2006_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2006_priv *priv = to_mis2006(client);
	int fps = ctrl->value;

	mis2006_sensor_set_fps_func(priv, fps);
	mis2006_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int mis2006_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = mis2006_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = mis2006_set_fps_direct(sd, ctrl);
			break;
...
static int mis2006_store_initial_regs(struct mis2006_priv *priv, void *arg)
{
...
static long mis2006_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct mis2006_priv *priv = to_mis2006(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			mis2006_store_initial_regs(priv, arg);
			break;
 ...
 */
static int mis2006_get_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct mis2006_priv *priv = to_mis2006(client);
	static struct mis2006_win_size win = {
		.name = "default",
 ...
 */
static int mis2006_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
 ...
 */
static int mis2006_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
 ...
 */
static int mis2006_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(mis2006_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int mis2006_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int mis2006_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	mis2006_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int mis2006_video_probe(struct i2c_client *client)
{
	struct mis2006_priv *priv = to_mis2006(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops mis2006_ctrl_ops = {
	.g_volatile_ctrl	= mis2006_g_volatile_ctrl,
	.s_ctrl				= mis2006_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops mis2006_subdev_core_ops = {
	.s_power	= mis2006_core_s_power,
	.g_ctrl		= mis2006_core_g_ctrl,
	.s_ctrl		= mis2006_core_s_ctrl,
	.ioctl		= mis2006_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int mis2006_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int mis2006_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops mis2006_subdev_video_ops = {
	.s_stream	= mis2006_s_stream,
	.cropcap	= mis2006_cropcap,
	.g_crop		= mis2006_g_crop,
	.g_mbus_config	= mis2006_g_mbus_config,
	.s_crop		= mis2006_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops mis2006_subdev_pad_ops = {
	.enum_mbus_code = mis2006_enum_mbus_code,
	.get_fmt	= mis2006_get_fmt,
	.set_fmt	= mis2006_set_fmt,
	.get_selection	= mis2006_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops mis2006_subdev_ops = {
	.core	= &mis2006_subdev_core_ops,
	.video	= &mis2006_subdev_video_ops,
	.pad	= &mis2006_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct mis2006_priv *priv)
{
 ...
 */
static int mis2006_match(struct mis2006_priv *priv)
{
	int pid;
 ...
 * @did:			driver ids
 */
static int mis2006_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	struct mis2006_priv	*priv;
	...
	if (dvp)
		MAX_FPS = MAX_FPS_MIPI;

	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"mis2006: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct mis2006_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!mis2006_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &mis2006_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &mis2006_ctrl_ops,
	...
	ret = mis2006_video_probe(client);
	if (ret < 0) {
		dev_err(&client->dev, "mis2006 probe fail\n");
		goto err_videoprobe;
	}
	...
	dev_err(&adapter->dev, "mis2006 Probed success, subdev:%p\n", &priv->subdev);
 ...
 * @client:			pointor to i2c client
 */
static int mis2006_remove(struct i2c_client *client)
{
	struct mis2006_priv       *priv = to_mis2006(client);
...
static const struct i2c_device_id mis2006_id[] = {
	{ "mis2006", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, mis2006_id);

static const struct of_device_id mis2006_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, mis2006_of_match);

static struct i2c_driver mis2006_i2c_driver = {
	.driver = {
		.name = "mis2006",
		.of_match_table = of_match_ptr(mis2006_of_match),
	},
	.probe    = mis2006_probe,
	.remove   = mis2006_remove,
	.id_table = mis2006_id,
};

module_i2c_driver(mis2006_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for mis2006 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.5");

/*sensor i2c write*/
static int mis2006_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int mis2006_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
 ...
 *
 * */
static int mis2006_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct mis2006_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = mis2006_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		mis2006_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
    ...
    priv->aec_parms.reg_frame_vts =
		mis2006_read(client,0x3200) << 8 | mis2006_read(client,0x3201);
	priv->aec_parms.reg_frame_hts =
		mis2006_read(client,0x3202) << 8 | mis2006_read(client,0x3203);
	priv->aec_parms.pclk_freq = priv->aec_parms.reg_frame_hts *
		priv->aec_parms.reg_frame_vts * MAX_FPS;
	printk("%s hts %d vts %d pclk %d\n",
			__func__,
			priv->aec_parms.reg_frame_hts,
			priv->aec_parms.reg_frame_vts,
...
static int mis2006_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct mis2006_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return mis2006_read(client, reg_addr);
}
/*write sensor register*/
static int mis2006_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct mis2006_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return mis2006_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct mis2006_priv *priv)
{
...
/*set sensor again*/
static int mis2006_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	...
	mis2006_write(client, 0x3102, (corse_gain<<5)|(fine_gain&0x1f));
...
/*set sensor dgain*/
static int mis2006_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	return D_GAIN_EFFECT_FRAMES;
}

static int __set_reg_exp_time(struct mis2006_priv *priv, int exp_time)
{
	int ret = 0;
	struct i2c_client *client = priv->client;
	...
	mis2006_write(client, 0x3100, exposure_time_msb);
	mis2006_write(client, 0x3101, exposure_time_lsb);
...
static int __set_reg_frame_vts(struct mis2006_priv *priv, int vts)
{
	int ret = 0;
	struct i2c_client *client = priv->client;

	mis2006_write(client, 0x3200, (vts >> 8) & 0xff);
	mis2006_write(client, 0x3201, vts & 0xff);
...
static int __get_reg_frame_vts(struct mis2006_priv *priv)
{
	struct i2c_client *client = priv->client;

	if (0 == priv->fps_info.reg_fps_value) {
		priv->fps_info.reg_fps_value = mis2006_read(client, 0x3200) << 8 | mis2006_read(client, 0x3201);
	}
	return priv->fps_info.reg_fps_value;
}

static void update_frame_vts_and_exp_ctrl(struct mis2006_priv *priv)
{
	struct i2c_client *client = priv->client;
    ...
    int const max_exp_ctrl      = priv->fps_info.to_fps_value - 1;   //Vts - 1
...
/*set sensor exp time*/
static int mis2006_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
...
/*sensor timer*/
static int mis2006_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int mis2006_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int mis2006_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int mis2006_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct mis2006_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = mis2006_read(client, 0x3000);
	id = value << 8;

	value = mis2006_read(client, 0x3001);
	id |= value;
...
/*get mclk*/
static int mis2006_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int mis2006_sensor_get_fps_func(void *arg)
{
	struct mis2006_priv *priv = arg;
...
static int mis2006_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type mis2006_sensor_get_bus_type_func(void *arg)
{
...
static int mis2006_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct mis2006_priv *priv = arg;
...
/*sensor power on*/
static int mis2006_sensor_set_power_on_func(void *arg)
{
	struct mis2006_priv *priv = arg;
...
/*sensor power off*/
static int mis2006_sensor_set_power_off_func(void *arg)
{
	struct mis2006_priv *priv = arg;
...
static int mis2006_fps_to_vts(struct mis2006_priv *priv, const int fps)
{
	...
	vts = vts * 100;
	vts = vts / fps_tmp ;
	priv->aec_parms.reg_frame_vts = vts;
...
static int mis2006_sensor_set_fps_func(void *arg, const int fps)
{
	...
	tmp = mis2006_fps_to_vts(priv, fps);
...
static AK_ISP_SENSOR_CB mis2006_cb = {
	.sensor_init_func = mis2006_sensor_init_func,
	.sensor_read_reg_func = mis2006_sensor_read_reg_func,
	.sensor_write_reg_func = mis2006_sensor_write_reg_func,
	.sensor_read_id_func = mis2006_sensor_read_id_func,
	.sensor_update_a_gain_func = mis2006_sensor_update_a_gain_func,
	.sensor_update_d_gain_func = mis2006_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func = mis2006_sensor_updata_exp_time_func,
	.sensor_timer_func = mis2006_sensor_timer_func,
	.sensor_set_standby_in_func = mis2006_sensor_set_standby_in_func,
	.sensor_set_standby_out_func = mis2006_sensor_set_standby_out_func,
	.sensor_probe_id_func = mis2006_sensor_probe_id_func,
	.sensor_get_resolution_func = mis2006_sensor_get_resolution_func,
	.sensor_get_mclk_func = mis2006_sensor_get_mclk_func,
	.sensor_get_fps_func = mis2006_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func = mis2006_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func = mis2006_sensor_get_bus_type_func,
	.sensor_get_parameter_func = mis2006_sensor_get_parameter_func,
	.sensor_set_power_on_func = mis2006_sensor_set_power_on_func,
	.sensor_set_power_off_func = mis2006_sensor_set_power_off_func,
	.sensor_set_fps_func = mis2006_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return mis2006_sensor_read_id_func(NULL);
}

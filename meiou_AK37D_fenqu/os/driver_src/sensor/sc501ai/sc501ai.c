...
struct sc501ai_priv {
	...
	u32	cfmt_code;
	const struct sc501ai_win_size	*win;
	...
	struct sc501ai_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB sc501ai_cb;
...
static void sc501ai_set_fps_async(struct sc501ai_priv *priv);
static int __sc501ai_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int sc501ai_sensor_read_id_func(void *arg);   //no use IIC bus
static int sc501ai_sensor_get_resolution_func(void *arg, int *width, int *height);
static int sc501ai_sensor_set_fps_func(void *arg, const int fps);
static int sc501ai_fps_to_vts(struct sc501ai_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 sc501ai_codes[] = {
 ...
 * General functions
 */
static struct sc501ai_priv *to_sc501ai(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct sc501ai_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct sc501ai_priv, hdl)->subdev;
}
 ...
 * @enable:			enable flags
 */
static int sc501ai_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int sc501ai_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int sc501ai_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int sc501ai_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc501ai_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = sc501ai_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc501ai_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc501ai_priv *priv = to_sc501ai(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int sc501ai_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc501ai_priv *priv = to_sc501ai(client);
	int fps = ctrl->value;
	int vts = sc501ai_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc501ai_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = sc501ai_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = sc501ai_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = sc501ai_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc501ai_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc501ai_priv *priv = to_sc501ai(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc501ai_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc501ai_priv *priv = to_sc501ai(client);
	int fps = ctrl->value;

	sc501ai_sensor_set_fps_func(priv, fps);
	sc501ai_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc501ai_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = sc501ai_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = sc501ai_set_fps_direct(sd, ctrl);
			break;
...
static int sc501ai_store_initial_regs(struct sc501ai_priv *priv, void *arg)
{
...
static long sc501ai_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc501ai_priv *priv = to_sc501ai(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			sc501ai_store_initial_regs(priv, arg);
			break;
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct sc501ai_priv *priv = to_sc501ai(client);
	static struct sc501ai_win_size win = {
		.name = "default",
 ...
 */
static int sc501ai_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
...
{
	if (code->pad || code->index >= ARRAY_SIZE(sc501ai_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int sc501ai_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int sc501ai_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	sc501ai_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int sc501ai_video_probe(struct i2c_client *client)
{
	struct sc501ai_priv *priv = to_sc501ai(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops sc501ai_ctrl_ops = {
	.g_volatile_ctrl	= sc501ai_g_volatile_ctrl,
	.s_ctrl				= sc501ai_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops sc501ai_subdev_core_ops = {
	.s_power	= sc501ai_core_s_power,
	.g_ctrl		= sc501ai_core_g_ctrl,
	.s_ctrl		= sc501ai_core_s_ctrl,
	.ioctl		= sc501ai_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int sc501ai_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int sc501ai_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops sc501ai_subdev_video_ops = {
	.s_stream	= sc501ai_s_stream,
	.cropcap	= sc501ai_cropcap,
	.g_crop		= sc501ai_g_crop,
	.g_mbus_config	= sc501ai_g_mbus_config,
	.s_crop		= sc501ai_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops sc501ai_subdev_pad_ops = {
	.enum_mbus_code = sc501ai_enum_mbus_code,
	.get_fmt	= sc501ai_get_fmt,
	.set_fmt	= sc501ai_set_fmt,
	.get_selection	= sc501ai_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops sc501ai_subdev_ops = {
	.core	= &sc501ai_subdev_core_ops,
	.video	= &sc501ai_subdev_video_ops,
	.pad	= &sc501ai_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct sc501ai_priv *priv)
{
 ...
 */
static int sc501ai_match(struct sc501ai_priv *priv)
{
	int pid;
			...
			const struct i2c_device_id *did)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"sc501ai: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct sc501ai_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!sc501ai_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &sc501ai_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &sc501ai_ctrl_ops,
	...
	ret = sc501ai_video_probe(client);
	if (ret < 0) {
	...
	dev_err(&adapter->dev, "sc501ai Probed success, subdev:%p\n", &priv->subdev);
...
module_i2c_driver(sc501ai_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc501ai sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.1");

/*sensor i2c write*/
static int sc501ai_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int sc501ai_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
...
static void sc501ai_agc_mode(struct sc501ai_priv *priv)
{
	int value;
	struct i2c_client *client = priv->client;

	value = sc501ai_read(client, 0x3e03) & 0xf0;
	value |= 0xb;
	sc501ai_write(client, 0x3e03, value);
}
 ...
 *
 * */
static int sc501ai_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	/*cleaned_0x54_SC500AI_24MInput_MIPI_2lane_396Mbps_10bit_2880x1620_15fps.ini*/
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct sc501ai_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = sc501ai_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		sc501ai_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
...
#if 0
	int value;
	value = sc501ai_read(client, 0x320e)&0x7f;
	pr_err("reg:%04x , read back value:%04x\n", 0x320e, value);

	value = sc501ai_read(client, 0x320f)&0xff;
	pr_err("reg:%04x , read back value:%04x\n", 0x320f, value);

	value = sc501ai_read(client, 0x320c)&0xff;
	pr_err("reg:%04x , read back value:%04x\n", 0x320c, value);

	value = sc501ai_read(client, 0x320d)&0xff;
	pr_err("reg:%04x , read back value:%04x\n", 0x320d, value);
#endif
	return 0;
}
...
static int sc501ai_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct sc501ai_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc501ai_read(client, reg_addr);
}
/*write sensor register*/
static int sc501ai_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct sc501ai_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc501ai_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct sc501ai_priv *priv)
{
...
/*set sensor again*/
static int sc501ai_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	...
	if(a_gain<=384) {
		 ...
		 * */
		corse_gain = 0x3;
		fine_gain = 0x40 + (a_gain - 256) / 4; /*128/32=4*/
		//blooming_dc = 0x05;
		//precharge_lag = 0x1b;
	} else if(a_gain<=768) {
		/*
		 ...
		 * */
		corse_gain = 0x23;
		fine_gain = 0x40 + (a_gain - 1 - 384) / 6;/*384/64=6*/
		//precharge_lag = 0x18;
	} else if(a_gain<=1536) {
		/*
		 * (3,6]
		 ...
		 * */
		corse_gain = 0x27;
		fine_gain = 0x40 + (a_gain - 1 - 768) / 12;/*768/64=12*/
		//precharge_lag = 0x18;
	} else if(a_gain<=3072) {
		/*
		 * (6,12]
		 ...
		 * */
		corse_gain = 0x2f;
		fine_gain = 0x40 + (a_gain - 1 - 1536) / 24;/*1536/64=24*/
		//precharge_lag = 0x10;
	}  else if(a_gain<=6144) {
		/*
		 * (12,24]
		 ...
		 * */
		corse_gain = 0x3f;
		fine_gain = 0x40 + (a_gain - 1 - 3072) / 48;/*3072/64=48*/
		//precharge_lag = 0x10;
	} else {
		corse_gain = 0x3f;
		fine_gain = 0x7f;
	...
	sc501ai_write(client, 0x3e08, corse_gain);
	sc501ai_write(client, 0x3e09, fine_gain);

#if 0
	sc501ai_write(client, 0x3812, 0x00);
	//sc501ai_write(client, 0x363c, blooming_dc);
	//sc501ai_write(client, 0x330e, precharge_lag);
	if (a_gain >= 1536)/*6x*/
		sc501ai_write(client, 0x5799, 0x07);
	else if (a_gain <= 1024)/*4x*/
		sc501ai_write(client, 0x5799, 0x00);
	sc501ai_write(client, 0x3812, 0x30);
#endif

	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int sc501ai_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	/*
	...
	sc501ai_write(client, 0x3e06, corse_gain);
	sc501ai_write(client, 0x3e07, fine_gain);

	return D_GAIN_EFFECT_FRAMES;
}
/*set sensor exp time*/
static int __set_reg_exp_time(struct sc501ai_priv *priv, int exp_time)
{
	int ret = 0;
	struct i2c_client *client = priv->client;
	...
	sc501ai_write(client, 0x3e00, exposure_time_ext);
	sc501ai_write(client, 0x3e01, exposure_time_msb);
	sc501ai_write(client, 0x3e02, exposure_time_lsb);
...
static int __set_reg_frame_vts(struct sc501ai_priv *priv, int vts)
{
	int ret = 0;
	struct i2c_client *client = priv->client;

	sc501ai_write(client, 0x320e, (vts >> 8) & 0x7f);
	sc501ai_write(client, 0x320f, vts & 0xff);
...
static int __get_reg_frame_vts(struct sc501ai_priv *priv)
{
	struct i2c_client *client = priv->client;

	return (sc501ai_read(client, 0x320e) & 0x7f) << 8 | sc501ai_read(client, 0x320f);
}

static void update_frame_vts_and_exp_ctrl(struct sc501ai_priv *priv)
{
	struct i2c_client *client = priv->client;
...
/*set sensor exp time*/
static int sc501ai_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	struct sc501ai_priv *priv = arg;
	struct i2c_client *client = priv->client;
...
/*sensor timer*/
static int sc501ai_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int sc501ai_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int sc501ai_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int sc501ai_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct sc501ai_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = sc501ai_read(client, 0x3107);
	id = value << 8;

	value = sc501ai_read(client, 0x3108);
	id |= value;
...
/*get mclk*/
static int sc501ai_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int sc501ai_sensor_get_fps_func(void *arg)
{
	struct sc501ai_priv *priv = arg;
...
static int sc501ai_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type sc501ai_sensor_get_bus_type_func(void *arg)
{
...
static int sc501ai_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct sc501ai_priv *priv = arg;
...
/*sensor power on*/
static int sc501ai_sensor_set_power_on_func(void *arg)
{
	struct sc501ai_priv *priv = arg;
...
/*sensor power off*/
static int sc501ai_sensor_set_power_off_func(void *arg)
{
	struct sc501ai_priv *priv = arg;
...
static int sc501ai_sensor_set_fps_func(void *arg, const int fps)
{
	...
	tmp = sc501ai_fps_to_vts(priv, fps);
...
static AK_ISP_SENSOR_CB sc501ai_cb = {
	.sensor_init_func							= sc501ai_sensor_init_func,
	.sensor_read_reg_func						= sc501ai_sensor_read_reg_func,
	.sensor_write_reg_func						= sc501ai_sensor_write_reg_func,
	.sensor_read_id_func						= sc501ai_sensor_read_id_func,
	.sensor_update_a_gain_func					= sc501ai_sensor_update_a_gain_func,
	.sensor_update_d_gain_func					= sc501ai_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func				= sc501ai_sensor_updata_exp_time_func,
	.sensor_timer_func							= sc501ai_sensor_timer_func,
	.sensor_set_standby_in_func					= sc501ai_sensor_set_standby_in_func,
	.sensor_set_standby_out_func				= sc501ai_sensor_set_standby_out_func,
	.sensor_probe_id_func						= sc501ai_sensor_probe_id_func,
	.sensor_get_resolution_func					= sc501ai_sensor_get_resolution_func,
	.sensor_get_mclk_func						= sc501ai_sensor_get_mclk_func,
	.sensor_get_fps_func						= sc501ai_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func			= sc501ai_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func					= sc501ai_sensor_get_bus_type_func,
	.sensor_get_parameter_func					= sc501ai_sensor_get_parameter_func,
	.sensor_set_power_on_func					= sc501ai_sensor_set_power_on_func,
	.sensor_set_power_off_func					= sc501ai_sensor_set_power_off_func,
	.sensor_set_fps_func						= sc501ai_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return sc501ai_sensor_read_id_func(NULL);
}

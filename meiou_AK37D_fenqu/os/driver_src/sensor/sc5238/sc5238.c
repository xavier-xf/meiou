...
#define SENSOR_ID				0x5238
#define SENSOR_MCLK				24
#define SENSOR_REGADDR_BYTE		2
#define SENSOR_DATA_BYTE		1
#define SENSOR_OUTPUT_WIDTH		2592
#define SENSOR_OUTPUT_HEIGHT	1944
...
static const struct v4l2_ctrl_ops sc5238_ctrl_ops;
...
struct sc5238_priv {
	...
	u32	cfmt_code;
	const struct sc5238_win_size	*win;
	...
	struct sc5238_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB sc5238_cb;
...
static void sc5238_set_fps_async(struct sc5238_priv *priv);
static int __sc5238_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int sc5238_sensor_read_id_func(void *arg);   //no use IIC bus
static int sc5238_sensor_get_resolution_func(void *arg, int *width, int *height);
static int sc5238_sensor_set_fps_func(void *arg, const int fps);
static int sc5238_fps_to_vts(struct sc5238_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 sc5238_codes[] = {
 ...
 * General functions
 */
static struct sc5238_priv *to_sc5238(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct sc5238_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct sc5238_priv, hdl)->subdev;
}
 ...
 * @enable:			enable flags
 */
static int sc5238_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int sc5238_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int sc5238_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int sc5238_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5238_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = sc5238_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5238_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5238_priv *priv = to_sc5238(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int sc5238_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5238_priv *priv = to_sc5238(client);
	int fps = ctrl->value;
	int vts = sc5238_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5238_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = sc5238_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = sc5238_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = sc5238_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5238_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5238_priv *priv = to_sc5238(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5238_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5238_priv *priv = to_sc5238(client);
	int fps = ctrl->value;

	sc5238_sensor_set_fps_func(priv, fps);
	sc5238_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5238_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = sc5238_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = sc5238_set_fps_direct(sd, ctrl);
			break;
...
static int sc5238_store_initial_regs(struct sc5238_priv *priv, void *arg)
{
...
static long sc5238_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5238_priv *priv = to_sc5238(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			sc5238_store_initial_regs(priv, arg);
			break;
 ...
 */
static int sc5238_get_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct sc5238_priv *priv = to_sc5238(client);
	static struct sc5238_win_size win = {
		.name = "default",
 ...
 */
static int sc5238_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
 ...
 */
static int sc5238_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
 ...
 */
static int sc5238_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(sc5238_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int sc5238_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int sc5238_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	sc5238_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int sc5238_video_probe(struct i2c_client *client)
{
	struct sc5238_priv *priv = to_sc5238(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops sc5238_ctrl_ops = {
	.g_volatile_ctrl	= sc5238_g_volatile_ctrl,
	.s_ctrl				= sc5238_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops sc5238_subdev_core_ops = {
	.s_power	= sc5238_core_s_power,
	.g_ctrl		= sc5238_core_g_ctrl,
	.s_ctrl		= sc5238_core_s_ctrl,
	.ioctl		= sc5238_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int sc5238_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int sc5238_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops sc5238_subdev_video_ops = {
	.s_stream	= sc5238_s_stream,
	.cropcap	= sc5238_cropcap,
	.g_crop		= sc5238_g_crop,
	.g_mbus_config	= sc5238_g_mbus_config,
	.s_crop		= sc5238_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops sc5238_subdev_pad_ops = {
	.enum_mbus_code = sc5238_enum_mbus_code,
	.get_fmt	= sc5238_get_fmt,
	.set_fmt	= sc5238_set_fmt,
	.get_selection	= sc5238_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops sc5238_subdev_ops = {
	.core	= &sc5238_subdev_core_ops,
	.video	= &sc5238_subdev_video_ops,
	.pad	= &sc5238_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct sc5238_priv *priv)
{
 ...
 */
static int sc5238_match(struct sc5238_priv *priv)
{
	int pid;
 ...
 * @did:			driver ids
 */
static int sc5238_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	struct sc5238_priv	*priv;
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"sc5238: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct sc5238_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!sc5238_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &sc5238_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &sc5238_ctrl_ops,
	...
	dev_err(&adapter->dev, "sc5238 Probed success, subdev:%p\n", &priv->subdev);
 ...
 * @client:			pointor to i2c client
 */
static int sc5238_remove(struct i2c_client *client)
{
	struct sc5238_priv       *priv = to_sc5238(client);
...
static const struct i2c_device_id sc5238_id[] = {
	{ "sc5238", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, sc5238_id);

static const struct of_device_id sc5238_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, sc5238_of_match);

static struct i2c_driver sc5238_i2c_driver = {
	.driver = {
		.name = "sc5238",
		.of_match_table = of_match_ptr(sc5238_of_match),
	},
	.probe    = sc5238_probe,
	.remove   = sc5238_remove,
	.id_table = sc5238_id,
};

module_i2c_driver(sc5238_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc5238 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.1");

/*sensor i2c write*/
static int sc5238_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int sc5238_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
...
static void sc5238_agc_mode(struct sc5238_priv *priv)
{
	int value;
	struct i2c_client *client = priv->client;

	value = sc5238_read(client, 0x3e03) & 0xf0;
	value |= 0xb;
	sc5238_write(client, 0x3e03, value);
}
 ...
 *
 * */
static int sc5238_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	/*cleaned_0x20_SC5335P_MIPI_24MInput_2lane_396Mbps_10bit_2600x1952_12.5fps.ini*/
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct sc5238_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = sc5238_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		sc5238_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
	...
	sc5238_agc_mode(priv);

	priv->aec_parms.reg_frame_hts =
		sc5238_read(client,0x320c) << 8 | sc5238_read(client,0x320d);
	priv->aec_parms.reg_frame_vts =
		sc5238_read(client,0x320e) << 8 | sc5238_read(client,0x320f);
	priv->aec_parms.pclk_freq = priv->aec_parms.reg_frame_hts *
		priv->aec_parms.reg_frame_vts * MAX_FPS;
	pr_debug("%s hts %d vts %d pclk %d\n",
			__func__,
			priv->aec_parms.reg_frame_hts,
			priv->aec_parms.reg_frame_vts,
...
static int sc5238_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct sc5238_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc5238_read(client, reg_addr);
}
/*write sensor register*/
static int sc5238_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct sc5238_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc5238_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct sc5238_priv *priv)
{
...
/*set sensor again*/
static int sc5238_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	...
	sc5238_write(client, 0x3e08, corse_gain);
	sc5238_write(client, 0x3e09, fine_gain);

	sc5238_write(client,0x3812,0x00);
	if(corse_gain == 0x03) {

		sc5238_write(client,0x3301,0x18);
		sc5238_write(client,0x3630,0x30);
		sc5238_write(client,0x3633,0x23);
		sc5238_write(client,0x3622,0xf6);
		sc5238_write(client,0x363a,0x83);

	}else if(corse_gain == 0x07){

		sc5238_write(client,0x3301,0x20);
		sc5238_write(client,0x3630,0x23);
		sc5238_write(client,0x3633,0x33);
		sc5238_write(client,0x3622,0xf6);
		sc5238_write(client,0x363a,0x87);

	}else if(corse_gain == 0x0f){

		sc5238_write(client,0x3301,0x28);
		sc5238_write(client,0x3630,0x24);
		sc5238_write(client,0x3633,0x43);
		sc5238_write(client,0x3622,0xf6);
		sc5238_write(client,0x363a,0x9f);

	}else if(corse_gain == 0x1f && fine_gain < 0x3f){

		sc5238_write(client,0x3301,0x38);
		sc5238_write(client,0x3630,0x28);
		sc5238_write(client,0x3633,0x43);
		sc5238_write(client,0x3622,0xf6);
		sc5238_write(client,0x363a,0x9f);
		...
		sc5238_write(client,0x3301,0x40);
		sc5238_write(client,0x3630,0x19);
		sc5238_write(client,0x3633,0x55);
		sc5238_write(client,0x3622,0x16);
		sc5238_write(client,0x363a,0x9f);

	}
	sc5238_write(client,0x3812,0x30);

	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int sc5238_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	/*
	 * max dgain < 16 times = 2048
	 * */
	struct sc5238_priv *priv = arg;
	struct i2c_client *client = priv->client;
	unsigned int corse_gain, fine_gain;
	...
	sc5238_write(client, 0x3e06, corse_gain);
	sc5238_write(client, 0x3e07, fine_gain);

	return D_GAIN_EFFECT_FRAMES;
}
/*set sensor exp time*/
static int sc5238_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	/*
	 * exp_time=1 mean half line exptime
	 * max exp_time = {R0x320e[6:0],R0x320f} * 2 - 10
	 *
	 * now:
	 * at max_exptime_15fps
	 * = {R0x320e[6:0],R0x320f} * 2 - 10
	 * = 2000 * 2 - 10
	 * = 4000 - 10
	 * = 3990
	 */
	...
	exp_time = exp_time <= priv->fps_info.to_fps_value * 2 - 4 ?
	   	exp_time : priv->fps_info.to_fps_value * 2 - 4;

	pr_debug("%s, exp:%d\n", __func__, exp_time);
	exposure_time_ext =(exp_time>>12)&0xf;
	exposure_time_msb =(exp_time>>4)&0xff;
	exposure_time_lsb =((exp_time)&0xf)<<4;
	//tem=sc5238_read(client, 0x3e02)&0x0f;
	//exposure_time_lsb =exposure_time_lsb|tem;
	sc5238_write(client, 0x3e01,exposure_time_msb);
	sc5238_write(client, 0x3e02,exposure_time_lsb);

	//tem = sc5238_read(client, 0x3e00) & 0xf0;
	//tem |= exposure_time_ext;
	sc5238_write(client, 0x3e00, exposure_time_ext);
...
/*sensor timer*/
static int sc5238_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int sc5238_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int sc5238_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int sc5238_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct sc5238_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = sc5238_read(client, 0x3107);
	id = value << 8;

	value = sc5238_read(client, 0x3108);
	id |= value;
...
/*get mclk*/
static int sc5238_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int sc5238_sensor_get_fps_func(void *arg)
{
	struct sc5238_priv *priv = arg;
...
static int sc5238_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type sc5238_sensor_get_bus_type_func(void *arg)
{
...
static int sc5238_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct sc5238_priv *priv = arg;
...
/*sensor power on*/
static int sc5238_sensor_set_power_on_func(void *arg)
{
	struct sc5238_priv *priv = arg;
...
/*sensor power off*/
static int sc5238_sensor_set_power_off_func(void *arg)
{
	struct sc5238_priv *priv = arg;
...
static int sc5238_fps_to_vts(struct sc5238_priv *priv, const int fps)
{
	 ...
	 * mipi:
	 * HTS=(0x320c,0x320d)*2
	 * VTS=(0x300e[6:0],0x320f)
	 * fps = pclk / HTS / VTS
	 *
	 * now pclk=79200000hz
	 * HTS=0x578*2=1400*2=2800
	 * VTS=0x7d0=2000 at 15fps
	 *
	 * so :
	 * vts = 84000000 / 2800 / fps = 30000 / fps
	 *
	 */
	int vts;
    int fps_tmp;
...
static int sc5238_sensor_set_fps_func(void *arg, const int fps)
{
	...
	tmp = sc5238_fps_to_vts(priv, fps);
...
static void sc5238_set_fps_async(struct sc5238_priv *priv)
{
	...
	if (priv->fps_info.to_fps != priv->fps_info.current_fps) {
		...
		sc5238_write(client, 0x320e, (value >> 8) & 0x7f);
		sc5238_write(client, 0x320f, value & 0xff);
		priv->fps_info.current_fps = priv->fps_info.to_fps;
	}
}

static AK_ISP_SENSOR_CB sc5238_cb = {
	.sensor_init_func = sc5238_sensor_init_func,
	.sensor_read_reg_func = sc5238_sensor_read_reg_func,
	.sensor_write_reg_func = sc5238_sensor_write_reg_func,
	.sensor_read_id_func = sc5238_sensor_read_id_func,
	.sensor_update_a_gain_func = sc5238_sensor_update_a_gain_func,
	.sensor_update_d_gain_func = sc5238_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func = sc5238_sensor_updata_exp_time_func,
	.sensor_timer_func = sc5238_sensor_timer_func,
	.sensor_set_standby_in_func = sc5238_sensor_set_standby_in_func,
	.sensor_set_standby_out_func = sc5238_sensor_set_standby_out_func,
	.sensor_probe_id_func = sc5238_sensor_probe_id_func,
	.sensor_get_resolution_func = sc5238_sensor_get_resolution_func,
	.sensor_get_mclk_func = sc5238_sensor_get_mclk_func,
	.sensor_get_fps_func = sc5238_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func = sc5238_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func = sc5238_sensor_get_bus_type_func,
	.sensor_get_parameter_func = sc5238_sensor_get_parameter_func,
	.sensor_set_power_on_func = sc5238_sensor_set_power_on_func,
	.sensor_set_power_off_func = sc5238_sensor_set_power_off_func,
	.sensor_set_fps_func = sc5238_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return sc5238_sensor_read_id_func(NULL);
}

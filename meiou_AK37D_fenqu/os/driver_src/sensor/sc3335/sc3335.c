...
#define MIPI_MBPS				337	/*original 333MHZ, now raise 100MHZ*/
#define MIPI_LANES				2
#define MAX_FPS					20
...
static const struct v4l2_ctrl_ops sc3335_ctrl_ops;
...
struct sc3335_priv {
	...
	u32	cfmt_code;
	const struct sc3335_win_size	*win;
	...
	struct sc3335_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB sc3335_cb;
...
static void sc3335_set_fps_async(struct sc3335_priv *priv);
static int __sc3335_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int sc3335_sensor_read_id_func(void *arg);   //no use IIC bus
static int sc3335_sensor_get_resolution_func(void *arg, int *width, int *height);
static int sc3335_sensor_set_fps_func(void *arg, const int fps);
static int sc3335_fps_to_vts(struct sc3335_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 sc3335_codes[] = {
 ...
 * General functions
 */
static struct sc3335_priv *to_sc3335(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct sc3335_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct sc3335_priv, hdl)->subdev;
}
 ...
 * @enable:			enable flags
 */
static int sc3335_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int sc3335_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int sc3335_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int sc3335_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc3335_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = sc3335_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc3335_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc3335_priv *priv = to_sc3335(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int sc3335_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc3335_priv *priv = to_sc3335(client);
	int fps = ctrl->value;
	int vts = sc3335_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc3335_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = sc3335_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = sc3335_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = sc3335_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc3335_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc3335_priv *priv = to_sc3335(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc3335_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc3335_priv *priv = to_sc3335(client);
	int fps = ctrl->value;

	sc3335_sensor_set_fps_func(priv, fps);
	sc3335_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc3335_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = sc3335_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = sc3335_set_fps_direct(sd, ctrl);
			break;
...
static int sc3335_store_initial_regs(struct sc3335_priv *priv, void *arg)
{
...
static long sc3335_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc3335_priv *priv = to_sc3335(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			sc3335_store_initial_regs(priv, arg);
			break;
 ...
 */
static int sc3335_get_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct sc3335_priv *priv = to_sc3335(client);
	static struct sc3335_win_size win = {
		.name = "default",
 ...
 */
static int sc3335_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
 ...
 */
static int sc3335_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
 ...
 */
static int sc3335_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(sc3335_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int sc3335_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int sc3335_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	sc3335_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int sc3335_video_probe(struct i2c_client *client)
{
	struct sc3335_priv *priv = to_sc3335(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops sc3335_ctrl_ops = {
	.g_volatile_ctrl	= sc3335_g_volatile_ctrl,
	.s_ctrl				= sc3335_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops sc3335_subdev_core_ops = {
	.s_power	= sc3335_core_s_power,
	.g_ctrl		= sc3335_core_g_ctrl,
	.s_ctrl		= sc3335_core_s_ctrl,
	.ioctl		= sc3335_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int sc3335_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int sc3335_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops sc3335_subdev_video_ops = {
	.s_stream	= sc3335_s_stream,
	.cropcap	= sc3335_cropcap,
	.g_crop		= sc3335_g_crop,
	.g_mbus_config	= sc3335_g_mbus_config,
	.s_crop		= sc3335_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops sc3335_subdev_pad_ops = {
	.enum_mbus_code = sc3335_enum_mbus_code,
	.get_fmt	= sc3335_get_fmt,
	.set_fmt	= sc3335_set_fmt,
	.get_selection	= sc3335_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops sc3335_subdev_ops = {
	.core	= &sc3335_subdev_core_ops,
	.video	= &sc3335_subdev_video_ops,
	.pad	= &sc3335_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct sc3335_priv *priv)
{
 ...
 */
static int sc3335_match(struct sc3335_priv *priv)
{
	int pid;
 ...
 * @did:			driver ids
 */
static int sc3335_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	struct sc3335_priv	*priv;
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"sc3335: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct sc3335_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!sc3335_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &sc3335_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &sc3335_ctrl_ops,
	...
	dev_err(&adapter->dev, "sc3335 Probed success, subdev:%p\n", &priv->subdev);
...
static const struct of_device_id sc3335_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, sc3335_of_match);

static struct i2c_driver sc3335_i2c_driver = {
	.driver = {
		.name = "sc3335",
		.of_match_table = of_match_ptr(sc3335_of_match),
	},
	.probe    = sc3335_probe,
...
module_i2c_driver(sc3335_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc3335 sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.7");

/*sensor i2c write*/
static int sc3335_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int sc3335_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
...
static void sc3335_agc_mode(struct sc3335_priv *priv)
{
	int value;
	struct i2c_client *client = priv->client;

	value = sc3335_read(client, 0x3e03) & 0xf0;
	value |= 0xb;
	sc3335_write(client, 0x3e03, value);
}
 ...
 *
 * */
static int sc3335_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct sc3335_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = sc3335_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		sc3335_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
...
static int sc3335_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct sc3335_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc3335_read(client, reg_addr);
}
/*write sensor register*/
static int sc3335_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct sc3335_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc3335_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct sc3335_priv *priv)
{
	struct i2c_client *client = priv->client;
	...
	priv->aec_parms.version = sc3335_read(client, 0x3109);
	return 0;
}

/*set sensor again*/
static int sc3335_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	 ...
	 * 2020/09/15
	 *
	 * 一、 SC3335 增益切换时，在 frame start 后需要写入如下逻辑：
	 * （当增益不切换时，请勿执行以下逻辑）
	 * i2c_write 0x3812,0x00;
	 * i2c_write 0x363c, value1; //按照.ini 配置文件，根据不同的模拟增益值调整寄存器的值
	 * i2c_write 0x330e, value2; //按照.ini 配置文件，根据不同的模拟增益值调整寄存器的值
	 * i2c_write 0x3812,0x30;
	 * 说明：
	 * 0x3812 为 group hold 寄存器，其操作方式为：
	 * a). i2c_write: 0x3812,0x00
	 * b). i2c_write: address1, value1;
	 * i2c_write: address2, value2;
	 * …
	 * c). i2c_write: 0x3812,0x30
	 * 注意：
	 * 1) Group hold 不支持空操作，即：不支持只有操作 a 和 c，而无操作 b。
	 * 2) 一帧内不允许操作 2 次 group hold。
	 * 3) Group hold 操作不能放在 sensor 初始化序列内； Sensor 初始化序列结束后需要加 10ms 延时。
	 * 4) 对于无法支持“在 frame start 之后进行 i2c 操作”的主控，每帧执行一次 group hold 操作是
	 * 有风险的，此处建议：每隔几帧执行一次逻辑操作（会出现增益、逻辑不匹配情况，但不会出异常帧）；
	 * 或者不使用 group hold 操作，直接写增益逻辑寄存器（需要客户端实测增益逻辑切换点的亮度变动风
	 * 险）。具体需要和 FAE 沟通
	 *
	 * 二、高温 DPC 逻辑，主要是优化高温噪声。需要添加以下逻辑：
	 * 每帧、或者每隔几帧执行以下操作：
	 * if (TotalGain>=15.875x) {
	 * i2c_write 0x5799 0x07
	 * }
	 * else if (TotalGain <=10x) {
	 * i2c_write 0x5799 0x00
	 * }
	 *
	 *
	 * 20201123接到思特威梁工微信：高温逻辑由10倍进16倍出改为4倍进6倍出。
	 *
	 */
	...
	if(a_gain<512) {
		 ...
		 * */
		corse_gain = 0x3;
		fine_gain = 0x40 + (a_gain - 256) / 4; /*256/64=4*/
		blooming_dc = 0x05;
		precharge_lag = 0x1b;
	} else if(a_gain<1024) {
		/*
		 ...
		 * */
		corse_gain = 0x7;
		fine_gain = 0x40 + (a_gain - 512) / 8;/*512/64=8*/
		precharge_lag = 0x18;
	} else if(a_gain<2048) {
		/*
		 * [4,8)
		 ...
		 * */
		corse_gain = 0xf;
		fine_gain = 0x40 + (a_gain - 1024) / 16;/*1024/64=16*/
		precharge_lag = 0x18;
	} else if(a_gain<4096) {
		/*
		 * [8,16)
		 ...
		 * */
		corse_gain = 0x1f;
		fine_gain = 0x40 + (a_gain - 2048) / 32;/*2048/64=32*/
		precharge_lag = 0x10;
	} else {
		corse_gain = 0x1f;
		fine_gain = 0x7f;
	...
	if (priv->aec_parms.version != 1)
		blooming_dc++;

	sc3335_write(client, 0x3e08, corse_gain);
	sc3335_write(client, 0x3e09, fine_gain);

	sc3335_write(client, 0x3812, 0x00);
	sc3335_write(client, 0x363c, blooming_dc);
	sc3335_write(client, 0x330e, precharge_lag);
	if (a_gain >= 1536)/*6x*/
		sc3335_write(client, 0x5799, 0x07);
	else if (a_gain <= 1024)/*4x*/
		sc3335_write(client, 0x5799, 0x00);
	sc3335_write(client, 0x3812, 0x30);

	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int sc3335_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	/*
	...
	if(d_gain<512) {
		 ...
		 * */
		corse_gain = 0x0;
		fine_gain = 0x80 + (d_gain - 256) / 2;/*256/(0x100-0x80)=2*/
		fine_gain &= 0xfe;
	} else if(d_gain<1024) {
		/*
		 * [2,4)
		 ...
		 * */
		corse_gain = 0x1;
		fine_gain = 0x80 + (d_gain - 512) / 4;/*512/(0x100-0x80)=4*/
		fine_gain &= 0xfe;
	} else if(d_gain<2048) {
		/*
		 * [4,8)
		 ...
		 * */
		corse_gain = 0x3;
		fine_gain = 0x80 + (d_gain - 1024) / 8;/*1024/(0x100-0x80)=8*/
		fine_gain &= 0xfe;
	} else if(d_gain<4096) {
		/*
		 * [8,16)
		 ...
		 * */
		corse_gain = 0x7;
		fine_gain = 0x80 + (d_gain - 2048) / 16;/*2048/(0x100-0x80)=16*/
		fine_gain &= 0xfe;
	} else if(d_gain<8192) {
		/*
		 * [16,32)
		 ...
		 * */
		corse_gain = 0xf;
		fine_gain = 0x80 + (d_gain - 4096) / 32;/*4096/(0x100-0x80)=32*/
		fine_gain &= 0xfe;
	} else {
		corse_gain = 0xf;
		fine_gain = 0xfe;
	...
	sc3335_write(client, 0x3e06, corse_gain);
	sc3335_write(client, 0x3e07, fine_gain);

	return D_GAIN_EFFECT_FRAMES;
}
/*set sensor exp time*/
static int sc3335_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	/*
	 ...
	 *
	 * now:
	 * at max_exptime_20fps
	 * = {R0x320e[6:0],R0x320f} * 2 - 8
	 * = 0x546 * 2 - 8
	 * = 1350 * 2 - 8
	 * = 2700 - 8
	 * = 2692
	...
	//tem=sc3335_read(client, 0x3e02)&0x0f;
	//exposure_time_lsb =exposure_time_lsb|tem;
	sc3335_write(client, 0x3e01,exposure_time_msb);
	sc3335_write(client, 0x3e02,exposure_time_lsb);

	//tem = sc3335_read(client, 0x3e00) & 0xf0;
	//tem |= exposure_time_ext;
	sc3335_write(client, 0x3e00, exposure_time_ext);
...
/*sensor timer*/
static int sc3335_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int sc3335_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int sc3335_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int sc3335_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct sc3335_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = sc3335_read(client, 0x3107);
	id = value << 8;

	value = sc3335_read(client, 0x3108);
	id |= value;
...
/*get mclk*/
static int sc3335_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int sc3335_sensor_get_fps_func(void *arg)
{
	struct sc3335_priv *priv = arg;
...
static int sc3335_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type sc3335_sensor_get_bus_type_func(void *arg)
{
...
static int sc3335_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct sc3335_priv *priv = arg;
...
/*sensor power on*/
static int sc3335_sensor_set_power_on_func(void *arg)
{
	struct sc3335_priv *priv = arg;
...
/*sensor power off*/
static int sc3335_sensor_set_power_off_func(void *arg)
{
	struct sc3335_priv *priv = arg;
...
static int sc3335_fps_to_vts(struct sc3335_priv *priv, const int fps)
{
	 ...
	 * mipi:
	 * HTS=(0x320c,0x320d)
	 * VTS=(0x300e[6:0],0x320f)*2
	 * fps = pclk / VTS / HTS
	 *
	 * now pclk=mipiclk*lanes/bits=333.75M*2/10=66.75MHz,
	 * HTS=0x4e2*2=1250*2=2500
	 * VTS=0x546=1350 at 20fps
	 *
	 * but I find HTS*VTS*FPS=2500*1350*20=67.5MHZ, so I use the frequcy.
	 *
	 *
	 * dvp:
	 * same as mipi
...
static int sc3335_sensor_set_fps_func(void *arg, const int fps)
{
	...
	tmp = sc3335_fps_to_vts(priv, fps);
...
static void sc3335_set_fps_async(struct sc3335_priv *priv)
{
	...
	if (priv->fps_info.to_fps != priv->fps_info.current_fps) {
		...
		sc3335_write(client, 0x320e, (value >> 8) & 0x7f);
		sc3335_write(client, 0x320f, value & 0xff);
		priv->fps_info.current_fps = priv->fps_info.to_fps;
	}
}

static AK_ISP_SENSOR_CB sc3335_cb = {
	.sensor_init_func = sc3335_sensor_init_func,
	.sensor_read_reg_func = sc3335_sensor_read_reg_func,
	.sensor_write_reg_func = sc3335_sensor_write_reg_func,
	.sensor_read_id_func = sc3335_sensor_read_id_func,
	.sensor_update_a_gain_func = sc3335_sensor_update_a_gain_func,
	.sensor_update_d_gain_func = sc3335_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func = sc3335_sensor_updata_exp_time_func,
	.sensor_timer_func = sc3335_sensor_timer_func,
	.sensor_set_standby_in_func = sc3335_sensor_set_standby_in_func,
	.sensor_set_standby_out_func = sc3335_sensor_set_standby_out_func,
	.sensor_probe_id_func = sc3335_sensor_probe_id_func,
	.sensor_get_resolution_func = sc3335_sensor_get_resolution_func,
	.sensor_get_mclk_func = sc3335_sensor_get_mclk_func,
	.sensor_get_fps_func = sc3335_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func = sc3335_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func = sc3335_sensor_get_bus_type_func,
	.sensor_get_parameter_func = sc3335_sensor_get_parameter_func,
	.sensor_set_power_on_func = sc3335_sensor_set_power_on_func,
	.sensor_set_power_off_func = sc3335_sensor_set_power_off_func,
	.sensor_set_fps_func = sc3335_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return sc3335_sensor_read_id_func(NULL);
}

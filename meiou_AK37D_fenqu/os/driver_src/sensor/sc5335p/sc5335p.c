/*
 * sc5335p (internal model name: sc5332) Camera Driver
 *
 * Copyright anyka
 *
...
static const struct v4l2_ctrl_ops sc5335p_ctrl_ops;
static const struct v4l2_ctrl_config config_sensor_get_id = {
	.ops	= &sc5335p_ctrl_ops,
	.id		= SENSOR_GET_ID,
	.name	= "get sensor id",
...
struct sc5335p_priv {
	...
	u32	cfmt_code;
	const struct sc5335p_win_size	*win;
	...
	struct sc5335p_aec_parms aec_parms;
...
static AK_ISP_SENSOR_CB sc5335p_cb;
...
static void sc5335p_set_fps_async(struct sc5335p_priv *priv);
static int __sc5335p_sensor_probe_id_func(struct i2c_client *client);  //use IIC bus
static int sc5335p_sensor_read_id_func(void *arg);   //no use IIC bus
static int sc5335p_sensor_get_resolution_func(void *arg, int *width, int *height);
static int sc5335p_sensor_set_fps_func(void *arg, const int fps);
static int sc5335p_fps_to_vts(struct sc5335p_priv *priv, const int fps);
static int call_sensor_sys_init(void);
static int call_sensor_sys_deinit(void);

static u32 sc5335p_codes[] = {
 ...
 * General functions
 */
static struct sc5335p_priv *to_sc5335p(const struct i2c_client *client)
{
	return container_of(i2c_get_clientdata(client), struct sc5335p_priv,
			    subdev);
}

static struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct sc5335p_priv, hdl)->subdev;
}
 ...
 * @enable:			enable flags
 */
static int sc5335p_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static int sc5335p_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
 ...
 */
static int sc5335p_s_ctrl(struct v4l2_ctrl *ctrl)
{
	return 0;
}
 ...
 * @on:			power flags
 */
static int sc5335p_core_s_power(struct v4l2_subdev *sd, int on)
{
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5335p_get_sensor_id(struct v4l2_control *ctrl)
{
	ctrl->value = sc5335p_sensor_read_id_func(NULL);   //no use IIC bus
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5335p_get_sensor_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5335p_priv *priv = to_sc5335p(client);

	ctrl->value = (int)&priv->cb_info;
	return 0;
}

static int sc5335p_get_max_exp_for_fps(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5335p_priv *priv = to_sc5335p(client);
	int fps = ctrl->value;
	int vts = sc5335p_fps_to_vts(priv, fps);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5335p_core_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case GET_SENSOR_ID:
			ret = sc5335p_get_sensor_id(ctrl);
			break;

		case GET_SENSOR_CB:
			ret = sc5335p_get_sensor_cb(sd, ctrl);
			break;

		case GET_MAX_EXP_FOR_FPS:
			ret = sc5335p_get_max_exp_for_fps(sd, ctrl);
			break;
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5335p_set_isp_timing_cb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5335p_priv *priv = to_sc5335p(client);
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5335p_set_fps_direct(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5335p_priv *priv = to_sc5335p(client);
	int fps = ctrl->value;

	sc5335p_sensor_set_fps_func(priv, fps);
	sc5335p_set_fps_async(priv);
	return 0;
}
 ...
 * @ctrl:			pointer to ctrl
 */
static int sc5335p_core_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	int ret;

	switch (ctrl->id) {
		case SET_ISP_MISC_CALLBACK:
			ret = sc5335p_set_isp_timing_cb(sd, ctrl);
			break;

		case SET_FPS_DIRECT:
			ret = sc5335p_set_fps_direct(sd, ctrl);
			break;
...
static int sc5335p_store_initial_regs(struct sc5335p_priv *priv, void *arg)
{
...
static long sc5335p_core_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct sc5335p_priv *priv = to_sc5335p(client);
	int ret = 0;
	...
	switch (cmd) {
		case AK_SENSOR_SET_INIT:
			sc5335p_store_initial_regs(priv, arg);
			break;
 ...
 */
static int sc5335p_get_fmt(struct v4l2_subdev *sd,
		...
		struct v4l2_subdev_format *format)
{
	...
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
	struct sc5335p_priv *priv = to_sc5335p(client);
	static struct sc5335p_win_size win = {
		.name = "default",
 ...
 */
static int sc5335p_set_fmt(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_format *format)
{
	struct v4l2_mbus_framefmt *mf = &format->format;
 ...
 */
static int sc5335p_get_selection(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
 ...
 */
static int sc5335p_enum_mbus_code(struct v4l2_subdev *sd,
		struct v4l2_subdev_pad_config *cfg,
		struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad || code->index >= ARRAY_SIZE(sc5335p_codes))
		return -EINVAL;
 ...
 * @a:				return crop
 */
static int sc5335p_g_crop(struct v4l2_subdev *sd, struct v4l2_crop *a)
{
 ...
 */
static int sc5335p_cropcap(struct v4l2_subdev *sd, struct v4l2_cropcap *a)
{
	sc5335p_sensor_get_resolution_func(NULL, &a->bounds.width, &a->bounds.height);
 ...
 * @a:				return crop capbilities
 */
static int sc5335p_video_probe(struct i2c_client *client)
{
	struct sc5335p_priv *priv = to_sc5335p(client);
	int ret;
...
/*ctrl ops*/
static const struct v4l2_ctrl_ops sc5335p_ctrl_ops = {
	.g_volatile_ctrl	= sc5335p_g_volatile_ctrl,
	.s_ctrl				= sc5335p_s_ctrl,
};

/*core ops*/
static struct v4l2_subdev_core_ops sc5335p_subdev_core_ops = {
	.s_power	= sc5335p_core_s_power,
	.g_ctrl		= sc5335p_core_g_ctrl,
	.s_ctrl		= sc5335p_core_s_ctrl,
	.ioctl		= sc5335p_core_ioctl,
};
 ...
 * @cfg:			return config
 */
static int sc5335p_g_mbus_config(struct v4l2_subdev *sd,
 ...
 * @crop:			crop to set
 */
static int sc5335p_s_crop(struct v4l2_subdev *sd, const struct v4l2_crop *crop)
{
...
/*video ops*/
static struct v4l2_subdev_video_ops sc5335p_subdev_video_ops = {
	.s_stream	= sc5335p_s_stream,
	.cropcap	= sc5335p_cropcap,
	.g_crop		= sc5335p_g_crop,
	.g_mbus_config	= sc5335p_g_mbus_config,
	.s_crop		= sc5335p_s_crop,
};

/*pad ops*/
static const struct v4l2_subdev_pad_ops sc5335p_subdev_pad_ops = {
	.enum_mbus_code = sc5335p_enum_mbus_code,
	.get_fmt	= sc5335p_get_fmt,
	.set_fmt	= sc5335p_set_fmt,
	.get_selection	= sc5335p_get_selection,
};

/*sensor driver subdev ops*/
static struct v4l2_subdev_ops sc5335p_subdev_ops = {
	.core	= &sc5335p_subdev_core_ops,
	.video	= &sc5335p_subdev_video_ops,
	.pad	= &sc5335p_subdev_pad_ops,
};
 ...
 * @priv:			sensor struct
 */
static int sensor_of_parse(struct i2c_client *client, struct sc5335p_priv *priv)
{
 ...
 */
static int sc5335p_match(struct sc5335p_priv *priv)
{
	int pid;
 ...
 * @did:			driver ids
 */
static int sc5335p_probe(struct i2c_client *client,
			const struct i2c_device_id *did)
{
	struct sc5335p_priv	*priv;
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		dev_err(&adapter->dev,
			"sc5335p: I2C-Adapter doesn't support SMBUS\n");
		return -EIO;
	}

	priv = devm_kzalloc(&client->dev, sizeof(struct sc5335p_priv), GFP_KERNEL);
	if (!priv) {
		dev_err(&adapter->dev,
	...
	if (!sc5335p_match(priv)) {
		ret = -ENODEV;
		goto err_clk;
	}

	/*subdev init*/
	v4l2_i2c_subdev_init(&priv->subdev, client, &sc5335p_subdev_ops);
	priv->subdev.flags |= /*V4L2_SUBDEV_FL_HAS_EVENTS | */V4L2_SUBDEV_FL_HAS_DEVNODE;
	v4l2_ctrl_handler_init(&priv->hdl, 2);
	v4l2_ctrl_new_std(&priv->hdl, &sc5335p_ctrl_ops,
	...
	ret = sc5335p_video_probe(client);
	if (ret < 0) {
	...
	dev_err(&adapter->dev, "sc5335p Probed success, subdev:%p\n", &priv->subdev);
 ...
 * @client:			pointor to i2c client
 */
static int sc5335p_remove(struct i2c_client *client)
{
	struct sc5335p_priv       *priv = to_sc5335p(client);
...
static const struct i2c_device_id sc5335p_id[] = {
	{ "sc5335p", 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, sc5335p_id);

static const struct of_device_id sc5335p_of_match[] = {
	/*donot changed compatible, must the same as dts*/
	{.compatible = "anyka,sensor0", },
	{},
};
MODULE_DEVICE_TABLE(of, sc5335p_of_match);

static struct i2c_driver sc5335p_i2c_driver = {
	.driver = {
		.name = "sc5335p",
		.of_match_table = of_match_ptr(sc5335p_of_match),
	},
	.probe    = sc5335p_probe,
	.remove   = sc5335p_remove,
	.id_table = sc5335p_id,
};

module_i2c_driver(sc5335p_i2c_driver);

MODULE_DESCRIPTION("SoC Camera driver for sc5335p sensor");
MODULE_AUTHOR("Anyka Microelectronic");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("1.0.0");

/*sensor i2c write*/
static int sc5335p_write(const struct i2c_client *client, int reg, int value)
{
...
/*sensor i2c read*/
static int sc5335p_read(const struct i2c_client *client, int reg)
{
	struct i2c_transfer_struct trans = {
		.client = client,
		.reg_bytes = SENSOR_REGADDR_BYTE,
...
static void sc5335p_agc_mode(struct sc5335p_priv *priv)
{
	int value;
	struct i2c_client *client = priv->client;

	value = sc5335p_read(client, 0x3e03) & 0xf0;
	value |= 0xb;
	sc5335p_write(client, 0x3e03, value);
}
 ...
 *
 * */
static int sc5335p_sensor_init_func(void *arg, const AK_ISP_SENSOR_INIT_PARA *npara)
{
	/*cleaned_0x20_SC5335P_MIPI_24MInput_2lane_396Mbps_10bit_2600x1952_12.5fps.ini*/
	int i;
	AK_ISP_SENSOR_REG_INFO *preg_info;
	struct sc5335p_priv *priv = arg;
	struct i2c_client *client = priv->client;
	...
	for (i = 0; i < para->num; i++) {
#if 0
		{
			...
			value = sc5335p_read(client, preg_info->reg_addr);
			pr_err("reg:%x read value:%x\n", preg_info->reg_addr, value);
		}
#endif
		sc5335p_write(client, preg_info->reg_addr, preg_info->value);
#if 0
		{
			int value;
...
static int sc5335p_sensor_read_reg_func(void *arg, const int reg_addr)
{
	struct sc5335p_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc5335p_read(client, reg_addr);
}
/*write sensor register*/
static int sc5335p_sensor_write_reg_func(void *arg, const int reg_addr, int value)
{
	struct sc5335p_priv *priv = arg;
	struct i2c_client *client = priv->client;

	return sc5335p_write(client, reg_addr, value);
}
...
static int aec_parms_init(struct sc5335p_priv *priv)
{
...
/*set sensor again*/
static int sc5335p_sensor_update_a_gain_func(void *arg, const unsigned int a_gain)
{
	/*
	...
	sc5335p_write(client, 0x3e08, corse_gain);
	sc5335p_write(client, 0x3e09, fine_gain);

#if 0
	sc5335p_write(client, 0x3812, 0x00);
	//sc5335p_write(client, 0x363c, blooming_dc);
	//sc5335p_write(client, 0x330e, precharge_lag);
	if (a_gain >= 1536)/*6x*/
		sc5335p_write(client, 0x5799, 0x07);
	else if (a_gain <= 1024)/*4x*/
		sc5335p_write(client, 0x5799, 0x00);
	sc5335p_write(client, 0x3812, 0x30);
#endif

	return A_GAIN_EFFECT_FRAMES;
}
/*set sensor dgain*/
static int sc5335p_sensor_update_d_gain_func(void *arg, const unsigned int d_gain)
{
	/*
	...
	sc5335p_write(client, 0x3e06, corse_gain);
	sc5335p_write(client, 0x3e07, fine_gain);

	return D_GAIN_EFFECT_FRAMES;
}
/*set sensor exp time*/
static int sc5335p_sensor_updata_exp_time_func(void *arg, unsigned int exp_time)
{
	/*
	...
	//tem=sc5335p_read(client, 0x3e02)&0x0f;
	//exposure_time_lsb =exposure_time_lsb|tem;
	sc5335p_write(client, 0x3e01,exposure_time_msb);
	sc5335p_write(client, 0x3e02,exposure_time_lsb);

	//tem = sc5335p_read(client, 0x3e00) & 0xf0;
	//tem |= exposure_time_ext;
	sc5335p_write(client, 0x3e00, exposure_time_ext);
...
/*sensor timer*/
static int sc5335p_sensor_timer_func(void *arg)
{
	return 0;
}
/*standby in*/
static int sc5335p_sensor_set_standby_in_func(void *arg)
{
	return 0;
}
/*standby out*/
static int sc5335p_sensor_set_standby_out_func(void *arg)
{
	return 0;
}
/*low level read sensor ID, user i2c ops*/
static int sc5335p_sensor_probe_id_func(void *arg)  //use IIC bus
{
	struct sc5335p_priv *priv = arg;
	struct i2c_client *client = priv->client;
	int id;
	...
	value = sc5335p_read(client, 0x3107);
	id = value << 8;

	value = sc5335p_read(client, 0x3108);
	id |= value;
...
/*get mclk*/
static int sc5335p_sensor_get_mclk_func(void *arg)
{
	return SENSOR_MCLK;
}
/*get current fps*/
static int sc5335p_sensor_get_fps_func(void *arg)
{
	struct sc5335p_priv *priv = arg;
...
static int sc5335p_sensor_get_valid_coordinate_func(void *arg, int *x, int *y)
{
...
/*get bus type*/
static enum sensor_bus_type sc5335p_sensor_get_bus_type_func(void *arg)
{
...
static int sc5335p_sensor_get_parameter_func(void *arg, int param, void *value)
{
	int ret = 0;
	struct sc5335p_priv *priv = arg;
...
/*sensor power on*/
static int sc5335p_sensor_set_power_on_func(void *arg)
{
	struct sc5335p_priv *priv = arg;
...
/*sensor power off*/
static int sc5335p_sensor_set_power_off_func(void *arg)
{
	struct sc5335p_priv *priv = arg;
...
static int sc5335p_sensor_set_fps_func(void *arg, const int fps)
{
	...
	tmp = sc5335p_fps_to_vts(priv, fps);
...
static void sc5335p_set_fps_async(struct sc5335p_priv *priv)
{
	...
	if (priv->fps_info.to_fps != priv->fps_info.current_fps) {
		...
		sc5335p_write(client, 0x320e, (value >> 8) & 0x7f);
		sc5335p_write(client, 0x320f, value & 0xff);
		priv->fps_info.current_fps = priv->fps_info.to_fps;
	}
}

static AK_ISP_SENSOR_CB sc5335p_cb = {
	.sensor_init_func = sc5335p_sensor_init_func,
	.sensor_read_reg_func = sc5335p_sensor_read_reg_func,
	.sensor_write_reg_func = sc5335p_sensor_write_reg_func,
	.sensor_read_id_func = sc5335p_sensor_read_id_func,
	.sensor_update_a_gain_func = sc5335p_sensor_update_a_gain_func,
	.sensor_update_d_gain_func = sc5335p_sensor_update_d_gain_func,
	.sensor_updata_exp_time_func = sc5335p_sensor_updata_exp_time_func,
	.sensor_timer_func = sc5335p_sensor_timer_func,
	.sensor_set_standby_in_func = sc5335p_sensor_set_standby_in_func,
	.sensor_set_standby_out_func = sc5335p_sensor_set_standby_out_func,
	.sensor_probe_id_func = sc5335p_sensor_probe_id_func,
	.sensor_get_resolution_func = sc5335p_sensor_get_resolution_func,
	.sensor_get_mclk_func = sc5335p_sensor_get_mclk_func,
	.sensor_get_fps_func = sc5335p_sensor_get_fps_func,
	.sensor_get_valid_coordinate_func = sc5335p_sensor_get_valid_coordinate_func,
	.sensor_get_bus_type_func = sc5335p_sensor_get_bus_type_func,
	.sensor_get_parameter_func = sc5335p_sensor_get_parameter_func,
	.sensor_set_power_on_func = sc5335p_sensor_set_power_on_func,
	.sensor_set_power_off_func = sc5335p_sensor_set_power_off_func,
	.sensor_set_fps_func = sc5335p_sensor_set_fps_func,
};

static int sensor_id_func(void)
{
	return sc5335p_sensor_read_id_func(NULL);
}

...
static void akmci_pinctrl_default(struct akmci_host *host);

extern void l2_dump_registers(void);

static unsigned int slot_index = 0;
static struct mmc_host* mci_host[MCI_SLOT_NUM] = {NULL};
	...
	while (1) {
			...
			for (i = 0; i < 3; i++) {
				if(host->detect_mode == AKMCI_DETECT_MODE_CLK)
				{
					...
					if(mci_active)
					{
						dev_dbg(&pdev->dev, "Data transmission is in progress. MCI_CLK will be pulled down, and can not function as the detection pin.!");
	...
	if (curr_status != host->card_status) {
		...
		if (curr_status == CARD_PLUGED) {
			...
			if(host->gpio_power.gpio >= 0){
				...
				gpio_set_value(host->gpio_power.gpio, host->gpio_power.value);
			}
...
static int akmci_probe(struct platform_device *pdev)
{
	...
	pr_info("card_status@probe =%d.(1=CARD_PLUGED, 0=CARD_UNPLUGED)\n",host->card_status);
	/*
	...
	if(host->gpio_power.gpio >= 0)
	{
			...
			if(host->card_status == CARD_UNPLUGED)
			{
				// CARD_POWER_DISABLE
				host->gpio_power.value  =  host->gpio_power.power_invert ^ (CARD_POWER_DISABLE);
				pr_info("[default setting] %s %s using gpio%d with gpio_val %d\n",(CARD_POWER_DISABLE? "enable":"disable"),(host->mci_mode == MCI_MODE_MMC_SD ? "tf_card circuit": "sdio_card circuit"),
				host->gpio_power.gpio,
...
static int akmci_suspend(struct device *dev)
{
	...
	if (host->clk)
		clk_disable_unprepare(host->clk);
	akmci_pinctrl_idle(host); //as pinctrl_pm_select_sleep_state(dev);
...
static struct dev_pm_ops akmci_pm = {
	SET_SYSTEM_SLEEP_PM_OPS(akmci_suspend, akmci_resume)
};

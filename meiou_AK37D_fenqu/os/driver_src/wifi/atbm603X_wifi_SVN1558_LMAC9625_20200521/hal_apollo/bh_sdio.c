...
rx_check:
	if (WARN_ON(read_ctrl_func(
			hw_priv, &ctrl_reg))){
			atbm_printk_err("%s:read_ctrl_func err\n",__func__);
			goto err;
	}
rx_continue:
	...
	if (WARN_ON(!skb_rx)){
		atbm_printk_err("%s:can not get skb,rx_continue_cnt(%d)\n",__func__,rx_continue_cnt);
		goto err;
	}
	...
	if (WARN_ON(read_data_func(hw_priv, data, alloc_len))){
		atbm_printk_err("%s:read_data_func err,rx_continue_cnt(%d)\n",__func__,rx_continue_cnt);
		atbm_bh_halt(hw_priv);
		goto err;
	}
...
#ifndef CONFIG_SDIO_TXRX_WORKQUEUE
		...
		*/
		...
		mutex_unlock(&hw_priv->conf_mutex);
		{
			...
			atbm_for_each_vif(hw_priv, priv, i) {
				...
				ATBM_CANCEL_PENDDING_WORK(&priv->set_channel_work,atbm_set_channel_work);
				#endif
			}
		}
	}

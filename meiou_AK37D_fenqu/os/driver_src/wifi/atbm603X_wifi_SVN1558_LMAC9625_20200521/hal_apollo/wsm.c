...
//				printk("%s %d if_id=%d\n",__func__,__LINE__,interface_link_id);
		...
		if (((s8)(rx.rcpiRssi)>5) &&
			...
			atbm_priv_vif_list_read_unlock(&priv->vif_lock);
			atbm_printk_err("wsm_receive_indication:rcpiRssi %d\n",(s8)(rx.rcpiRssi));
			return 0;
		}
#else
		if (!rx.rcpiRssi &&
		    ...
		    ieee80211_is_beacon(hdr->frame_control))) {
			atbm_priv_vif_list_read_unlock(&priv->vif_lock);
			atbm_printk_err("wsm_receive_indication: rcpiRssi is zero\n");
			return 0;
		}
#endif
#ifndef CONFIG_RATE_HW_CONTROL
		...
		* convert RCPI to RSSI here */
		...
		if (!rx.status && unlikely(ieee80211_is_deauth(fctl))) {
			if (priv->join_status == ATBM_APOLLO_JOIN_STATUS_STA) {
				...
				atbm_printk_always("%s:receive deauthen bssid[%pM],join_bssid[%pM]\n",__func__,hdr->addr3,priv->join_bssid);
				...
				if(do_unjoin == true){
					...
					atbm_priv_vif_list_read_unlock(&priv->vif_lock);
					atbm_printk_err( "%s:receive unknown deauthen bssid[%pM]\n",__func__,hdr->addr3);
					return 0;
				}
			}
		}
...
//		hw_priv->bh_error = 1;
	...
	if (hw_priv->bh_error) {
		...
		if (atbm_wait_event_timeout_stay_awake(hw_priv,hw_priv->bh_evt_wq,!(hw_priv->hw_bufs_used),timeout,true) <= 0) {
			atbm_printk_err( "%s:+++++  bh_error=1 have txframe pending hw_bufs_used %d,timeout =%d\n", __func__,(u32)hw_priv->hw_bufs_used,(u32)timeout);
			if(!(hw_priv->hw_bufs_used)){
			/* Get a timestamp of "oldest" frame */
                	  for (i = 0; i < 4; ++i)
                        	pending |= atbm_queue_get_xmit_timestamp(
			   ...
			   atbm_printk_err("%s <WARNING hw_bufs_use==0,pending %x,but wait imeout!!!!!!\n",__func__,pending);
			...
			//				
			atbm_printk_err("%s:  bh_error=1 have txframe pending hw_bufs_used %d,hw_noconfirm_tx %d,timeout =%d\n", __func__,(u32)hw_priv->hw_bufs_used,hw_priv->hw_noconfirm_tx,(u32)timeout);
#ifdef SDIO_BUS
			       ...
			       struct atbm_queue *queue)
{
	...
	case NL80211_IFTYPE_STATION:
		if (unlikely((priv->join_status == ATBM_APOLLO_JOIN_STATUS_STA) &&
			...
			if (ieee80211_is_auth(fctl))
				...
				*the channel.
				*/
				atbm_printk_err("%s:drop the off channel pkg(%x)\n",__func__,fctl);
				action = doDrop;
			}
		}
		break;
	...
	case NL80211_IFTYPE_MONITOR:
		if(priv->join_status == ATBM_APOLLO_JOIN_STATUS_SIMPLE_MONITOR){
			atbm_printk_err("%s:monitor send\n",__func__);
			txpriv->offchannel_if_id = ATBM_WIFI_GENERIC_IF_ID;
			action = doTx;
			break;
		}

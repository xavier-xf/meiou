...
static int atbm_ioctl_stop_rx(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	if(status == NULL){
		atbm_printk_err("%s:atbm_kzalloc err\n",__func__);
		ret = -ENOMEM;
		goto exit;
	}
...
static int atbm_ioctl_rx_monitor_mac(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if(ieee80211_get_channel_mode(local,NULL) != CHAN_MODE_FIXED){
		atbm_printk_err("%s:no fix channel\n",__func__);
		ret =  -EINVAL;
		goto exit;
	}
	...
	*/
	...
	if(pos_next - pos > 1){
		atbm_printk_err("%s: params err3 [%p][%p]\n",__func__,pos_next,pos);
		ret = -EINVAL;
		goto exit;
	}
	pos_end = memchr(pos,ATBM_TAIL,len);
	...
	if(atbm_accsii_to_int(pos,pos_next - pos,&enable) == false){		
		atbm_printk_err("%s:enable err(%s),len(%d)\n",__func__,pos,len);
		ret = -EINVAL;
		goto exit;
	}
	...
	*/
	...
	if(pos_next - pos > 1){
		atbm_printk_err("%s: params err33 [%p][%p]\n",__func__,pos_next,pos);
		ret = -EINVAL;
		goto exit;
	}
	...
	if((index != 0)&&(index != 1)){
		atbm_printk_err("%s:index err(%s),len(%d)\n",__func__,pos,len);
		ret = -EINVAL;
		goto exit;
	}
	...
	if(enable){
		...
		*/
		if(len != 17){
			atbm_printk_err("%s: params err4 %d\n",__func__,len);
			ret = -EINVAL;
			goto exit;
		}
		...
		atbm_printk_err("%s:diable monitor do not need params\n",__func__);
		ret = -EINVAL;
		goto exit;
	}
	
	if(atbm_internal_mac_monitor(&local->hw,&mac_moniter) == false){
		atbm_printk_err("%s:triger mac monitor err\n",__func__);
		ret = -EINVAL;
		goto exit;
	}
exit:
...
static int atbm_ioctl_rx_statistic(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if(ETF_bStartRx == 0){
		atbm_printk_err("%s:start rx not running\n",__func__);
		ret = -EOPNOTSUPP;
		goto exit;
	}

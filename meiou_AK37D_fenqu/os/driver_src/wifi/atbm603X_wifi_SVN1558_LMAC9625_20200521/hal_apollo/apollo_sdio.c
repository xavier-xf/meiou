...
#ifdef CONFIG_SDIO_TXRX_WORKQUEUE	
	struct workqueue_struct __rcu *tx_workqueue;
	struct work_struct tx_complete_work;
	struct work_struct rx_complete_work;
	struct workqueue_struct __rcu *rx_workqueue;	
#else
	struct task_struct		__rcu *sdio_rx_thread;	
	struct task_struct		__rcu *sdio_tx_thread;
#endif
	const struct atbm_platform_data *pdata;
	spinlock_t		lock;
	spinlock_t		bh_lock;
	unsigned long	tx_thread_action;
	unsigned long   rx_thread_action;
...
static int atbm_sdio_wait_rx_action(struct sbus_priv		*self)
{
	unsigned long idle_period = msecs_to_jiffies(40);
	unsigned long period = idle_period;
	
	set_current_state(TASK_INTERRUPTIBLE);

	while (!kthread_should_stop()) {
		if (test_and_clear_bit(THREAD_RX_WAKEUP,
				       &self->rx_thread_action)) {
			try_to_freeze();
		...
		if(period == 0){
			...
			printk_once("[atbm_log]:%s timeout\n",__func__);
...
static int atbm_sdio_rx_thread(void *priv)
{
	...
	*/
	...
	/*
	*set our task freezeale 
	*/
	set_freezable();
	while(!atbm_sdio_wait_rx_action(hw_priv->sbus_priv)){
		atbm_sdio_rx_bh(hw_priv);		
	};
	atbm_printk_init("%s:exit\n",__func__);
	return 0;
}
static int atbm_sdio_wait_tx_action(struct sbus_priv		*self)
{
	unsigned long idle_period = msecs_to_jiffies(1000);
	unsigned long period = idle_period;
	
	set_current_state(TASK_INTERRUPTIBLE);

	while (!kthread_should_stop()) {
		if (test_and_clear_bit(THREAD_TX_WAKEUP,
				       &self->tx_thread_action)) {
			try_to_freeze();
			__set_current_state(TASK_RUNNING);
			return 0;
		}
...
static int atbm_sdio_tx_thread(void *priv)
{
	...
	*/
	...
	while(!atbm_sdio_wait_tx_action(hw_priv->sbus_priv)){
		
		hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
		atbm_sdio_tx_bh(hw_priv);
...
static int atbm_sdio_xmit_init(struct sbus_priv *self)
{
	...
	bh= create_singlethread_workqueue(ieee80211_alloc_name(hw_priv->hw,"tx_workqueue")/*"tx_workqueue"*/);

	if(bh == NULL)
		return -1;

	spin_lock_bh(&self->bh_lock);
	rcu_assign_pointer(self->tx_workqueue,(struct workqueue_struct *)bh);
	spin_unlock_bh(&self->bh_lock);
#else
	self->tx_thread_action = 0;
	bh = kthread_create(&atbm_sdio_tx_thread, hw_priv, ieee80211_alloc_name(hw_priv->hw,"sdio_tx"));
	if (IS_ERR(bh)){
		self->sdio_tx_thread = NULL;
		atbm_printk_err("%s:sdio_rx_thread err\n",__func__);
		return -1;
	}else {
		spin_lock_bh(&self->bh_lock);
		rcu_assign_pointer(self->sdio_tx_thread,bh);
		spin_unlock_bh(&self->bh_lock);
	}
#endif
...
#ifdef CONFIG_SDIO_TXRX_WORKQUEUE
	spin_lock_bh(&self->bh_lock);
	bh = rcu_dereference(self->tx_workqueue);
	rcu_assign_pointer(self->tx_workqueue,NULL);
	spin_unlock_bh(&self->bh_lock);
	if(bh){
		synchronize_rcu();
		flush_workqueue((struct workqueue_struct *)bh);
		destroy_workqueue((struct workqueue_struct *)bh);
	}
#else
	spin_lock_bh(&self->bh_lock);
	bh = rcu_dereference(self->sdio_tx_thread);
	rcu_assign_pointer(self->sdio_tx_thread,NULL);
	spin_unlock_bh(&self->bh_lock);
	if (bh){
		synchronize_rcu();
		kthread_stop(bh);
...
static int atbm_sdio_rev_init(struct sbus_priv *self)
{
	...
	bh = create_singlethread_workqueue(ieee80211_alloc_name(hw_priv->hw,"rx_workqueue")/*"rx_workqueue"*/);
	if(bh == NULL)
		return -1;
	spin_lock_bh(&self->bh_lock);
	rcu_assign_pointer(self->rx_workqueue,(struct workqueue_struct *)bh);
	spin_unlock_bh(&self->bh_lock);
#else	
	self->rx_thread_action = 0;
	bh= kthread_create(&atbm_sdio_rx_thread, hw_priv, 
							ieee80211_alloc_name(hw_priv->hw,"sdio_rx"));
	if (IS_ERR(bh)){
		self->sdio_rx_thread = NULL;
		atbm_printk_err("%s:sdio_rx_thread err\n",__func__);
		return -1;
	}else {
		spin_lock_bh(&self->bh_lock);
		rcu_assign_pointer(self->sdio_rx_thread,(struct task_struct *)bh);
		spin_unlock_bh(&self->bh_lock);
		/*
		*wake up rx thread
		*/
...
#ifdef CONFIG_SDIO_TXRX_WORKQUEUE
	spin_lock_bh(&self->bh_lock);
	bh = rcu_dereference(self->rx_workqueue);
	rcu_assign_pointer(self->rx_workqueue,NULL);
	spin_unlock_bh(&self->bh_lock);
	if(bh){
		synchronize_rcu();
		flush_workqueue((struct workqueue_struct *)bh);
...
#else
	spin_lock_bh(&self->bh_lock);
	bh = rcu_dereference(self->sdio_rx_thread);
	rcu_assign_pointer(self->sdio_rx_thread,NULL);
	spin_unlock_bh(&self->bh_lock);
	if (bh){
		synchronize_rcu();
		kthread_stop((struct task_struct *)bh);
	}
#endif
	return 0;
}
...
static int atbm_sdio_xmit_schedule(struct sbus_priv *self)
{
	...
	bh = rcu_dereference(self->tx_workqueue);
	if(bh)
		queue_work((struct workqueue_struct *)bh,&self->tx_complete_work);
#else
	if(test_and_set_bit(THREAD_TX_WAKEUP, &self->tx_thread_action) == 0){
		bh = rcu_dereference(self->sdio_tx_thread);
		if(bh)
...
static int atbm_sdio_rev_schedule(struct sbus_priv *self)
{
	...
	bh = rcu_dereference(self->rx_workqueue);
	if(bh)
		queue_work((struct workqueue_struct *)bh,&self->rx_complete_work);
#else
	if(test_and_set_bit(THREAD_RX_WAKEUP, &self->rx_thread_action) == 0){
		bh = rcu_dereference(self->sdio_rx_thread);
		if(bh)
			wake_up_process((struct task_struct *)bh);
	}
...
static void atbm_sdio_miss_irq(struct sbus_priv *self) 
{
	...
	struct atbm_common *hw_priv = self->core;
	hw_priv->sbus_ops->lock(hw_priv->sbus_priv);
	atbm_sdio_irq_handler(self->func);
	hw_priv->sbus_ops->unlock(hw_priv->sbus_priv);
	#else
	atbm_gpio_irq(0,self);
	#endif
}

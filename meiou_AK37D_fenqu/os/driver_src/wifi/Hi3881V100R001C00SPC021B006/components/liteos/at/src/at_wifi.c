...
hi_u32 at_is_hex_char(hi_char ch)
{
    if (((ch >= '0') && (ch <= '9')) ||
        ((ch >= 'A') && (ch <= 'F')) ||
        ((ch >= 'a') && (ch <= 'f'))) {
...
hi_u32 at_is_hex_string(const hi_char *data, hi_u16 len)
{
    hi_u16 i;
    for (i = 0; i < len; i++) {
        if (at_is_hex_char(data[i]) == HI_ERR_FAILURE) {
            return HI_ERR_FAILURE;
        }
    }
    return HI_ERR_SUCCESS;
...
hi_u32 cmd_sta_connect_get_key(hi_s32 argc, const hi_char *argv[], hi_wifi_fast_assoc_request *fast_assoc_req)
{
    if ((fast_assoc_req->req.auth != HI_WIFI_SECURITY_OPEN) && (argc == 5)) { /* 5:命令参数个数 */
        ...
        if ((atoi(argv[3]) == HI_WIFI_SECURITY_WEP) && (len != 9) && (len != 17) && /* 3:加密方式 9:17:密码长度 */
            ...
            return HI_ERR_FAILURE;
        } else if ((len == HI_WIFI_AP_KEY_LEN_MAX + 2) &&       /* 2: len plus 2 */
            (at_is_hex_string(buf + 1, HI_WIFI_AP_KEY_LEN_MAX) == HI_ERR_FAILURE)) {
            return HI_ERR_FAILURE;
        }
...
hi_u32 cmd_sta_connect(hi_s32 argc, const hi_char *argv[])
{
    ...
    if (argc == 4) { /* 4:命令参数个数 */
        ...
        if ((atoi(argv[2]) == HI_WIFI_SECURITY_WEP) && (len != 9) && (len != 17) && /* 2:加密方式 9:17:密码长度 */
            ...
            (len < HI_WIFI_AP_KEY_LEN_MIN + 2))) { /* 2:引号长度 */
            return HI_ERR_FAILURE;
        } else if ((len == HI_WIFI_AP_KEY_LEN_MAX + 2) && (at_is_hex_string(buf + 1, HI_WIFI_AP_KEY_LEN_MAX) != HI_ERR_SUCCESS)) {
            return HI_ERR_FAILURE;
        }

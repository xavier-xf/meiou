			       ...
			       const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	(void)vsnprintf(format, maxlen, fmt, ap);
	va_end(ap);
...
#else
#define eapol_auth_logger(args...) do { } while (0)
#define eapol_auth_vlogger(args...) do { } while (0)
...
static void eapol_auth_tx_req(struct eapol_state_machine *sm)
{
	...
	if (sm->flags & EAPOL_SM_WAIT_START) {
		wpa_warning_log4(MSG_DEBUG, "EAPOL: Drop EAPOL TX to " "%02x:xx:xx:%02x:%02x:%02x"
			   " while waiting for EAPOL-Start",
			   (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
		return;
	}
...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (state->aWhile > 0) {
		state->aWhile--;
		if (state->aWhile == 0) {
			wpa_error_log4(MSG_DEBUG, "IEEE 802.1X: " "%02x:xx:xx:%02x:%02x:%02x"
				   " - aWhile --> 0",
				   (state->addr)[0], (state->addr)[3], (state->addr)[4], (state->addr)[5]);
		}
	}
	...
	(void)eloop_register_timeout(1, 0, eapol_port_timers_tick, eloop_ctx, state);
}
...
#endif
{
	...
	if (sm == NULL) {
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.1X state machine allocation "
			   "failed");
		return NULL;
	}
#ifndef HISI_CODE_CROP
...
#ifndef LOS_CONFIG_HOSTAPD_EAP_CIPHERS
...
#endif /* LOS_CONFIG_HOSTAPD_EAP_CIPHERS */
#ifdef CONFIG_WPS
...
void eapol_auth_free(struct eapol_state_machine *sm)
{
	...
	(void)eloop_cancel_timeout(eapol_port_timers_tick, NULL, sm);
	(void)eloop_cancel_timeout(eapol_sm_step_cb, sm, NULL);
	if (sm->eap)
	...
	if (eapol_sm_sta_entry_alive(eapol, addr) && sm->eap) {
		...
		if (sm->eap_if->aaaEapResp) {
			...
			if (sm->eap_if->aaaEapRespData == NULL) {
				wpa_warning_log0(MSG_DEBUG, "EAPOL: aaaEapResp set, "
					   "but no aaaEapRespData available");
				return;
			}
			if(sm->eapol->cb.aaa_send) {
...
void eapol_auth_step(struct eapol_state_machine *sm)
{
	...
	(void)eloop_register_timeout(0, 0, eapol_sm_step_cb, sm, NULL);
}
...
static void eapol_auth_initialize(struct eapol_state_machine *sm)
{
	...
	(void)eloop_cancel_timeout(eapol_port_timers_tick, NULL, sm);
	(void)eloop_register_timeout(1, 0, eapol_port_timers_tick, NULL, sm);
}
...
{
	wpa_warning_log4(MSG_DEBUG, "EAPOL: External reauthentication trigger for "
		   "%02x:xx:xx:%02x:%02x:%02x", (sm->addr)[0], (sm->addr)[3], (sm->addr)[4], (sm->addr)[5]);
	sm->reAuthenticate = TRUE;
	eapol_auth_step(sm);
}
...
int eapol_auth_set_conf(struct eapol_state_machine *sm, const char *param,
			const char *value)
{
	wpa_warning_log4(MSG_DEBUG, "EAPOL: External configuration operation for "

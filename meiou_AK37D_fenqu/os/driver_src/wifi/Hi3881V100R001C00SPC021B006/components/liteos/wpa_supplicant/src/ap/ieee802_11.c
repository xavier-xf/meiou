...
prepare_auth_resp_fils(struct hostapd_data *hapd,
		       ...
		       int *is_pub);
#endif /* CONFIG_FILS */
#ifdef LOS_CONFIG_HOSTAPD_MGMT
static void handle_auth(struct hostapd_data *hapd,
			const struct ieee80211_mgmt *mgmt, size_t len,
			int rssi, int from_queue);
#endif /* LOS_CONFIG_HOSTAPD_MGMT */
...
u16 hostapd_own_capab_info(struct hostapd_data *hapd)
{
	...
	if (dfs < 0) {
		wpa_warning_log1(MSG_WARNING, "Failed to check if DFS is required; ret=%d",
			   dfs);
		dfs = 0;
	}
			   ...
			   int iswep)
{
	...
	if (auth_transaction == 1) {
		if (!sta->challenge) {
			...
			(void)rc4_skip(key, sizeof(key), 0,
	...
	(void)wpa_auth_sm_event(sta->wpa_sm, WPA_AUTH);
	os_free(sta->challenge);
	sta->challenge = NULL;
			   ...
			   const u8 *ies, size_t ies_len, const char *dbg)
{
	...
	wpa_warning_log4(MSG_DEBUG, "authentication reply: STA=" "%02x:xx:xx:%02x:%02x:%02x",
		   (dst)[0], (dst)[3], (dst)[4], (dst)[5]);
	wpa_warning_log4(MSG_DEBUG, "authentication reply: STA="
		   " auth_alg=%d auth_transaction=%d resp(status_code)=%d (IE len=%lu)",
		   auth_alg, auth_transaction,
		   resp, (unsigned long) ies_len);
	wpa_warning_buf(MSG_DEBUG, "authentication reply: STA=" "(dbg=%s)",
			dbg, strlen(dbg));
	if (hostapd_drv_send_mlme(hapd, reply, rlen, 0) < 0) {
		wpa_warning_log0(MSG_INFO, "send_auth_reply: send failed");
	} else {
...
#ifndef CONFIG_SAE_CROP
static void sae_set_state(struct sta_info *sta, enum sae_state state,
			  const char *reason)
{
	(void)reason;
	wpa_warning_two_buf(MSG_DEBUG, "SAE: State %s -> %s ",
		   sae_state_txt(sta->sae->state), strlen(sae_state_txt(sta->sae->state)), sae_state_txt(state), strlen(sae_state_txt(state)));
	wpa_warning_log4(MSG_DEBUG, "for peer " "%02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_buf(MSG_DEBUG, " (%s)", reason, strlen(reason));
	sta->sae->state = state;
}
#endif
					     ...
					     struct sta_info *sta, int update)
{
	...
	if (!password) {
		wpa_warning_log0(MSG_DEBUG, "SAE: No password available");
		return NULL;
	}
	...
	if (update &&
	    sae_prepare_commit(hapd->own_addr, sta->addr,
			       ...
			       sta->sae) < 0) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Could not pick PWE");
		return NULL;
	}
	...
	if (update &&
	    sae_prepare_commit(hapd->own_addr, sta->addr,
			       (u8 *) password, os_strlen(password), NULL,
			       sta->sae) < 0) {
		wpa_warning_log0(MSG_DEBUG, "SAE: Could not pick PWE");
		return NULL;
	}
	...
	if (buf == NULL)
		return NULL;
	sae_write_commit(sta->sae, buf, sta->sae->tmp ?
			 sta->sae->tmp->anti_clogging_token : NULL, NULL);
#endif
	return buf;
}
...
#else
		if (open >= 5) /* 5: default anti_clogging_threshold */
#endif
			return 1;
	}
...
#else
	if (open + dl_list_len(&hapd->sae_commit_queue) >= 5)
#endif
		return 1;
			   ...
			   const u8 *token, size_t token_len)
{
	...
	if (token_idx == 0 || token_idx != WPA_GET_BE16(token)) {
		wpa_warning_log2(MSG_DEBUG, "SAE: Invalid anti-clogging token from "
			   " - token_idx 0x%04x, expected 0x%04x",
			   WPA_GET_BE16(token), token_idx);
...
#else
	if (sta->sae->sync > 1) { /* 2: default sae_sync */
#endif
		wpa_warning_log2(MSG_DEBUG, "Sync (%d) > dot11RSNASAESync(%d)", sta->sae->sync, hapd->conf->sae_sync);
#ifndef CONFIG_SAE_CROP
...
static void auth_sae_retransmit_timer(void *eloop_ctx, void *eloop_data)
{
	...
	sta->sae->sync++;
#ifndef CONFIG_SAE_CROP
	wpa_warning_log4(MSG_DEBUG, "SAE: Auth SAE retransmit timer for " "%02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_log1(MSG_DEBUG, "SAE: Auth SAE retransmit timer for "
		   " (sync=%d)", sta->sae->sync);
	wpa_warning_buf(MSG_DEBUG, "SAE: Auth SAE retransmit timer for "
		   " (state=%s)",
		   sae_state_txt(sta->sae->state), strlen(sae_state_txt(sta->sae->state)));
#endif
	...
	if (ret != WLAN_STATUS_SUCCESS) {
		wpa_warning_log1(MSG_INFO, "SAE: Failed to retransmit: ret=%d", ret);
	}
}
...
void sae_accept_sta(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (sta->sae->tmp && sta->sae->tmp->vlan_id > 0) {
		wpa_warning_log1(MSG_DEBUG, "SAE: Assign STA to VLAN ID %d",
			   sta->sae->tmp->vlan_id);
		wpa_warning_log4(MSG_DEBUG, "mac %02x:xx:xx:%02x:%02x:%02x",
				(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]));
		os_memset(&vlan_desc, 0, sizeof(vlan_desc));
		vlan_desc.notempty = 1;
		vlan_desc.untagged = sta->sae->tmp->vlan_id;
		if (!hostapd_vlan_valid(hapd->conf->vlan, &vlan_desc)) {
			wpa_warning_log1(MSG_INFO,
...
#ifdef LOS_CONFIG_MESH
	if (hapd->conf->mesh & MESH_ENABLED)
		wpa_auth_pmksa_add_sae(hapd->wpa_mesh_auth, sta->addr, sta->sae->pmk, sta->sae->pmkid);
	else
#endif /* LOS_CONFIG_MESH */
		       ...
		       int *sta_removed)
{
	...
	*sta_removed = 0;
	...
	if (auth_transaction != 1 && auth_transaction != 2)
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
#ifndef CONFIG_SAE_CROP
	wpa_warning_log4(MSG_DEBUG, "SAE: Peer " "%02x:xx:xx:%02x:%02x:%02x",
		   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	wpa_warning_buf(MSG_DEBUG, "SAE: Peer " " state=%s",
		   sae_state_txt(sta->sae->state), strlen(sae_state_txt(sta->sae->state)));
	wpa_warning_log1(MSG_DEBUG, "SAE: Peer " " auth_trans=%u", auth_transaction);
#endif
	switch (sta->sae->state) {
	case SAE_NOTHING:
		if (auth_transaction == 1) {
	...
	case SAE_ACCEPTED:
		if (auth_transaction == 1 &&
			...
			wpa_warning_log4(MSG_DEBUG, "SAE: remove the STA (" "%02x:xx:xx:%02x:%02x:%02x"
				   ") doing reauthentication",
				   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
			ap_free_sta(hapd, sta);
#ifndef LOS_CONFIG_NO_PMKSA
			wpa_auth_pmksa_remove(hapd->wpa_mesh_auth, sta->addr);
#endif /* LOS_CONFIG_NO_PMKSA */
		} else if (auth_transaction == 1) {
			wpa_warning_log0(MSG_DEBUG, "SAE: Start reauthentication");
			ret = auth_sae_send_commit(hapd, sta, bssid, 1);
			if (ret)
				return ret;
#ifndef CONFIG_SAE_CROP
		...
		break;
	default:
		wpa_error_log1(MSG_ERROR, "SAE: invalid state %d",
			   sta->sae->state);
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
	return WLAN_STATUS_SUCCESS;
...
#ifndef CONFIG_SAE_ONE_ECC_CURVE
static void sae_pick_next_group(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	wpa_warning_log1(MSG_DEBUG, "SAE: Previously selected group: %d", sae->group);
	...
	wpa_warning_log1(MSG_DEBUG, "SAE: Selected new group: %d", groups[i]);
}
#endif /* CONFIG_SAE_ONE_ECC_CURVE */
			    ...
			    u16 auth_transaction, u16 status_code)
{
	...
	if (hapd->conf->sae_reflection_attack && auth_transaction == 1) {
		wpa_warning_log0(MSG_DEBUG, "SAE: TESTING - reflection attack");
	...
	if (hapd->conf->sae_commit_override && auth_transaction == 1) {
		wpa_warning_log0(MSG_DEBUG, "SAE: TESTING - commit override");
		send_auth_reply(hapd, mgmt->sa, mgmt->bssid, WLAN_AUTH_SAE,
	...
	if (auth_transaction == 1) {
		...
		if (resp == SAE_SILENTLY_DISCARD) {
			wpa_warning_log4(MSG_DEBUG,
				   "SAE: Drop commit message from " "%02x:xx:xx:%02x:%02x:%02x" " due to reflection attack",
				   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
			goto remove_sta;
		}
#ifndef CONFIG_SAE_NO_PW_ID
		...
		if (token && check_sae_token(hapd, sta->addr, token, token_len)
		    < 0) {
			wpa_warning_log4(MSG_DEBUG, "SAE: Drop commit message with "
				   "incorrect token from " "%02x:xx:xx:%02x:%02x:%02x",
				   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto remove_sta;
		}
		...
		if (!token && use_sae_anti_clogging(hapd) && !allow_reuse) {
			wpa_warning_log4(MSG_DEBUG,
				   ...
				   "%02x:xx:xx:%02x:%02x:%02x", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
			   ...
			   int rssi)
{
	...
	if (queue_len >= 15) {
		wpa_warning_log4(MSG_DEBUG,
			   "SAE: No more room in message queue - drop the new frame from %02x:xx:xx:%02x:%02x:%02x",
			   mgmt->sa[0], mgmt->sa[3], mgmt->sa[4], mgmt->sa[5]);
		return;
	}

	wpa_warning_log1(MSG_DEBUG, "SAE: Queue Authentication message from mac for processing (queue_len %u)",
		   queue_len);
	wpa_warning_log4(MSG_DEBUG, "mac %02x:xx:xx:%02x:%02x:%02x",
		   			mgmt->sa[0], mgmt->sa[3], mgmt->sa[4], mgmt->sa[5]);
				 ...
				 struct wpabuf *data, int pub))
{
	...
	if (auth_alg == WLAN_AUTH_FILS_SK_PFS) {
		...
		if ((size_t) (end - pos) < elem_len) {
			wpa_warning_log0(MSG_DEBUG, "FILS: No room for Element");
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto fail;
		}
		...
		sta->fils_dh_ss = crypto_ecdh_set_peerkey(sta->fils_ecdh, 1,
							  pos, elem_len);
		if (!sta->fils_dh_ss) {
			wpa_warning_log0(MSG_DEBUG, "FILS: ECDH operation failed");
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto fail;
		}
	...
	if (ieee802_11_parse_elems(pos, end - pos, &elems, 1) == ParseFailed) {
		wpa_warning_log0(MSG_DEBUG, "FILS: Could not parse elements");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	if (!elems.rsn_ie ||
	    wpa_parse_wpa_ie_rsn(elems.rsn_ie - 2, elems.rsn_ie_len + 2,
				 &rsn) < 0) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No valid RSN element");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	if (!elems.fils_nonce) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No FILS Nonce field");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	if (pmksa)
		wpa_warning_log0(MSG_DEBUG, "FILS: Found matching PMKSA cache entry");

	/* FILS Session */
	if (!elems.fils_session) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No FILS Session element");
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
		       ...
		       int *is_pub)
{
	...
	if (!pmksa && erp_resp) {
		...
		if (fils_rmsk_to_pmk(wpa_auth_sta_key_mgmt(sta->wpa_sm),
				     ...
				     pmk_buf, &pmk_len)) {
			wpa_warning_log0(MSG_DEBUG, "FILS: Failed to derive PMK");
			*resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			wpabuf_free(data);
			data = NULL;
			goto fail;
	...
	if (!pmk) {
		wpa_warning_log0(MSG_DEBUG, "FILS: No PMK available");
		*resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		wpabuf_free(data);
		data = NULL;
		goto fail;
			...
			int rssi, int from_queue)
{
...
#endif /* HISI_CODE_CROP */
	u8 resp_ies[2 + WLAN_AUTH_CHALLENGE_LEN] = {0};
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.auth)) {
		wpa_warning_log1(MSG_INFO, "handle_auth - too short payload (len=%lu)",
			   (unsigned long) len);
		return;
	}
	...
	wpa_warning_log4(MSG_DEBUG, "authentication: STA=" "%02x:xx:xx:%02x:%02x:%02x",
		   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
	wpa_error_log4(MSG_DEBUG, "authentication: STA=" " auth_alg=%d "
		   "auth_transaction=%d status_code=%d wep=%d",
		   auth_alg, auth_transaction,
		   status_code, !!(fc & WLAN_FC_ISWEP));
	wpa_warning_two_buf(MSG_DEBUG, "authentication: STA="
		   "%s " "%s",
		   challenge ? " challenge" : "", challenge ? strlen(" challenge") : strlen(""),
		   (fc & WLAN_FC_RETRY) ? " retry" : "", (fc & WLAN_FC_RETRY) ? strlen(" retry") : strlen(""));
	...
	if (!(((hapd->conf->auth_algs & WPA_AUTH_ALG_OPEN) &&
		...
		wpa_warning_log1(MSG_INFO, "Unsupported authentication algorithm (%d)",
			   auth_alg);
		resp = WLAN_STATUS_NOT_SUPPORTED_AUTH_ALG;
		goto fail;
	}
	...
	if (!(auth_transaction == 1 || auth_alg == WLAN_AUTH_SAE ||
		...
		wpa_warning_log1(MSG_INFO, "Unknown authentication transaction number (%d)",
			   auth_transaction);
		resp = WLAN_STATUS_UNKNOWN_AUTH_TRANSACTION;
		goto fail;
	}
	...
	if (os_memcmp(mgmt->sa, hapd->own_addr, ETH_ALEN) == 0) {
		wpa_warning_log4(MSG_INFO, "Station " "%02x:xx:xx:%02x:%02x:%02x" " not allowed to authenticate",
			   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
		goto fail;
	}
	...
	if (hapd->conf->no_auth_if_seen_on) {
		...
		if (other) {
			...
			wpa_warning_two_buf(MSG_DEBUG, "%s: Reject authentication from " " since STA has been seen on %s",
				   hapd->conf->iface, strlen(hapd->conf->iface),
				   hapd->conf->no_auth_if_seen_on, strlen(hapd->conf->no_auth_if_seen_on));
			wpa_warning_log4(MSG_DEBUG, "%02x:xx:xx:%02x:%02x:%02x",
					(mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
	...
	if (sta) {
		...
		if ((hapd->conf->mesh & MESH_ENABLED) &&
			...
			wpa_warning_log4(MSG_DEBUG, "Mesh peer " "%02x:xx:xx:%02x:%02x:%02x"
...
#endif
#ifdef CONFIG_MESH
		if ((hapd->conf->mesh & MESH_ENABLED) && (g_mesh_reject_peer_flag)) {
			wpa_error_log0(MSG_ERROR, "handle_auth: reject mesh peer now.");
			goto fail;
		}
#endif /* CONFIG_MESH */
...
#ifdef LOS_CONFIG_MESH
		/* hisi insert into driver */
		os_memset(&params, 0, sizeof(params));
		params.addr = mgmt->sa;
		if ((hapd->driver->sta_add == NULL) ||
			((hapd->driver->sta_add != NULL) && (hapd->driver->sta_add(hapd->drv_priv, &params)))) {
			wpa_error_log4(MSG_ERROR, "Driver failed to insert " "%02x:xx:xx:%02x:%02x:%02x", (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
			ap_free_sta(hapd, sta);
			goto fail;
		}
#endif /* LOS_CONFIG_MESH */
	...
	case WLAN_AUTH_FT:
		...
		if (sta->wpa_sm == NULL)
			sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth,
							sta->addr, NULL);
		if (sta->wpa_sm == NULL) {
			wpa_warning_log0(MSG_DEBUG, "FT: Failed to initialize WPA "
				   "state machine");
			resp = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto fail;
		}
	...
	case WLAN_AUTH_SAE:
		...
		if (status_code == WLAN_STATUS_SUCCESS &&
			...
			if (sta->wpa_sm == NULL)
				sta->wpa_sm =
					wpa_auth_sta_init(hapd->wpa_mesh_auth,
							  sta->addr, NULL);
			if (sta->wpa_sm == NULL) {
				wpa_warning_log0(MSG_DEBUG,
...
int hostapd_get_aid(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	/* get a unique AID */
	if (sta->aid > 0) {
		wpa_warning_log1(MSG_DEBUG, "  old AID %d", sta->aid);
		return 0;
	}
	...
	hapd->sta_aid |= BIT(j);
#endif /* HISI_CODE_CROP */
	wpa_warning_log1(MSG_DEBUG, "  new AID %d", sta->aid);
	return 0;
}
				 ...
				 u8 owe_dh_len)
{
	...
	if (wpa_auth_sta_get_pmksa(sta->wpa_sm)) {
		wpa_warning_log0(MSG_DEBUG, "OWE: Using PMKSA caching");
		return WLAN_STATUS_SUCCESS;
	}

	group = WPA_GET_LE16(owe_dh);
	if (!owe_group_supported(hapd, group)) {
		wpa_warning_log1(MSG_DEBUG, "OWE: Unsupported DH group %u", group);
	...
	if (!secret) {
		wpa_warning_log0(MSG_DEBUG, "OWE: Invalid peer DH public key");
		return WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
			 ...
			 const u8 *owe_dh, size_t owe_dh_len)
{
	...
	if (!rsn_ie || rsn_ie_len < 2) {
		wpa_warning_log4(MSG_DEBUG, "OWE: Invalid RSNE from %02x:xx:xx:%02x:%02x:%02x",
		   	   (peer)[0], (peer)[3], (peer)[4], (peer)[5]);
		return WLAN_STATUS_INVALID_IE;
	}
	rsn_ie -= 2;
	rsn_ie_len += 2;
	...
	if (res) {
		wpa_warning_log4(MSG_DEBUG, "Failed to parse RSNE from %02x:xx:xx:%02x:%02x:%02x",
		   	   (peer)[0], (peer)[3], (peer)[4], (peer)[5]);
		wpa_warning_log1(MSG_DEBUG, "Failed to parse RSNE from mac (res=%d)", res);
		wpa_hexdump(MSG_DEBUG, "RSNE", rsn_ie, rsn_ie_len);
		return wpa_res_to_status_code(res);
	}
	if (!(data.key_mgmt & WPA_KEY_MGMT_OWE)) {
		wpa_warning_log1(MSG_DEBUG, "OWE: Unexpected key mgmt 0x%x" , (unsigned int) data.key_mgmt);
		wpa_warning_log4(MSG_DEBUG, "from %02x:xx:xx:%02x:%02x:%02x",
		   	   (peer)[0], (peer)[3], (peer)[4], (peer)[5]);
		return WLAN_STATUS_AKMP_NOT_VALID;
	}
	if (!owe_dh) {
		wpa_warning_log4(MSG_DEBUG,
			   "OWE: No Diffie-Hellman Parameter element from %02x:xx:xx:%02x:%02x:%02x",
			    (peer)[0], (peer)[3], (peer)[4], (peer)[5]);
		return WLAN_STATUS_AKMP_NOT_VALID;
	}
		       ...
		       const u8 *owe_dh, size_t owe_dh_len)
{
	...
	if (!rsn_ie || rsn_ie_len < 2) {
		wpa_warning_log0(MSG_DEBUG, "OWE: No RSNE in (Re)AssocReq");
	...
	wpa_warning_log2(MSG_DEBUG, "OWE: Update status %d, ie len %d for peer ",
				status, (unsigned int) ie_len);
	wpa_warning_log4(MSG_DEBUG, "peer %02x:xx:xx:%02x:%02x:%02x",
				(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]));
			   ...
			   const u8 *ies, size_t ies_len, int reassoc)
{
	...
	if (hapd->conf->wps_state && elems.wps_ie) {
		wpa_warning_log0(MSG_DEBUG, "STA included WPS IE in (Re)Association "
		...
		if (sta->wps_ie && wps_is_20(sta->wps_ie)) {
			wpa_warning_log0(MSG_DEBUG, "WPS: STA supports WPS 2.0");
			sta->flags |= WLAN_STA_WPS2;
		}
		wpa_ie = NULL;
		wpa_ie_len = 0;
		if (sta->wps_ie && wps_validate_assoc_req(sta->wps_ie) < 0) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Invalid WPS IE in "
				   "(Re)Association Request - reject");
			return WLAN_STATUS_INVALID_IE;
		}
	} else if (hapd->conf->wps_state && wpa_ie == NULL) {
		wpa_warning_log0(MSG_DEBUG, "STA did not include WPA/RSN IE in "
	...
	if (hapd->conf->wpa && wpa_ie) {
		...
		if (sta->wpa_sm == NULL)
			sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth,
							...
							p2p_dev_addr);
		if (sta->wpa_sm == NULL) {
			wpa_warning_log0(MSG_WARNING, "Failed to initialize WPA "
				   "state machine");
			return WLAN_STATUS_UNSPECIFIED_FAILURE;
		}
	...
	(void)reassoc;
		...
		if (wpa_auth_uses_sae(sta->wpa_sm) &&
			...
			wpa_warning_log4(MSG_DEBUG, "SAE: " "%02x:xx:xx:%02x:%02x:%02x"
				   " using PMKSA caching", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		} else if (wpa_auth_uses_sae(sta->wpa_sm) &&
			   sta->auth_alg != WLAN_AUTH_SAE &&
			   !(sta->auth_alg == WLAN_AUTH_FT &&
			     wpa_auth_uses_ft_sae(sta->wpa_sm))) {
			wpa_warning_log4(MSG_DEBUG, "SAE: " "%02x:xx:xx:%02x:%02x:%02x" " tried to use "
				   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
			wpa_warning_log1(MSG_DEBUG, "SAE: "
				   "SAE AKM after non-SAE auth_alg %u", sta->auth_alg);
	...
	pfs_fail:
		...
		wpa_warning_log0(MSG_DEBUG, "HS 2.0: OSEN association");
		if (sta->wpa_sm == NULL)
			sta->wpa_sm = wpa_auth_sta_init(hapd->wpa_auth,
							sta->addr, NULL);
		if (sta->wpa_sm == NULL) {
			...
			u16 reason_code)
{
	...
	if (hostapd_drv_send_mlme(hapd, &reply, send_len, 0) < 0)
		wpa_warning_buf(MSG_INFO, "Failed to send deauth: %s",
			   ...
			   const u8 *ies, size_t ies_len, int rssi)
{
	...
	if (hostapd_drv_send_mlme(hapd, reply, send_len, 0) < 0) {
		wpa_warning_buf(MSG_INFO, "Failed to send assoc resp: %s",
			   strerror(errno), strlen(strerror(errno)));
		res = WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
			   ...
			   u8 *owe_buf, size_t owe_buf_len, u16 *reason)
{
	...
	if (sta->owe_pmk && sta->external_dh_updated) {
		wpa_warning_log0(MSG_DEBUG, "OWE: Using previously derived PMK");
		*reason = WLAN_STATUS_SUCCESS;
		return owe_buf;
	}
			 ...
			 int reassoc, int rssi)
{
	...
	if (len < IEEE80211_HDRLEN + (reassoc ? sizeof(mgmt->u.reassoc_req) :
		...
		wpa_warning_log2(MSG_INFO, "handle_assoc(reassoc=%d) - too short payload (len=%lu)",
	...
	if (reassoc) {
		...
		wpa_warning_log4(MSG_DEBUG, "reassociation request: STA = %02x:xx:xx:%02x:%02x:%02x",
			   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		wpa_warning_log2(MSG_DEBUG, "reassociation request: STA="
			   " capab_info=0x%02x listen_interval=%d",
			   capab_info, listen_interval);
		wpa_warning_log4(MSG_DEBUG, "reassociation request: STA="
			   " current_ap=" "%02x:xx:xx:%02x:%02x:%02x",
			   (mgmt->u.reassoc_req.current_ap)[0], (mgmt->u.reassoc_req.current_ap)[3],
			   (mgmt->u.reassoc_req.current_ap)[4], (mgmt->u.reassoc_req.current_ap)[5]);
		wpa_warning_log1(MSG_DEBUG, "reassociation request: STA="  " seq_ctrl=0x%x", seq_ctrl);
		wpa_warning_buf(MSG_DEBUG, "reassociation request: STA=" "%s",
					(fc & WLAN_FC_RETRY) ? " retry" : "",
					(fc & WLAN_FC_RETRY) ? strlen(" retry") : strlen(""));
		...
		wpa_warning_log4(MSG_DEBUG, "association request: STA=" "%02x:xx:xx:%02x:%02x:%02x",
			   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]));
		wpa_warning_log3(MSG_DEBUG, "association request: STA="
			   " capab_info=0x%02x listen_interval=%d "
			   "seq_ctrl=0x%x", capab_info, listen_interval, seq_ctrl);
		wpa_warning_buf(MSG_DEBUG, "association request: STA="
			   "%s", (fc & WLAN_FC_RETRY) ? " retry" : "",
			   (fc & WLAN_FC_RETRY) ? strlen(" retry") : strlen(""));
	...
	if ((sta->flags & WLAN_STA_MFP) && sta->sa_query_timed_out) {
		wpa_warning_buf(MSG_DEBUG, "Allowing %sassociation after timed out "
			   "SA Query procedure", reassoc ? "re" : "", reassoc ? strlen("re") : strlen(""));
			    ...
			    const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.disassoc)) {
		wpa_warning_log1(MSG_INFO, "handle_disassoc - too short payload (len=%lu)",
			   (unsigned long) len);
		return;
	}

	wpa_warning_log4(MSG_DEBUG, "disassocation: STA=" "%02x:xx:xx:%02x:%02x:%02x",
		   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
	wpa_warning_log1(MSG_DEBUG, " reason_code=%d",
		   le_to_host16(mgmt->u.disassoc.reason_code));

	sta = ap_get_sta(hapd, mgmt->sa);
	if (sta == NULL) {
		wpa_warning_log4(MSG_INFO, "Station " "%02x:xx:xx:%02x:%02x:%02x" " trying to disassociate, but it is not associated",
			   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
		return;
	}
			  ...
			  struct hostapd_frame_info *fi)
{
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.beacon)) {
		wpa_warning_log1(MSG_INFO, "handle_beacon - too short payload (len=%lu)",
			   (unsigned long) len);
		return;
	}
			 ...
			 unsigned int freq)
{
	...
	wpa_warning_log4(MSG_DEBUG, "RX_ACTION category %u action %u"
		  " len %d freq %u",
		   mgmt->u.action.category, *action, (int) len, freq);
	wpa_warning_log4(MSG_DEBUG, "sa " "%02x:xx:xx:%02x:%02x:%02x",
		   (mgmt->sa)[0], (mgmt->sa)[3], (mgmt->sa)[4], (mgmt->sa)[5]);
	wpa_warning_log4(MSG_DEBUG, " da " "%02x:xx:xx:%02x:%02x:%02x",
		   (mgmt->da)[0], (mgmt->da)[3], (mgmt->da)[4], (mgmt->da)[5]);
	sta = ap_get_sta(hapd, mgmt->sa);

	if (mgmt->u.action.category != WLAN_ACTION_PUBLIC &&
	    (sta == NULL || !(sta->flags & WLAN_STA_ASSOC))) {
		wpa_warning_log1(MSG_DEBUG, "IEEE 802.11: Ignored Action "
	...
	case WLAN_ACTION_VENDOR_SPECIFIC:
		...
		break;
#endif /* CONFIG_P2P */
#ifndef LOS_CONFIG_HOSTAPD_RRM
	case WLAN_ACTION_RADIO_MEASUREMENT:
		hostapd_handle_radio_measurement(hapd, (const u8 *) mgmt, len);
		return 1;
#endif /* LOS_CONFIG_HOSTAPD_RRM */
	}
	...
	if (!is_multicast_ether_addr(mgmt->da) &&
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "IEEE 802.11: Return unknown Action "
		...
		if (hostapd_drv_send_mlme(hapd, resp, len, 0) < 0) {
			wpa_error_log0(MSG_ERROR, "IEEE 802.11: Failed to send "
				   "Action frame");
		}
		os_free(resp);
	}
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	if (!is_broadcast_ether_addr(mgmt->bssid) &&
		...
		wpa_warning_log4(MSG_INFO, "MGMT: BSSID=" "%02x:xx:xx:%02x:%02x:%02x" " not our address",
			   (mgmt->bssid)[0], (mgmt->bssid)[3], (mgmt->bssid)[4], (mgmt->bssid)[5]);
		return 0;
	}
	...
	case WLAN_FC_STYPE_AUTH:
		wpa_warning_log0(MSG_DEBUG, "mgmt::auth");
		handle_auth(hapd, mgmt, len, ssi_signal, 0);
		ret = 1;
		break;
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	case WLAN_FC_STYPE_ASSOC_REQ:
		wpa_warning_log0(MSG_DEBUG, "mgmt::assoc_req");
		handle_assoc(hapd, mgmt, len, 0, ssi_signal);
		ret = 1;
		break;
	case WLAN_FC_STYPE_REASSOC_REQ:
		wpa_warning_log0(MSG_DEBUG, "mgmt::reassoc_req");
		handle_assoc(hapd, mgmt, len, 1, ssi_signal);
		ret = 1;
		break;
	case WLAN_FC_STYPE_DISASSOC:
		wpa_warning_log0(MSG_DEBUG, "mgmt::disassoc");
	...
	case WLAN_FC_STYPE_ACTION:
		wpa_warning_log0(MSG_DEBUG, "mgmt::action");
			   ...
			   size_t len, int ok)
{
	...
	if (!sta) {
		wpa_warning_log4(MSG_DEBUG, "handle_auth_cb: STA " "%02x:xx:xx:%02x:%02x:%02x"
			   " not found",
			   (mgmt->da)[0], (mgmt->da)[3], (mgmt->da)[4], (mgmt->da)[5]);
		return;
	}
	...
	if (len < IEEE80211_HDRLEN + sizeof(mgmt->u.auth)) {
		wpa_warning_log1(MSG_INFO, "handle_auth_cb - too short payload (len=%lu)",
			   (unsigned long) len);
		goto fail;
	}
			    ...
			    size_t len, int reassoc, int ok)
{
	...
	sta = ap_get_sta(hapd, mgmt->da);
	if (!sta) {
		wpa_warning_log4(MSG_INFO, "handle_assoc_cb: STA " "%02x:xx:xx:%02x:%02x:%02x" " not found",
			   (mgmt->da)[0], (mgmt->da)[3], (mgmt->da)[4], (mgmt->da)[5]);
		return;
	}
...
#else
	if ((!hapd->conf->wpa) ||
#endif /* LOS_CONFIG_HOSTAPD_SECURITY */
	    sta->auth_alg == WLAN_AUTH_FT) {
		/*
...
#endif /* LOS_CONFIG_NO_VLAN */
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	(void)hostapd_set_sta_flags(hapd, sta);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
	...
	if ((sta->auth_alg == WLAN_AUTH_FILS_SK ||
		...
		wpa_warning_log0(MSG_DEBUG, "FILS: TK configuration failed");
			     ...
			     size_t len, int ok)
{
	...
	sta = ap_get_sta(hapd, mgmt->da);
	if (!sta) {
		wpa_warning_log4(MSG_DEBUG, "handle_deauth_cb: STA " "%02x:xx:xx:%02x:%02x:%02x"
			   " not found", (mgmt->da)[0], (mgmt->da)[3], (mgmt->da)[4], (mgmt->da)[5]);
		return;
	}
	if (ok) {
		wpa_warning_log4(MSG_DEBUG, "STA " "%02x:xx:xx:%02x:%02x:%02x" " acknowledged deauth",
			   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	}
	else {
		wpa_warning_log4(MSG_DEBUG, "STA " "%02x:xx:xx:%02x:%02x:%02x" " did not acknowledge "
			   "deauth", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	}
			       ...
			       size_t len, int ok)
{
	...
	sta = ap_get_sta(hapd, mgmt->da);
	if (!sta) {
		wpa_warning_log4(MSG_DEBUG, "handle_disassoc_cb: STA " "%02x:xx:xx:%02x:%02x:%02x"
			   " not found", (mgmt->da)[0], (mgmt->da)[3], (mgmt->da)[4], (mgmt->da)[5]);
		return;
	}
	if (ok) {
		wpa_warning_log4(MSG_DEBUG, "STA " "%02x:xx:xx:%02x:%02x:%02x" " acknowledged disassoc",
			   (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	}
	else {
		wpa_warning_log4(MSG_DEBUG, "STA " "%02x:xx:xx:%02x:%02x:%02x" " did not acknowledge "
			   "disassoc", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	}
			     ...
			     size_t len, int ok)
{
	...
	sta = ap_get_sta(hapd, mgmt->da);
	if (!sta) {
		wpa_warning_log4(MSG_DEBUG, "handle_action_cb: STA " "%02x:xx:xx:%02x:%02x:%02x"
			   " not found", (mgmt->da)[0], (mgmt->da)[3], (mgmt->da)[4], (mgmt->da)[5]);
		return;
	}
			...
			u16 stype, int ok)
{
	...
	switch (stype) {
	case WLAN_FC_STYPE_AUTH:
		wpa_warning_log0(MSG_DEBUG, "mgmt::auth cb");
		handle_auth_cb(hapd, mgmt, len, ok);
		break;
	case WLAN_FC_STYPE_ASSOC_RESP:
		wpa_warning_log0(MSG_DEBUG, "mgmt::assoc_resp cb");
		handle_assoc_cb(hapd, mgmt, len, 0, ok);
		break;
	case WLAN_FC_STYPE_REASSOC_RESP:
		wpa_warning_log0(MSG_DEBUG, "mgmt::reassoc_resp cb");
		handle_assoc_cb(hapd, mgmt, len, 1, ok);
		break;
	case WLAN_FC_STYPE_PROBE_RESP:
		wpa_msgdump_log1(MSG_EXCESSIVE, "mgmt::proberesp cb ok=%d", ok);
		break;
	case WLAN_FC_STYPE_DEAUTH:
		wpa_warning_log0(MSG_DEBUG, "mgmt::deauth cb");
		handle_deauth_cb(hapd, mgmt, len, ok);
		break;
	case WLAN_FC_STYPE_DISASSOC:
		wpa_warning_log0(MSG_DEBUG, "mgmt::disassoc cb");
		handle_disassoc_cb(hapd, mgmt, len, ok);
		break;
#ifndef LOS_CONFIG_HOSTAPD_RRM
	case WLAN_FC_STYPE_ACTION:
		wpa_warning_log1(MSG_DEBUG, "mgmt::action cb ok=%d", ok);
		handle_action_cb(hapd, mgmt, len, ok);
		break;
#endif /* LOS_CONFIG_HOSTAPD_RRM */
	default:
		wpa_warning_log1(MSG_INFO, "unknown mgmt cb frame subtype %d", stype);
		break;
	}
}
#endif /* CONFIG_P2P */
		       ...
		       const u8 *buf, size_t len, int ack)
{
	...
	if (sta->flags & WLAN_STA_PENDING_POLL) {
		wpa_warning_log4(MSG_DEBUG, "STA " "%02x:xx:xx:%02x:%02x:%02x",
			(sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
		wpa_warning_buf(MSG_DEBUG, " %s pending " "activity poll",
			   ack ? "ACKed" : "did not ACK", ack ? strlen("ACKed") : strlen("did not ACK"));
		if (ack)
			     ...
			     const u8 *data, size_t len, int ack)
{
	...
	if (sta == NULL || !(sta->flags & WLAN_STA_ASSOC)) {
		wpa_warning_log4(MSG_DEBUG, "Ignore TX status for Data frame to STA "
			   "%02x:xx:xx:%02x:%02x:%02x" " that is not currently associated",
			   dst[0], dst[3], dst[4], dst[5]);
		return;
	}
...
void hostapd_client_poll_ok(struct hostapd_data *hapd, const u8 *addr)
{
	...
	wpa_warning_log4(MSG_DEBUG, "STA " "%02x:xx:xx:%02x:%02x:%02x" " ACKed pending "
		   "activity poll", (sta->addr)[0], (sta->addr)[3], (sta->addr)[4], (sta->addr)[5]);
	sta->flags &= ~WLAN_STA_PENDING_POLL;
}

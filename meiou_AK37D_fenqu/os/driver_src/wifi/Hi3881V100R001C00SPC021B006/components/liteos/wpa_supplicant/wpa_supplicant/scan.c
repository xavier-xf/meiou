...
static void wpa_supplicant_gen_assoc_event(struct wpa_supplicant *wpa_s)
{
	...
	(void)os_memset(&data, 0, sizeof(data));
...
#ifndef HISI_CODE_CROP
	(void)os_get_reltime(&wpa_s->scan_trigger_time);
#endif /* HISI_CODE_CROP */
#ifdef CONFIG_WPS
	wpa_s->scan_runs++;
#endif /* CONFIG_WPS */
...
wpa_supplicant_build_filter_ssids(struct wpa_config *conf, size_t *num_ssids)
{
	...
	*num_ssids = 0;
	...
	for (ssid = conf->ssid; ssid; ssid = ssid->next) {
		if (!ssid->ssid || !ssid->ssid_len)
			continue;
		(void)os_memcpy(ssids[*num_ssids].ssid, ssid->ssid, ssid->ssid_len);
...
void wpa_supplicant_set_default_scan_ies(struct wpa_supplicant *wpa_s)
{
	...
	(void)wpa_drv_get_ext_capa(wpa_s, type);
	...
	if (default_ies)
		(void)wpa_drv_set_default_scan_ies(wpa_s, wpabuf_head(default_ies),
...
#endif /* CONFIG_P2P */
		(void)wpa_drv_get_ext_capa(wpa_s, WPA_IF_STATION);
...
#endif /* CONFIG_WPS */
#ifdef LOS_CONFIG_MESH
	hi_wpa_mesh_add_scan_req_ie(wpa_s, &extra_ie);
#endif /* LOS_CONFIG_MESH */
			      ...
			      size_t max_ssids, const u8 *ssid, size_t ssid_len)
{
	...
	if (params->num_ssids + 1 > max_ssids) {
		wpa_warning_log0(MSG_DEBUG, "Over max scan SSIDs for manual request");
		return;
	}

	wpa_warning_buf(MSG_DEBUG, "Scan SSID (manual request): %s",
		   wpa_ssid_txt(ssid, ssid_len), strlen(wpa_ssid_txt(ssid, ssid_len)));
...
#ifndef HISI_CODE_CROP
		wpa_warning_log1(MSG_DEBUG, "Over max scan SSIDs from scan req: %u",
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
...
#endif /* HISI_CODE_CROP */
	if ((wpa_s->conf->ap_scan == 2) || (wpa_s->num_ssids_from_scan_req != 0))
		max_ssids = 1;
	else {
	...
	if (connect_without_scan) {
		...
		if (ssid) {
#ifndef HISI_CODE_CROP
			wpa_warning_log0(MSG_DEBUG, "Start a pre-selected network "
				   "without scan step");
#endif /* HISI_CODE_CROP */
			wpa_supplicant_associate(wpa_s, NULL, ssid);
#ifdef LOS_WPA_PATCH
...
#endif /* HISI_CODE_CROP */
#ifdef LOS_WPA_PATCH
	params.prefix_ssid_scan_flag = wpa_s->prefix_ssid_scan_flag;
	wpa_s->prefix_ssid_scan_flag = WPA_FLAG_OFF;
	params.fast_connect_flag = g_fast_connect_flag ? WPA_FLAG_ON : WPA_FLAG_OFF;
	if ((!g_usr_scanning_flag) && g_connecting_flag &&
	    (g_connecting_ssid != NULL) && (wpa_s->current_ssid == g_connecting_ssid)) {
		wpa_warning_log0(MSG_DEBUG, "Use specific SSID for connecting option \n");

		params.ssids[0].ssid = g_connecting_ssid->ssid;
		params.ssids[0].ssid_len = g_connecting_ssid->ssid_len;
		params.num_ssids = 1;
		params.bssid = g_connecting_ssid->bssid;
		if (g_connecting_ssid->frequency) {
			params.freqs = os_malloc(sizeof(int) * 2);
			if (params.freqs) {
				params.freqs[0] = g_connecting_ssid->frequency;
				params.freqs[1] = 0;
			}
		}
		goto ssid_list_set;
...
#ifndef HISI_CODE_CROP
		wpa_warning_log0(MSG_DEBUG, "Use specific SSIDs from SCAN command");
#endif /* HISI_CODE_CROP */
		goto ssid_list_set;
	}
	...
	if ((wpa_s->p2p_in_provisioning || wpa_s->show_group_started) &&
		wpa_s->go_params) {
		wpa_warning_log2(MSG_DEBUG, "P2P: Use specific SSID for scan during P2P group formation (p2p_in_provisioning=%d show_group_started=%d)",
			   wpa_s->p2p_in_provisioning,
	...
	if (wpa_s->p2p_in_invitation) {
		if (wpa_s->current_ssid) {
			wpa_warning_log0(MSG_DEBUG, "P2P: Use specific SSID for scan during invitation");
			params.ssids[0].ssid = wpa_s->current_ssid->ssid;
			params.ssids[0].ssid_len =
				wpa_s->current_ssid->ssid_len;
			params.num_ssids = 1;
		} else {
			wpa_warning_log0(MSG_DEBUG, "P2P: No specific SSID known for scan during invitation");
		}
		goto ssid_list_set;
	}
#endif /* CONFIG_P2P */
...
#endif /* HISI_CODE_CROP */
	if (ret) {
		wpa_msg(wpa_s, MSG_DEBUG, "AP scan failed");
		...
		wpa_msg(wpa_s, MSG_DEBUG, "AP scan ok");
#ifndef HISI_CODE_CROP
...
void wpa_supplicant_update_scan_int(struct wpa_supplicant *wpa_s, int sec)
{
	...
	if (cancelled) {
		(void)eloop_register_timeout(new_int.sec, new_int.usec,
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	if (res == 1) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "Setting scan request: %d.%06d sec",
			sec, usec);
#endif /* HISI_CODE_CROP */
		(void)eloop_register_timeout(sec, usec, wpa_supplicant_scan, wpa_s, NULL);
	}
}
				      ...
				      int sec, int usec)
{
	...
	(void)eloop_register_timeout(sec, usec,
	...
	while (ssid) {
		...
		if (params.num_filter_ssids < wpa_s->max_match_sets &&
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "add to filter ssid: %s",
				wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
			(void)os_memcpy(params.filter_ssids[params.num_filter_ssids].ssid,
				  ssid->ssid, ssid->ssid_len);
...
scan:
	...
	if (ssid || !wpa_s->first_sched_scan) {
		wpa_s->sched_scan_timed_out = 0;
		(void)eloop_register_timeout(wpa_s->sched_scan_timeout, 0,
...
void wpa_supplicant_cancel_delayed_sched_scan(struct wpa_supplicant *wpa_s)
{
	...
	(void)eloop_cancel_timeout(wpa_supplicant_delayed_sched_scan_timeout,
			     wpa_s, NULL);
}
...
void wpa_supplicant_cancel_sched_scan(struct wpa_supplicant *wpa_s)
{
	...
	(void)eloop_cancel_timeout(wpa_supplicant_sched_scan_timeout, wpa_s, NULL);
	(void)wpa_supplicant_stop_sched_scan(wpa_s);
}
#endif /* HISI_CODE_CROP */
...
static void dump_scan_res(struct wpa_scan_results *scan_res)
{
	...
	wpa_msgdump_log0(MSG_EXCESSIVE, "Sorted scan results");
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (r->flags & WPA_SCAN_LEVEL_DBM) {
			...
			wpa_msgdump_log4(MSG_EXCESSIVE, "%02x:xx:xx:%02x:%02x:%02x",
				   (r->bssid)[0], (r->bssid)[3], (r->bssid)[4], (r->bssid)[5]);
			wpa_msgdump_log4(MSG_EXCESSIVE, " freq=%d qual=%d "
				   "noise=%d level=%d",
				   r->freq, r->qual,
				   r->noise, r->level);
			wpa_msgdump_log3(MSG_EXCESSIVE,
				   "flags=0x%x age=%u est=%u",
				   r->flags,
				   r->age, r->est_throughput);
			wpa_msgdump_two_buf(MSG_EXCESSIVE,
				   "noise=%s snr=%s",
				   noise_valid ? "" : "~", noise_valid ? strlen("") : strlen("~"),
				   r->snr >= GREAT_SNR ? "*" : "", r->snr >= GREAT_SNR ? strlen("*") : strlen(""));
		} else {
			wpa_msgdump_log4(MSG_EXCESSIVE, "%02x:xx:xx:%02x:%02x:%02x",
				   (r->bssid)[0], (r->bssid)[3], (r->bssid)[4], (r->bssid)[5]);
			wpa_msgdump_log4(MSG_EXCESSIVE, " freq=%d qual=%d "
				   "noise=%d level=%d",
				   r->freq, r->qual, r->noise, r->level);
			wpa_msgdump_log3(MSG_EXCESSIVE, "flags=0x%x age=%u est=%u",
		     ...
		     struct wpa_scan_results *res)
{
	...
	if (res->num != j) {
		wpa_warning_log1(MSG_DEBUG, "Filtered out %d scan results",
			   (int) (res->num - j));
		res->num = j;
	}
}
...
#else
	if (capab == CAPAB_HT || capab == CAPAB_HT40) {
#endif /* HISI_CODE_CROP */
				...
				struct scan_info *info, int new_scan)
{
	...
	if (scan_res->fetch_time.sec == 0) {
		 ...
		 * does not set this.
		 */
		(void)os_get_reltime(&scan_res->fetch_time);
	}
#ifndef HISI_CODE_CROP
...
#ifdef CONFIG_MESH
	if (wpas_mesh_searching(wpa_s) == HISI_OK) {
#ifndef HISI_CODE_CROP
		wpa_dbg(wpa_s, MSG_DEBUG, "MESH: Order scan results with MESH "
			"provisioning rules");
#endif /* HISI_CODE_CROP */
		wpa_bss_mesh_scan_result_sort(wpa_s);
	}
#endif /* CONFIG_MESH */
		       ...
		       struct wpa_scan_results *scan_res)
{
	...
	if ((wifi_dev != NULL) &&
		((wifi_dev->iftype == HI_WIFI_IFTYPE_STATION) ||
		(wifi_dev->iftype == HI_WIFI_IFTYPE_MESH_POINT))) {
		hi_at_printf("+NOTICE:SCANFINISH\r\n");
	}
	wpa_events.event = HI_WIFI_EVT_SCAN_DONE;
...
#ifdef LOS_WPA_PATCH
	params->prefix_ssid_scan_flag = src->prefix_ssid_scan_flag;
	params->fast_connect_flag = src->fast_connect_flag;
#endif /* LOS_WPA_PATCH */
#ifndef HISI_CODE_CROP
...
int wpas_start_pno(struct wpa_supplicant *wpa_s)
{
	...
	if ((wpa_s->wpa_state > WPA_SCANNING) &&
		...
		wpa_error_log0(MSG_ERROR, "PNO: In assoc process");
		return -EAGAIN;
	}
	...
	if (wpa_s->wpa_state == WPA_SCANNING) {
		...
		if (wpa_s->sched_scanning) {
			wpa_warning_log0(MSG_DEBUG, "Schedule PNO on completion of "
				   "ongoing sched scan");
			wpa_supplicant_cancel_sched_scan(wpa_s);
	...
	if (num_match_ssid == 0) {
		wpa_warning_log0(MSG_DEBUG, "PNO: No configured SSIDs");
		return -1;
	}
	...
	while (ssid) {
		if (!wpas_network_disabled(wpa_s, ssid)) {
			...
			(void)os_memcpy(params.filter_ssids[i].ssid, ssid->ssid,
				  ssid->ssid_len);
...
int wpas_sched_scan_plans_set(struct wpa_supplicant *wpa_s, const char *cmd)
{
	...
	if (!cmd[0]) {
		wpa_warning_log0(MSG_DEBUG, "Clear sched scan plans");
...
fail:
	os_free(scan_plans);
	wpa_error_log0(MSG_ERROR, "invalid scan plans list");
	return -1;
}

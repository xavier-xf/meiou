...
#if defined(CONFIG_NO_CONFIG_WRITE)
#define NO_CONFIG_WRITE
#endif

#ifndef CONFIG_DRIVER_HISILICON
#define WPA_METHOD "physical_push_button virtual_push_button physical_display keypad"
#else
#define WPA_METHOD "virtual_push_button virtual_display keypad"
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_printf(MSG_ERROR, "Line %d (len=%lu "
			   "min_len=%ld)", line, (unsigned long) res_len, (long) data->param3);
		wpa_printf(MSG_ERROR, "too short %s", data->name, strlen(data->name));
#endif /* CONFIG_PRINT_NOUSE */
		os_free(tmp);
		return -1;
	}
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_printf(MSG_ERROR, "Line %d (len=%lu "
			   "min_len=%ld)", line, (unsigned long) res_len, (long) data->param4);
		wpa_printf(MSG_ERROR, "too short %s", data->name, strlen(data->name));
#endif /* CONFIG_PRINT_NOUSE */
		os_free(tmp);
		return -1;
...
static char * wpa_config_write_string_ascii(const u8 *value, size_t len)
{
	...
	if (buf == NULL)
		return NULL;
	buf[0] = '"';
	(void)os_memcpy(buf + 1, value, len);
	buf[len + 1] = '"';
	buf[len + 2] = '\0';
...
static char * wpa_config_write_string_hex(const u8 *value, size_t len)
{
	...
	if (buf == NULL)
		return NULL;
	(void)wpa_snprintf_hex(buf, 2 * len + 1, value, len);
				...
				int line, const char *value)
{
	...
	if (*end) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_error_buf(MSG_ERROR, "invalid number \"%s\"",
			   value, strlen(value));
#endif /* CONFIG_PRINT_NOUSE */
		return -1;
	}
	...
	*dst = val;
#ifndef CONFIG_PRINT_NOUSE
	wpa_msgdump_log2(MSG_MSGDUMP, "%d (0x%x)", *dst, *dst);
	wpa_msgdump_buf(MSG_MSGDUMP, "%s", data->name, strlen(data->name));
#endif /* CONFIG_PRINT_NOUSE */
	if (data->param3 && *dst < (long) data->param3) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_error_log2(MSG_ERROR, "(value=%d "
		  	 "min_value=%ld)", *dst,
		   	(long) data->param3);
		wpa_error_buf(MSG_ERROR, "too small %s", data->name, strlen(data->name));
#endif /* CONFIG_PRINT_NOUSE */
	...
	if (data->param4 && *dst > (long) data->param4) {
		...
		wpa_error_log2(MSG_ERROR, "(value=%d "
		  	 "max_value=%ld)", *dst,
		   	(long) data->param4);
		wpa_error_buf(MSG_ERROR, "too large %s", data->name, strlen(data->name));
#endif /* CONFIG_PRINT_NOUSE */
				      ...
				      u8 abort_on_error, u8 masked)
{
	...
	while (pos && *pos) {
		...
		if (hwaddr_masked_aton(pos, addr, &addr[ETH_ALEN], masked)) {
			...
			wpa_error_two_buf(MSG_INFO,
				   "Ignore likely truncated %s address '%s'",
				   name, strlen(name), pos, strlen(pos));
		} else {
			...
			buf = n;
			(void)os_memmove(buf + 2 * ETH_ALEN, buf,
				   count * 2 * ETH_ALEN);
			os_memcpy(buf, addr, 2 * ETH_ALEN);
			count++;
			wpa_msgdump_buf(MSG_MSGDUMP,
				   "%s: addr=", name, strlen(name));
			wpa_msgdump_log4(MSG_MSGDUMP,
				   "%02x:xx:xx:%02x:%02x:%02x",
				   addr[0], addr[3], addr[4], addr[5]);
			wpa_msgdump_log4(MSG_MSGDUMP, " mask=" "%02x:xx:xx:%02x:%02x:%02x",
				   (&addr[ETH_ALEN])[0], (&addr[ETH_ALEN])[3], (&addr[ETH_ALEN])[4], (&addr[ETH_ALEN])[5]);
		}
				       ...
				       const char *value)
{
	if (value[0] == '\0' || os_strcmp(value, "\"\"") == 0 ||
		...
		wpa_msgdump_log0(MSG_MSGDUMP, "BSSID hint any");
		return 0;
	}
	...
	if (*value == '"') {
		...
		if (len < 8 || len > 63) {
			...
			wpa_printf(MSG_ERROR, "Invalid passphrase "
				   "length %lu",
				   (unsigned long) len);
			wpa_printf(MSG_ERROR, "(expected: 8..63) '%s'.",
				   value, strlen(value));
#endif /* CONFIG_PRINT_NOUSE */
			return -1;
		}
...
static int wpa_config_parse_cipher(int line, const char *value)
{
	...
	if (val < 0) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_printf(MSG_ERROR, "invalid cipher '%s'.",
			   value, strlen(value));
#endif /* CONFIG_PRINT_NOUSE */
		return -1;
	}
				  ...
				  const char *value)
{
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_error_log2(MSG_ERROR, "Line %d: not allowed group cipher "
		 	  "(0x%x).", line, val);
#endif /* CONFIG_PRINT_NOUSE */
		return -1;
	}
	...
	if (ssid->group_cipher == val)
		return 1;
#ifndef CONFIG_PRINT_NOUSE
	wpa_msgdump_log1(MSG_MSGDUMP, "group: 0x%x", val);
#endif /* CONFIG_PRINT_NOUSE */
					...
					struct wpa_ssid *ssid)
{
	...
	(void)os_memcpy(buf, "hash:", 5);
	(void)wpa_snprintf_hex(buf + 5, 32 + 1, ssid->eap.password, 16);
				    ...
				    const char *value, int idx)
{
	...
	if (buf == NULL) {
#ifndef CONFIG_PRINT_NOUSE
		wpa_printf(MSG_ERROR, "Invalid WEP key %d",
			   idx);
		wpa_printf(MSG_ERROR, " wpa_config_parse_wep_key: '%s'.",
			   value, strlen(value));
#endif /* CONFIG_PRINT_NOUSE */
		return -1;
	}
...
#ifndef CONFIG_PRINT_NOUSE
		wpa_printf(MSG_ERROR, "Too long WEP key %d",
			   idx);
		wpa_printf(MSG_ERROR, "wpa_config_parse_wep_key: '%s'.",
			   value, strlen(value));
#endif /* CONFIG_PRINT_NOUSE */
					    ...
					    const char *value)
{
	...
	if (value[0] == '\0' || os_strcmp(value, "\"\"") == 0 ||
		...
		wpa_msgdump_log0(MSG_MSGDUMP, "GO P2P Device Address any");
		return 0;
	}
	if (hwaddr_aton(value, ssid->go_p2p_dev_addr)) {
		wpa_error_buf(MSG_ERROR, "Invalid GO P2P Device Address '%s'.",
			   value, strlen(value));
		return -1;
	}
	ssid->bssid_set = 1;
	wpa_msgdump_log4(MSG_MSGDUMP, "GO P2P Device Address " "%02x:xx:xx:%02x:%02x:%02x",
		   (ssid->go_p2p_dev_addr)[0], (ssid->go_p2p_dev_addr)[3], (ssid->go_p2p_dev_addr)[4], (ssid->go_p2p_dev_addr)[5]);
	return 0;
}
				     ...
				     const char *value)
{
	...
	if (hwaddr_aton(pos, p->addr)) {
		wpa_error_buf(MSG_ERROR, "Invalid psk_list address '%s'",
			   pos, strlen(pos));
		os_free(p);
		return -1;
	}
	pos += 17;
	if (*pos != '-') {
		wpa_error_buf(MSG_ERROR, "Invalid psk_list '%s'",
			   pos, strlen(pos));
		os_free(p);
		return -1;
	}
	...
	if (hexstr2bin(pos, p->psk, PMK_LEN) || pos[PMK_LEN * 2] != '\0') {
		wpa_error_buf(MSG_ERROR, "Invalid psk_list PSK '%s'",
			   pos, strlen(pos));
		os_free(p);
		return -1;
	}
				...
				struct wpa_ssid *ssid)
{
	...
	for (prio = 0; prio < config->num_prio; prio++) {
		if (nlist[prio]->priority < ssid->priority) {
			(void)os_memmove(&nlist[prio + 1], &nlist[prio],
...
struct wpa_ssid * wpa_config_add_network(struct wpa_config *config)
{
	...
	if (last)
		...
		config->ssid = ssid;
#ifndef HISI_CODE_CROP
	(void)wpa_config_update_prio_list(config);
#endif /* HISI_CODE_CROP */
	return ssid;
}
...
#ifndef CONFIG_PRINT_NOUSE
		   if (line) {
				wpa_error_two_buf(MSG_ERROR, "wpa_config_set: failed to "
			  		 "parse %s '%s'.", var, strlen(var), value, strlen(value));
			}
#endif /* CONFIG_PRINT_NOUSE */
			ret = -1;
		}
...
#ifndef CONFIG_PRINT_NOUSE
	   if (line) {
	   	 wpa_error_buf(MSG_ERROR, "wpa_config_set: unknown network field "
				   "'%s'.", var, strlen(var));
	   }
#endif /* CONFIG_PRINT_NOUSE */
		ret = -1;
...
{
#ifndef CONFIG_NO_PBKDF2
	(void)pbkdf2_sha1(ssid->passphrase, ssid->ssid, ssid->ssid_len, 4096,
		ssid->psk, PMK_LEN);
#ifndef CONFIG_PRINT_NOUSE
			...
			const char *value, int line)
{
	...
	if (os_strcmp(var, "eap") == 0) {
		...
		if (cred->eap_method == NULL)
			return -1;
		(void)os_memcpy(cred->eap_method, &method, sizeof(method));
		return 0;
	}
	...
	if (os_strcmp(var, "roaming_consortium") == 0) {
		...
		(void)os_memcpy(cred->roaming_consortium, val, len);
	...
	if (os_strcmp(var, "required_roaming_consortium") == 0) {
		...
		(void)os_memcpy(cred->required_roaming_consortium, val, len);
	...
	if (os_strcmp(var, "roaming_partner") == 0) {
		...
		*pos++ = '\0';
		...
		(void)os_memcpy(p->fqdn, val, pos - val);
		...
		*pos++ = '\0';
		...
		(void)os_memcpy(p->country, pos, os_strlen(pos) + 1);
...
char * wpa_config_get_cred_no_key(struct wpa_cred *cred, const char *var)
{
	...
	if (os_strcmp(var, "roaming_consortium") == 0) {
		...
		if (buf == NULL)
			return NULL;
		(void)wpa_snprintf_hex(buf, buflen, cred->roaming_consortium,
	...
	if (os_strcmp(var, "required_roaming_consortium") == 0) {
		...
		if (buf == NULL)
			return NULL;
		(void)wpa_snprintf_hex(buf, buflen, cred->required_roaming_consortium,
...
#ifdef LOS_WPA_PATCH
	/* add default pmf setting for hisi. */
	config->pmf = MGMT_FRAME_PROTECTION_OPTIONAL;
	if (g_sta_opt_set.usr_pmf_set_flag == WPA_FLAG_ON) {
		config->pmf = g_sta_opt_set.pmf;
	} else {
		g_sta_opt_set.pmf = config->pmf;
	}
#endif /* LOS_WPA_PATCH */
...
#endif /* HISI_CODE_CROP */
	config->config_methods = WPA_METHOD;
	return config;
}
...
void wpa_config_debug_dump_networks(struct wpa_config *config)
{
	...
	for (prio = 0; prio < config->num_prio; prio++) {
		...
		wpa_warning_log1(MSG_DEBUG, "Priority group %d",
			   ssid->priority);
		while (ssid) {
				       ...
				       const char *pos)
{
	...
	if (has_newline(pos)) {
		wpa_error_buf(MSG_ERROR, "invalid %s value with newline",
			   data->name, strlen(data->name));
		return -1;
	}
				      ...
				      const char *pos)
{
	...
	(void)line;
	if (!pos[0] || !pos[1]) {
		wpa_warning_log0(MSG_DEBUG, "Invalid country set");
		return -1;
	}
	config->country[0] = pos[0];
	config->country[1] = pos[1];
	wpa_warning_log2(MSG_DEBUG, "country='%c%c'",
	...
	int line, const char *so)
{
	...
	(void)config;
	(void)line;
	wpa_warning_buf(MSG_DEBUG, "load_dynamic_eap=%s", so, strlen(so));
	ret = eap_peer_method_load(so);
	if (ret == -2) {
		wpa_warning_log0(MSG_DEBUG, "This EAP type was already loaded - not "
			   "reloading.");
	} else if (ret) {
		wpa_error_buf(MSG_ERROR, "Failed to load dynamic EAP "
			   "method '%s'.", so, strlen(so));
		return -1;
	}
				   ...
				   const char *pos)
{
	...
	if (uuid_str2bin(pos, config->uuid)) {
		wpa_error_log1(MSG_ERROR, "Line %d: invalid UUID", line);
		return -1;
	}
	(void)uuid_bin2str(config->uuid, buf, sizeof(buf));
	wpa_warning_buf(MSG_DEBUG, "uuid=%s", buf, strlen(buf));
	return 0;
}
					 ...
					 const char *pos)
{
	...
	if (hexstr2bin(pos, config->os_version, 4)) {
		wpa_error_log1(MSG_ERROR, "Line %d: invalid os_version", line);
		return -1;
	}
	wpa_warning_log1(MSG_DEBUG, "os_version=%08x",
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	(void)data;
	if (!len) {
		wpa_error_log1(MSG_ERROR, "Line %d: "
	...
	if (tmp) {
		...
		wpa_error_log0(MSG_ERROR, "Can not allocate "
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (config->num_sec_device_types >= MAX_SEC_DEVICE_TYPES) {
		wpa_error_log1(MSG_ERROR, "Line %d: too many sec_device_type "
			   "items", line);
		return -1;
	}
...
fail:
	os_free(pref);
	wpa_error_log1(MSG_ERROR, "Line %d: Invalid p2p_pref_chan list", line);
	return -1;
}
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	if (ret < 0) {
		wpa_error_log1(MSG_ERROR, "Line %d: Invalid p2p_no_go_freq", line);
		return -1;
	}

	wpa_warning_log1(MSG_DEBUG, "P2P: p2p_no_go_freq with %u items",
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	(void)line;
	if (groups == NULL) {
		wpa_error_buf(MSG_ERROR, "Invalid sae_groups '%s'",
			   pos, strlen(pos));
		return -1;
	}
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	(void)data;
	if (!len) {
		wpa_error_log1(MSG_ERROR, "Line %d: invalid ap_vendor_elements",
			   line);
		return -1;
	}
	...
	if (tmp) {
		...
		if (hexstr2bin(pos, p, len)) {
			wpa_error_log1(MSG_ERROR, "Line %d: invalid "
		...
		wpa_error_log0(MSG_ERROR, "Cannot allocate memory for "
			   "ap_vendor_elements");
		return -1;
	}
	...
	struct wpa_config *config, int line, const char *pos)
{
	...
	(void)line;
	(void)pos;
	wpa_warning_log0(MSG_DEBUG, "no_ctrl_interface -> ctrl_interface=NULL");
	os_free(config->ctrl_interface);
...
int wpa_config_process_global(struct wpa_config *config, char *pos, int line)
{
	...
	for (i = 0; i < NUM_GLOBAL_FIELDS; i++) {
		...
		if (field->parser(field, config, line, pos + flen + 1)) {
			wpa_error_buf(MSG_ERROR, "failed to "
				   "parse '%s'.", pos, strlen(pos));
			ret = -1;
		}
	...
	if (i == NUM_GLOBAL_FIELDS) {
		...
		if (os_strncmp(pos, "wmm_ac_", 7) == 0) {
			...
			if (tmp == NULL) {
				if (line < 0)
					return -1;
				wpa_error_buf(MSG_ERROR, "invalid line "
					   "'%s'", pos, strlen(pos));
				return -1;
			}
			*tmp++ = '\0';
			if (hostapd_config_wmm_ac(config->wmm_ac_params, pos,
						  tmp)) {
				wpa_error_log1(MSG_ERROR, "Line %d: invalid WMM "
					   "AC item", line);
				return -1;
			}
		}
		...
		if (line < 0)
			return -1;
		wpa_error_buf(MSG_ERROR, "unknown global field '%s'.",
			   pos, strlen(pos));
		ret = -1;
	}

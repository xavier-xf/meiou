...
int wpa_set_wep_keys(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	for (i = 0; i < NUM_WEP_KEYS; i++) {
		...
		set = 1;
		(void)wpa_drv_set_key(wpa_s, WPA_ALG_WEP, NULL,
				     ...
				     int sec, int usec)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "Setting authentication timeout: %d sec "
		"%d usec", sec, usec);
#endif /* HISI_CODE_CROP */
	(void)eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
	wpa_s->last_auth_timeout_sec = sec;
	(void)eloop_register_timeout(sec, usec, wpa_supplicant_timeout, wpa_s, NULL);
}
...
{
	wpa_warning_log0(MSG_DEBUG, "Cancelling authentication timeout");
	(void)eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s, NULL);
#ifndef HISI_CODE_CROP
	(void)wpa_blacklist_del(wpa_s, wpa_s->bssid);
...
void wpa_supplicant_initiate_eapol(struct wpa_supplicant *wpa_s)
{
	...
	(void)os_memset(&eapol_conf, 0, sizeof(eapol_conf));
#ifndef HISI_WPA_KEY_MGMT_CROP
...
#else
	eapol_conf.eap_disabled = (wpa_s->key_mgmt != WPA_KEY_MGMT_WPS);
#endif /* HISI_WPA_KEY_MGMT_CROP */
#ifndef HISI_CODE_CROP
...
void wpa_clear_keys(struct wpa_supplicant *wpa_s, const u8 *addr)
{
	...
	for (i = 0; i < max; i++) {
		if (wpa_s->keys_cleared & BIT(i))
			continue;
		(void)wpa_drv_set_key(wpa_s, WPA_ALG_NONE, NULL, i, 0, NULL, 0,
				NULL, 0);
	}
	if (!(wpa_s->keys_cleared & BIT(0)) && addr &&
	    !is_zero_ether_addr(addr)) {
		(void)wpa_drv_set_key(wpa_s, WPA_ALG_NONE, addr, 0, 0, NULL, 0, NULL,
				0);
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
		/* MLME-SETPROTECTION.request(None) */
		(void)wpa_drv_mlme_setprotection(
			wpa_s, addr,
...
static void wpa_supplicant_start_bgscan(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->current_ssid) {
		if (bgscan_init(wpa_s, wpa_s->current_ssid, name)) {
			wpa_warning_log0(MSG_DEBUG, "Failed to initialize "
				"bgscan");
			/*
...
static void wpa_supplicant_start_autoscan(struct wpa_supplicant *wpa_s)
{
	if (autoscan_init(wpa_s, 0))
		wpa_warning_log0(MSG_DEBUG, "Failed to initialize autoscan");
}
			      ...
			      enum wpa_states state)
{
...
#endif /* LOS_WPA_PATCH */
	struct wpa_ssid *ssid_temp = NULL;
#ifndef HISI_CODE_CROP
	...
	if (wifi_dev != NULL)
		ssid_temp = wpa_config_get_network(wpa_s->conf, wifi_dev->network_id);
	if ((ssid_temp != NULL) && (state == WPA_COMPLETED) && (ssid_temp->mode == WPAS_MODE_INFRA)) {
		...
		g_reconnect_set.try_count = 0;
		wpas_connect_work_done(wpa_s);
		g_connecting_flag = WPA_FLAG_OFF;
		g_connecting_ssid = wpa_s->current_ssid;
	}
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if ((wifi_dev != NULL) && (wifi_dev->iftype == HI_WIFI_IFTYPE_STATION) &&
			...
			hi_at_printf("+NOTICE:CONNECTED\r\n");
		}
		/*
		 * After fast connection, station must reset frequency to scan for all channels
		 * in next automatic reconnection.
		 */
		if ((g_connecting_ssid != NULL) && (g_connecting_ssid->mode == WPAS_MODE_INFRA) &&
			(g_fast_connect_flag == WPA_FLAG_ON)) {
			g_connecting_ssid->frequency = 0;
		}
		if ((g_wpa_event_cb != NULL) && (wpa_s->ifmsh == NULL) && (ssid != NULL) &&
			(g_sta_delay_report_flag == WPA_FLAG_OFF)) {
			wpa_events.event = HI_WIFI_EVT_CONNECTED;
			if (ssid->ssid != NULL) {
				os_memcpy(wpa_events.info.wifi_connected.ssid, ssid->ssid, ssid->ssid_len);
			}
			os_memcpy(wpa_events.info.wifi_connected.bssid, wpa_s->bssid, HI_WIFI_MAC_LEN);
			os_memcpy(wpa_events.info.wifi_connected.ifname, wpa_s->ifname, WIFI_IFNAME_MAX_SIZE);
			wpa_events.info.wifi_connected.ssid_len = ssid->ssid_len;
			g_wpa_event_cb(&wpa_events);
		} else {
			wpa_error_log0(MSG_DEBUG, "connected, no callback report.");
		}
...
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
		(void)wpa_drv_set_operstate(wpa_s, 1);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
...
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
		(void)wpa_drv_set_operstate(wpa_s, 0);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
#ifndef IEEE8021X_EAPOL
...
#endif /* HISI_CODE_CROP */
	if (state == WPA_COMPLETED) {
		if (g_reconnect_set.enable == WPA_FLAG_ON)
			wpa_drv_set_delay_report(wpa_s, WPA_FLAG_ON);
		if (g_fast_connect_flag == WPA_FLAG_ON)
			(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_QUICK_CONNECT_OK);
	}
}
...
void wpa_supplicant_terminate_proc(struct wpa_global *global)
{
	...
	(void)global;
#endif /* CONFIG_WPS */
	wpa_error_log1(MSG_ERROR, "eloop_terminate : %d", ELOOP_TASK_WPA);
	eloop_terminate(ELOOP_TASK_WPA);
}
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	(void)wpa_config_read(wpa_s->confanother, conf);
...
static void wpa_supplicant_reconfig(int sig, void *signal_ctx)
{
	...
	if (wpa_debug_reopen_file() < 0) {
		...
		wpa_warning_log0(MSG_DEBUG, "Could not reopen debug log file");
	}
}
#endif
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	const u8 *bss_osen;
#endif /* CONFIG_HS20 */
	const u8 *bss_hisi_mesh = NULL;
	if (bss) {
		bss_wpa = wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE);
		bss_rsn = wpa_bss_get_ie(bss, WLAN_EID_RSN);
#ifdef LOS_CONFIG_MESH
		bss_hisi_mesh = wpa_bss_get_hi_mesh_ie(bss);
#endif /* LOS_CONFIG_MESH */
...
#endif /* CONFIG_PRINT_NOUSE */
		proto = WPA_PROTO_WPA;
	} else if ((bss_hisi_mesh != NULL) && (bss->caps & IEEE80211_CAP_PRIVACY)) {
		(void)os_memset(&ie, 0, sizeof(ie));
		ie.group_cipher = WPA_CIPHER_CCMP;
		ie.pairwise_cipher = WPA_CIPHER_CCMP;
		ie.key_mgmt = WPA_KEY_MGMT_PSK;
		if ((ssid != NULL) && (ssid->proto & WPA_PROTO_RSN) &&
		  (ie.group_cipher & ssid->group_cipher) &&
		  (ie.pairwise_cipher & ssid->pairwise_cipher) &&
		  (ie.key_mgmt & ssid->key_mgmt)) {
#ifndef HISI_CODE_CROP
			wpa_dbg(wpa_s, MSG_DEBUG, "RSN: using IEEE 802.11i/D9.0 (hisi mesh sta)");
#endif /* HISI_CODE_CROP */
			proto = WPA_PROTO_RSN;
		} else {
#ifndef HISI_CODE_CROP
			wpa_dbg(wpa_s, MSG_DEBUG, "wpa_supplicant_set_suites cipher or proto dismatch.");
#endif /* HISI_CODE_CROP */
			return -1;
		}
#ifdef CONFIG_HS20
...
#endif /* CONFIG_OWE */
#ifndef CONFIG_PRINT_NOUSE
			wpa_warning_log0(MSG_DEBUG, "WPA: Set cipher suites "
...
#ifndef HISI_CODE_CROP
	wpa_warning_log4(MSG_DEBUG, "WPA: Selected cipher suites: group %d "
		"pairwise %d key_mgmt %d proto %d",
		ie.group_cipher, ie.pairwise_cipher, ie.key_mgmt, proto);
#ifdef CONFIG_IEEE80211W
	if (ssid->ieee80211w) {
		wpa_warning_log1(MSG_DEBUG, "WPA: Selected mgmt group cipher %d",
			ie.mgmt_group_cipher);
	}
#endif /* CONFIG_IEEE80211W */
#endif /* HISI_CODE_CROP */
...
#ifndef HISI_CODE_CROP
		wpa_warning_log0(MSG_DEBUG, "RSN: using KEY_MGMT SAE");
#endif /* HISI_CODE_CROP */
#ifdef CONFIG_IEEE80211R
	} else if (sel & WPA_KEY_MGMT_FT_PSK) {
		wpa_s->key_mgmt = WPA_KEY_MGMT_FT_PSK;
#ifndef HISI_CODE_CROP
		wpa_warning_log0(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT FT/PSK");
#endif /* HISI_CODE_CROP */
#endif /* CONFIG_IEEE80211R */
#endif /* CONFIG_SAE */
#ifdef CONFIG_IEEE80211W
	...
	if (0) {
		...
		if (bss && ssid->bssid_set && ssid->ssid_len == 0 &&
			...
			u8 psk[PMK_LEN];
				(void)pbkdf2_sha1(ssid->passphrase, bss->ssid, bss->ssid_len,
				    4096, psk, PMK_LEN);
#ifndef CONFIG_PRINT_NOUSE
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	if (ssid->mode == WPAS_MODE_MESH) {
#ifdef CONFIG_MESH
		if (wpa_s->ifmsh) {
			if (bss && (g_mesh_connect_flag == WPA_FLAG_ON) && (hi_wpa_mesh_peer(wpa_s, bss) != HISI_OK)) {
				wpa_error_log0(MSG_ERROR, "hi_wpa_mesh_peer fail.");
			}
			return;
		}
		if (bss)
		...
		wpa_msg(wpa_s, MSG_INFO, MESH_GROUP_STARTED "ssid=\"%s\" id=%d",
			...
			ssid->id);
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_MESH_START_OK);
#else /* CONFIG_MESH */
...
#endif /* CONFIG_SME */
#ifndef HISI_CODE_CROP
	(void)wpas_abort_ongoing_scan(wpa_s);
#endif /* HISI_CODE_CROP */
...
skip_ht40:
	...
	*freq = vht_freq;
	...
	wpa_warning_log3(MSG_DEBUG, "IBSS: VHT setup freq cf1 %d, cf2 %d, bw %d",
...
#ifdef LOS_CONFIG_MESH
		    wpa_bss_get_ie(bss, WLAN_EID_RSN) || wpa_bss_get_hi_mesh_ie(bss)) &&
#else
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (deinit) {
		if (work->started) {
			...
			(void)eloop_cancel_timeout(wpa_supplicant_timeout, wpa_s,
					     NULL);
		}
		wpas_connect_work_free(cwork);
		return;
...
#ifndef HISI_CODE_CROP
	(void)os_memcpy(prev_bssid, wpa_s->bssid, ETH_ALEN);
#endif /* HISI_CODE_CROP */
	...
	if (bss &&
...
#endif /* HISI_CODE_CROP */
		(void)os_memset(wpa_s->bssid, 0, ETH_ALEN);
		(void)os_memcpy(wpa_s->pending_bssid, bss->bssid, ETH_ALEN);
#ifndef HISI_CODE_CROP
		if (bssid_changed)
		...
		if (bss)
			...
			(void)os_memset(wpa_s->pending_bssid, 0, ETH_ALEN);
	}
#ifndef HISI_CODE_CROP
	if (!wpa_s->pno)
		wpa_supplicant_cancel_sched_scan(wpa_s);
#endif /* HISI_CODE_CROP */
	wpa_supplicant_cancel_scan(wpa_s);
	wpa_msg(wpa_s, MSG_DEBUG, "Trying to assoc");
	/* Starting new association, so clear the possibly used WPA IE from the
	 * previous association. */
	(void)wpa_sm_set_assoc_wpa_ie(wpa_s->wpa, NULL, 0);
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPA_NONE) {
		...
		(void)wpa_supplicant_set_wpa_none_key(wpa_s, ssid);
	}
#endif /* HISI_WPA_KEY_MGMT_CROP */
	...
	if (bss) {
		...
		if (!wpas_driver_bss_selection(wpa_s) || ssid->bssid_set ||
		    wpa_s->key_mgmt == WPA_KEY_MGMT_WPS) {
			wpa_warning_log4(MSG_DEBUG, "Limit connection to BSSID "
				   "%02x:xx:xx:%02x:%02x:%02x",
				   (bss->bssid)[0], (bss->bssid)[3], (bss->bssid)[4], (bss->bssid)[5]);
			wpa_warning_log3(MSG_DEBUG, "Limit connection to BSSID "
				   " freq=%u MHz based on scan results "
				   "(bssid_set=%d wps=%d)", bss->freq, ssid->bssid_set,
				   wpa_s->key_mgmt == WPA_KEY_MGMT_WPS);
	...
	if (params.mgmt_frame_protection != NO_MGMT_FRAME_PROTECTION && bss) {
		...
		if (rsn && wpa_parse_wpa_ie(rsn, 2 + rsn[1], &ie) == 0) {
			if (ie.capabilities & WPA_CAPABILITY_MFPC) {
#ifndef HISI_CODE_CROP
				wpa_dbg(wpa_s, MSG_DEBUG, "WPA: Selected AP supports MFP: require MFP");
#endif /* HISI_CODE_CROP */
				params.mgmt_frame_protection = MGMT_FRAME_PROTECTION_OPTIONAL;
			} else {
/* default set is MGMT_FRAME_PROTECTION_OPTIONAL, so add NO_MGMT_FRAME_PROTECTION check*/
				params.mgmt_frame_protection = NO_MGMT_FRAME_PROTECTION;
			}
		} else {
			params.mgmt_frame_protection = NO_MGMT_FRAME_PROTECTION;
	...
	if (wep_keys_set &&
		...
		(void)wpa_set_wep_keys(wpa_s, ssid);
	}
#endif /* HISI_CODE_CROP */
...
int wpa_supplicant_remove_network(struct wpa_supplicant *wpa_s, int id)
{
	...
	if (!ssid) {
		if (g_wpa_rm_network == HI_WPA_RM_NETWORK_START)
			(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_STA_RM_NETWORK_OK);
		return -1;
	}
...
#ifdef LOS_WPA_PATCH
	if ((g_wpa_rm_network == HI_WPA_RM_NETWORK_START) && (wpa_s->wpa_state <= WPA_SCANNING))
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_STA_RM_NETWORK_OK);
#endif /* LOS_WPA_PATCH */
	return 0;
}
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (wpa_s->reassociate && !wpa_s->disconnected &&
		...
		if (wpa_s->sched_scanning) {
			wpa_warning_log0(MSG_DEBUG, "Stop ongoing sched_scan to add "
				    ...
				    struct wpa_ssid *ssid)
{
	...
	if (ssid == NULL) {
		...
		if (was_disabled != ssid->disabled) {
			wpas_notify_network_enabled_changed(wpa_s, ssid);
			if (wpa_s->sched_scanning) {
				wpa_warning_log0(MSG_DEBUG, "Stop ongoing sched_scan "
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (ssid && ssid == wpa_s->current_ssid && wpa_s->current_ssid &&
		...
		wpa_warning_log0(MSG_DEBUG, "Already associated with the "
	...
	wpa_s->reassociate = 1;
	g_fast_connect_scan_flag = WPA_FLAG_OFF;
	if (wpa_s->connect_without_scan ||
		...
		if (g_fast_connect_flag == WPA_FLAG_ON) {
			wpa_supplicant_req_scan(wpa_s, 0, 0);
			g_fast_connect_scan_flag = WPA_FLAG_ON;
		} else {
					   ...
					   const char *pkcs11_module_path)
{
	...
	if (wpa_supplicant_init_eapol(wpa_s)) {
		...
		if (pkcs11_engine_path != NULL && pkcs11_module_path != NULL)
			(void)wpas_set_pkcs11_engine_and_module_path(wpa_s, NULL,
							       NULL);
...
static int select_driver(struct wpa_supplicant *wpa_s, int i)
{
	...
	if (wpa_drivers[i]->global_init && global->drv_priv[i] == NULL) {
		...
		if (global->drv_priv[i] == NULL) {
			wpa_error_buf(MSG_ERROR, "Failed to initialize driver "
				   "'%s'", wpa_drivers[i]->name, strlen(wpa_drivers[i]->name));
			return -1;
		}
	}
#endif /* HISI_CODE_CROP */
			     ...
			     const u8 *buf, size_t len)
{
	...
	if (wpa_s->ignore_auth_resp) {
		wpa_warning_log0(MSG_INFO, "RX EAPOL - ignore_auth_resp active!");
		return;
	}
#endif /* CONFIG_TESTING_OPTIONS */
	...
	if (wpa_s->wpa_state < WPA_ASSOCIATED ||
		...
		if (wpa_s->pending_eapol_rx) {
			(void)os_get_reltime(&wpa_s->pending_eapol_rx_time);
			(void)os_memcpy(wpa_s->pending_eapol_rx_src, src_addr,
				  ETH_ALEN);
		}
		return;
	}
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->l2 && l2_packet_get_own_addr(wpa_s->l2, wpa_s->own_addr, sizeof(wpa_s->own_addr))) {
#ifndef HISI_CODE_CROP
		wpa_msg(wpa_s, MSG_ERROR, "Failed to get own L2 address");
#endif /* HISI_CODE_CROP */
		return -1;
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->bridge_ifname[0]) {
		wpa_warning_log0(MSG_DEBUG, "Receiving packets from bridge "
...
{
	wpa_warning_log0(MSG_DEBUG, "Daemonize..");
...
int wpas_init_ext_pw(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->ext_pw == NULL) {
		wpa_warning_log0(MSG_DEBUG, "EXT PW: Failed to initialize backend");
		return -1;
	}
...
#endif /* CONFIG_PRINT_NOUSE */
	(void)eloop_cancel_timeout(radio_start_next_work, radio, NULL);
	os_free(radio);
}
...
#endif /* HISI_CODE_CROP */
	(void)eloop_cancel_timeout(radio_start_next_work, radio, NULL);
	(void)eloop_register_timeout(0, 0, radio_start_next_work, radio, NULL);
}
...
#ifdef LOS_CONFIG_P2P
u8 device_names[] = {"hisi_p2p"};
u8 manufacturers[] = {"HUAWEI"};
u8 model_names[] = {"HUAWEI HISI"};
u8 model_numbers[] = {"HUAWEI HISI"};
u8 mydevice_type[] = {0x00, 0x0a, 0x00, 0x50, 0xf2, 0x04, 0x00, 0x05};
u8 serial_number[] = {"0123456789ABCDEF"};
u8 mycfg_meth[] = "physical_display virtual_push_button keypad";
u8 country[] = {"CN"};
#endif /* LOS_CONFIG_P2P */
				     ...
				     const struct wpa_interface *iface)
{
	...
	(void)os_strlcpy(wpa_s->ifname, iface->ifname, sizeof(wpa_s->ifname));
	...
	if (iface->bridge_ifname) {
		...
		(void)os_strlcpy(wpa_s->bridge_ifname, iface->bridge_ifname,
			   sizeof(wpa_s->bridge_ifname));
	}
#endif /* HISI_CODE_CROP */
	...
	if (wpa_supplicant_init_wpa(wpa_s) < 0)
		return -1;
#ifdef LOS_CONFIG_P2P
	wpa_s->conf->config_methods = os_strdup((const char *)mycfg_meth);
	os_memcpy(wpa_s->conf->country, country, 2);
	wpa_s->conf->device_name = (char *)device_names;
	wpa_s->conf->manufacturer = (char *)manufacturers;
	wpa_s->conf->model_name = (char *)model_names;
	wpa_s->conf->model_number = (char *)model_numbers;
	wpa_s->conf->serial_number = (char *)serial_number;
	os_memcpy(wpa_s->conf->device_type, mydevice_type, 8);
#endif /* LOS_CONFIG_P2P */
	...
	/* use the default value set in wpa_sm_init */
	 ...
	 */
#ifdef CONFIG_DRIVER_HISILICON
	wpa_drv_get_drv_flags(wpa_s, &(wpa_s->drv_flags));
	...
	if (!wpa_s->gas_server) {
		wpa_error_log0(MSG_ERROR, "Failed to initialize GAS server");
		return -1;
	}
#endif /* CONFIG_GAS_SERVER */
	...
	if (wpa_s->gas == NULL) {
		wpa_error_log0(MSG_ERROR, "Failed to initialize GAS query");
		return -1;
	}
...
#ifdef CONFIG_EAP_PROXY
{
	...
	if (wpa_s->mnc_len > 0) {
		...
		wpa_warning_log0(MSG_DEBUG, "eap_proxy: IMSI not available");
	}
}
	...
	(void)wpas_sched_scan_plans_set(wpa_s, wpa_s->conf->sched_scan_plans);
#endif /* HISI_CODE_CROP */
...
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
		(void)wpa_drv_set_countermeasures(wpa_s, 0);
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
		wpa_clear_keys(wpa_s, NULL);
	}

	wpa_supplicant_cleanup(wpa_s);
	if ((wifi_dev != NULL) && (wifi_dev->iftype == HI_WIFI_IFTYPE_P2P_DEVICE)) {
		wpas_p2p_deinit_iface(wpa_s);
	}
	wpa_sta_delay_report_deinit(wifi_dev);
#ifndef HISI_CODE_CROP
						 ...
						 struct wpa_supplicant *parent)
{
...
#endif /* HISI_CODE_CROP */
#ifdef CONFIG_P2P
	if (g_p2p_start_flag == WPA_FLAG_ON) {
		t_iface.p2p_mgmt = 1;
	}
#endif /* CONFIG_P2P */
	...
	wpa_warning_buf(MSG_DEBUG, "Added interface %s", wpa_s->ifname, strlen(wpa_s->ifname));
	wpa_supplicant_set_state(wpa_s, WPA_DISCONNECTED);
#ifdef CONFIG_DRIVER_HISILICON
#ifdef CONFIG_P2P
...
#endif /* HISI_CODE_CROP */
#ifdef LOS_WPA_PATCH
	hi_free_wifi_dev(hi_get_wifi_dev_by_priv(wpa_s));
#endif /* LOS_WPA_PATCH */
	return 0;
}
...
static void wpas_periodic(void *eloop_ctx, void *timeout_ctx)
{
	...
	for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		...
		bss_count = dl_list_len(&wpa_s->bss);
		iface_count++;
#ifndef HISI_CODE_CROP
#ifdef CONFIG_AP
		ap_periodic(wpa_s);
...
void wpa_register_timeout_periodic_cleanup(struct wpa_supplicant *wpa_s)
{
	if ((wpa_s == NULL) || (wpa_s->global == NULL))
		return;
	eloop_cancel_timeout(wpas_periodic, wpa_s->global, NULL);
	eloop_register_timeout(WPA_SUPPLICANT_CLEANUP_INTERVAL, 0, wpas_periodic, wpa_s->global, NULL);
}
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	if (params->wpa_debug_file_path)
		(void)wpa_debug_open_file(params->wpa_debug_file_path);
	else
	...
	if (ret) {
		...
		if (ret == -2)
			wpa_error_log0(MSG_ERROR, "Two or more EAP methods used "
				   "the same EAP type.");
		return NULL;
	}
	...
	wpa_warning_log0(MSG_DEBUG, "wpa_supplicant v" VERSION_STR);

	if (eloop_init(ELOOP_TASK_WPA)) {
	...
	if (wifi_display_init(global) < 0) {
		wpa_error_log0(MSG_ERROR, "Failed to initialize Wi-Fi Display");
		wpa_supplicant_deinit(global);
		return NULL;
	}
...
{
	int i;
	wpa_error_log0(MSG_ERROR, "wpa_supplicant_deinit enter\n");
...
struct wpa_global * wpa_supplicant_init(void)
{
	...
	(void)eloop_register_timeout(WPA_SUPPLICANT_CLEANUP_INTERVAL, 0,
				   wpas_periodic, global, NULL);
...
{
	(void)global;
	return eloop_run(ELOOP_TASK_WPA);
}
...
void wpa_supplicant_deinit(struct wpa_global *global)
{
	...
	(void)eloop_cancel_timeout(wpas_periodic, global, NULL);

	while (global->ifaces)
		(void)wpa_supplicant_remove_iface(global, global->ifaces, 1);
...
int wpas_network_disabled(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt) && !ssid->psk_set &&
		(!ssid->passphrase || ssid->ssid_len != 0)
#ifdef CONFIG_EXT_PASSWORD
		&& !ssid->ext_psk
#endif /* CONFIG_EXT_PASSWORD */
#ifndef CONFIG_SAE_NO_PW_ID
...
#ifdef CONFIG_MEM_ONLY_PSK
		&& !ssid->mem_only_psk)
#else
	   )
#endif /* CONFIG_MEM_ONLY_PSK */
		return 1;
...
int wpas_get_ssid_pmf(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	if (ssid == NULL || ssid->ieee80211w == MGMT_FRAME_PROTECTION_DEFAULT) {
		...
		if (ssid &&
			(ssid->key_mgmt &
			 ~(WPA_KEY_MGMT_NONE | WPA_KEY_MGMT_WPS)) == 0) {
#endif /* HISI_WPA_KEY_MGMT_CROP */
			/*
...
{
#ifdef LOS_WPA_PATCH
	// if mesh,the disconnect does not work.
	if ((wpa_s == NULL) || (wpa_s->ifmsh != NULL)) {
		wpa_error_log0(MSG_ERROR, "wpas_request_disconnection mesh should not use wpa disconnect cmd !");
		return;
	}
#endif /* LOS_WPA_PATCH */
#ifdef CONFIG_SME
...
#ifdef LOS_WPA_PATCH
	if (g_reconnect_set.pending_flag) {
		wpa_error_log0(MSG_ERROR, "wpas_request_disconnection cancel reconnect_timeout !");
		(void)eloop_cancel_timeout(wpa_supplicant_reconnect_timeout, wpa_s, NULL);
		g_reconnect_set.pending_flag = WPA_FLAG_OFF;
	}
				...
				unsigned int len)
{
	...
	(void)os_memset(freqs_data, 0, sizeof(struct wpa_used_freq_data) * len);
...
void wpas_vendor_elem_update(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_DEBUG, "Update vendor elements");
			    ...
			    const u8 *elem, size_t len)
{
	...
	for (; ie + 1 < end; ie += 2 + ie[1]) {
		...
		if (wpabuf_len(wpa_s->vendor_elem[frame]) == len) {
			...
			(void)os_memmove(ie, ie + len, end - (ie + len));

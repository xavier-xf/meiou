...
static void wpas_wps_assoc_with_cred(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Continuing association after eapol_cb");
	if (!use_fast_assoc ||
...
{
	(void)eloop_cancel_timeout(wpas_wps_assoc_with_cred, wpa_s, (void *) 0);
	(void)eloop_cancel_timeout(wpas_wps_assoc_with_cred, wpa_s, (void *) 1);
}
...
int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s)
{
...
#ifndef HISI_CODE_CROP
		wpa_warning_log4(MSG_DEBUG, "WPS: PIN registration with " "%02x:xx:xx:%02x:%02x:%02x"
			   " did not succeed - continue trying to find "
			   "suitable AP", (bssid)[0], (bssid)[3], (bssid)[4], (bssid)[5]);
		(void)wpa_blacklist_add(wpa_s, bssid);
#endif /* HISI_CODE_CROP */
	...
	(void)eloop_cancel_timeout(wpas_wps_timeout, wpa_s, NULL);
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS && wpa_s->current_ssid &&
		...
		wpa_warning_log1(MSG_DEBUG, "WPS: Network configuration replaced - "
		...
		wpa_supplicant_deauthenticate(wpa_s,
					      WLAN_REASON_DEAUTH_LEAVING);
		if (disabled) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Current network is "
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Checking whether fast association "
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS: Continue association from timeout");
		wpas_wps_assoc_with_cred_cancel(wpa_s);
		(void)eloop_register_timeout(0, 10000,
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPS && wpa_s->current_ssid) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Registration completed - waiting "
					 ...
					 const struct wps_credential *cred)
{
	...
	if (bss == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: The AP was not found from BSS "
			   "table - use credential as-is");
		return;
	}

	wpa_warning_log0(MSG_DEBUG, "WPS: AP found from BSS table");
	...
	if (ie == NULL && (ssid->proto & WPA_PROTO_WPA) &&
		 ...
		 */
		wpa_warning_log0(MSG_DEBUG, "WPS: The AP did not yet advertise WPA "
...
#endif /* HISI_CODE_CROP */
		(ssid->pairwise_cipher & WPA_CIPHER_TKIP)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Add CCMP into the credential "
	...
	if (wpa2 && !(ssid->proto & WPA_PROTO_RSN) &&
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Add WPA2 into the credential "
			   "based on scan results");
		if (wpa_s->conf->ap_scan == 1)
					...
					struct wpa_ssid *new_ssid)
{
	...
	for (ssid = wpa_s->conf->ssid, next = ssid ? ssid->next : NULL; ssid;
		...
		wpa_warning_log1(MSG_DEBUG, "Remove duplicate network %d", ssid->id);
#ifdef CONFIG_WPS
				   ...
				   const struct wps_credential *cred)
{
	...
	if ((wpa_s->conf->wps_cred_processing == 1 ||
		...
		if (buf) {
			(void)wpa_snprintf_hex(buf, blen,
	...
	wpa_warning_log1(MSG_DEBUG, "WPS: Authentication Type 0x%x",
		   cred->auth_type);
	wpa_warning_log1(MSG_DEBUG, "WPS: Encryption Type 0x%x", cred->encr_type);
	wpa_warning_log1(MSG_DEBUG, "WPS: Network Key Index %d", cred->key_idx);
	wpa_hexdump_key(MSG_DEBUG, "WPS: Network Key",
			cred->key, cred->key_len);
	wpa_warning_log4(MSG_DEBUG, "WPS: MAC Address " "%02x:xx:xx:%02x:%02x:%02x",
		   (cred->mac_addr)[0], (cred->mac_addr)[3], (cred->mac_addr)[4], (cred->mac_addr)[5]);

	auth_type = cred->auth_type;
	if (auth_type == (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Workaround - convert mixed-mode "
	...
	if (auth_type != WPS_AUTH_OPEN &&
	    ...
	    auth_type != WPS_AUTH_WPA2PSK) {
		wpa_warning_log1(MSG_DEBUG, "WPS: Ignored credentials for "
	...
	if (auth_type == WPS_AUTH_WPAPSK || auth_type == WPS_AUTH_WPA2PSK) {
		if (cred->key_len < 8 || cred->key_len > 2 * PMK_LEN) {
			wpa_error_log1(MSG_ERROR, "WPS: Reject PSK credential with "
	...
	if (ssid && (ssid->key_mgmt & WPA_KEY_MGMT_WPS)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Replace WPS network block based "
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Create a new network based on the "
			   "received credential");
	...
	if (ssid->key_mgmt & WPA_KEY_MGMT_PSK) {
		if (cred->key_len == 2 * PMK_LEN) {
			if (hexstr2bin((const char *) cred->key, ssid->psk,
				       PMK_LEN)) {
				wpa_error_log0(MSG_ERROR, "WPS: Invalid Network "
					   "Key");
				return -1;
			}
			ssid->psk_set = 1;
			...
			wpa_error_log1(MSG_ERROR, "WPS: Invalid Network Key "
				   "length %lu",
				   (unsigned long) cred->key_len);
			return -1;
		}
	...
	if (wpa_s->conf->update_config &&
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Failed to update configuration");
		return -1;
	}
...
static void wpas_wps_clear_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	(void)timeout_ctx;
	wpa_warning_log0(MSG_DEBUG, "WPS: Clear WPS network from timeout");
	wpas_clear_wps(wpa_s);
}
					  ...
					  struct wps_event_fail *fail)
{
	 ...
	 */
	wpa_warning_log0(MSG_DEBUG, "WPS: Register timeout to clear WPS network");
	(void)eloop_cancel_timeout(wpas_wps_clear_timeout, wpa_s, NULL);
	(void)eloop_register_timeout(0, 100000, wpas_wps_clear_timeout, wpa_s, NULL);
...
static void wpas_wps_reenable_networks(struct wpa_supplicant *wpa_s)
{
	...
	(void)eloop_cancel_timeout(wpas_wps_reenable_networks_cb, wpa_s, NULL);
	...
	if (changed) {
		...
		if (wpa_s->conf->update_config &&
		    wpa_config_write(wpa_s->confname, wpa_s->conf)) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Failed to update "
				   "configuration");
		}
#endif /* CONFIG_NO_CONFIG_WRITE */
	}
...
static void wpa_supplicant_wps_event_success(struct wpa_supplicant *wpa_s)
{
	 ...
	 */
	(void)eloop_register_timeout(10, 0, wpas_wps_reenable_networks_cb, wpa_s,
			       NULL);
					       ...
					       struct wps_event_er_ap *ap)
{
	...
	(void)uuid_bin2str(ap->uuid, uuid_str, sizeof(uuid_str));
	if (ap->pri_dev_type)
		(void)wps_dev_type_bin2str(ap->pri_dev_type, dev_type,
				     sizeof(dev_type));
	else
		dev_type[0] = '\0';
	...
	struct wpa_supplicant *wpa_s, struct wps_event_er_enrollee *enrollee)
{
	...
	(void)uuid_bin2str(enrollee->uuid, uuid_str, sizeof(uuid_str));
	if (enrollee->pri_dev_type)
		(void)wps_dev_type_bin2str(enrollee->pri_dev_type, dev_type,
				     sizeof(dev_type));
	else
		dev_type[0] = '\0';
	...
	struct wps_event_er_ap_settings *ap_settings)
{
	...
	(void)uuid_bin2str(ap_settings->uuid, uuid_str, sizeof(uuid_str));
	...
	struct wps_event_er_set_selected_registrar *ev)
{
	...
	(void)uuid_bin2str(ev->uuid, uuid_str, sizeof(uuid_str));
	switch (ev->state) {
...
static void wpas_wps_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_TIMEOUT "Requested operation timed "
		"out");
	wpa_events.event = HI_WIFI_EVT_WPS_TIMEOUT;
					      ...
					      const u8 *bssid)
{
	...
	if (bssid) {
		...
		if (count > 1) {
			wpa_warning_log0(MSG_DEBUG, "WPS: More than one SSID found "
		       ...
		       int p2p_group, int multi_ap_backhaul_sta)
{
	...
	(void)eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wpas_wps_timeout,
			       wpa_s, NULL);
				 ...
				 const u8 *ssid_val, size_t ssid_len, int freq)
{
	...
	if (ssid == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Could not add network");
		return -1;
	}
	ssid->temporary = 1;
#ifdef CONFIG_P2P
	...
	if (peer_pubkey_hash) {
		...
		(void)wpa_snprintf_hex_uppercase(hash + 8, sizeof(hash) - 8,
					   peer_pubkey_hash,
...
#endif /* CONFIG_P2P */
	if (pin)
		(void)os_snprintf(val, sizeof(val), "\"pin=%s dev_pw_id=%u%s\"",
			    pin, dev_pw_id, hash);
	else if (pin == NULL && dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER) {
		(void)os_snprintf(val, sizeof(val), "\"dev_pw_id=%u%s\"",
				dev_pw_id, hash);
	} else {
		if (wps_generate_pin(&rpin) < 0) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Could not generate PIN");
			return -1;
		}
		(void)os_snprintf(val, sizeof(val), "\"pin=%08d dev_pw_id=%u%s\"",
				rpin, dev_pw_id, hash);
	}
...
int wpas_wps_start_pin(struct wpa_supplicant *wpa_s, const u8 *bssid,
		       const char *pin, int p2p_group, u16 dev_pw_id)
{
	(void)os_get_reltime(&wpa_s->wps_pin_start_time);
...
int wpas_wps_cancel(struct wpa_supplicant *wpa_s)
{
#ifdef CONFIG_WPS_AP
	if (wpa_s->ap_iface) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Cancelling in AP mode");
	...
	if (wpa_s->wpa_state == WPA_SCANNING ||
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Cancel operation - cancel scan");
		wpa_supplicant_cancel_scan(wpa_s);
		wpas_clear_wps(wpa_s);
	} else if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Cancel operation - "
			   "deauthenticate");
		wpa_s->own_disconnect_req = 1;
		wpa_supplicant_deauthenticate(wpa_s,
					      WLAN_REASON_DEAUTH_LEAVING);
			       ...
			       size_t psk_len)
{
	...
	if (is_zero_ether_addr(p2p_dev_addr)) {
		wpa_warning_log4(MSG_DEBUG,
			   "Received new WPA/WPA2-PSK from WPS for STA " "%02x:xx:xx:%02x:%02x:%02x",
		...
		wpa_warning_log4(MSG_DEBUG,
			   ...
			   (mac_addr)[0], (mac_addr)[3], (mac_addr)[4], (mac_addr)[5]);
		wpa_warning_log4(MSG_DEBUG,
			   " P2P Device Addr " "%02x:xx:xx:%02x:%02x:%02x",
			   (p2p_dev_addr)[0], (p2p_dev_addr)[3], (p2p_dev_addr)[4], (p2p_dev_addr)[5]);
	}
...
static u16 wps_fix_config_methods(u16 config_methods)
{
	if ((config_methods &
		...
		wpa_warning_log0(MSG_INFO, "WPS: Converting display to "
	...
	if ((config_methods &
		...
		wpa_warning_log0(MSG_INFO, "WPS: Converting push_button to "
			      ...
			      struct wps_context *wps)
{
	...
	(void)uuid_bin2str(wps->uuid, buf, sizeof(buf));
}
				       ...
				       struct wps_context *wps)
{
	...
	if (wpa_s->conf->wps_vendor_ext_m1) {
		...
		if (!wps->dev.vendor_ext_m1) {
			wpa_error_log0(MSG_ERROR, "WPS: Cannot "
...
int wpas_wps_init(struct wpa_supplicant *wpa_s)
{
	...
	if ((wps->config_methods & (WPS_CONFIG_DISPLAY | WPS_CONFIG_LABEL)) ==
		...
		wpa_error_log0(MSG_ERROR, "WPS: Both Label and Display config "
	...
	if (wps->registrar == NULL) {
		wpa_warning_log0(MSG_DEBUG, "Failed to initialize WPS Registrar");
		os_free(wps);
		return -1;
	}
#endif /* CONFIG_WPS_AP */
			    ...
			    struct wpa_ssid *ssid, struct wpa_bss *bss)
{
	...
	if (eap_is_wps_pbc_enrollee(&ssid->eap)) {
		if (!wps_ie) {
			wpa_warning_log0(MSG_DEBUG, "   skip - non-WPS AP");
			return 0;
		}

		if (!wps_is_selected_pbc_registrar(wps_ie)) {
			wpa_warning_log0(MSG_DEBUG, "   skip - WPS AP "
		...
		wpa_warning_log0(MSG_DEBUG, "   selected based on WPS IE "
			   "(Active PBC)");
		wpabuf_free(wps_ie);
		return 1;
	}
			      ...
			      struct wpa_bss *bss)
{
	...
	if (wps_ie) {
		...
		if (bss->beacon_ie_len) {
			...
			if (bcn_wps == NULL) {
				wpa_warning_log0(MSG_DEBUG, "WPS: Mandatory WPS IE "
			      ...
			      struct wpa_bss *selected, struct wpa_ssid *ssid)
{
	...
	wpa_warning_log4(MSG_DEBUG, "WPS: Check whether PBC session overlap is "
		   "present in scan results; selected BSSID " "%02x:xx:xx:%02x:%02x:%02x",
		   (selected->bssid)[0], (selected->bssid)[3], (selected->bssid)[4], (selected->bssid)[5]);
	...
	if (wps_ie) {
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Selected BSS does not include "
			   "WPS IE?!");
		sel_uuid = NULL;
	}
	...
	for (i = 0; i < wpa_s->num_wps_ap; i++) {
		...
		wpa_warning_log4(MSG_DEBUG, "WPS: Another BSS in active PBC mode: "
			   "%02x:xx:xx:%02x:%02x:%02x", (ap->bssid)[0], (ap->bssid)[3], (ap->bssid)[4], (ap->bssid)[5]);
...
{
	wpa_warning_log0(MSG_DEBUG, "WPS ER: Terminated");
...
void wpas_wps_update_config(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_CONFIG_METHODS) {
		...
		if ((wps->config_methods &
		     (WPS_CONFIG_DISPLAY | WPS_CONFIG_LABEL)) ==
		    (WPS_CONFIG_DISPLAY | WPS_CONFIG_LABEL)) {
			wpa_error_log0(MSG_ERROR, "WPS: Both Label and Display "
		       ...
		       const u8 *ssid, size_t ssid_len, int freq)
{
	...
	if (wpa_s->conf->wps_nfc_dh_pubkey == NULL ||
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: Missing DH params - "
	...
	if (wps->dh_privkey == NULL || wps->dh_pubkey == NULL) {
		...
		wpabuf_free(wps->dh_privkey);
		wps->dh_privkey = NULL;
		wpa_warning_log0(MSG_DEBUG, "WPS: Failed to get DH priv/pub key");
		return -1;
	}
	...
	if (wps->dh_ctx == NULL) {
		...
		wpabuf_free(wps->dh_privkey);
		wps->dh_privkey = NULL;
		wpa_warning_log0(MSG_DEBUG, "WPS: Failed to initialize DH context");
		return -1;
	}
			     ...
			     struct wps_parse_attr *attr)
{
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Request reconnection with new network "
				    ...
				    const struct wpabuf *wps)
{
	...
	if (wps_parse_msg(wps, &attr)) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Ignore invalid data from NFC tag");
		return -1;
	}
	...
	wpa_warning_log0(MSG_DEBUG, "WPS: Ignore unrecognized NFC tag");
	return -1;
}
...
#endif /* CONFIG_P2P */
			wpa_warning_log0(MSG_DEBUG, "WPS: Could not parse NDEF");
			return -1;
		}
		wps = tmp;
	}
					...
					const struct wpabuf *data)
{
	...
	if (wps == NULL)
		return -1;
	wpa_warning_log0(MSG_DEBUG, "WPS: Received application/vnd.wfa.wsc "
		   "payload from NFC connection handover");
	wpa_hexdump_buf(MSG_DEBUG, "WPS: NFC payload", wps);
	if (wpabuf_len(wps) < 2) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Too short Wi-Fi Handover Select "
			   "Message");
		goto out;
	}
	pos = wpabuf_head(wps);
	...
	if (ret < 0) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Could not parse WSC attributes in "
	...
	if (attr.oob_dev_password == NULL ||
		...
		wpa_warning_log0(MSG_DEBUG, "WPS: No Out-of-Band Device Password "
	...
	if (attr.ssid == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No SSID included in Wi-Fi Handover "
			   "Select Message");
		ret = -1;
		goto out;
	}
	...
	if (attr.rf_bands)
		wpa_warning_log1(MSG_DEBUG, "WPS: RF Bands: %d", *attr.rf_bands);

	if (attr.ap_channel) {
		u16 chan = WPA_GET_BE16(attr.ap_channel);

		wpa_warning_log1(MSG_DEBUG, "WPS: AP Channel: %d", chan);
...
{
	wpa_warning_log0(MSG_DEBUG, "NFC: WPS connection handover reported");
				    ...
				    const struct wpabuf *sel)
{
	...
	wpa_hexdump_buf(MSG_DEBUG, "WPS: NFC payload", wps);
	if (wpabuf_len(wps) < 2) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Too short Wi-Fi Handover Request "
			   "Message");
		goto out;
	}
	pos = wpabuf_head(wps);
	...
	if (attr.uuid_e == NULL) {
		wpa_warning_log0(MSG_DEBUG, "WPS: No UUID-E included in Wi-Fi "
	...
	dev_pw_id = WPA_GET_BE16(attr.oob_dev_password +
				 WPS_OOB_PUBKEY_HASH_LEN);
	if (dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER) {
		wpa_warning_log0(MSG_DEBUG, "WPS: Unexpected OOB Device Password ID "
					...
					struct wpa_scan_res *res)
{
	...
	if (ap) {
		if (ap->type != type) {
			wpa_warning_log4(MSG_DEBUG, "WPS: AP " "%02x:xx:xx:%02x:%02x:%02x",
				   (res->bssid)[0], (res->bssid)[3], (res->bssid)[4], (res->bssid)[5]);
			wpa_warning_log2(MSG_DEBUG, "WPS: AP " " changed type %d -> %d", ap->type, type);
			ap->type = type;
#ifndef HISI_CODE_CROP
			if (type != WPS_AP_NOT_SEL_REG)
				(void)wpa_blacklist_del(wpa_s, ap->bssid);
#endif /* HISI_CODE_CROP */
		}
	...
	if (uuid)
		os_memcpy(ap->uuid, uuid, WPS_UUID_LEN);
	wpa_warning_log4(MSG_DEBUG, "WPS: AP " "%02x:xx:xx:%02x:%02x:%02x",
		   (ap->bssid)[0], (ap->bssid)[3], (ap->bssid)[4], (ap->bssid)[5]);
	wpa_warning_log1(MSG_DEBUG, " type %d added", ap->type);
out:
	wpabuf_free(wps);
}
...
void wpas_wps_notify_assoc(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	if (ap == NULL)
		return;
	ap->tries++;
	(void)os_get_reltime(&ap->last_attempt);
}

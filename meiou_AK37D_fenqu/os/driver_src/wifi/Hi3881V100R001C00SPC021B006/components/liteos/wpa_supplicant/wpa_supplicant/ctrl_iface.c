...
struct wpa_ssid *g_connecting_ssid = NULL;
...
static int set_bssid_filter(struct wpa_supplicant *wpa_s, char *val)
{
	...
	while (pos) {
		...
		filter = n;
		(void)os_memcpy(filter + count * ETH_ALEN, addr, ETH_ALEN);
		count++;
...
static int set_disallow_aps(struct wpa_supplicant *wpa_s, char *val)
{
	...
	while (pos) {
		...
		if (os_strncmp(pos, "bssid ", 6) == 0) {
			...
			bssid = n;
			(void)os_memcpy(bssid + count * ETH_ALEN, addr, ETH_ALEN);
			count++;
			...
			if ((end - pos) & 0x01 ||
				...
				os_free(ssid);
				os_free(bssid);
				wpa_warning_log0(MSG_DEBUG, "Invalid disallow_aps "
					   "SSID value '%s'", pos);
				return -1;
			}
	...
	wpa_warning_log1(MSG_DEBUG, "disallow_aps_ssid_count %d", (int) ssid_count);
	...
	wpa_warning_log0(MSG_DEBUG, "Disconnect and try to find another network "
...
static int wpas_ctrl_set_blob(struct wpa_supplicant *wpa_s, char *pos)
{
	...
	*pos++ = '\0';
	...
	if (hexstr2bin(pos, blob->data, len / 2) < 0) {
		wpa_warning_log0(MSG_DEBUG, "CTRL: Invalid blob hex data");
		wpa_config_free_blob(blob);
		return -1;
	}
	blob->len = len / 2;
...
#endif /* HISI_EAP_TRIM */
	if (os_strcasecmp(cmd, "dot11RSNAConfigPMKLifetime") == 0) {
		...
		if (val < 0 || val > 0xff) {
			ret = -1;
			wpa_warning_log1(MSG_DEBUG, "WPS: Invalid "
		...
		wpa_warning_log1(MSG_DEBUG, "WPS: Testing - dummy_cred=%d",
			   wps_testing_dummy_cred);
	} else if (os_strcasecmp(cmd, "wps_corrupt_pkhash") == 0) {
		wps_corrupt_pkhash = atoi(value);
		wpa_warning_log1(MSG_DEBUG, "WPS: Testing - wps_corrupt_pkhash=%d",
			   wps_corrupt_pkhash);
		...
		wpa_warning_log1(MSG_DEBUG, "TDLS: tdls_testing=0x%x", tdls_testing);
#endif /* CONFIG_TDLS_TESTING */
	} else if (os_strcasecmp(cmd, "tdls_disabled") == 0) {
		int disabled = atoi(value);
		wpa_warning_log1(MSG_DEBUG, "TDLS: tdls_disabled=%d", disabled);
		if (disabled) {
	...
	struct wpa_supplicant *wpa_s, char *addr)
{
	...
	if (os_strcmp(addr, "*") == 0) {
		/* remove everyone */
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE TDLS_TEARDOWN *");
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*pos++ = '\0';
	...
	if (freq_params.freq == 0) {
		wpa_warning_log0(MSG_INFO, "tdls_chanswitch: Invalid freq provided");
		return -1;
	}
					     ...
					     char *cmd)
{
	...
	if (cmd == NULL || os_strcmp(cmd, "any") == 0) {
		...
		if (hwaddr_aton(cmd + 13, p2p_dev_addr)) {
			wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE WPS_PBC: invalid ");
			return -1;
		}
...
#else
	return wpas_wps_start_pbc(wpa_s, _bssid, 0, 0);
#endif /* CONFIG_MULTI_AP */
}
	...
	struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	...
	for (pos = cmd, len = 0; *pos != '\0'; pos++) {
		...
		pin[len++] = *pos;
		if (len == 9) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Too long PIN");
			return -1;
		}
	}
	if (len != 4 && len != 8) {
		wpa_warning_log1(MSG_DEBUG, "WPS: Invalid PIN length %d", (int) len);
		return -1;
	}
	pin[len] = '\0';
	...
	if (len == 8) {
		...
		if (!wps_pin_valid(pin_val)) {
			wpa_warning_log0(MSG_DEBUG, "WPS: Invalid checksum digit");
					      ...
					      int ndef)
{
	...
	if (buf == NULL) {
		wpa_warning_log0(MSG_DEBUG, "P2P: Could not generate NFC handover request");
		return -1;
	}
					 ...
					 char *cmd)
{
	...
	pos = os_strchr(role, ' ');
	if (pos == NULL) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Missing type in handover report");
		return -1;
	}
	*pos++ = '\0';
	...
	pos = os_strchr(type, ' ');
	if (pos == NULL) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Missing request message in handover report");
		return -1;
	}
	*pos++ = '\0';
	...
	pos2 = os_strchr(pos, ' ');
	if (pos2 == NULL) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Missing select message in handover report");
		return -1;
	}
	*pos2++ = '\0';
	...
	len = os_strlen(pos);
	if (len & 0x01) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Invalid request message length in handover report");
		return -1;
	}
	len /= 2;
	...
	if (req == NULL) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Failed to allocate memory for request message");
		return -1;
	}
	if (hexstr2bin(pos, wpabuf_put(req, len), len) < 0) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Invalid request message hexdump in handover report");
		wpabuf_free(req);
		return -1;
	}
	...
	len = os_strlen(pos2);
	if (len & 0x01) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Invalid select message length in handover report");
		wpabuf_free(req);
		return -1;
	}
	len /= 2;
	...
	if (sel == NULL) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Failed to allocate memory for select message");
		wpabuf_free(req);
		return -1;
	}
	if (hexstr2bin(pos2, wpabuf_put(sel, len), len) < 0) {
		wpa_warning_log0(MSG_DEBUG, "NFC: Invalid select message hexdump in handover report");
		wpabuf_free(req);
		wpabuf_free(sel);
		return -1;
	}
	...
	if (ret)
		wpa_warning_log0(MSG_DEBUG, "NFC: Failed to process reported handover messages");
					     ...
					     char *cmd)
{
	...
	*pin++ = '\0';
	...
	if (hwaddr_aton(cmd, bssid)) {
		wpa_warning_buf(MSG_DEBUG, "CTRL_IFACE WPS_REG: invalid BSSID '%s'",
			   cmd, strlen(cmd));
		return -1;
	}
					      ...
					      char *rsp)
{
	...
	*pos++ = '\0';
	...
	if (ssid == NULL) {
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: Could not find SSID id=%d "
			   "to update", id);
		return -1;
	}
	...
	wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: 802.1X not included");
	return -1;
#endif /* IEEE8021X_EAPOL */
}
#endif /* HISI_CODE_CROP */
					    ...
					    char *buf, size_t buflen)
{
	...
	{
		char uuid_str[100];
		(void)uuid_bin2str(wpa_s->wps->uuid, uuid_str, sizeof(uuid_str));
					   ...
					   char *cmd)
{
	...
	*pos++ = '\0';
	...
	(void)os_memcpy(ssid->bssid, bssid, ETH_ALEN);
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	const u8 *ie, *ie2, *wmm_ie, *p2p, *mesh;
#ifdef LOS_WPA_PATCH
	char tmp_ssid[SSID_MAX_LEN + 1] = {0};
#endif /* LOS_WPA_PATCH */
#ifdef LOS_CONFIG_MESH
	struct hi_mesh_ap_param mesh_ap_param = {0};
	const u8 *hisi_mesh = wpa_bss_get_hi_mesh_ie(bss);
#endif /* LOS_CONFIG_MESH */
	...
	wmm_ie = wpa_bss_get_vendor_ie(bss, WMM_IE_VENDOR_TYPE);
	if (wmm_ie)
		pos = wpa_supplicant_ie_txt(pos, end, "WMM", wmm_ie, 2 + wmm_ie[1]);
#ifndef HISI_CODE_CROP
	...
	if (!ie && !ie2 && (bss->caps & IEEE80211_CAP_PRIVACY)) {
		...
		if (hisi_mesh != NULL) {
			...
			} else if (g_mesh_sta_flag) {
				ret = os_snprintf(pos, end - pos, "[WPA-PSK][RSN-PSK]");
			} else {
...
#ifdef LOS_CONFIG_MESH
	if (hisi_mesh) {
		ret = os_snprintf(pos, end - pos, "[HISI_MESH]");
		if (os_snprintf_error(end - pos, ret))
			return -1;
		pos += ret;
		if (((wpa_s->ifmsh != NULL) || g_mesh_sta_flag) &&
			(hi_wpa_get_mesh_ap_param(bss, &mesh_ap_param) == HISI_OK)) {
			if (mesh_ap_param.mesh_is_mbr) {
				ret = os_snprintf(pos, end - pos, "[MBR]");
				if (os_snprintf_error(end - pos, ret))
					return -1;
				pos += ret;
			}
			if (mesh_ap_param.accept_sta) {
				ret = os_snprintf(pos, end - pos, "[ACPTS]");
				if (os_snprintf_error(end - pos, ret))
					return -1;
				pos += ret;
			}
			if (mesh_ap_param.accept_peer) {
				ret = os_snprintf(pos, end - pos, "[ACPTP]");
				if (os_snprintf_error(end - pos, ret))
					return -1;
				pos += ret;
			}
			ret = os_snprintf(pos, end - pos, "[PNUM%d]", mesh_ap_param.peering_num);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
			ret = os_snprintf(pos, end - pos, "[BPRI%d]", mesh_ap_param.mesh_bcn_priority);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
			ret = os_snprintf(pos, end - pos, "[RANK%d]", mesh_ap_param.mesh_rank);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
			ret = os_snprintf(pos, end - pos, "[CHSW%d]", mesh_ap_param.switch_status);
			if (os_snprintf_error(end - pos, ret))
				return -1;
			pos += ret;
		}
	...
	os_memcpy(tmp_ssid, bss->ssid, bss->ssid_len);
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
		...
		if (ret < 0 || ret >= end - pos) {
			os_memset(pos, 0, end - pos); /* clean incomplete result info, set 0 as the end of buf string */
			return pos - buf;
		}
		pos += ret;
	}
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	id = atoi(cmd);
	wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: MESH_GROUP_ADD id=%d", id);

	ssid = wpa_config_get_network(wpa_s->conf, id);
	if (ssid == NULL) {
		wpa_warning_log1(MSG_DEBUG,
			   "CTRL_IFACE: Could not find network id=%d", id);
		return -1;
	}
	if (ssid->mode != WPAS_MODE_MESH) {
		wpa_warning_log0(MSG_DEBUG,
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	wpa_warning_buf(MSG_DEBUG, "CTRL_IFACE: MESH_GROUP_REMOVE ifname=%s", cmd, strlen(cmd));
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (os_strncmp(cmd, "any", 3) == 0) {
#ifndef HISI_CODE_CROP
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: SELECT_NETWORK any");
#endif /* HISI_CODE_CROP */
		ssid = NULL;
	} else {
		id = atoi(cmd);
#ifndef HISI_CODE_CROP
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: SELECT_NETWORK id=%d", id);
#endif /* HISI_CODE_CROP */
		ssid = wpa_config_get_network(wpa_s->conf, id);
		if (ssid == NULL) {
#ifndef HISI_CODE_CROP
			wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: Could not find "
				   "network id=%d", id);
#endif /* HISI_CODE_CROP */
			return -1;
		}
		...
		if (ssid->disabled == 2) {
#ifndef HISI_CODE_CROP
			wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: Cannot use "
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (os_strcmp(cmd, "all") == 0) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: ENABLE_NETWORK all");
		ssid = NULL;
	} else {
		id = atoi(cmd);
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: ENABLE_NETWORK id=%d", id);
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	/* cmd: "<network id>" or "all" */
	if (os_strcmp(cmd, "all") == 0) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: DISABLE_NETWORK all");
		ssid = NULL;
	} else {
		id = atoi(cmd);
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: DISABLE_NETWORK id=%d", id);
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	int ret = 0;
#endif /* LOS_WPA_PATCH */
	wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: ADD_NETWORK");
#ifdef LOS_WPA_PATCH
	wifi_dev = hi_get_wifi_dev_by_name(wpa_s->ifname);
	if (wifi_dev == NULL) {
		return -1;
	}
#endif /* LOS_WPA_PATCH */
...
#endif /* LOS_WPA_PATCH */
#ifndef HISI_CODE_CROP
	wpa_error_log1(MSG_ERROR, "wpa_supplicant_ctrl_iface_add_network: add network id : %d\n", ssid->id);
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	/* cmd: "<network id>" or "all" */
	if (os_strcmp(cmd, "all") == 0) {
		if (g_reconnect_set.pending_flag) {
			wpa_error_log0(MSG_ERROR, "wpa_supplicant_ctrl_iface_remove_network cancel reconnect_timeout !");
			(void)eloop_cancel_timeout(wpa_supplicant_reconnect_timeout, wpa_s, NULL);
			g_reconnect_set.pending_flag = WPA_FLAG_OFF;
		}
		(void)eloop_cancel_timeout(wpa_supplicant_reconnect_restart, wpa_s, NULL);
...
#endif /* CONFIG_WPS */
			(void)wpa_config_remove_network(wpa_s->conf, id);
		}
		return 0;
	}
#endif /* HISI_CODE_CROP */
	...
	id = atoi(cmd);
#ifndef HISI_CODE_CROP
	wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: REMOVE_NETWORK id=%d", id);
#endif /* HISI_CODE_CROP */
	result = wpa_supplicant_remove_network(wpa_s, id);
	if (result == -1) {
#ifndef HISI_CODE_CROP
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: Could not find network "
			   "id=%d", id);
#endif /* HISI_CODE_CROP */
		return -1;
	}
	if (result == -2) {
#ifndef HISI_CODE_CROP
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: Not able to remove the "
			   "network id=%d", id);
#endif /* HISI_CODE_CROP */
		return -1;
	}
	...
	char *name, char *value)
{
	...
	if (os_strcmp(name, "ssid") == 0) {
		if (hi_wpa_ssid_config_set(ssid, name, value) == HISI_OK)
			goto SSID_SET;
		else
			return -1;
	}
...
SSID_SET:
	...
	if ((os_strcmp(name, "psk") == 0 &&
		...
		(void)wpa_config_update_prio_list(wpa_s->conf);
#endif /* HISI_CODE_CROP */
	...
	struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	...
	*name++ = '\0';
	...
	ssid = wpa_config_get_network(wpa_s->conf, id);
	if (ssid == NULL) {
		wpa_msgdump_log1(MSG_EXCESSIVE, "CTRL_IFACE: Could not find network "
			   "id=%d", id);
		return -1;
	}
					      ...
					      char *buf, size_t buflen)
{
	...
	wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: ADD_CRED");
				 ...
				 struct wpa_cred *cred)
{
	...
	if (cred == NULL) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: Could not find cred");
		return -1;
	}
	...
	while (ssid) {
		if (ssid->parent_cred == cred) {
			...
			wpa_warning_log1(MSG_DEBUG, "Remove network id %d since it "
				   "used the removed credential", ssid->id);
			res = os_snprintf(str, sizeof(str), "%d", ssid->id);
			if (os_snprintf_error(sizeof(str), res))
				str[sizeof(str) - 1] = '\0';
			ssid = ssid->next;
			(void)wpa_supplicant_ctrl_iface_remove_network(wpa_s, str);
		} else
			ssid = ssid->next;
	}
						 ...
						 char *cmd)
{
	...
	if (os_strcmp(cmd, "all") == 0) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: REMOVE_CRED all");
		cred = wpa_s->conf->cred;
		while (cred) {
			prev = cred;
			cred = cred->next;
			(void)wpas_ctrl_remove_cred(wpa_s, prev);
		}
		return 0;
	}
	...
	id = atoi(cmd);
	wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: REMOVE_CRED id=%d", id);
					      ...
					      char *cmd)
{
	...
	*value++ = '\0';
	...
	cred = wpa_config_get_cred(wpa_s->conf, id);
	if (cred == NULL) {
		wpa_warning_log1(MSG_DEBUG, "CTRL_IFACE: Could not find cred id=%d",
			   id);
		return -1;
	}
...
static int wpa_supplicant_ctrl_iface_save_config(struct wpa_supplicant *wpa_s)
{
	...
	if (!wpa_s->conf->update_config) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: SAVE_CONFIG - Not allowed "
	...
	if (ret) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: SAVE_CONFIG - Failed to "
			   "update configuration");
	} else {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE: SAVE_CONFIG - Configuration"
			   " updated");
	}
					 ...
					 size_t buflen)
{
	...
	if (os_strncmp(cmd, "RANGE=", 6) == 0) {
		if (os_strncmp(cmd + 6, "ALL", 3) == 0) {
			...
			if ((ctmp = os_strchr(cmd + 6, '-')) == NULL) { /*lint !e158*/
				wpa_warning_log0(MSG_INFO, "Wrong BSS range "
					   "format");
				return 0;
			}
...
{
	wpa_warning_log0(MSG_DEBUG, "Dropping SA without deauthentication");
					  ...
					  char *addr)
{
	...
	if (!ssid) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE ROAM: No network "
	...
	if (!bss) {
		wpa_warning_log0(MSG_DEBUG, "CTRL_IFACE ROAM: Target AP not found "
			   "from BSS table");
		return -1;
	}
	...
	wpa_s->reassociate = 1;
	(void)wpa_supplicant_connect(wpa_s, bss, ssid);
...
static int p2ps_ctrl_parse_cpt_priority(const char *pos, u8 *cpt)
{
	...
	for (i = 0; (token = cstr_token(pos, ": \t", &last)); i++) {
		...
		if (token_len  == 3 &&
			...
			wpa_error_log0(MSG_ERROR, "P2PS: Unsupported CPT name");
			cpt[0] = 0;
			return -1;
		}
...
static int p2p_ctrl_asp_provision_resp(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	wpa_warning_buf(MSG_DEBUG, "p2p_ctrl_asp_provision_resp: %s", cmd, strlen(cmd));
...
static int p2p_ctrl_asp_provision(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	wpa_warning_buf(MSG_DEBUG, "p2p_ctrl_asp_provision: %s", cmd, strlen(cmd));
	if (hwaddr_aton(cmd, addr))
		return -1;
...
static int p2p_ctrl_service_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*pos++ = '\0';
	...
	wpa_warning_buf(MSG_DEBUG, "Unknown service '%s'", cmd, strlen(cmd));
	return -1;
}
...
static int p2p_ctrl_invite_persistent(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (max_oper_chwidth < 0)
		return -1;
#endif /* HISI_CODE_CROP */
	return wpas_p2p_invite(wpa_s, _peer, ssid, NULL, freq, pref_freq);
}
...
static int p2p_ctrl_invite_group(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*pos = '\0';
	pos += 6;
	if (hwaddr_aton(pos, peer)) {
		wpa_warning_buf(MSG_DEBUG, "P2P: Invalid MAC address '%s'", pos, strlen(pos));
		return -1;
	}
	...
	if (pos) {
		...
		if (hwaddr_aton(pos, go_dev_addr)) {
			wpa_warning_buf(MSG_DEBUG, "P2P: Invalid MAC address '%s'",
				   pos, strlen(pos));
			return -1;
		}
		go_dev = go_dev_addr;
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	while ((token = str_token(cmd, " ", &context))) {
		if (sscanf(token, "persistent=%d", &group_id) == 1) {
			continue;
#ifdef CONFIG_ACS
	...
	return wpas_p2p_group_add(wpa_s, persistent, freq);
}
			 ...
			 char *buf, size_t buflen)
{
	...
	while (i < MAX_WPA_P2P_PEER_NUM) {
		...
		if (memcpy_s(g_p2p_peers[i].bssid, ETH_ALEN, info->p2p_device_addr, ETH_ALEN) != EOK)
			return -1;
		res |= os_snprintf(g_p2p_peers[i].device_name , sizeof(g_p2p_peers[i].device_name), "%s", info->device_name);
		if (res < 0) {
			return -1;
		}
		g_p2p_peers[i].device_name[res] = '\0';
		g_p2p_peers[i].config_methods = info->config_methods;
		g_p2p_peers[i].listen_freq = dev->listen_freq;
		ssid = wpas_p2p_get_persistent(wpa_s, info->p2p_device_addr, NULL, 0);
		if (ssid != NULL) {
			g_p2p_peers[i].persistent_id = ssid->id;
		} else {
			g_p2p_peers[i].persistent_id = -1;//not the persistent group
		}
		res = os_snprintf(g_p2p_peers[i].manufacturer , sizeof(g_p2p_peers[i].manufacturer), "%s", info->manufacturer);
		if (res < 0) {
			return -1;
		}
		g_p2p_peers[i].manufacturer[res] = '\0';
		i++;
		addr_ptr = (u8 *)info->p2p_device_addr;
		g_p2p_peers_discoverd++;
		next = 1;
	}
	return 0;
#endif
	...
	if (group_capab &&
		...
		wpa_warning_log4(MSG_DEBUG,
			   ...
			   (info->p2p_device_addr)[0], (info->p2p_device_addr)[3], (info->p2p_device_addr)[4], (info->p2p_device_addr)[5]);
		group_capab = 0;
	}
				  ...
				  const char *param)
{
	...
	for (i = 0; i < wpa_s->global->p2p_disallow_freq.num; i++) {
		...
		wpa_warning_log2(MSG_DEBUG, "P2P: Disallowed frequency range %u-%u",
			   freq->min, freq->max);
	}
#endif /* HISI_CODE_CROP */
	...
	*param++ = '\0';
	...
	if (os_strcmp(cmd, "noa") == 0) {
		...
		if (count == 0 && duration > 0)
			return -1;
		wpa_warning_log3(MSG_DEBUG, "CTRL_IFACE: P2P_SET GO NoA: count=%d "
	...
	if (os_strcmp(cmd, "disabled") == 0) {
		...
		wpa_warning_buf(MSG_DEBUG, "P2P functionality %s",
			   wpa_s->global->p2p_disabled ?
			   "disabled" : "enabled", wpa_s->global->p2p_disabled ? strlen("disabled") : strlen("enabled"));
	...
	if (os_strcmp(cmd, "conc_pref") == 0) {
		if (os_strcmp(param, "sta") == 0)
			...
			wpa_warning_log0(MSG_INFO, "Invalid conc_pref value");
			return -1;
		}
		wpa_warning_buf(MSG_DEBUG, "Single channel concurrency preference: "
			   "%s", param, strlen(param));
		return 0;
	}
	...
	wpa_warning_buf(MSG_DEBUG, "CTRL_IFACE: Unknown P2P_SET field value '%s'",
		   cmd, strlen(cmd));
...
static int p2p_ctrl_iface_p2p_lo_start(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	{
		wpa_warning_buf(MSG_DEBUG,
			   "CTRL: Invalid P2P LO Start parameter: '%s'", cmd, strlen(cmd));
		return -1;
	}
...
static int wpa_supplicant_pktcnt_poll(struct wpa_supplicant *wpa_s, char *buf,
				      size_t buflen)
{
	struct hostap_sta_driver_data sta = {0};
	int ret;
...
static void wpa_supplicant_ctrl_iface_flush(struct wpa_supplicant *wpa_s)
{
	...
	(void)eloop_cancel_timeout(wpa_supplicant_stop_countermeasures, wpa_s, NULL);
	...
	(void)wpa_drv_radio_disable(wpa_s, 0);
	wpa_blacklist_clear(wpa_s);
	wpa_s->extra_blacklist_count = 0;
	(void)wpa_supplicant_ctrl_iface_remove_network(wpa_s, "all");
	(void)wpa_supplicant_ctrl_iface_remove_cred(wpa_s, "all");
	...
	(void)wpa_sm_set_param(wpa_s->wpa, RSNA_PMK_LIFETIME, 43200);
	(void)wpa_sm_set_param(wpa_s->wpa, RSNA_PMK_REAUTH_THRESHOLD, 70);
	(void)wpa_sm_set_param(wpa_s->wpa, RSNA_SA_TIMEOUT, 60);
	...
	(void)eloop_cancel_timeout(wpas_network_reenabled, wpa_s, NULL);
...
static void wpas_ctrl_radio_work_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (deinit) {
		if (work->started)
			(void)eloop_cancel_timeout(wpas_ctrl_radio_work_timeout,
					     work, NULL);
	...
	if (!ework->timeout)
		ework->timeout = 10;
	(void)eloop_register_timeout(ework->timeout, 0, wpas_ctrl_radio_work_timeout,
			       work, NULL);
}
				    ...
				    char *buf, size_t buflen)
{
	...
	(void)os_strlcpy(ework->type, "ext:", sizeof(ework->type));
	(void)os_memcpy(ework->type + 4, cmd, type_len);
...
static int wpas_ctrl_radio_work_done(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	dl_list_for_each(work, &wpa_s->radio->work, struct wpa_radio_work, list)
	{
		...
		wpa_dbg(wpa_s, MSG_DEBUG,
			...
			ework->id, ework->type);
		(void)eloop_cancel_timeout(wpas_ctrl_radio_work_timeout, work, NULL);
		wpa_s->ext_work_in_progress = 0;
		radio_work_done(work);
		os_free(ework);
		return 3; /* "OK\n" */
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
		*reply_len = -1;
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_SCAN_OK);
		return;
	}
#else
	(void)reply;
	...
	if (params) {
		...
		if (pos) {
			...
			if (hwaddr_aton(pos, bssid)) {
#ifndef HISI_CODE_CROP
				wpa_error_buf(MSG_ERROR, "Invalid BSSID %s", pos, strlen(pos));
#endif /* HISI_CODE_CROP */
				*reply_len = -1;
				goto done;
			}
		...
		while (pos && *pos != '\0') {
			if (os_strncmp(pos, "ssid ", 5) == 0) {
				...
				ssid = ns;
				if ((end - pos) <= SSID_MAX_LEN) {
					ssid[ssid_count].ssid_len = end - pos;
					os_memcpy(ssid[ssid_count].ssid, pos, ssid[ssid_count].ssid_len);
				} else {
					*reply_len = -1;
					goto done;
				}
#ifndef HISI_CODE_CROP
				ssid[ssid_count].ssid_len = strlen(ssid[ssid_count].ssid);
	...
	wpa_s->prefix_ssid_scan_flag = g_ssid_prefix_flag;
	...
	if (!wpa_s->scanning &&
		...
		wpa_s->manual_scan_freqs = manual_scan_freqs;
		manual_scan_freqs = NULL;
		wpa_warning_log0(MSG_DEBUG, "Stop ongoing sched_scan to allow requested full scan to proceed");
		wpa_supplicant_cancel_sched_scan(wpa_s);
#endif /* HISI_CODE_CROP */
		wpa_s->scan_req = MANUAL_SCAN_REQ;
		wpa_supplicant_req_scan(wpa_s, 0, 0);
...
#ifndef HISI_CODE_CROP
		wpa_warning_log0(MSG_DEBUG, "Ongoing scan action - reject new request");
		*reply_len = os_snprintf(reply, reply_size, "FAIL-BUSY\n");
#endif /* HISI_CODE_CROP */
		*reply_len = -1;
	}
...
{
	wpa_warning_log0(MSG_DEBUG, "External MGMT TX - done waiting");
...
static void wpas_ctrl_event_test_cb(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_warning_log1(MSG_DEBUG, "TEST: Send %d control interface event messages",
		   count);
	for (i = 0; i < count; i++) {
...
static int wpas_ctrl_reset_pn(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_INFO, "TESTING: Reset PN");
...
static int wpas_ctrl_resend_assoc(struct wpa_supplicant *wpa_s)
{
	...
	wpa_warning_log0(MSG_INFO, "TESTING: Resend association request");
...
static int wpa_sta_status(struct wpa_supplicant *wpa_s)
{
	if ((g_sta_status == NULL) || (wpa_s == NULL)) {
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_STA_STATUS_FAIL);
		return HISI_FAIL;
	}
	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		if (memcpy_s(g_sta_status->bssid, ETH_ALEN, wpa_s->bssid, ETH_ALEN) != EOK) {
			(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_STA_STATUS_FAIL);
			return HISI_FAIL;
		}
		if (hi_freq_to_channel(wpa_s->assoc_freq, &g_sta_status->channel) != HISI_OK) {
			(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_STA_STATUS_FAIL);
			return HISI_FAIL;
		}
		if ((ssid != NULL) && (ssid->ssid_len > 0) && (ssid->ssid != NULL)) {
			if (memcpy_s(g_sta_status->ssid, sizeof(g_sta_status->ssid), ssid->ssid, ssid->ssid_len) != EOK) {
				(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_STA_STATUS_FAIL);
				return HISI_FAIL;
			}
		}
	}
	g_sta_status->status = (wpa_s->wpa_state == WPA_COMPLETED) ? HI_WIFI_CONNECTED : HI_WIFI_DISCONNECTED;
	(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_STA_STATUS_OK);
	return HISI_OK;
}


#ifdef LOS_CONFIG_MESH
static int wpa_mesh_deauth_peer(struct wpa_supplicant *wpa_s, const char *addr_txt)
{
	struct sta_info *sta = NULL;
	u8 addr[ETH_ALEN] = { 0 };
	if ((g_hapd == NULL) || (wpa_s->ifmsh == NULL) || (g_hapd != wpa_s->ifmsh->bss[0])) {
		wpa_error_log0(MSG_ERROR, "wpa_mesh_deauth_peer fail.");
		goto OUT;
	}
	if (hwaddr_aton(addr_txt, addr)) {
		goto OUT;
	}

	sta = ap_get_sta(g_hapd, addr);
	if (sta == NULL) {
		wpa_error_log0(MSG_ERROR, "station not exist!");
		goto OUT;
	}

	if (sta->plink_state >= PLINK_IDLE) {
		wpas_mesh_peer_remove(wpa_s, addr);
		hi_wpa_join_mesh_completed(wpa_s);
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_MESH_DEAUTH_OK);
		return HISI_OK;
	}
	hostapd_drv_sta_remove(g_hapd, addr);
	(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_MESH_DEAUTH_OK);
	return HISI_OK;
OUT:
	(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_MESH_DEAUTH_FAIL);
	return HISI_FAIL;
}
#endif /* LOS_CONFIG_MESH */


static int hapd_deauth_peer(struct hostapd_data *hapd, const char *addr_txt)
{
	struct sta_info *sta = NULL;
	u8 addr[ETH_ALEN] = { 0 };
	if (hapd == NULL) {
		wpa_error_log0(MSG_ERROR, "hapd_deauth_peer fail.");
		goto OUT;
	}
	if (hwaddr_aton(addr_txt, addr)) {
		goto OUT;
	...
	(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_AP_DEAUTH_OK);
	return HISI_OK;
OUT:
	(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_AP_DEAUTH_FAIL);
	return HISI_FAIL;
}


static int wpa_supplicant_iface_remove(struct wpa_supplicant *wpa_s)
{
	int res = 0;
	if ((wpa_s == NULL) || (wpa_s->global == NULL)) {
		wpa_error_log0(MSG_ERROR, "wpa_s is null.");
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_REMOVE_IFACE_FLAG);
		return HISI_FAIL;
	}
	wpa_s->reassociate = 0;
	wpa_s->disconnected = 1;
	wpa_supplicant_cancel_scan(wpa_s);
	res = wpa_supplicant_remove_iface(wpa_s->global, wpa_s, 0);
	(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_REMOVE_IFACE_FLAG);
	return res;
}
...
#ifdef LOS_CONFIG_MESH
static void wpa_fill_mesh_peer_info(struct sta_info *sta)
{
	if ((sta == NULL) || (g_mesh_sta_info == NULL))
		return;
	if (sta->flags & WLAN_STA_AUTHORIZED) {
		g_mesh_sta_info->mesh_role = MESH_STA;
	} else {
		g_mesh_sta_info->mesh_role = MESH_AP;
		g_mesh_sta_info->mesh_is_mbr = sta->mesh_is_mbr;
		g_mesh_sta_info->mesh_bcn_priority = sta->mesh_bcn_priority;
		g_mesh_sta_info->mesh_block = sta->plink_state == PLINK_BLOCKED ? WPA_FLAG_ON : WPA_FLAG_OFF;
	}
	(void)os_memcpy(g_mesh_sta_info->mac, sta->addr, ETH_ALEN);
}
#endif /* LOS_CONFIG_MESH */


static int wpa_supplicant_show_sta(hi_wifi_iftype iftype)
{
	struct sta_info *sta = NULL;
	unsigned int sta_count = 0;
	g_sta_num = WPA_MIN(g_sta_num, WIFI_DEFAULT_MAX_NUM_STA);
#ifdef LOS_CONFIG_MESH
	if ((g_hapd == NULL) || (g_mesh_get_sta_flag && (g_mesh_sta_info == NULL)) ||
#else
	if ((g_hapd == NULL) ||
#endif /* LOS_CONFIG_MESH */
	((iftype == HI_WIFI_IFTYPE_AP) && (g_ap_sta_info == NULL))) {
		g_sta_num = 0;
		return -1;
	}
	for (sta = g_hapd->sta_list; sta != NULL; sta = sta->next) {
		if (sta_count >= g_sta_num) {
			break;
		}
		if (iftype == HI_WIFI_IFTYPE_AP) {
			(void)os_memcpy(g_ap_sta_info->mac, sta->addr, ETH_ALEN);
			g_ap_sta_info++;
		}
#ifdef LOS_CONFIG_MESH
		if (g_mesh_get_sta_flag == WPA_FLAG_ON) {
			if ((sta->plink_state != PLINK_ESTAB) && (sta->plink_state != PLINK_BLOCKED) &&
				!(sta->flags & WLAN_STA_AUTHORIZED)) {
				continue;
			}
			wpa_fill_mesh_peer_info(sta);
			g_mesh_sta_info++;
		}
#endif /* LOS_CONFIG_MESH */
		sta_count++;
		...
		while (end - pos > 2) {
			...
			if (s_id == WLAN_EID_MEASURE_REPORT && s_len > 3) {
				...
				case MEASURE_TYPE_LCI:
					if (lci[0])
						break;
					(void)wpa_snprintf_hex(lci, sizeof(lci),
							 pos, s_len);
					break;
				case MEASURE_TYPE_LOCATION_CIVIC:
					if (civic[0])
						break;
					(void)wpa_snprintf_hex(civic, sizeof(civic),
							 pos, s_len);
					break;
				}
			}
					 ...
					 char *cmd)
{
	...
	if (!type) {
		wpa_warning_log0(MSG_INFO, "CTRL: MAC_RAND_SCAN no type specified");
		return -1;
	}
...
#ifdef LOS_WPA_PATCH
static int wpa_supplicant_ctrl_iface_add(struct wpa_supplicant *wpa_s, char *ifname)
{
	struct wpa_interface iface;
	struct wpa_supplicant *new_wpa_s = NULL;
	struct hisi_wifi_dev *wifi_dev = hi_get_wifi_dev_by_name(ifname);
	if ((wpa_s == NULL) || (wifi_dev == NULL) ||
		(wpa_s->driver == NULL) || (wpa_s->conf == NULL)) {
		wpa_error_log0(MSG_ERROR, "wpa_s or wifi dev is null.");
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_ADD_IFACE_ERROR);
		return HISI_FAIL;
	}
	os_memset(&iface, 0, sizeof(iface));
	iface.ifname = ifname;
	iface.driver = wpa_s->driver->name;
	iface.driver_param = (const char *)(wpa_s->conf->driver_param);
	iface.ctrl_interface = wpa_s->conf->ctrl_interface;
	new_wpa_s = wpa_supplicant_add_iface(wpa_s->global, &iface, NULL);
	if (new_wpa_s == NULL) {
		wpa_warning_log0(MSG_ERROR, "wpa_supplicant_add_iface fail.");
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_ADD_IFACE_ERROR);
		return HISI_FAIL;
	}
	LOS_TaskLock();
	wifi_dev->priv = new_wpa_s;
	LOS_TaskUnlock();
	(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_ADD_IFACE_OK);
	return HISI_OK;
}
#endif /* LOS_WPA_PATCH */
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if ((buf == NULL) || (resp_len == NULL))
		return NULL;
	if (os_strncmp(buf, "SOFTAP_START ", 13) == 0) {
		hostapd_main(buf + 13);
		goto OUT;
	} else if (os_strncmp(buf, "WPA_START ", 10) == 0) {
		wpa_supplicant_main(buf + 10);
		goto OUT;
	} else if (os_strncmp(buf, "ELOOP_TERMINATE ", 16) == 0) {
		wpa_error_log1(MSG_ERROR, "eloop_terminate : %d", atoi(buf + 16));
		eloop_terminate(atoi(buf + 16));
		goto OUT;
	} else if (os_strcmp(buf, "REMOVE_IFACE") == 0) {
		if (wpa_supplicant_iface_remove(wpa_s) < 0)
		goto OUT;
	} else if (os_strncmp(buf, "ADD_IFACE ", 10) == 0) {
		if (wpa_supplicant_ctrl_iface_add(wpa_s, buf + 10) < 0)
		goto OUT;
	}
	wifi_dev = hi_get_wifi_dev_by_priv(wpa_s);
	if (wifi_dev == NULL) {
		wpa_error_log0(MSG_ERROR, "ctrl iface process: get wifi dev failed\n");
		goto OUT;
	}

	if (os_strncmp(buf, "AP_DEAUTH ", 10) == 0) {
		if (wifi_dev->iftype == HI_WIFI_IFTYPE_AP)
			hapd_deauth_peer((struct hostapd_data *)wpa_s, buf + 10);
		goto OUT;
	} else if (os_strcmp(buf, "SHOW_STA") == 0) {
		if ((wifi_dev->iftype == HI_WIFI_IFTYPE_AP) || (wifi_dev->iftype == HI_WIFI_IFTYPE_MESH_POINT)) {
			if (wpa_supplicant_show_sta(wifi_dev->iftype) < 0) {
				(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_SHOW_STA_ERROR);
			} else {
				(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_SHOW_STA_OK);
			}
		} else {
			(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_SHOW_STA_ERROR);
		}
	...
	if ((wpa_s == NULL) || (wifi_dev->iftype == HI_WIFI_IFTYPE_AP) || (eloop_is_running(ELOOP_TASK_WPA) != HISI_OK))
		goto OUT;

#ifdef CONFIG_MESH
	if (os_strcmp(buf, "JOIN_MESH") == 0) {
		if (wpa_s->ifmsh != NULL)
			hi_wpa_join_mesh(wpa_s);
		goto OUT;
	}
#endif
	if ((os_strncmp(buf, "STA_SET_DELAY_RP ", 17) == 0) && (wifi_dev->iftype == HI_WIFI_IFTYPE_STATION)) {
		wpa_drv_set_delay_report(wpa_s, atoi(buf + 17));
		goto OUT;
	}
	...
	if (reply == NULL) {
		wpa_error_log0(MSG_ERROR, "ctrl_iface_process fail to malloc buffer.");
		*resp_len = 1;
		return NULL;
	}
	os_memcpy(reply, "OK\n", 3);
	...
	if (os_strncmp(buf, "STATUS", 6) == 0) {
		...
		g_p2p_status_buf = reply;
		g_p2p_status_len = reply_len;
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_GET_STATUS_OK);
#endif /* LOS_CONFIG_P2P */
#ifndef HISI_CODE_CROP
		...
		reply_len = p2p_ctrl_peer(wpa_s, buf + 9, reply,
					      reply_size);
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_GET_PEER_OK);
...
#endif /* LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT */
#ifdef LOS_CONFIG_P2P
	} else if (os_strcmp(buf, "P2P_UPDATE_CONFIG") == 0) {
		...
		if (wpa_supplicant_ctrl_iface_ctrl_rsp(
			 ...
			 */
			(void)eloop_register_timeout(0, 0, wpas_ctrl_eapol_response,
					       wpa_s, NULL);
		...
		if (g_sta_delay_report_flag == WPA_FLAG_ON)
			wpa_drv_set_delay_report(wpa_s, WPA_FLAG_OFF);
		g_sta_delay_report_flag = WPA_FLAG_OFF;
		wpas_request_disconnection(wpa_s);
#ifdef LOS_WPA_PATCH
	} else if (os_strcmp(buf, "STA_STATUS") == 0) {
		wpa_sta_status(wpa_s);
#ifdef LOS_CONFIG_MESH
	} else if (os_strncmp(buf, "MESH_DEAUTH ", 12) == 0) {
		wpa_mesh_deauth_peer(wpa_s, buf + 12);
#endif /* LOS_CONFIG_MESH */
#endif /* LOS_WPA_PATCH */
		...
		reply_len = wpa_supplicant_ctrl_iface_scan_results(
			wpa_s, reply, reply_size);
#ifdef LOS_WPA_PATCH
		g_scan_result_buf = reply;
		g_result_len = reply_len;
		(void)LOS_EventWrite(&g_wpa_event, WPA_EVENT_GET_SCAN_RESULT_OK);
#endif /* LOS_WPA_PATCH */
#ifndef HISI_CODE_CROP
		...
		(void)wpas_ctrl_iface_erp_flush(wpa_s);
					   ...
					   char *cmd)
{
	...
	(void)os_memset(&iface, 0, sizeof(iface));
...
fail:
	if (create_iface)
		(void)wpa_drv_if_remove(global->ifaces, WPA_IF_STATION, iface.ifname);
	return -1;
}
					     ...
					     size_t reply_size)
{
	...
	if (!fst_parse_attach_command(cmd, ifname, sizeof(ifname), &cfg)) {
		...
		if (wpa_s) {
			if (wpa_s->fst) {
				wpa_warning_log0(MSG_INFO, "FST: Already attached");
				return -1;
			}
						...
						char *buf, size_t *resp_len)
{
	...
	(void)os_memcpy(reply, "OK\n", 3);
	reply_len = 3;

	if (os_strcmp(buf, "PING") == 0) {
		(void)os_memcpy(reply, "PONG\n", 5);
		reply_len = 5;
		...
		(void)os_memcpy(reply, "UNKNOWN COMMAND\n", 16);
		reply_len = 16;
	}

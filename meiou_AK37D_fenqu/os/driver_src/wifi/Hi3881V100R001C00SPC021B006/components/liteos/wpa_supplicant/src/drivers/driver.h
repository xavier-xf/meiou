...
struct wpa_driver_scan_params {
	...
	const u8 *bssid;
#ifdef LOS_WPA_PATCH
	int prefix_ssid_scan_flag;
	int fast_connect_flag;
#endif /* LOS_WPA_PATCH */
#ifndef HISI_CODE_CROP
	/**
...
struct wpa_driver_ops {
	...
	/**
	 * get_drv_flags - Get driver capabilities
	 * @priv: private driver interface data
	 *
	 * Returns: 0 on success, -1 on failure
	 *
	 * Get driver/firmware/hardware capabilities.
	 */
   int (*get_drv_flags)(void *priv, u64 *drv_flag);
	...
	/* sta disconnect delay report set */
	int (*set_delay_report)(const char *ifname, int enable_flag);
...
#ifdef CONFIG_DRIVER_HISILICON
	/**
	 * get_p2p_mac_addr - Get P2P GO/GC MAC address
	 * @priv: private driver interface data
	 * @type: Interface type
	 * @puc_mac_addr: Pointer to P2P GO/GC MAC
	 * Returns: 0 on success, -1 on failure
	 *
	 * This optional function can be used to get the P2P GO/GC MAC address of the
	 * device from the driver interface code. This is only needed if the
	 * l2_packet implementation for the OS does not provide easy access to
	 * a MAC address. */
	int (*get_p2p_mac_addr)(void *priv, enum wpa_driver_if_type type, unsigned char *puc_mac_addr);
#endif /* CONFIG_DRIVER_HISILICON */
#endif /* CONFIG_P2P */
#ifndef LOS_CONFIG_HISI_DRIVER_NOT_SUPPORT
	/**
...
union wpa_event_data {
	 ...
	 *
	 * @peer: Peer address
	 * @mesh_bcn_priority: hisi mesh Beacon priority
	 *
	 * Notification of hisi new candidate mesh peer.
	 */
	struct hi_mesh_peer {
		const u8 *peer;
		u8 mesh_bcn_priority;

 ...
 * @ingroup hi_wifi_basic
 *
 * Struct of report datarate.CNcomment:吞吐量数据.CNend
 */
typedef struct {
...
*
* @attention  NULL
* @param  vap_res_num   [IN]  Type #const unsigned char, vap num[rang: 1-2].CNcomment:vap资源个数，取值[1-2].CNend
* @param  user_res_num  [IN]  Type #const unsigned char, user resource num[1-4].
*           CNcomment:用户资源个数，多vap时共享，取值[1-4].CNend
*
...
*           CNcomment:添加设备mac地址递增，删除设备回收对应的mac地址.CNend
*
* @attention  It is recommended that MAC addresses be forcibly converted to type unsigned char *.
*             CNcomment:建议将获取的MAC地址强转为unsigned char *类型.CNend
...
/**
* @ingroup  hi_wifi_basic
* @brief  The host sets device to deep sleep without turning off crystal.CNcomment:设置device深睡时不关闭晶体.CNend
*
* @par Description:
*           The host sets device to deep sleep without turning off crystal.CNcomment:设置device深睡时不关闭晶体.CNend
*
* @attention  set before setting up sleep, by default, the crystal is disabled when the device is in deep sleep.
* CNcomment:在设置device睡眠前进行设置，默认情况下，device深睡时关闭晶体.CNend
* @param  NULL.
*
* @retval #HISI_OK  Excute successfully
* @retval #Other           Error code
...
*
* @attention  NULL
* @param  event_cb  [IN]    Type #hi_wifi_driver_event_cb, event callback .CNcomment:回调函数.CNend
*
* @retval #HISI_OK        Execute successfully.
* @retval #HISI_FAIL      Execute failed.
* @par Dependency:
...
*
* @attention  shoule call after wifi init.CNcomment:须在WIFI初始化成功后调用.CNend
*
* @retval #HISI_OK        Execute successfully.
* @retval #Other           Error code
* @par Dependency:

...
{
    hcc_handler_stru *hcc_handler = (hcc_handler_stru *)(uintptr_t)arg;
...
hi_void hcc_trans_flow_ctrl_info_init(hcc_handler_stru *hcc_handle)
{
    ...
    oal_timer_init(&hcc_handle->hcc_transer_info.tx_flow_ctrl.flow_timer,
        hcc_handle->hcc_transer_info.tx_flow_ctrl.timeout, hcc_dev_flow_ctrl_timeout_isr, (uintptr_t)hcc_handle);
...
static hi_s32 hcc_host_rx(const hcc_handler_stru *hcc_handler, oal_netbuf_stru *netbuf)
{
    ...
    if (hcc_host_check_header_vaild(hcc_hdr) != HI_TRUE) {
        oal_print_hex_dump((hi_u8 *)hcc_hdr, HCC_HDR_TOTAL_LEN, 16, "invaild hcc_rx header: "); /* 16进制 */
        return -OAL_EFAIL;
    }
    ...
    if (HCC_HDR_LEN + hcc_hdr->pad_hdr + hcc_hdr->pad_payload > oal_netbuf_len(netbuf)) {
        ...
        oal_print_hex_dump((hi_u8 *)oal_netbuf_data(netbuf), (hi_s32)oal_netbuf_len(netbuf),
            32, "rx wrong data: "); /* group_size 等于 32 */
        return -OAL_EINVAL;
    }
    ...
    oal_print_hex_dump(oal_netbuf_data(netbuf) + HCC_HDR_TOTAL_LEN + hcc_hdr->pad_payload, hcc_hdr->pay_len,
        8, "hcc payload:"); /* group_size 等于 8 */
#endif
...
hi_s32 hcc_host_proc_rx_queue(hcc_handler_stru *hcc_handler, hcc_queue_type_enum  type)
{
    ...
    netbuf_head = &hcc_handler->hcc_transer_info.hcc_queues[HCC_RX].queues[type].data_queue;
    ...
    for (; ;) {
        ...
        hcc_hdr = (hcc_header_stru *)oal_netbuf_data(netbuf);
        if (hcc_host_check_header_vaild(hcc_hdr) != HI_TRUE) {
            oal_print_hex_dump((hi_u8 *)hcc_hdr, HCC_HDR_TOTAL_LEN, 16, "invaild hcc header: "); /* 16进制 */
            oam_error_log0(0, 0, "hcc_host_send_rx_queue:: invaild hcc_header");
            count++;
        ...
        if (pre_ret == HI_SUCCESS) {
            if (hcc_host_rx(hcc_handler, netbuf) == HI_SUCCESS) {
                if (rx_action->post_do != HI_NULL) {
                    ...
                    oal_print_hex_dump((hi_u8 *)hcc_hdr, HCC_HDR_TOTAL_LEN, 32, "hcc invaild header: "); /* len 32 */
                    oal_print_hex_dump(oal_netbuf_data(netbuf),
                        (hi_s32)oal_netbuf_len(netbuf), 32, "hcc invaild header(payload): "); /* 32 进制 */
                    oal_netbuf_free(netbuf);
                }
            } else {
                oal_netbuf_free(netbuf);
                                   ...
                                   const hi_void *data, hi_u32 ul_size)
{
    ...
    if (ul_size) {
        ...
        if (OAL_WARN_ON(ul_size + sizeof(struct hcc_descr_header) > oal_netbuf_len(netbuf))) {
            oal_io_print2("invaild request size:%u,max size:%u\r\n",
        ...
        /* lint -e124 */
        if (memcpy_s((hi_void *)((hi_u8 *)oal_netbuf_data(netbuf) + sizeof(struct hcc_descr_header)),
            ul_size, data, ul_size) != EOK) {
            return -OAL_EINVAL;
        }
    }
...
static hi_void  hcc_restore_assemble_netbuf_list(hcc_handler_stru *hcc_handler)
{
    ...
    if (!oal_netbuf_list_empty(assembled_head))
        oal_netbuf_splice_sync(&hcc_handler->hcc_transer_info.hcc_queues[HCC_TX].queues[type].data_queue,
                               assembled_head);
}
...
hi_s32 hcc_tx_netbuf_test_and_switch_high_pri_queue(hcc_handler_stru *hcc_handler,
    hcc_netbuf_queue_type pool_type, hi_u8 is_config_frame)
{
...
static hi_void hcc_tx_netbuf_free(oal_netbuf_stru *pst_netbuf)
{
    ...
    if (oal_likely(pst_cb_stru->destory)) {
        pst_cb_stru->destory(hcc_host_get_handler());
    }
                                   ...
                                   hi_u32 *remain_len)
{
    ...
    if (oal_netbuf_list_empty(next_assembled_head)) {
        ...
        if (assemble_len > HISDIO_HOST2DEV_SCATT_SIZE) {
            oam_error_log0(0, OAM_SF_ANY,
                "hcc_send_data_packet::assemble_len > HISDIO_HOST2DEV_SCATT_SIZE.");
        }
    ...
    if (mode == HCC_ASSEM_SEND) {
        hcc_build_next_assem_descr(hcc_handler, type, head, next_assembled_head,
    ...
    if (descr_netbuf == HI_NULL) {
        oam_error_log0(0, OAM_SF_ANY,
                       "hcc_send_data_packet::oal_netbuf_delist fail.descr_netbuf is HI_NULL.");
        ret = HI_SUCCESS;
        goto failed_get_assem_descr;
    }
...
hi_u8 hcc_config_frame(oal_netbuf_head_stru *head)
{
    ...
    if (!oal_netbuf_list_empty(head)) {
        oal_netbuf_stru *netbuf = (oal_netbuf_stru *)oal_netbuf_head_next(head);
        if (netbuf != HI_NULL) {
...
hi_s32 hcc_host_proc_tx_queue(hcc_handler_stru *hcc_handler, hcc_queue_type_enum type)
{
    ...
    hcc_handler->hcc_transer_info.tx_flow_ctrl.uc_hipriority_cnt = hisdio_large_pkt_get(priority_cnt) +
        hisdio_mgmt_pkt_get(priority_cnt);
...
hi_void dev_backtrace_info(const hi_char* buf, hi_s32 length)
{
    ...
    for (int i = 0; i < length / WORD_WIDTH; i++) {
    ...
    switch (hcc_ext_hdr->nest_sub_type) {
        ...
        case OAM_LOG_TYPE:
            oam_info_log4(0, 0, "[CLIENT]id 0x%08x:0x%08x 0x%08x 0x%08x\n",
                buf[LOG_ID], buf[LOG_P0], buf[LOG_P1], buf[LOG_P2]);
            break;
        case OAM_PM_TYPE:
...
static hi_s32 hcc_host_rx_register(hcc_handler_stru *hcc_handler, hcc_action_type_enum type,
...
hi_u32 hcc_send_control_msg(hcc_handler_stru *hcc_handler, hi_u8 sub_type, hi_u8* buf, int len)
{
    ...
    if (buf == HI_NULL || memcpy_s(event_payload, len, buf, len) != EOK) {
        goto exit;
    }
    ...
    /* 1. 初始化HCC头 */
    hcc_hdr_param_init(&param, HCC_ACTION_TYPE_OAM, WIFI_CONTROL_TYPE,
                       ((hi_u32)sizeof(frw_hcc_extend_hdr_stru)), HCC_FC_NONE, DATA_HI_QUEUE);
...
{
    hcc_handler_stru *hcc_handler = (hcc_handler_stru *)(uintptr_t)data;
    ...
    if (hi_wifi_get_heart_beat_enable() == HI_TRUE) {
        ...
        oam_info_log3(0, 0, "g_priority_cnt=0x%x, g_lopriority_cnt=0x%x, g_des_cnt=0x%x",
            g_priority_cnt, g_lopriority_cnt, g_des_cnt);
    }

    if (hcc_handler) {
        hi_s32 ret = oal_timer_start(&hcc_handler->hcc_transer_info.hcc_timer, HCC_HEARTBEAT_TIMEOUT);
        if (ret != 0 && ret != 1) {
...
hi_u32 hcc_host_init()
{
    ...
    oal_timer_init(&hcc_handler->hcc_transer_info.hcc_timer,
        HCC_HEARTBEAT_TIMEOUT,
        hcc_send_heartbeat,
        (unsigned long)(uintptr_t)hcc_handler);
...
sdio_probe_err:
    oal_channel_detectcard_to_core(0);
    oal_free(hcc_handler);
    return HI_FAIL;
}
...
hi_void hcc_host_exit(hcc_handler_stru *hcc)
{
    ...
    hcc_message_unregister(hcc, D2H_MSG_CREDIT_UPDATE);
    hcc_message_unregister(hcc, D2H_MSG_HIGH_PKT_LOSS);

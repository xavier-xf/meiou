...
hi_u32  wal_connect_new_sta_proc_ap(frw_event_mem_stru *event_mem)
{
    ...
    if (event_mem == HI_NULL || event_mem->puc_data == NULL) {
        oam_error_log0(0, OAM_SF_ASSOC, "{wal_connect_new_sta_proc_ap::event_mem/puc_data is null!}");
        return HI_ERR_CODE_PTR_NULL;
    }
    frw_event_stru *event = (frw_event_stru *)event_mem->puc_data;
    ...
    if (asoc_user_req_info == HI_NULL) {
        oam_error_log0(0, OAM_SF_ASSOC, "{wal_connect_new_sta_proc_ap::asoc_user_req_info is null!}");
        return HI_ERR_CODE_PTR_NULL;
    }
    ...
    /* 向内核标记填充了关联请求帧的ie信息 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 0, 0)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    station_info.filled |=  STATION_INFO_ASSOC_REQ_IES;
#endif

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 2, 0)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
    ...
    oam_warning_log3(event->event_hdr.vap_id, OAM_SF_ASSOC,
        ...
        user_mac_addr[3], user_mac_addr[4], user_mac_addr[5]); /* 3/4/5 MAC地址位数 */
...
hi_u32 wal_any_wait_query_result(hmac_vap_stru *hmac_vap, hi_wifi_any_peer_info *peer)
{
    ...
    if (g_query_completed_flag == HI_FALSE) {
        ...
        ret = (hi_u32)hi_wait_event_timeout(hmac_vap->query_wait_q,
            (HI_TRUE == g_query_completed_flag), (5 * HZ)); /* 5 频率,非wifi目录定义宏函数,误报告警,lin_t e26告警屏蔽 */
        if (ret == 0) {
                          ...
                          const hi_wifi_csi_entry *entry_list, hi_s32 entry_num)
{
    ...
    /* 安全编码规范 规则6.6 例外(1)对固定长度的数组进行初始化,或对固定长度的结构体进行内存初始化 */
...
hi_u32 wal_report_tx_params(frw_event_mem_stru *event_mem)
{
    ...
    if (event_mem == HI_NULL || event_mem->puc_data == HI_NULL) {
        oam_error_log0(0, OAM_SF_TX, "{wal_get_efuse_mac_from_dev::event_mem/puc_data is null!}");
        return HI_ERR_CODE_PTR_NULL;
    }

...
/* insmod 设置vap mode */
int g_mode = WAL_WIFI_MODE_STA_AP; /* wifi 默认STA_AP 共存模式 */
...
hi_s32 wal_cfg80211_set_default_mgmt_key(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev, hi_u8 key_index)
{
    ...
    if (oal_unlikely(wiphy == HI_NULL) || oal_unlikely(netdev == HI_NULL)) {
        oam_error_log2(0, OAM_SF_ANY, "{wal_cfg80211_set_default_mgmt_key::wiphy, netdev ptr %d, %d!}\r\n",
                       wiphy, netdev);
        return -HI_FAIL;;
    }

    payload_params.key_index = key_index;
    payload_params.unicast   = HI_FALSE;
    payload_params.multicast = HI_TRUE;
    ...
    ***************************************************************************/
    /* 填写事件头 */
    wal_write_msg_hdr_init(&write_msg, WLAN_CFGID_DEFAULT_KEY, sizeof(mac_setdefaultkey_param_stru));
    /* 填写消息体 */
    if (memcpy_s(write_msg.auc_value, sizeof(write_msg.auc_value), &payload_params,
        sizeof(mac_setdefaultkey_param_stru)) != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "{wal_cfg80211_set_default_mgmt_key::mem safe function err!}");
        return HI_FAIL;
    }

    ret = wal_send_cfg_event(netdev, WAL_MSG_TYPE_WRITE,
              WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_setdefaultkey_param_stru),
              (hi_u8 *)&write_msg, HI_FALSE, HI_NULL);

    if (oal_unlikely(ret != HI_SUCCESS)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_cfg80211_set_default_mgmt_key::return err code %d!}\r\n", ret);
        return -HI_FAIL;
    }
...
static hi_void wal_cfg80211_print_sched_scan_req_info(oal_cfg80211_sched_scan_request_stru  *request)
{
    ...
    oam_warning_log4(0, OAM_SF_SCAN,
        ...
        request->n_channels, request->scan_plans[0].interval, request->flags, request->min_rssi_thold);
    ...
    oam_warning_log2(0, OAM_SF_SCAN,
        "wal_cfg80211_print_sched_scan_req_info::channels[%d],interval[%d]ms",
        request->n_channels, request->interval);
#endif
/* HI1131C modify end */
    /* 打印ssid集合的信息 */
    ...
    oal_cfg80211_sched_scan_request_stru *request)
{
    ...
    for (hi_s32 l_loop = 0; l_loop < request->n_match_sets; l_loop++) {
        oal_cfg80211_ssid_stru *ssid_tmp = (oal_cfg80211_ssid_stru *)&(request->match_sets[l_loop].ssid);
...
static hi_void wal_cfg80211_fill_station_info(oal_station_info_stru  *sta_info, oal_station_info_stru  *stats)
{
    ...
    oam_info_log3(0, OAM_SF_CFG, "{wal_cfg80211_fill_station_info::legacy[%d],mcs[%d],flags[%d]}",
                  sta_info->txrate.legacy / 10, sta_info->txrate.mcs, /* 10: 单位转换 */
                  sta_info->txrate.flags);
}
...
static hi_s32 wal_cfg80211_mgmt_tx_cancel_wait(oal_wiphy_stru *wiphy, oal_net_device_stru *wdev, hi_u64 ull_cookie)
#else
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 7, 0))
oal_net_device_stru *wal_cfg80211_add_virtual_intf_send_event(oal_net_device_stru *netdev,
...
*****************************************************************************/
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 7, 0))
static oal_net_device_stru *wal_cfg80211_add_virtual_intf(oal_wiphy_stru *wiphy,
#else
static oal_wireless_dev *wal_cfg80211_add_virtual_intf(oal_wiphy_stru *wiphy,
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 7, 0))
    hi_char *puc_name,
#else
    const hi_char *puc_name,
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
    nl80211_iftype_uint8 name_assign_type,
#endif
    enum_nl80211_iftype type,
...
*****************************************************************************/
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0))
hi_s32 wal_cfg80211_del_virtual_intf(oal_wiphy_stru *wiphy, oal_net_device_stru *dev)
#else
...
#endif
{
    ...
    wal_msg_write_stru  write_msg;
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0))
    oal_wireless_dev *wdev = oal_netdevice_wdev(dev);
#endif
...
/* P2P 补充缺失的CFG80211接口 */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0))
hi_void wal_cfg80211_mgmt_frame_register(struct wiphy *wiphy, oal_net_device_stru *dev, hi_u16 frame_type, bool reg)
#else
    ...
    if (WLAN_BAND_2G == band) {
        ...
        if (beacon_info->head_len > us_offset) {
            puc_supported_rates_ie = mac_find_ie(MAC_EID_RATES,
                beacon_info->head + 24 + us_offset, beacon_info->head_len - us_offset); /* mac header长度24 */
            if (puc_supported_rates_ie != HI_NULL) {
...
#else
hi_s32 wal_cfg80211_scan(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev, oal_cfg80211_scan_request_stru *request)
#endif
#endif
{
    hmac_device_stru           *hmac_dev = HI_NULL;
    mac_vap_stru               *mac_vap = HI_NULL;
    hmac_scan_stru             *scan_mgmt = HI_NULL;
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
    oal_net_device_stru        *netdev = HI_NULL;
#endif
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
hi_u8 current_connect_bssid[WLAN_MAC_ADDR_LEN] = { 0 };
hi_u8 *wal_get_cur_connect_bssid(hi_void)
{
    return current_connect_bssid;
}
#endif
/*****************************************************************************
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    if (memcpy_s(current_connect_bssid, WLAN_MAC_ADDR_LEN, mac_cfg80211_connect_param.puc_bssid,
        WLAN_MAC_ADDR_LEN) != EOK) {
        oam_warning_log0(0, OAM_SF_ANY, "wal_cfg80211_connect:memcpy fail.");
        goto fail;
    }
#endif
    /* 解析内核下发的安全相关参数 */
...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0))
    /* 获取内核下发的pmf是使能的结果 */
    mac_cfg80211_connect_param.mfp = sme->mfp;
#endif

    oam_warning_log3(0, OAM_SF_ANY, "{wal_cfg80211_connect::start new conn,ssid_len=%d,auth_type=%d,privacy=%d}",
        sme->ssid_len, sme->auth_type, sme->privacy);
...
static hi_u32 wal_cfg80211_set_ssid(oal_net_device_stru *netdev, const hi_u8 *puc_ssid_ie, hi_u8 ssid_len)
{
    ...
    if ((ssid_len > 32) || (ssid_len == 0)) { /* 32: 长度上界 */
        oam_warning_log1(0, OAM_SF_ANY, "{wal_cfg80211_set_ssid: len[%d].}", ssid_len);
        return HI_FAIL;
    }
...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0))
...
#elif (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0))
...
*****************************************************************************/
static hi_u32 wal_cfg80211_set_channel_info(oal_net_device_stru *netdev, oal_ieee80211_channel *channel,
    hi_s32 l_bandwidth, hi_s32 l_center_freq1, hi_s32 l_channel)
{
    ...
    /* 14信道仅支持11b，而窄带不支持11b，此处禁止在14信道启动窄带ap */
    if ((g_bw != WAL_WIFI_BW_LEGACY_20M) && (l_channel == 14)) { /* 14信道 */
        oam_error_log0(0, 0, "wal_cfg80211_set_channel::invalid bw in channel 14.");
        return HI_FAIL;
    }
...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
hi_u32 wal_cfg80211_set_beacon_param(oal_net_device_stru *netdev, mac_vap_stru *mac_vap,
    oal_beacon_parameters *beacon_info, mac_beacon_param_stru *beacon_param, mac_beacon_operation_type_uint8 type)
{
    /* 获取 WPA/WPA2 信息元素 */
    if (beacon_info->tail == HI_NULL || beacon_info->head == HI_NULL) {
        oam_error_log2(0, OAM_SF_ANY, "{wal_cfg80211_add_beacon::beacon frame error tail = %d, head = %d!}\r\n",
                       beacon_info->tail, beacon_info->head);
        return HI_FAIL;
    }

    if (wal_parse_wpa_wpa2_ie(beacon_info, beacon_param) != HI_SUCCESS) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_add_beacon::Failed to parse wpa/wpa2 ie!}\r\n");
        return HI_FAIL;
    }

    if (wal_parse_ht_vht_ie(mac_vap, beacon_info, beacon_param) != HI_SUCCESS) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_add_beacon::Failed to parse HT/VHT ie!}\r\n");
        return HI_FAIL;
    }
    ...
    if ((mac_vap->channel.chan_number == 14) && (beacon_param->protocol != WLAN_LEGACY_11B_MODE)) { /* 14：代表信道号 */
        oam_error_log1(mac_vap->vap_id, OAM_SF_ANY,
            "{wal_cfg80211_add_beacon::ch 14 should in 11b, protocol is %d to 11b}", beacon_param->protocol);
        beacon_param->protocol = WLAN_LEGACY_11B_MODE;
    }

    /* 设置操作类型 */
    beacon_param->operation_type = type;
    /* 抛事件给驱动 */
...
hi_u32 wal_cfg80211_set_bw(oal_net_device_stru *netdev)
{
    ...
    if (wal_wifi_set_bw(netdev, g_bw) != HI_SUCCESS) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_set_bw::failed to set bw}");
        return HI_FAIL;
    }
    if ((oal_netdevice_flags(netdev) & OAL_IFF_RUNNING) == 0) {
        oal_netdevice_flags(netdev) |= OAL_IFF_RUNNING;
...
hi_s32 wal_cfg80211_add_beacon(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev, oal_beacon_parameters *beacon_info)
{
    ...
    if ((netdev == HI_NULL) || (beacon_info == HI_NULL) || (beacon_info->head == HI_NULL)) {
        oam_error_log3(0, OAM_SF_ANY, "{wal_cfg80211_add_beacon:: %p, %p, %p!}",
            (uintptr_t)netdev, (uintptr_t)beacon_info, (uintptr_t)beacon_info->head);
        return -HI_FAIL;
    }
    ...
    mac_vap_stru *mac_vap = oal_net_dev_priv(netdev);
    if (mac_vap == HI_NULL) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_cfg80211_add_beacon::pst_mac_vap = %p}", (uintptr_t)mac_vap);
        return -HI_FAIL;
    }

    /* 初始化beacon interval 和DTIM_PERIOD 参数 */
    beacon_param.l_interval    = beacon_info->interval;
    beacon_param.l_dtim_period = beacon_info->dtim_period;

    puc_ie = mac_get_ssid(beacon_info->head + MAC_80211_FRAME_LEN, (beacon_info->head_len - MAC_80211_FRAME_LEN),
                 &ssid_len);
    /* 隐藏SSID两种情况:(1)长度为0，(2)长度为实际ssid长度，但内容为空 */
    beacon_param.hidden_ssid = 0;
    if (mac_is_hide_ssid(puc_ie, ssid_len) == HI_TRUE) {
        beacon_param.hidden_ssid = 1;
    }

    if (wal_cfg80211_set_beacon_param(netdev, mac_vap, beacon_info,
        &beacon_param, MAC_ADD_BEACON) != HI_SUCCESS) {
        oam_warning_log0(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_add_beacon: set_beacon_param fail.}");
        return -HI_FAIL;
    }

    if (wal_cfg80211_set_ssid(netdev, puc_ie, ssid_len) != HI_SUCCESS) {
        return -HI_FAIL;
    }

    if (wal_parse_wmm_ie(netdev, mac_vap, beacon_info) != HI_SUCCESS) {
        oam_error_log0(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_add_beacon::Failed to parse wmm ie!}\r\n");
        return -HI_FAIL;
    }
    /* 启动AP */
    if (oal_unlikely(wal_start_vap(netdev) != HI_SUCCESS)) {
        oam_warning_log0(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_add_beacon::failed to start ap}");
        return -HI_FAIL;
    }
    /* 设置net_device里flags标志 */
    if (wal_cfg80211_set_bw(netdev) != HI_SUCCESS) {
        oam_warning_log0(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_add_beacon::fail set bw}");
        return -HI_FAIL;
    }
...
hi_s32 wal_cfg80211_set_beacon(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev, oal_beacon_parameters *beacon_info)
{
    mac_beacon_param_stru    beacon_param = {0};
    hi_u8                   *puc_ie;
    hi_u8                    ssid_len;
    ...
    if ((netdev == HI_NULL) || (beacon_info == HI_NULL) || (beacon_info->head == HI_NULL)) {
        oam_error_log3(0, OAM_SF_ANY, "{wal_cfg80211_set_beacon:: %p, %p, %p!}",
            (uintptr_t)netdev, (uintptr_t)beacon_info, (uintptr_t)beacon_info->head);
        return -HI_FAIL;
    }
    ...
    if (wal_cfg80211_set_beacon_param(netdev, mac_vap, beacon_info,
        &beacon_param, MAC_SET_BEACON) != HI_SUCCESS) {
        oam_warning_log0(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_add_beacon: set_beacon_param fail.}");
        return -HI_FAIL;
    }

    if ((ssid_len > 32) || (ssid_len == 0)) { /* 32: 长度上界 */
        oam_warning_log1(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_add_beacon: len[%d].}", ssid_len);
        return -HI_FAIL;
    }
...
hi_s32 wal_cfg80211_del_beacon(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev)
{
    wal_msg_write_stru write_msg;
    hi_unref_param(wiphy);

    if (netdev == HI_NULL) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_cfg80211_del_beacon:: %p}", (uintptr_t)netdev);
        return -HI_FAIL;
    }
    ...
    if (mac_vap == HI_NULL || mac_vap->vap_mode != WLAN_VAP_MODE_BSS_AP) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_del_beacon::pst_mac_vap null or vap_mode err}");
        return -HI_FAIL;
    }

    /* 如果netdev不是running状态，则不需要down */
    if ((oal_netdevice_flags(netdev) & OAL_IFF_RUNNING) == 0) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_del_beacon::vap is already down!}\r\n");
        return HI_SUCCESS;
    }
    ...
    wlan_p2p_mode_enum_uint8  p2p_mode = wal_wireless_iftype_to_mac_p2p_mode(wdev->iftype);
    if (WLAN_P2P_BUTT == p2p_mode) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_del_beacon::wal_wireless_iftype_to_mac_p2p_mode return BUFF}\r\n");
        return -HI_FAIL;
    }
    ...
    hi_u32 ret = wal_send_cfg_event(netdev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_start_vap_param_stru), (hi_u8 *)&write_msg, HI_FALSE, HI_NULL);
    if (oal_unlikely(ret != HI_SUCCESS)) {
        oam_warning_log1(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_del_beacon::failed to stop ap, error[%u]}", ret);
        return -HI_FAIL;
    }
...
hi_s32 wal_cfg80211_set_channel(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev,
    oal_ieee80211_channel *chan, oal_nl80211_channel_type channel_type)
{
    hi_unref_param(wiphy);

    if ((netdev == HI_NULL) || (chan == HI_NULL)) {
        oam_error_log2(0, OAM_SF_ANY, "{wal_cfg80211_set_channel:: %p, %p!}", (uintptr_t)netdev, (uintptr_t)chan);
        return -HI_FAIL;
    }

    oal_ieee80211_channel *channel = chan;
    hi_s32 l_bandwidth    = channel_type;
    hi_s32 l_center_freq1 = channel->center_freq;
    hi_s32 l_channel      = channel->hw_value;

    if (wal_cfg80211_set_channel_info(netdev, channel,l_bandwidth, l_center_freq1, l_channel) != HI_SUCCESS) {
        oam_error_log0(0, OAM_SF_ANY, "{wal_cfg80211_set_channel::wal_cfg80211_set_channel_info fail");
        return -HI_FAIL;
    }
...
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0))
{
    ...
    /* 参数合法性检查 */
    if ((netdev == HI_NULL) || (netdev->ieee80211_ptr == HI_NULL)|| (ap_settings == HI_NULL)) {
    ...
    /*****************************************************************************
        1.设置信道
    *****************************************************************************/
#if (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION) || (LINUX_VERSION_CODE >= KERNEL_VERSION(3,7,0))
    oal_ieee80211_channel *channel = netdev->ieee80211_ptr->preset_chandef.chan;
#else
    oal_ieee80211_channel *channel = netdev->ieee80211_ptr->channel;
#endif
    if (channel == HI_NULL) {
        oam_error_log1(0, OAM_SF_ANY, "{wal_cfg80211_start_ap::channel = %p}", (uintptr_t)channel);
        goto fail;
    }
...
#else
   hi_s32 l_bandwidth    = 1;    /* 1: NL80211_CHAN_WIDTH_20 */
   hi_s32 l_center_freq1 = channel->center_freq;
#endif
    hi_s32 l_channel      = channel->hw_value;
...
#endif
{
    ...
    if (mac_addr == HI_NULL) {
        ...
        oam_info_log3(mac_vap->vap_id, OAM_SF_ANY, "{wal_cfg80211_del_station::delete user:XX:XX:XX:%02X:%02X:%02X}",
            mac_addr[3], mac_addr[4], mac_addr[5]); /* 3, 4, 5: 数组下标 */
    }
#elif (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0))
...
static hi_u32 wal_cfg80211_mgmt_tx_parameter_check(oal_wiphy_stru *wiphy, oal_net_device_stru *netdev,
...
#endif
    if ((chan == HI_NULL) || (pull_cookie == HI_NULL) || (puc_buf == HI_NULL)) {
        oam_error_log3(0, OAM_SF_CFG,
            "{wal_cfg80211_mgmt_tx_parameter_check::chan or cookie or buf ptr is null, error %p, %p, %p!}\r\n",
            (uintptr_t)chan, (uintptr_t)pull_cookie, (uintptr_t)puc_buf);
        return HI_ERR_CODE_PTR_NULL;
    }
...
/*****************************************************************************
 ...
 功能描述  : 发送管理帧
*****************************************************************************/
hi_s32 wal_cfg80211_mgmt_tx(oal_wiphy_stru *wiphy,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
...
#endif
#else
    oal_cfg80211_mgmt_tx_params_stru *params,
#endif
    hi_u64 *pull_cookie)
{
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 34))
    hi_unref_param(channel_type);
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 36))
    hi_unref_param(channel_type_valid);
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 38))
    hi_unref_param(wait);
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
    if (wal_cfg80211_mgmt_tx_parameter_check(wiphy, netdev, params, pull_cookie) != HI_SUCCESS) {
        return -HI_ERR_CODE_PTR_NULL;
    }
...
#else
    if (wal_cfg80211_mgmt_tx_parameter_check(wiphy, netdev, chan, puc_buf, pull_cookie) != HI_SUCCESS) {
        return -HI_ERR_CODE_PTR_NULL;
    }
#endif
    ...
    *pull_cookie = p2p_info->ull_send_action_id++;   /* cookie值上层调用需要判断是否是这次的发送导致的callback */
...
#else
        oal_cfg80211_mgmt_tx_status(netdev, *pull_cookie, puc_buf, len, HI_TRUE, GFP_KERNEL);
#endif
        return HI_SUCCESS;
    }
    ...
    /* 发送失败，则尝试重传 */
    do {
        ret = wal_mgmt_do_tx(netdev, &mgmt_tx);
...
#else
    wiphy->custom_regulatory |= WIPHY_FLAG_CUSTOM_REGULATORY;
#endif

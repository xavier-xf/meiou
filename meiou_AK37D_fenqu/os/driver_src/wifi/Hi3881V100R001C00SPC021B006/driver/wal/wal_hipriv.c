...
#if defined (_PRE_WLAN_FEATURE_HIPRIV) || defined (_PRE_WLAN_FEATURE_SIGMA)
/*****************************************************************************
 功能描述  : 开启或关闭所有用户对应tid的amsdu发送功能并配置amsdu参数
*****************************************************************************/
...
hi_u32 wal_hipriv_amsdu_tx_on(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    /* 命令格式: hipriv wlan0 amsdu_tx_on 0|1 tidno max_num[1~12] */
...
hi_u32 wal_hipriv_create_proc(hi_void *proc_arg)
{
    ...
    g_proc_entry = create_proc_entry(WAL_HIPRIV_PROC_ENTRY_NAME, 420, NULL);    /* 420十进制对应八进制是0644 */
...
hi_u32 wal_get_add_wow_pattern_param(hi_u8 index, hi_char *pattern, hmac_cfg_wow_pattern_param_stru *cfg_wow_param)
{
    ...
    if ((index >= WOW_NETPATTERN_MAX_NUM) || (pattern == HI_NULL)) {
        oam_error_log1(0, 0, "wal_get_add_wow_pattern_param:: invalid_param, index[%d] or pattern null", index);
        return HI_FAIL;
    }
...
hi_u32 wal_hipriv_rx_proc(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    for (hi_u32 i = 0; i < 5; i++) { /* 常收命令有5个参数 */
...
hi_u32 wal_hipriv_tx_proc(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    for (hi_u32 i = 0; i < 5; i++) { /* 常发命令有5个参数 */
...
#if defined(_PRE_WLAN_FEATURE_HIPRIV) || defined(_PRE_WLAN_FEATURE_SIGMA)
    {"amsdu_tx_on",     wal_hipriv_amsdu_tx_on},    /* 开闭amsdu: amsdu_tx_on [0\1] */
...
/*****************************************************************************
 ...
 返 回 值  : 错误码
*****************************************************************************/
/* 6721行puc_cmd_id有赋值操作，pul_off_set作为wal_get_cmd_one_arg函数的参数，在其中进行了修改，lint_t e818告警屏蔽 */

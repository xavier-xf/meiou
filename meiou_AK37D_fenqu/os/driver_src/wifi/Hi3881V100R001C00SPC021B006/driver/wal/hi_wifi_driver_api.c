...
{
    printk("[VERSION]:Hi3881V100R001C00SPC021 2020-12-08 23:15:00\n");
...
hi_s32 hi_wifi_set_bandwidth(const hi_char *ifname, unsigned char ifname_len, hi_wifi_bw bw)
{
    ...
    if ((netdev->ieee80211_ptr != HI_NULL) && (netdev->ieee80211_ptr->iftype == NL80211_IFTYPE_AP)) {
        if (netdev->ieee80211_ptr->preset_chandef.chan == HI_NULL) {
            oam_error_log0(0, 0, "hi_wifi_set_bandwidth chan is null.");
            return HI_FAIL;
        }
        oal_ieee80211_channel *channel = netdev->ieee80211_ptr->preset_chandef.chan;
        if ((channel->hw_value == 14) && (bw != HI_WIFI_BW_LEGACY_20M)) { /* 14信道 */
            oam_error_log0(0, 0, "hi_wifi_set_bandwidth invalid bw in channel 14");
            return HI_FAIL;
        }
    }
...
hi_s32 hi_wifi_any_fetch_peer(hi_u8 index, hi_wifi_any_peer_info *peer)
{
    ...
    ret = wal_send_cfg_event(g_any_netdev,
                             WAL_MSG_TYPE_WRITE,
                             WAL_MSG_WRITE_MSG_HDR_LENGTH + 4, /* 4 长度增4 */
                             (hi_u8 *)&write_msg,
                             HI_FALSE,
                             HI_NULL);
    if (ret != HI_SUCCESS) {
...
hi_u8 hi_wifi_arp_offload_setting(const hi_char *ifname, hi_u8 en, hi_u32 ip)
{
    ...
    oam_info_log3(0, 0, "{hi_wifi_arp_offload_setting:: oper: %d, ip[xx.xx.%d.%d] succ.}",
        en, *(((hi_char *)&ip_addr_cfg.ip.ipv4) + 2), *(((hi_char *)&ip_addr_cfg.ip.ipv4) + 3)); /* 2/3 偏置 */
...
hi_u32 hi_wifi_set_tpc(const char* ifname, unsigned char ifname_len, hi_u32 tpc_value)
{
    ...
    if (ifname == HI_NULL) {
        oam_error_log0(0, 0, "{hi_wifi_set_tpc:: input point is NULL}");
        return HI_FAIL;
    }

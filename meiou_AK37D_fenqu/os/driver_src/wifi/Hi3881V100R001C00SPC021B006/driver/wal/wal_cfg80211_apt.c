...
hi_u32 hwal_send_others_bss_data(const  oal_netbuf_stru *netbuf)
{
    ...
    if ((filter & 0x1) == HI_FALSE) {       /* bit0 多播、广播数据帧没有打开 */
        if ((frame_hdr->frame_control.type == WLAN_DATA_BASICTYPE) && is_multicast) {
            return HI_SUCCESS;
        }
    }
    if (((filter >> 1) & 0x1) == HI_FALSE) { /* bit1 单播数据帧没有打开 */
        if ((frame_hdr->frame_control.type == WLAN_DATA_BASICTYPE) && (!is_multicast)) {
            return HI_SUCCESS;
        }
    }
    if (((filter >> 2) & 0x1) == HI_FALSE) { /* bit2 多播、广播管理帧没有打开 */
        if ((frame_hdr->frame_control.type == WLAN_MANAGEMENT) && is_multicast) {
            return HI_SUCCESS;
        }
    }
    if (((filter >> 3) & 0x1) == HI_FALSE) { /* bit3 单播管理帧没有打开 */
        if ((frame_hdr->frame_control.type == WLAN_MANAGEMENT) && (!is_multicast)) {
            return HI_SUCCESS;
        }
    }

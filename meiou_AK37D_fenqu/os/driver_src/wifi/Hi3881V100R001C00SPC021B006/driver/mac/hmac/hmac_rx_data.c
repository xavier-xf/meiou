...
/*****************************************************************************
 ...
 修改历史      :
  ...
  2.日    期   : 2016年06月20日
    作    者   : Hisilicon
    修改内容   : DTS2016061403606:修改wlan to wlan 数据转发为直接发送，不需要抛事件
*****************************************************************************/
...
{
    hi_u16 submsdu_len  = 0; /* submsdu的长度 */
    hi_u8  submsdu_pad_len = 0; /* submsdu的填充长度 */
                                                ...
                                                mac_ieee80211_frame_stru *frame_hdr)
{
    ...
    hmac_msdu_proc_state_stru msdu_state;    /* 记录MPDU的处理信息 */
	memset_s(&msdu, sizeof(hmac_msdu_stru), 0, sizeof(hmac_msdu_stru));
	memset_s(&msdu_state, sizeof(hmac_msdu_proc_state_stru), 0, sizeof(hmac_msdu_proc_state_stru));
    hmac_msdu_proc_status_enum_uint8 process_state = MAC_PROC_BUTT;   /* 解析AMSDU的状态 */
    hi_u32 ret;

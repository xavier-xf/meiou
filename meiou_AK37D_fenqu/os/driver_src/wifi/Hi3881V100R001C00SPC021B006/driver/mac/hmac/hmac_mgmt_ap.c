 ...
 * Description: AP side management surface processing.
 * Author: Hisilicon
 * Create: 2018-08-04
 */
...
/*****************************************************************************
  2 全局变量定义
*****************************************************************************/
static hi_u32 hmac_ap_up_update_sta_sup_rates(hi_u8 *puc_payload,
                                              hmac_user_stru *hmac_user,
                                              mac_status_code_enum_uint16 *pen_status_code,
                                              const hmac_sup_rates_info_stru *sup_rates_info);
static hi_u32 hmac_ap_prepare_assoc_req(hmac_user_stru *hmac_user, const hi_u8 *puc_payload,
                                        hi_u32 payload_len, hi_u8 mgmt_frm_type);
...
static hi_u32 hmac_check_open_action_sec_authenticator(const hmac_vap_stru *hmac_vap,
                                                       hi_u8 *puc_payload,
                                                       hi_u32 msg_len,
                                                       const hmac_user_stru *hmac_user,
                                                       mac_status_code_enum_uint16 *pen_status_code);
static hi_u32 hmac_chk_open_action_sec_cap(const mac_vap_stru *mac_vap, const hi_u8 *puc_ie,
    hi_u8 is_80211i_mode, hi_u8 offset, mac_status_code_enum_uint16 *pen_status_code);
#endif
...
static hi_void hmac_handle_connect_rsp_ap(const hmac_vap_stru *hmac_vap, const hmac_user_stru *hmac_user)
{
    ...
    /* 关联的STA mac地址 */
    if (memcpy_s((hi_u8 *)asoc_user_req_info.auc_user_mac_addr, WLAN_MAC_ADDR_LEN,
...
hi_void hmac_mesh_sae_auth_proc(const hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req)
{
    ...
    if (auth_seq > WLAN_AUTH_TRASACTION_NUM_TWO) {
        oam_warning_log1(hmac_vap->base_vap->vap_id, OAM_SF_AUTH,
                         "{hmac_ap_rx_auth_req::[MESH]auth recieve invalid seq, auth seq [%d]}", auth_seq);
        return;
    }
    oam_warning_log1(hmac_vap->base_vap->vap_id, OAM_SF_AUTH,
                     "{hmac_ap_rx_auth_req::[MESH]auth recieve valid seq, auth seq [%d]}", auth_seq);
    ...
    if (mac_addr_is_zero(mac_addr)) {
        oam_warning_log3(hmac_vap->base_vap->vap_id, OAM_SF_AUTH,
                         "{hmac_ap_rx_auth_req::[MESH]user mac:XX:XX:XX:%02X:%02X:%02X is all 0 and invaild!}",
    ...
    oam_warning_log3(hmac_vap->base_vap->vap_id, OAM_SF_AUTH,
                     "{hmac_ap_rx_auth_req::[MESH] report Auth Req to host,user mac:XX:XX:XX:%02X:%02X:%02X.}",
                     mac_addr[3], mac_addr[4], mac_addr[5]); /* 3 4 5 元素索引 */
}
#endif
...
static hi_void hmac_ap_rx_auth_req(const hmac_vap_stru *hmac_vap, const oal_netbuf_stru *auth_req)
{
    ...
    if (auth_alg == WLAN_WITP_AUTH_SHARED_KEY) {
        /* 获取challenge text */
        for (hi_u8 chtxt_index = 0; chtxt_index < WLAN_CHTXT_SIZE; chtxt_index++) {
...
static hi_u32 hmac_ap_rx_deauth_req(const hmac_vap_stru *hmac_vap, hi_u8 *puc_mac_hdr, hi_u8 is_protected)
{
    ...
    /* 增加接收到去认证帧时的维测信息 */
    oam_warning_log3(0, OAM_SF_AUTH, "{aput rx deauth frame, reason code = %d, sa[XX:XX:XX:XX:%X:%X]}",
                     us_err_code, sa_mac_addr[4], sa_mac_addr[5]); /* 4 5MAC地址最后2字节  */
    ...
    if ((HI_TRUE != ether_is_multicast(da_mac_addr)) && (is_protected != hmac_user->base_user->cap_info.pmf_active)) {
        oam_error_log2(hmac_vap->base_vap->vap_id, OAM_SF_AUTH,
                       "{hmac_ap_rx_deauth_req::PMF check failed %d %d.}", is_protected,
                       hmac_user->base_user->cap_info.pmf_active);
        return HI_FAIL;
    }
#ifdef _PRE_WLAN_FEATURE_MESH
    ...
    if ((hmac_vap->base_vap->vap_mode == WLAN_VAP_MODE_MESH) && (hmac_user->base_user->is_mesh_user == HI_TRUE)) {
        oam_warning_log4(hmac_vap->base_vap->vap_id, OAM_SF_AUTH,
            "{meshut rx deauth frame [mesh user], reason code = %d, sa[XX:XX:XX:%X:%X:%X]}",
            us_err_code, sa_mac_addr[3], sa_mac_addr[4], sa_mac_addr[5]); /* 3 4 5 元素索引 */

        hmac_handle_close_peer_mesh(hmac_vap, sa_mac_addr, WLAN_MAC_ADDR_LEN,
            HMAC_REPORT_DEAUTH, DMAC_DISASOC_MISC_KICKUSER);
    } else {
#endif
        /* 抛事件上报内核，已经去关联某个STA */
...
/*****************************************************************************
 ...
 修改历史      :
  ...
  2.日    期   : 2013年7月8日
    ...
    修改内容   : 添加函数检查内容
*****************************************************************************/
static hi_u8 hmac_ap_up_update_sta_cap_info(const hmac_vap_stru *hmac_vap,
                                            hi_u16 us_cap_info,
                                            const hmac_user_stru *hmac_user,
                                            mac_status_code_enum_uint16 *pen_status_code)
{
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2019年1月17日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
hi_u32 hmac_mesh_update_sta_sup_rates(hi_u8 *puc_sup_rates,
                                      hi_u8 sup_rates_len,
                                      hmac_user_stru *hmac_user, mac_status_code_enum_uint16 *pen_status_code)
{
                                   ...
                                   hi_u16 us_disasoc_reason_code, hi_u16 us_dmac_reason_code)
{
    ...
    *((hi_u32 *)(data + WLAN_MAC_ADDR_LEN)) =
    ...
    return hmac_send_event_to_host(hmac_vap->base_vap, (const hi_u8*)data,
        WLAN_MAC_ADDR_LEN + sizeof(hi_u32), HMAC_HOST_CTX_EVENT_SUB_TYPE_PEER_CLOSE_MESH);
}
#endif
    ...
    mac_status_code_enum_uint16 *pen_status_code, hi_u8 *puc_rsn_ie, hi_u8 *puc_wpa_ie)
{
    ...
    if (((rsn_found == HI_TRUE) && (puc_ie != HI_NULL)) && (hmac_check_assoc_req_security_cap_common(mac_vap,
        puc_ie, is_80211i_mode, index, pen_status_code) != HI_SUCCESS)) {
            return HI_FAIL;
    }

    oam_info_log1(mac_vap->vap_id, OAM_SF_ASSOC,
        "{hmac_chk_assoc_req_sec_cap_auth_encry::is_80211i_mode = %d}", is_80211i_mode);
    return HI_SUCCESS;
}
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2013年7月9日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
static hi_void hmac_ap_up_update_legacy_capability(const hmac_vap_stru *hmac_vap,
                                                   hmac_user_stru *hmac_user, hi_u16 us_cap_info)
{
    ...
    hmac_user_stru *hmac_user, mac_status_code_enum_uint16 *status_code, const hmac_sta_check_info *sta_check)
{
    ...
    if ((hmac_vap->base_vap->vap_mode == WLAN_VAP_MODE_MESH) && (sub_type == WLAN_FC0_SUBTYPE_ACTION)) {
        ...
        hi_u32 sec_rslt = hmac_check_open_action_sec_authenticator(hmac_vap, sta_user->puc_payload,
        ...
        hi_u32 ret = hmac_chk_assoc_req_sec_cap_auth(hmac_vap, sta_user->puc_mac_hdr,
            sta_user->puc_payload, sta_user->msg_len, hmac_user, status_code);
        if (ret != HI_SUCCESS) {
    ...
    hi_u16 ret_val = hmac_vap_check_ht_capabilities_ap(hmac_vap, sta_user->puc_payload,
        sta_check->msg_idx, sta_user->msg_len, hmac_user);
    ...
    hmac_user_stru *hmac_user, mac_status_code_enum_uint16 *status_code, hi_u16 msg_idx)
{
    ...
    rslt = hmac_check_opmode_notify(hmac_vap, update_sta_user->puc_mac_hdr, update_sta_user->puc_payload,
                                    msg_idx, update_sta_user->msg_len, hmac_user);
    if (rslt != HI_SUCCESS) {
...
/*****************************************************************************
 功能描述  : AP侧更新STA信息
  ...
  2.日    期   : 2013年8月27日
    ...
    修改内容   : 修改函数，添加安全相关的信息
*****************************************************************************/
static hi_u32 hmac_ap_up_update_sta_user(const hmac_vap_stru *hmac_vap,
    const hmac_update_sta_user_info_stru *update, hmac_user_stru *hmac_user, mac_status_code_enum_uint16 *status_code)
{
    hi_u16 msg_idx = 0;
                                        ...
                                        hi_u32 payload_len, hi_u8 mgmt_frm_type)
{
    ...
    /* DTS2014090307793 & DTS2014091602730 */
...
static hi_u32 hmac_ap_get_mac_and_user(const hmac_vap_stru *hmac_vap, const hi_u8 *puc_mac_hdr,
    hi_u8 *sta_mac_addr, hmac_user_stru **hmac_user, hi_u8 *user_idx)
{
    hi_u32                      rslt;
    ...
    if (rslt != HI_SUCCESS) {
        ...
        oam_warning_log3(hmac_vap->base_vap->vap_id, OAM_SF_ASSOC,
                         "{hmac_ap_up_rx_asoc_req::user mac:%XX:XX:XX:%02X:%02X:%02X.}",
    ...
    oam_warning_log1(hmac_vap->base_vap->vap_id, OAM_SF_ASSOC,
                     "{hmac_ap_get_mac_and_user::uc_user_idx[%d].}", *user_idx);
...
static hi_u32 hmac_ap_asoc_pmf_process(const hmac_vap_stru *hmac_vap,
...
{
    hi_u32                         rslt;
    hmac_update_sta_user_info_stru update_sta_user_info = { .puc_mac_hdr = asoc_req.puc_mac_hdr,
            .puc_payload = asoc_req.puc_payload, .msg_len = asoc_req.payload_len };
    ...
    if (*(asoc_info.status_code) != MAC_REJECT_TEMP) {
        ...
        rslt = hmac_ap_up_update_sta_user(asoc_req.hmac_vap, &update_sta_user_info,
                                          asoc_info.hmac_user, asoc_info.status_code);
        if ((rslt != HI_SUCCESS) || (*(asoc_info.status_code) != MAC_SUCCESSFUL_STATUSCODE)) {
            ...
            hmac_user_set_asoc_state(asoc_req.hmac_vap->base_vap,
                                     asoc_info.hmac_user->base_user, MAC_USER_STATE_AUTH_COMPLETE);
        }
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
        /* 同步ap带宽，能力等信息到dmac */
        ...
        if ((rslt != HI_SUCCESS) || (*(asoc_info.status_code) != MAC_SUCCESSFUL_STATUSCODE)) {
            oam_warning_log2(0, OAM_SF_CFG, "hmac_ap_up_update_sta_user fail rslt[%d] status_code[%d].",
                             rslt, *(asoc_info.status_code));
...
static hi_u32 hmac_ap_prepare_asoc_rsp(hmac_ap_asoc_req_stru asoc_req,
    hmac_ap_asoc_info_stru asoc_info, oal_netbuf_stru **asoc_rsp, hi_u32 *asoc_rsp_len)
{
    ...
    *asoc_rsp = (oal_netbuf_stru *)oal_netbuf_alloc(WLAN_MGMT_NETBUF_SIZE, 0, 4);   /* align 4 */
    ...
    if (memset_s(tx_ctl, sizeof(hmac_tx_ctl_stru), 0, oal_netbuf_cb_size()) != EOK) {
    ...
    switch (puc_data[MAC_ACTION_OFFSET_ACTION]) {
        case MAC_SP_ACTION_MESH_PEERING_CONFIRM:
            if (hmac_user == HI_NULL) {
                oam_warning_log0(hmac_user->base_user->vap_id, OAM_SF_ASSOC,
                    "{hmac_ap_up_rx_action_self_protected::rx self-protected confirm frame but user is NULL!.}");
                return;
            }
        ...
        case MAC_SP_ACTION_MESH_GROUP_KEY_ACK:
            if ((hmac_user != HI_NULL) && (hmac_user->base_user->user_asoc_state != MAC_USER_STATE_ASSOC) &&
                (hmac_user->base_user->is_mesh_user == HI_TRUE)) {
                /* 更新用户的RSSI统计信息 */
                hmac_user->base_user->rx_conn_rssi = wlan_rssi_lpf(hmac_user->base_user->rx_conn_rssi,
                                                                   rx_ctrl->rssi_dbm);
            }
...
/*****************************************************************************
 ...
 修改历史      :
  1.日    期   : 2014年2月21日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
static hi_void hmac_ap_up_rx_action(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf,
                                    hi_u8 is_protected)
{
    hmac_user_stru *hmac_user = HI_NULL;
    hmac_rx_ctl_stru *rx_ctrl = (hmac_rx_ctl_stru *)oal_netbuf_cb(netbuf);
...
hi_u32 hmac_mgmt_timeout_ap(hi_void *param)
{
    ...
    if (hmac_vap == HI_NULL) {
        oam_error_log0(hmac_user->base_user->vap_id, OAM_SF_AUTH,
    ...
    hi_u8 *puc_mac_hdr, hi_u8 *puc_payload, hi_u32 payload_len)
{
    ...
    if (status_code == MAC_SUCCESSFUL_STATUSCODE) {
        ...
        if (rslt != HI_SUCCESS) {
            oam_error_log1(0, OAM_SF_ASSOC,
                           "{hmac_mesh_up_rx_confirm_action::hmac_init_user_security_port Err=%d}", rslt);
            status_code = MAC_UNSPEC_FAIL;
        }
    }
    ...
    oam_warning_log1(mac_user->vap_id, OAM_SF_ASSOC,
                     "{hmac_mesh_up_rx_confirm_action::update_sta_user[%d] succ}", mac_user->us_assoc_id);

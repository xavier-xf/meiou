...
hi_u32 hmac_config_set_amsdu_tx_on(mac_vap_stru *mac_vap, hi_u16 us_len, const hi_u8 *puc_param)
{
    ...
    /* 开启AMSDU则为vap 下所有用户(除组播用户)申请amsdu指针 关闭则释放所有的指针 */
                              ...
                              wlan_cfgid_enum_uint16 cfg_id, hi_u16 us_len, const hi_u8 *puc_param)
{
    ...
    if (mac_vap == HI_NULL) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_config_send_event:: mac_vap is null.");
        return HI_FAIL;
    }
    ...
    const mac_cfg_add_vap_param_stru *param)
{
	...
	memset_s(&station_id_param, sizeof(mac_cfg_staion_id_param_stru), 0, sizeof(mac_cfg_staion_id_param_stru));
    hi_u32         ret;
    /* 设置mac地址 */
...
hi_u32 hmac_config_down_vap(mac_vap_stru *mac_vap, hi_u16 us_len, const hi_u8 *param)
{
    ...
    if (mac_vap->vap_state == MAC_VAP_STATE_INIT) {
        /* DTS2015120107074 判断VAP是否down，wal层通过网络设备down/up来判断,
           hmac层通过INIT状态判断,两种状态应该保持一致 */
...
hi_u32 hmac_config_set_bw(mac_vap_stru *mac_vap, hi_u16 us_len, const hi_u8 *puc_param)
{
    ...
    if ((event_set_bw->param == WLAN_BAND_ASSEMBLE_5M) || (event_set_bw->param == WLAN_BAND_ASSEMBLE_10M)) {
        hi_u16 txop[WLAN_WME_AC_BUTT] = {0xfa0, 0xbb8, 0x11f8, 0x1388}; /* 各AC的txop limit */
                                                 ...
                                                 hi_u8 init_flag)
{
    ...
    switch (payload_flag) {
        ...
        case RF_PAYLOAD_RAND:
            netbuf->data[0] = oal_gen_random(18, 1); /* 随机种子为18 */
            for (loop = 1; loop < size; loop++) {
                netbuf->data[loop] = oal_gen_random(18, 0); /* 随机种子为18 */
            }
            break;
        default:
            break;
                                                ...
                                                hi_u8 tid, const hi_u8 *ra_mac_addr, const hi_u8 *ta_mac_addr)
{
    ...
    if (memcpy_s(&netbuf->data[6], WLAN_MAC_ADDR_LEN, ta_mac_addr, WLAN_MAC_ADDR_LEN) != EOK) { /* 6 元素索引 */
        oal_netbuf_free(netbuf);
        return HI_NULL;
    }
    /* 帧体内容 最后6个字节保持为0x00 */
    for (loop = 0; loop < size - 50; loop++) { /* size减去50 */
        netbuf->data[14 + loop] = (hi_u8) loop; /* 循环索引加14 */
    }

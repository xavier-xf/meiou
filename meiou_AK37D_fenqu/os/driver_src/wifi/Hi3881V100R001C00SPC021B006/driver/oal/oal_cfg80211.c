                                      ...
                                      oal_gfp_enum_uint8          en_gfp)
{
    ...
    cfg80211_ready_on_channel(net_dev, cookie, chan, en_channel_type, duration, en_gfp);
#endif
#else
    hi_unref_param(wdev);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))|| (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
                                    oal_wireless_dev *wdev,
#else
...
hi_u32 oal_cfg80211_disconnected(oal_net_device_stru    *pst_net_device,
                                 hi_u16                  us_reason,
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION) && (LINUX_VERSION_CODE < KERNEL_VERSION(3, 16, 0))
                                 hi_u8            *puc_ie,
#else
                                 const hi_u8            *puc_ie,
#endif
                            ...
                            oal_gfp_enum_uint8      en_gfp)
{
    ...
    cfg80211_rx_mgmt(pst_dev, l_freq, puc_buf, ul_len, en_gfp);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0))
    cfg80211_rx_mgmt(pst_dev, l_freq, 0, puc_buf, ul_len, en_gfp);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3, 12, 0))
    cfg80211_rx_mgmt(pst_wdev, l_freq, 0, puc_buf, ul_len, en_gfp);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3, 18, 0))
    cfg80211_rx_mgmt(pst_wdev, l_freq, 0, puc_buf, ul_len, 0, en_gfp);
#else
    cfg80211_rx_mgmt(pst_wdev, l_freq, 0, puc_buf, ul_len, 0);
#endif
#elif (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)

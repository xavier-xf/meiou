...
static hi_s32 _oal_sdio_transfer_scatt(const oal_channel_stru *hi_sdio, hi_s32 rw, hi_u32 addr,
                                       struct scatterlist *sg, hi_u32 sg_len, hi_u32 rw_sz);
...
hi_s32 oal_sdio_send_msg(oal_channel_stru *hi_sdio, unsigned long val)
{
    ...
    if (val >= H2D_MSG_COUNT) {
        oam_error_log1(0, OAM_SF_ANY, "[Error]invaild param[%lu]!\n", val);
        return -OAL_EINVAL;
    }
    ...
    if (g_sdio_extend_func) {
        ...
        if (oal_likely(ret == HI_SUCCESS)) {
            ...
            oal_print_hex_dump((hi_u8 *)hi_sdio->sdio_extend,
                               sizeof(hisdio_extend_func), 32, "extend :"); /* 32 ���� */
    ...
    hi_s32 ret;
    {
        ...
        if (ret == HI_SUCCESS) {
            printk(KERN_DEBUG"=========extend buff:%d=====\n",
                   HISDIO_COMM_REG_SEQ_GET(hi_sdio->sdio_extend->credit_info));
            oal_print_hex_dump((hi_u8 *)hi_sdio->sdio_extend,
                               sizeof(struct hisdio_extend_func), 32, "extend :"); /* 32 ���� */
        }
#endif
    }
    return ret;
...
#ifdef _PRE_SDIO_FEATURE_FLAT_MEMORY
static hi_s32 oal_sdio_transfer_cal_size(const oal_channel_stru *hi_sdio, hi_s32 rw, const oal_netbuf_head_stru *head, hi_u32 *buf_len)
{
    hi_s32 ret = OAL_SUCC;
    hi_u32 queue_len;
    hi_u32 sum_len = 0;
    ...
    skb_queue_walk_safe(head, netbuf, tmp) {
        if (!oal_is_aligned((uintptr_t)oal_netbuf_data(netbuf), 4)) {
        ...
        if (rw == SDIO_WRITE) {
            memcpy_s(hi_sdio->tx_flat_buff + sum_len, SDIO_MAX_FLAT_MEMORY_SIZE, oal_netbuf_data(netbuf), oal_netbuf_len(netbuf));
        }
    ...
    if (WARN_ON(sum_len > SDIO_MAX_FLAT_MEMORY_SIZE)) {
        return -OAL_EINVAL;
    }
...
static hi_s32 oal_sdio_transfer_copy(const oal_channel_stru *hi_sdio,
    const oal_netbuf_head_stru *head,
    hi_u8* buf,
    hi_u32 len)
{
    oal_netbuf_stru *netbuf = HI_NULL;
    ...
    hi_s32 index =0;
    skb_queue_walk_safe(head, netbuf, tmp) {
        memcpy_s(oal_netbuf_data(netbuf), oal_netbuf_len(netbuf), buf + offset, oal_netbuf_len(netbuf));
...
static hi_s32 oal_sdio_transfer_flat_memory(const oal_channel_stru *hi_sdio,
    const oal_netbuf_head_stru *head,
    hi_s32 rw)
{
    hi_s32 ret = OAL_SUCC;
    ...
    if (hi_sdio == HI_NULL) {
        oam_error_log0(0, OAM_SF_ANY, "hi_sdio is NULL.");
        return -OAL_EFAIL;
    }
    if (rw != SDIO_READ && rw != SDIO_WRITE) {
        oam_error_log1(0, OAM_SF_ANY, "rw is invalid. rw: %d", rw);
        return -OAL_EFAIL;
    }
    ...
    if (oal_unlikely(oal_sdio_get_state(hi_sdio, OAL_SDIO_ALL) != HI_TRUE)) {
        sdio_release_host(hi_sdio->func);
        oam_error_log0(0, OAM_SF_ANY, "hi_sdio statu is invalid.");
        return -OAL_EFAIL;
    }
    sdio_release_host(hi_sdio->func);

    hi_u32 buf_len = 0;
    ret = oal_sdio_transfer_cal_size(hi_sdio, rw, head, &buf_len);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "oal_sdio_transfer_cal_size failed. ret: %d", ret);
        return ret;
    }

    sdio_claim_host(hi_sdio->func);
    if (rw == SDIO_WRITE) {
        ret = oal_sdio_writesb(hi_sdio->func, 0, hi_sdio->tx_flat_buff, buf_len);
        if (ret != OAL_SUCC) {
            oam_error_log2(0, OAM_SF_ANY, "oal_sdio_writesb failed. ret: %d, buf_len:%u", ret, buf_len);
        }
    } else {
        ret = oal_sdio_readsb(hi_sdio->func, hi_sdio->rx_flat_buff, 0, buf_len);
        if (ret != OAL_SUCC) {
            oam_error_log2(0, OAM_SF_ANY, "oal_sdio_readsb failed. ret: %d, buf_len:%u", ret, buf_len);
        }
    }
    sdio_release_host(hi_sdio->func);
    if (ret != OAL_SUCC) {
    ...
    if (rw == SDIO_READ) {
        ret = oal_sdio_transfer_copy(hi_sdio, head, hi_sdio->rx_flat_buff, buf_len);
        if (ret != OAL_SUCC) {
            oam_error_log1(0, OAM_SF_ANY, "oal_sdio_transfer_copy failed. ret: %d", ret);
            return ret;
        }
    }
    return ret;
...
hi_s32 oal_sdio_transfer_rx_reserved_buff(const oal_channel_stru *hi_sdio)
{
    ...
    if (hi_sdio->scatt_info[SDIO_READ].max_scatt_num < seg_nums) {
        oam_error_log2(0, OAM_SF_ANY, "{sdio seg nums :%u large than rx scatt num %u!}",
                       seg_nums, hi_sdio->scatt_info[SDIO_READ].max_scatt_num);
        return -OAL_EINVAL;
    }
...
#ifdef _PRE_SDIO_FEATURE_FLAT_MEMORY
    ret = oal_sdio_readsb(hi_sdio->func, hi_sdio->rx_flat_buff, 0, ul_extend_len);
#else
    ret = _oal_sdio_transfer_scatt(hi_sdio, SDIO_READ, HISDIO_REG_FUNC1_FIFO, sg, seg_nums, ul_extend_len);
#endif
    if (oal_unlikely(ret)) {
        printk("sdio trans revered mem failed! oal_sdio_transfer_commit, ret=%d\n", ret);
    }
    return ret;
}
...
static hi_s32 oal_sdio_clear_int_status(const oal_channel_stru *hi_sdio, hi_u8 int_stat)
{
    ...
    if (oal_unlikely(ret)) {
        printk("[SDIO][Err]faild to clear sdio func1 interrupt!ret=%d\n", ret);
        return ret;
    }
    return HI_SUCCESS;
}
 ...
 */
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
hi_void oal_sdio_get_dev_pm_state(oal_channel_stru *hi_sdio, unsigned long *pst_ul_f1,
...
hi_s32 oal_sdio_dev_init(oal_channel_stru *hi_sdio)
{
    ...
    oal_disable_sdio_state(hi_sdio, OAL_SDIO_ALL);
#endif
    sdio_en_timeout(func)  = 1000; /* ��ʱʱ��Ϊ1000  */
    ...
    if (wlan_resume_state_get() == 0) { /* host����ָ����������жϣ���ֹ���ݶ�ʧ */
#endif
#endif
    ...
    hi_s32 incr_addr, struct scatterlist *sg, hi_u32 sg_len, hi_u32 blocks, hi_u32 blksz)
{
...
#ifdef CONFIG_SDIO_DEBUG
    printk("wait for %s tranfer over.\n", write ? "write" : "read");
#endif
    if (cmd.error) {
        return cmd.error;
    }
    ...
    do {
        ...
        for_each_sg(data.sg, sg_t, data.sg_len, i) {
            ...
            oal_print_hex_dump(sg_virt(sg_t), sg_t->length, 32, "sg buf  :"); /* 32 ���� */
        }
    } while (0);
...
hi_s32 oal_sdio_transfer_netbuf_list(const oal_channel_stru *hi_sdio, const oal_netbuf_head_stru *head, hi_s32 rw)
{
    ...
    hi_s32 ret;
#ifndef _PRE_SDIO_FEATURE_FLAT_MEMORY
    hi_u32 idx = 0;
    hi_u32 queue_len;
    hi_u32 sum_len = 0;
    hi_u8 sg_realloc = 0;
    ...
    skb_queue_walk_safe(head, netbuf, tmp) {
        ...
        if (!oal_is_aligned((uintptr_t)oal_netbuf_data(netbuf), 4)) { /* 4 �ֽڶ��� */
            oam_error_log0(0, OAM_SF_ANY, "{oal_sdio_transfer_netbuf_list netbuf 4 aligned fail!}");
            return -OAL_EINVAL;
        }
...
#ifdef _PRE_SDIO_FEATURE_FLAT_MEMORY
    ret = oal_sdio_transfer_flat_memory(hi_sdio, head, rw);
#else
...
#else
    hi_s32 ret = hisi_sdio_rescan(val);
...
hi_s32 oal_sdio_func_probe_resume(void)
{
    ...
    if (oal_wait_for_completion_timeout(&g_sdio_driver_complete, TIMEOUT_MUTIPLE_10 * HZ)) {
...
hi_s32 oal_sdio_func_probe(oal_channel_stru *hi_sdio)
{
    ...
    if (times <= 0) {
        oam_print_err("hisi sdio load fail.\n");
        goto failed_sdio_enum;
    }
...
detect_retry:
    ...
    if (oal_wait_for_completion_timeout(&g_sdio_driver_complete, TIMEOUT_MUTIPLE_5 * HZ)) {
        printk("hisi sdio load sucuess, sdio enum done.\n");
    } else {
        printk("sdio enum timeout, reason[%s]\n", g_sdio_enum_err_str);
...
#ifdef _PRE_SDIO_FEATURE_FLAT_MEMORY
    hi_sdio->tx_flat_buff = (hi_void *)oal_memalloc(SDIO_MAX_FLAT_MEMORY_SIZE);
    if (hi_sdio->tx_flat_buff == HI_NULL) {
        printk("[E]alloc tx_flat_buff failed [%u]\n", SDIO_MAX_FLAT_MEMORY_SIZE);
        goto failed_rx_reserved_buff_alloc;
    }
    hi_sdio->tx_flat_buff_len = SDIO_MAX_FLAT_MEMORY_SIZE;
    printk("{alloc %u bytes tx_flat_buff!} \n", SDIO_MAX_FLAT_MEMORY_SIZE);

    hi_sdio->rx_flat_buff = (hi_void *)oal_memalloc(SDIO_MAX_FLAT_MEMORY_SIZE);
    if (hi_sdio->rx_flat_buff == HI_NULL) {
        printk("[E]alloc rx_flat_buff failed [%u]\n", SDIO_MAX_FLAT_MEMORY_SIZE);
        goto failed_rx_reserved_buff_alloc;
    }
    hi_sdio->rx_flat_buff_len = SDIO_MAX_FLAT_MEMORY_SIZE;
    printk("{alloc %u bytes rx_flat_buff!} \n", SDIO_MAX_FLAT_MEMORY_SIZE);
#endif
...
#ifdef CONFIG_HISDIO_H2D_SCATT_LIST_ASSEMBLE
    tx_scatt_buff_len = HISDIO_HOST2DEV_SCATT_SIZE
...
hi_s32 oal_sdio_reinit(void)
{
    ...
    sdio_en_timeout(pst_func) = 1000; /* ��ʱʱ��Ϊ1000ms  */

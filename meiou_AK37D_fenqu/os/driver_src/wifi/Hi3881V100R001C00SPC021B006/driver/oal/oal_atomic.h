...
#ifdef _PRE_LOSCFG_KERNEL_SMP
#ifndef G_OAL_SMP_SPIN_LOCK
#define G_OAL_SMP_SPIN_LOCK
static SPIN_LOCK_INIT(g_oal_smp_spinlock);
#endif
#endif
...
static inline int oal_atomic_add_return(int i, oal_atomic *v)
{
...
#ifdef _PRE_LOSCFG_KERNEL_SMP
    LOS_SpinLockSave(&g_oal_smp_spinlock, &int_save);
#else
    ...
    v->counter = val += i;
#ifdef _PRE_LOSCFG_KERNEL_SMP
    LOS_SpinUnlockRestore(&g_oal_smp_spinlock, int_save);
#else

...
#define LWIP_PBUF_STRUCT_SIZE               hi_byte_align(sizeof(oal_lwip_buf), 4) /* struct pbuf 结构体大小 */
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(4, 7, 0)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
enum ieee80211_band {
...
typedef struct _oal_hisi_eapol_stru {
    ...
    hi_u8                   eapol_cnt;    /* 当前链表中eapol数量 */
    hi_u16                  enqueue_time; /* 记录的eapol帧最早入队时间 单位s 用于超时老化 */
    hi_void*                context;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,0)) || (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)

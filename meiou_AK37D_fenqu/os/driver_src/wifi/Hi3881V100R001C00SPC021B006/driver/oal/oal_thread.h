...
/* struct sched_param change file */
...
#define g_stLosTask         OsCurrTaskGet()
#else
...
#ifdef _PRE_LOSCFG_KERNEL_SMP
    ret = LOS_TaskCpuAffiSet(ul_taskid, 0x2);
    if (ret != 0) {
        oal_io_print1("Failed to set cpu affi %s thread\n", pc_thread_name);
        return NULL;
    }
#endif
...
#ifndef _PRE_LOSCFG_KERNEL_SMP
static inline oal_kthread_stru *oal_get_current(hi_void)
{

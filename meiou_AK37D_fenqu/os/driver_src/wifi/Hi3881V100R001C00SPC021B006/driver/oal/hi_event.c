 ...
 * Description: os event interface.
 * Author: Hisilicon
 * Create: 2019-03-04
 */
...
#ifdef _PRE_LOSCFG_KERNEL_SMP
#ifndef G_EVENT_SMP_SPIN_LOCK
#define G_EVENT_SMP_SPIN_LOCK
static SPIN_LOCK_INIT(g_event_smp_spinlock);
#endif
#endif
...
hi_u32 hi_event_init(hi_u8 max_event_cnt, hi_pvoid event_space)
{
    if (!g_event_ctrl.init) {
        ...
        memset_s(g_event_ctrl.item, sizeof(hi_event_item) * max_event_cnt, 0,
                 sizeof(hi_event_item) * max_event_cnt);
...
#ifdef _PRE_LOSCFG_KERNEL_SMP
    LOS_SpinLockSave(&g_event_smp_spinlock, &int_save);
#else
...
#ifdef _PRE_LOSCFG_KERNEL_SMP
    LOS_SpinUnlockRestore(&g_event_smp_spinlock, int_save);
#else

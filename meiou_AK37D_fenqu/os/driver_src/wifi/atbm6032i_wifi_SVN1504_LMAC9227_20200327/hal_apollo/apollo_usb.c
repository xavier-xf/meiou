					...
					u16 value, u16 index, void *pdata,u16 len)
{
	...
	do {
		...
		if (status < 0) {
			atbm_printk_err("%s:err(%d)addr[%x] len[%d],b_write %d request %d\n",__func__,status,value|(index<<16),len,b_write, request);
		} else if(status != len) {
...
static int atbm_usb_hw_write_port(struct sbus_priv *self, u32 addr, const void *pdata,int len)
{
	...
	if(atomic_read(&self->drvobj->ctrl_err)){
		ret = -ENODEV;		
		atbm_printk_err("%s: ctrl_err\n",__func__);
		goto exit;
	}
	
	if((!!pdata)^(!!len)){
		atbm_printk_err("%s: len(%d),pdata(%p)\n",__func__,len,pdata);
		ret = -EINVAL;
		goto exit;
	}
	
	if(len > ATBM_USB_EP0_MAX_SIZE){
		atbm_printk_err("%s: len(%d)\n",__func__,len);
		ret = -EINVAL;
		goto exit;
	}
	...
	if (!urb) {
		atbm_printk_err("%s: alloc urb err\n",__func__);
		ret = -ENOMEM;
		goto exit;
	}
	...
	if(ctrl == NULL){		
		atbm_printk_err("%s: alloc ctrl err\n",__func__);
		ret = -ENOMEM;
		goto exit;
	}
...
static int atbm_usb_wait_anchor_empty_timeout(struct sbus_priv *self,int timeout)
{
	...
	if(ret == 0){
		atbm_printk_err("%s:tx_submitted cancle timeout(%d) \n",__func__,usb_anchor_empty(&hw_priv->sbus_priv->drvobj->tx_submitted));
	}
	atbm_printk_bus("Unlink Tx Urb(%d)\n",ret);
exit:
	return ret;
}
static void atbm_destroy_wsm_cmd(struct atbm_common *hw_priv)
{
	/*
	*flush work
	*/
...
static int atbm_reinit_firmware(struct atbm_common *hw_priv)
{
...
int atbm_lmac_restart(struct sbus_priv *self)
{
	...
	*/
	if(atbm_usb_module_trylock()){ 
		...
		atbm_printk_err("%s:Skip Restart\n",__func__);
	}
	return ret;
}
...
static void atbm_usb_rev_skb_cb(struct atbm_common *hw_priv,struct sk_buff *skb)
{
...
static void atbm_usb_receive_data_complete(struct urb *urb)
{
	...
	if(0){
		...
		if(!in_irq()){
			...
			skb->pkt_type = ATBM_RX_DERICTLY_DATA_FRAME;
			if(run_inirq)
				printk_once("[atbm_log]%s:urb in softirq\n",__func__);
			run_inirq = 0;
		}else {
			...
			if(run_inirq == 0)
				printk_once("[atbm_log]%s:urb in hardirq\n",__func__);
			run_inirq = 1;
		}
#endif		
			 ...
			 struct ieee80211_vif *vif)
{
	...
	if(hw_priv != priv->hw_priv){
		atbm_printk_err("%s:hw_priv != priv->hw_priv\n",__func__);
		return;
	}

	if(priv->join_status != ATBM_APOLLO_JOIN_STATUS_STA){
		atbm_printk_err("%s:priv not sta mode\n",__func__);
		return;
	}
...
{	
	...
	if(hw_priv == NULL){
		atbm_printk_err( "%s:hw_priv == NULL\n",__func__);
		goto sync_end;
	}
	hw = hw_priv->hw;

	if(hw_priv != hw->priv){
		atbm_printk_err("%s:hw_priv != hw->priv\n",__func__);
		goto sync_end;
	}
	/*
...
/*
	...
	*/
	atbm_printk_init("self->tx_hwChanId %d\n",self->tx_hwChanId);
...
static void __atbm_usb_disconnect(struct usb_interface *intf)
{
	...
	//	mdelay(2000);
		...
		if(pdev)
		{
			...
			atbm_printk_exit("we have get dev,so put it in the end\n");
			usb_put_dev(pdev);
		}
...
static void atbm_usb_disconnect_flush(struct atbm_common *hw_priv)
{
	...
	*/
	...
	if(hw_priv->wsm_cmd.cmd != 0xFFFF){
		...
		if(hw_priv->wsm_cmd.ptr){
			hw_priv->wsm_cmd.ptr = NULL;
			atbm_printk_err("%s:cancle new cmd\n",__func__);
		}else {
			atbm_printk_err("%s:cancle old cmd\n",__func__);
		}
		wake_up(&hw_priv->wsm_cmd_wq);
	}
	spin_unlock_bh(&hw_priv->wsm_cmd.lock);
...
static void atbm_usb_prepare_disconnect(struct atbm_common *hw_priv)
{
	...
	*/
	...
	if(hw_priv->etf_test_v2)
	{
		...
		atbm_printk_always("[%s]:usb device removed,stop etf test @_@ @_@\n",__func__);
...
{
	atbm_printk_pm("===----===start resume state\n");
	self->suspend=0;
...
{
	int ret;
	usb_platform_dev = platform_device_alloc("atbmusbwifi",0);
...
{
	int loop =0;
	atbm_printk_exit("%s:wifi_tx_urb_pending(%d)\n",__func__,wifi_tx_urb_pending);

...
*/
	...
	*/
	if((frame->frame_control & cpu_to_le16(0x00F0)) ==
	       cpu_to_le16(0x00F0)){	   
		skb->pkt_type = IEEE80211_SDATA_QUEUE_SPECIAL_TYPE;
		atbm_skb_queue_tail(&(vif_to_sdata(priv->vif)->skb_queue), skb);
		ieee80211_queue_work(hw_priv->hw, &(vif_to_sdata(priv->vif)->work));
		*skb_p = NULL;
		return;
	}
#endif

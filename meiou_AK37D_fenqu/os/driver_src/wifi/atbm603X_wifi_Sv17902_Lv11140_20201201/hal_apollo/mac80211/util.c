					    ...
					    int show_errors)
{
	...
	case ATBM_OUI_MICROSOFT:
		...
		case 1:
			...
			elems->wpa = pos;
			elems->wpa_len = elen;
			encry_type_count = pos[11];
			encry_type_count = ((encry_type_count << 8) | pos[10]);
			
			//atbm_printk_err("ATBM_OUI_MICROSOFT = 221 encry_type_count = %d\n",encry_type_count);
			for(i = 0;i<encry_type_count;i++){
				offsert = 4*(i+1);
				if(elen < (11 + offsert)){
					atbm_printk_err("%s,outof range elen[%d] 11 + offsert[%d]  encry_type_count[%d]\n",__func__,
						elen,11 + offsert,encry_type_count);
					break;
				}
				elems->encry_info |= 1<<pos[11+offsert];
			}
			break;
			       ...
			       u64 filter, u32 crc)
{
	...
	while (left >= 2) {
		...
		case ATBM_WLAN_EID_RSN:
			elems->rsn = pos;
			elems->rsn_len = elen;
			encry_type_count = pos[7];
			encry_type_count = ((encry_type_count << 8) | pos[6]);
		//	encry_type_count = (pos[7] << 8 | pos[6]);
		//	atbm_printk_err("rsn = 48 encry_type_count = %d\n",encry_type_count);
			for(i = 0 ; i < encry_type_count;i++){
				offsert = 4*(i+1);
				if(elen < (7 + offsert)){
					atbm_printk_err("%s,outof range elen[%d] 11 + offsert[%d] encry_type_count[%d] \n",__func__,elen,7 + offsert,encry_type_count);
					break;
				}
				elems->encry_info |= (1<<pos[7 + offsert]);
			}
			break;
	  ...
	  */
	  ...
	  if(ieee80211_is_probe_req(mgmt->frame_control)){
		  ...
		  if(atbm_ie){
			  ...
			  *send probe response
			  */
			  atbm_printk_err("[probe req] from [%pM] channel[%d] special ie[%d][%d][%s]\n",mgmt->sa,freq,atbm_ie[0],atbm_ie[1],special_data);
			  ...
			  atbm_printk_err("[probe req] from [%pM] channel[%d] \n",mgmt->sa,freq);
		  }
	  }else {
		  /*
		  *other frame
...
*/
bool ieee80211_sdata_send_special_frame(struct ieee80211_sub_if_data *sdata,
													  u16 frame_type,
													  u8 *a1,
													  u8 *a2,
													  u8 *a3,
													  ...
													  size_t frame_len)
{
	...
	*/
	...
	skb = atbm_dev_alloc_skb(frame_len + sdata->local->hw.extra_tx_headroom + sizeof(*hdr));
	...
	hdr->frame_control = cpu_to_le16(frame_type);
	memcpy(hdr->addr1, a1, ETH_ALEN);
	memcpy(hdr->addr2, a2, ETH_ALEN);
	memcpy(hdr->addr3, a3, ETH_ALEN);
	memcpy(atbm_skb_put(skb, frame_len),frame,frame_len);
				  ...
				  struct sk_buff *skb)
{
	...
	*/
	...
	if (ieee80211_is_probe_resp(mgmt->frame_control)) {
		...
		freq = (freq - 2412)/5 + 1;
		ie = atbm_ieee80211_find_ie(ATBM_WLAN_EID_PRIVATE,mgmt->u.probe_resp.variable,
				                   skb->len-offsetof(struct atbm_ieee80211_mgmt, u.probe_resp.variable));		
		if(ie){
			u8 special_data[255]={0};
			ie[2+ie[1]] = 0;
			atbm_printk_err("[probe resp] from [%pM] channel[%d] special ie[%d][%d][%s]\n",mgmt->sa,freq,ie[0],ie[1],ie+2);			
			...
			//atbm_printk_err("[probereq] from [%pM] channel[%d] \n",mgmt->sa,freq);
			return ;
		}

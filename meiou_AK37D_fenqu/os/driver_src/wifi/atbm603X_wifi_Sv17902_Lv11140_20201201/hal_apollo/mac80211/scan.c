...
static void ieee80211_scan_try_spilt(struct ieee80211_local *local)
{
		...
		if((result->sta.ie_len != sta_node_target->sta.ie_len)){
			if((result->sta.ie_len == 0)){
				...
				*/
				...
				{
				/*
				* only save the new special ie , if return same ap_info have special ie save this
				*/
...
static bool ieee80211_prep_hw_scan(struct ieee80211_local *local)
{
	...
	do {
		...
		ieee80211_scan_try_spilt(local);

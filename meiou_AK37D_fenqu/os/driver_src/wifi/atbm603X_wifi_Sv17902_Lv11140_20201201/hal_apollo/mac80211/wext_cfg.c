...
extern bool atbm_internal_update_ap_conf(struct ieee80211_sub_if_data *sdata,
...
static const struct iw_priv_args atbm_privtab[] = {
		...
		{SIOCIWFIRSTPRIV + 0x1E, IW_PRIV_TYPE_CHAR | 500, 0, "best_ch_scan"},
		...
		{SIOCIWFIRSTPRIV + 0x18, IW_PRIV_TYPE_CHAR | 500, 0, "best_ch_scan"},
#ifdef CONFIG_ATBM_STA_LISTEN
...
static int atbm_ioctl_ie_ipc_clear_insert_data(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
        int ret = 0,i;
...
int atbm_ioctl_best_ch_scan_result(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
	...
	atbm_printk_wext("atbm_ioctl_best_ch_scan_result:\n");
	for(i = 0;i<CHANNEL_NUM;i++){
		scan_result.channel_ap_num[i] = results.n_aps[i];
		scan_result.busy_ratio[i] = results.busy_ratio[i];
		atbm_printk_wext("channel[%d],channel_ap_num[%d]\n",i+1,scan_result.channel_ap_num[i]);
	}
/*
	for(i = 0;i<CHANNEL_NUM;i++){
		scan_result.busy_ratio[i] = results.busy_ratio[i];
...
static int atbm_ioctl_get_Tjroom(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if((ret = copy_to_user(wrqu->data.pointer, pbuff, strlen(pbuff))) != 0){
		atbm_kfree(pbuff);
		return -EINVAL;
	}
...
static int atbm_ioctl_subtype(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	union iwreq_data *wdata = (union iwreq_data *)wrqu;
	u8 a1[6]={0x10,0x12,0x13,0x14,0x15,0x16};
	u8 a2[6]={0x10,0x12,0x13,0x14,0x15,0x17};
	u8 a3[6]={0x10,0x12,0x13,0x14,0x15,0x18};
	u8 frame[6] = {0xaa,0xff,0xaa,0xff,0xaa,0xff};
	ieee80211_sdata_send_special_frame(sdata,0xf0,a1,a2,a3,frame,6);
	return 0;
}
#endif
...
static int atbm_ioctl_edca_params(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if (!ieee80211_sdata_running(sdata)){
		atbm_printk_err("interface not running! \n");
		ret =  -ENETDOWN;
		goto exit;
	}
	...
	if(!ptr){
		atbm_printk_err("atbm_kzalloc fail \n");
		ret =  -ENOMEM;
		goto exit;
	}
	
	if(copy_from_user(ptr, wdata->data.pointer, wdata->data.length)){
		atbm_printk_err("copy_from_user fail \n");
		ret =  -EINVAL;
		goto exit;
	}
	...
	if(pos == NULL){
		atbm_printk_err("%d,atbm_skip_space fail \n",__LINE__);
		ret = -EINVAL;
		goto exit;
	}
	...
	*/
	...
	if(atbm_internal_edca_update(sdata,queue,aifs,cw_min,cw_max,txop) == false){
		atbm_printk_err("%d,atbm_internal_edca_update fail \n",__LINE__);
		ret = -EINVAL;
		goto exit;
	}
exit:
...
static int atbm_ioctl_get_edca_params(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if(copy_to_user(ptr, (char *)edca_value, sizeof(struct edca_data) * 4) != 0){
		atbm_printk_err("copy_to_user fail\n");
		return -EINVAL;
	}
...
int get_rate_delta_gain(s8 *dst)
{
	...
	if(access_file(file,readbuf,sizeof(readbuf),1) > 0)
	{
		pdata[count] = readbuf;
		count++;
...
static int atbm_ioctl_set_rate_txpwr_by_file(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	int if_id = -1;
	int rate_idx=0,useFlag = 0;
	s8 rate_txpower[12] = {0};//validfalg,data
	char *pbuffer = NULL;
	char *pRxData = NULL;
...
static int atbm_ioctl_common_cmd(struct net_device *dev, struct iw_request_info *info, void *wrqu, char *extra)
{
	...
	if(memcmp(ptr, "get_rssi", 8) == 0)
		...
		ret = atbm_ioctl_set_efuse(dev, info, wrqu, extra);
	else if(memcmp(ptr, "setEfuse_tjroom", 18) == 0)
		ret = atbm_ioctl_set_efuse(dev, info, wrqu, extra);
	...
	}else if(memcmp(ptr,"stations_show",8) == 0){
		ret = atbm_ioctl_associate_sta_status(dev, info, wrqu, extra);
	}
#ifdef ATBM_PRIVATE_IE
...
{	
	...
	{
		gthreshold_param.chip_crystal_type = 1;
	}
	atbm_direct_read_reg_32(hw_priv, 0x16101010, &pin_reg);
	if (pin_reg & BIT(5))
	{
		gthreshold_param.chip_crystal_type |= BIT(1);
	}
	if (pin_reg & BIT(27))
	{
		gthreshold_param.chip_crystal_type |= BIT(2);
	}
	atbm_direct_write_reg_32(hw_priv, 0x17400000, pin_reg17400000);

	atbm_printk_always("crystal:%d\n",gthreshold_param.chip_crystal_type);
#else
	return 0;
#endif
}
...
static void get_test_threshold(char *param)
{
	...
	if(strlen(param) != 0)
	{
		sscanf(param, "cfg:%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
			&Freq, &txEvm, &rxEvm, &txEvmthreshold,&rxEvmthreshold,&Txpwrmax, 
			&Txpwrmin, &Rxpwrmax, &Rxpwrmin, &rssifilter, &cableloss, &default_dcxo,&noFreqCali);
		...
		gthreshold_param.noFfreqCaliFalg = noFreqCali;
	}
	else
	{
	...
	atbm_printk_always("Freq:%d,txEvm:%d,rxEvm:%d,txevmthreshold:%d,rxevmthreshold:%d,Txpwrmax:%d,Txpwrmin:%d,Rxpwrmax:%d,Rxpwrmin:%d,rssifilter:%d,cableloss:%d,default_dcxo:%d,noFreqCali:%d\n",
		...
		gthreshold_param.noFfreqCaliFalg);
}
...
static int Test_FreqOffset_v2(struct atbm_common *hw_priv, u32 *dcxo, int *pfreqErrorHz)
{
	...
	if(gthreshold_param.noFfreqCaliFalg == 1)
	{
...
#endif
	if(gthreshold_param.chip_crystal_type != 2)
	{
		if(gthreshold_param.freq_ppm != 0)

...
static u32 gsl_ts_get_status(struct i2c_client *client)
{
	...
	else
	{
		...
		if(tmp == 0)
		{
			pr_info("\033[31m" "error: chip status is incorrect" "\033[0m");
		}
		else if(tmp == 0x5a5a5a5a)
		{
			pr_info("\033[32m" "success: chip status is correct" "\033[0m");
		}
	}
	msleep(10);
...
static void process_gslX680_data(struct gsl_ts *ts)
{
	...
	{	
		...
		//	input_mt_report_slot_state(ts->input, MT_TOOL_FINGER, false);
			...
			//printk("process_gslx680_data  X= %d Y= %d  ID= %d\n",last_y, last_x,i);
		
			input_mt_slot(ts->input, i);		
			input_report_abs(ts->input, ABS_MT_TRACKING_ID, i);
			input_report_abs(ts->input, ABS_MT_TOUCH_MAJOR, 0);
			input_report_abs(ts->input, ABS_MT_POSITION_X, last_x+1);
			input_report_abs(ts->input, ABS_MT_POSITION_Y, last_y+1);	
			input_report_abs(ts->input, ABS_MT_WIDTH_MAJOR, 1);
		#endif
			id_sign[i]=0;
		}

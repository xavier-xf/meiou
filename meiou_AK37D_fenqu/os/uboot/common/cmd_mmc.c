...
static int do_mmcinfo(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (curr_device < 0) {
		if (get_mmc_num() > 0) {
			curr_device = 0;
			printf("cdh:MMC device available:curr_device=%d\n", curr_device);
		}
		else {
			puts("No MMC device available\n");
			return 1;
	...
	if (mmc) {
		puts("find MMC device available\n");
		mmc_init(mmc);
...
static int do_mmcops(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (state != MMC_INVALID) {
		...
		if (addr > (void *)(CONFIG_SYS_SDRAM_BASE + gd->ram_size - CONFIG_SYS_SDRAM_PROTECT_SIZE)){

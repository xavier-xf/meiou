...
extern int do_read_dtbcfg(unsigned long *lcdbase, unsigned long *fdt_base);
...
#else
		//uchar *d = dest;  //waring by 20200221
#endif
...
#endif
		for (i = 0; i < count; ++i) {
			//uchar c, bits;   //waring by 20200221

			//c = *s++;              //waring by 20200221
			//bits = video_fontdata[c * VIDEO_FONT_HEIGHT + row]; //waring by 20200221
...
{
	if(panel_info.vl_bpix == LCD_COLOR24){
		*line_length = (panel_info.vl_col * 24) / 8;
	}else{

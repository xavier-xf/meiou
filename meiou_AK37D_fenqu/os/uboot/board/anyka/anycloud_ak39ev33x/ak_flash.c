...
* @PARAM  
* @RETURN 
* @NOTE: 该函数将在芯片内部的BGR的电压输出送到VCM2 引脚，方便万用表测量
*/
#if 0
u32  efuse_make_BGR_output_to_vcm2(void)
{
...
/**
* @BRIEF  efuse_burn_voltage_close
* @AUTHOR Zou Tianxiang
* @DATE   2016-11 -7
* @PARAM  
* @PARAM  
* @RETURN 
* @NOTE: 关闭EFUSE 烧录电压，其实就是关闭VCM3 和VCM2
* @NOTE: 这是内部函数不对外使用
*/
static void efuse_burn_voltage_close(void)
{
	//关闭VCM3的电源开关， 即从2.5V的输出降到0V
...
* @PARAM  
* @RETURN 
* @NOTE: 读取efuse 中的trimming值，并设置8C寄存器的第19位， 让这个trimming影响BGR电压
*/
//u32  efuse_read_for_trimming()
unsigned long efuse_read(void)
{
	unsigned long efuse_reg1_val;
	unsigned long efuse_reg2_val;
	unsigned long trimming;
...
*/
	//关闭EFUSE 烧录电压
	efuse_burn_voltage_close();
	
	//delay 3ms 让AVDD2V5下降到0
	mdelay(3);
	...
	REG32_EFUSE(EFUSE_CFG_REG) |= ((0x1<<31) | (0x1<<29));
	...
	debug("efuse_reg2_val = %x\n", (u32)efuse_reg2_val);

	trimming = ((efuse_reg2_val>>16) & 0xf);

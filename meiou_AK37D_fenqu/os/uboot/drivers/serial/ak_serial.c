...
int _serial_getc(const int dev_index)
{
	...
	reg = readl(base + UART_STATUS_REG);
	reg |= (UART_RX_TO_FLAG | UART_RX_FP_ERR);
	writel(reg, base + UART_STATUS_REG);
	...
	while(1)
	{
		 ...
		 */
		if((readl(base + UART_STATUS_REG) & (0x1 << 17))) {
			/* add clean l2 buffer uart buffer status */
			reg = readl(0x2014008C);
			reg |= (0x1<<17);
			writel(reg, 0x2014008C);
			
			reg = readl(base + UART_CONF_REG);
			reg |= UART_CLEAR_RX_STAT;
			writel(reg, base + UART_CONF_REG);
		}
	}
...
int _serial_tstc(const int dev_index)
{
	...
	regval = (readl(base + UART_COUNT_REG)>>13) & 0x7f;
	if (l2_uartbuffer_offset != regval) {
		return 1;
	}

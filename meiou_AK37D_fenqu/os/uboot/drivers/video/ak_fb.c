...
int ak_lcd_parameter_parse_dt1(const void *blob, struct ak_fb *fb_dev)
{
    ...
    fb_dev->reset_pin = fdtdec_get_valoffset_int(blob, node, "reset-pins", 1, -1);
    if (fb_dev->reset_pin < 0) {
#ifdef LCD_RESET_PIN
        fb_dev->reset_pin = LCD_RESET_PIN;
#else
        printf("could not get pannel device reset pin\n");
        ret = 1;
        goto lcd_para_exit;
#endif
    }
    fb_dev->reset_pin_active = fdtdec_get_valoffset_int(blob, node, "reset-pins", 2, -1);
    if (fb_dev->reset_pin_active < 0) {
#ifdef LCD_RESET_PIN_ACTIVE
        fb_dev->reset_pin_active = LCD_RESET_PIN_ACTIVE;
#else
        fb_dev->reset_pin_active = 0;
#endif
    } else {
        fb_dev->reset_pin_active = !fb_dev->reset_pin_active;
    }

    fb_dev->power_pin = fdtdec_get_valoffset_int(blob, node, "pwr-gpio", 1, -1);
    if (fb_dev->power_pin < 0) {
#ifdef LCD_PWR_PIN
        fb_dev->power_pin = LCD_PWR_PIN;
#endif
    }
    fb_dev->power_pin_active = fdtdec_get_valoffset_int(blob, node, "pwr-gpio", 2, -1);
    if (fb_dev->power_pin_active < 0) {
#ifdef LCD_PWR_PIN_ACTIVE
        fb_dev->power_pin_active = LCD_PWR_PIN_ACTIVE;
#else
        fb_dev->power_pin_active = 0;
#endif
    } else {
        fb_dev->power_pin_active = !fb_dev->power_pin_active;
    }
...
int ak_lcd_parameter_parse_dt1(const void *blob, struct ak_fb *fb_dev)
{
	...
	fb_dev->power_pin = LCD_PWR_PIN;
#ifdef LCD_PWR_PIN_ACTIVE
    fb->dev->power_pin_active = LCD_PWR_PIN_ACTIVE;
#else
    fb->dev->power_pin_active = 0;
#endif
#endif
...
static void rgb_panel_info_cfg(struct ak_fb *fb_dev)
{
    ...
    */
    if (fb_dev->backlight_pin >= 0) {
        ...
        gpio_direction_output(fb_dev->backlight_pin, fb_dev->backlight_pin_active);
        mdelay(1);
    }
#endif
#if 0
...
/*
 * mpu_pclk_reset-
 * configure all clocks relate to mpu
 *
 * @fb_dev:         pointor to fb device
 */
static void mpu_pclk_reset(struct ak_fb *fb_dev)
{
    struct lcdc_panel_info *panel_info = fb_dev->panel_info;
    struct lcdc_rgb_info *rgb_info = fb_dev->rgb_info;
    ...
    regval = __raw_readl(AK_LCD_PCLK);
    regval |= (1 << 8 | ((panel_info->pclk_div) << 1) | 1);
    __raw_writel(regval, AK_LCD_PCLK);
}

/*
* wr_cycle_len - wr_low_len >= 2 (18/16bits mode)
* wr_cycle_len/2 - wr_low_len >= 2 (8/9bits mode)
*/
static void mpu_clk_set(struct lcdc_panel_info *panel_info,struct ak_fb *fb_dev)
{
    unsigned int regval;

    /*pclk_div setting  &  pclk enable  & inform to change the pclk */

    /*write timing setting1: wr_cycle */
    regval = __raw_readl(AK_LCD_MPU_1);
    regval &= ~(0x7f << 6);
    regval |= panel_info->wr_cycle << 6;
    __raw_writel(regval, AK_LCD_MPU_1);

    /*write timing setting2: wr_low */
    regval = __raw_readl(AK_LCD_MPU_1);
    regval &= ~(0x1f << 1);
    regval |= panel_info->wr_low << 1;
    __raw_writel(regval, AK_LCD_MPU_1);
}

/*
 * mpu_panel_set -
 * set timing and so on suit for mpu
 *
 * @panel_info:     panel infomations
 * @fb_dev:         pointor to fb device
 */
static int mpu_panel_set(struct lcdc_panel_info *panel_info,struct ak_fb *fb_dev)
{
    unsigned int regval;
    ...
    /*mpu interface buswidth */
    regval = __raw_readl(AK_LCD_MPU_1);
    regval &= ~(0x3 << 14);
    regval |= panel_info->data_width << 14;
    __raw_writel(regval, AK_LCD_MPU_1);

    /*mpu color mode */
    regval = __raw_readl(AK_LCD_MPU_1);
    regval &= ~(0x1 << 13);
    regval |= panel_info->color_mode << 13;
    __raw_writel(regval, AK_LCD_MPU_1);

    /*mpu rs's polarity select */
    regval = __raw_readl(AK_LCD_MPU_1);
    regval &= ~(0x1);
    regval |= panel_info->a0_pol;
    __raw_writel(regval, AK_LCD_MPU_1);

    /*lcd interface mode select: MPU interface */
    regval = __raw_readl(AK_LCD_TOP_CONFIG);
    regval &= ~(0x3<<5);
    regval |= (0x1<<5);
    __raw_writel(regval, AK_LCD_TOP_CONFIG);
    ...
    regval = __raw_readl(AK_PANEL_SIZE);
    regval &= ~(0xffffffff);
    regval |= (panel_info->width <<11)| (panel_info->height);
    __raw_writel(regval, AK_PANEL_SIZE);
...
/*
 * mpu_reflash_for_open_pclk -
 *
 * @fb_dev:         pointor to fb device
 */
static void mpu_reflash_for_open_pclk(struct ak_fb *fb_dev)
{
    u32 status = 0;
    u32 regval;

    /* disable the RGB, YUV1, YUV2, OSD channel */
    regval = __raw_readl(AK_LCD_TOP_CONFIG);
    regval &= (~0xf);
    __raw_writel(regval, AK_LCD_TOP_CONFIG);
    ...
    /*mpu reflash start command */
    regval = __raw_readl(AK_LCD_GO);
    regval = (1<<3)|(0<<2)|0;
    __raw_writel(regval, AK_LCD_GO);

    while(1){
        status = __raw_readl(AK_LCD_INT_STATUS);
        if((status & (1<<18)) == (1<<18))
            debug("fifo alarm!\n");
...
/*
 * mpu_set_16bit_buswidth -
 * set buswith to 16bits
 *
 * @fb_dev:         pointor to fb device
 */
static void mpu_set_16bit_buswidth(struct ak_fb *fb_dev)
{
    u32 reg_val;

    reg_val = __raw_readl(AK_LCD_MPU_1);
	mdelay(1);       //这个延时必须存在
    reg_val &= ~(0x3<<14);
    reg_val |= (MPU16BITS<<14);
	mdelay(1);       //这个延时必须存在
    __raw_writel(reg_val, AK_LCD_MPU_1);
}

/*
 * mpu_recover_buswidth -
 * recover buswith
 *
 * @fb_dev:         pointor to fb device
 */
static void mpu_recover_buswidth(struct ak_fb *fb_dev)
{
    struct lcdc_panel_info *panel_info = fb_dev->panel_info;
    u32 reg_val;
    ...
    reg_val = __raw_readl(AK_LCD_MPU_1);
    reg_val &= ~(0x3<<14);
    reg_val |= (panel_info->data_width<<14);
    __raw_writel(reg_val, AK_LCD_MPU_1);
}

/*
 * mpu_read_reg -
 * read register of mpu lcd
 *
 * @ctl:            is command or data
 * @data:           data to read
 * @count:          number of data
 * @fb_dev:         pointor to fb device
 */
static void mpu_read_reg(int ctl, unsigned int *data,unsigned count,struct ak_fb *fb_dev)
{
    unsigned int regval,i;

    /* write command*/
    regval = __raw_readl(AK_REG_CONFIGURE);
    regval &= ~(0xffffffff);
    regval = (ctl | (0<<24));
    __raw_writel(regval, AK_REG_CONFIGURE);
    udelay(50);

    for(i = 0;  i < count; i++)
    {
        /* set all signal to high */
        regval = __raw_readl(AK_MPU_READ);
        regval = ((0<<29) | (1<<27)|(1<<26)|(1<<24));
        __raw_writel(regval, AK_MPU_READ);
        udelay(100);

        /* set cs signal to low*/
        regval = __raw_readl(AK_MPU_READ);
        regval = ((0<<29) | (1<<27)|(0<<26)|(1<<24));
        __raw_writel(regval, AK_MPU_READ);
        udelay(100);
        /* set RD to LOW*/
        regval = __raw_readl(AK_MPU_READ);
        regval = ((0<<29) | (0<<27)|(0<<26)|(1<<24));
        __raw_writel(regval, AK_MPU_READ);
        udelay(100);

        /*set RD to HIGH*/
        regval = __raw_readl(AK_MPU_READ);
        regval = ((0<<29) | (1<<28)|(1<<27)|(0<<26)|(1<<24));
        __raw_writel(regval, AK_MPU_READ);
        udelay(100);

        /*Clear 28 bit and end reading data. */
        regval = __raw_readl(AK_MPU_READ);
        regval = ((0<<29) |(1<<27)|(0<<26)|(1<<24));
        __raw_writel(regval, AK_MPU_READ);
        udelay(100);
        ...
        /* read data*/
        data[i] =  __raw_readl(AK_MPU_READ) & 0xffffff;

        /* set all signal to high */
        regval = __raw_readl(AK_MPU_READ);
        regval = ((1<<29) |(1<<27)|(1<<26)|(1<<24));
        __raw_writel(regval, AK_MPU_READ);
        udelay(100);
    }
}
 ...
 * @fb_dev:         pointor to fb device
 */
static void mpu_panel_readid(struct ak_fb *fb_dev)
{
    u32 id_dat[4] = {0};
    u32 id_val = 0;

    mpu_set_16bit_buswidth(fb_dev);
    mpu_read_reg(0x04, id_dat, 4,fb_dev);
    mpu_recover_buswidth(fb_dev);

    id_dat[1] &= 0xff;
    id_dat[2] &= 0xff;
    id_dat[3] &= 0xff;

    id_val = ((id_dat[1]<<16) | (id_dat[2]<<8) | (id_dat[3]));

    printf("MPU PANEL ID %x\n", id_val);

    mpu_set_16bit_buswidth(fb_dev);
    mpu_read_reg(0x0C, id_dat, 2, fb_dev);
    printf("mpu pix format = %x\n", id_dat[1]);
    mpu_recover_buswidth(fb_dev);
}

/*
 * mpu_write_reg -
 * write register of mpu lcd
 *
 * @ctl:            is command or data
 * @data:           data to write
 * @fb_dev:         pointor to fb device
 */
static int mpu_write_reg(int ctl, unsigned int data,struct ak_fb *fb_dev)
{
    mpu_set_16bit_buswidth(fb_dev);
    if (LCD_MPU_CMD == ctl)
    {
        __raw_writel(AK_LCD_MPU_CMD | data, AK_REG_CONFIGURE);
        udelay(50);
    }
    else if (LCD_MPU_DATA == ctl)
    {
        __raw_writel(AK_LCD_MPU_DATA | data, AK_REG_CONFIGURE);
        udelay(50);
    }
...
/*
 * mpu_panel_cmd_setting -
 * send comman to mpu lcd to initialled
 *
 * @child:          pointor to device node
 * @fb_dev:         pointor to fb device
 */
static void mpu_panel_cmd_setting(struct ak_fb *fb_dev)
{
    int num_inits;
    u32 config;
    int i,j;
    ...
    for(i = 0; i < num_inits; i++) {
		config = fdtdec_get_valoffset_int(blob, node, "panel-init-list", i, 0);
        // debug("config 0x%x\n", config);

        /*0xFFF means end of data flag, and next will be a command.*/
        if (config == 0xFFF) {
            j = 0;
            continue;
        }
        ...
        if (j == 0) {
            mpu_write_reg(LCD_MPU_CMD, config,fb_dev);
            if(config == 0x11){
                mdelay(120);/* Delay 120ms*/
            }
        }else{
            mpu_write_reg(LCD_MPU_DATA, config,fb_dev);
        }
        j++;
    }
}

static void mpu_panel_init(struct ak_fb *fb_dev)
{
	struct lcdc_panel_info *panel_info = fb_dev->panel_info;

	ak_lcd_parameter_parse_dt4(fb_dev);

    mpu_pclk_reset(fb_dev);
    mpu_clk_set(panel_info, fb_dev);
    mpu_panel_set(panel_info, fb_dev);

    /* reset mpu lcd panel */
    if (fb_dev->reset_pin >= 0) {
        gpio_direction_output(fb_dev->reset_pin, !fb_dev->reset_pin_active);
        mdelay(10);/* Delay 10ms*/
        gpio_set_pin_level(fb_dev->reset_pin, fb_dev->reset_pin_active);
        mdelay(10);/* Delay 10ms*/
        gpio_set_pin_level(fb_dev->reset_pin, !fb_dev->reset_pin_active);
        mdelay(120);/* Delay 120ms*/
    }

    /*根据芯片设计的信息，需要GO刷屏后才能打开PCLK，命令波形才能正常输出*/
    mpu_reflash_for_open_pclk(fb_dev);
    /*read mpu panel ID */
    mpu_panel_readid(fb_dev);
    /*mpu panel setting */
    mpu_panel_cmd_setting(fb_dev);
}


/*
 * according lcd dtb panel-data-width parameter to reset sharepin
 */
static void mpu_panel_sharepin_cfg(struct ak_fb *fb_dev)
{
	u32 regval;

	if (fb_dev->panel_info->data_width == MPU8BITS) {
#ifdef CONFIG_37_E_CODE
        /*
         * 8bit mode: MPU_RD,MPU_A0,MPU_CS,MPU_WR,MPU_D0~MPU_D7, use GPIO34~GPIO45
         */
        regval = __raw_readl(SHARE_PIN_CFG3_REG);
		regval &= ~((0x7<<12)|(0x7<<15)|(0x7<<18)|(0x7<<21)|(0x7<<24)|(0x7<<27));
		regval |= ((0x2<<12)|(0x2<<15)|(0x2<<18)|(0x2<<21)|(0x2<<24)|(0x2<<27));
		__raw_writel(regval, SHARE_PIN_CFG3_REG);

        regval = __raw_readl(SHARE_PIN_CFG4_REG);
		regval &= ~((0x7<<0)|(0x7<<3)|(0x7<<6)|(0x7<<9)|(0x7<<12)|(0x7<<15));
		regval |= ((0x2<<0)|(0x2<<3)|(0x2<<6)|(0x2<<9)|(0x2<<12)|(0x2<<15));
		__raw_writel(regval, SHARE_PIN_CFG4_REG);
		
		/*
		* cfg driver strength
		*/
		regval = __raw_readl(PAD_DRV_CFG2_REG);
		regval &= ~((0x3<<4)|(0x3<<6)|(0x3<<8)|(0x3<<10)|(0x3<<12)|(0x3<<14)|
                (0x3<<16)|(0x3<<18)|(0x3<<20)|(0x3<<22)|(0x3<<24)|(0x3<<26));
		regval |= ((0x0<<4)|(0x0<<6)|(0x3<<8)|(0x1<<10)|(0x2<<12)|(0x2<<14)|
                (0x2<<16)|(0x2<<18)|(0x2<<20)|(0x2<<22)|(0x2<<24)|(0x2<<26));
		__raw_writel(regval, PAD_DRV_CFG2_REG);
		
		/*
		* cfg slew rate
		*/
		regval = __raw_readl(PAD_SL_CFG1_REG);
		regval &= ~((0xfff<<2));
		__raw_writel(regval, PAD_SL_CFG1_REG);
#endif
	} else if (fb_dev->panel_info->data_width == MPU9BITS) {
        /* todo */
	} else if (fb_dev->panel_info->data_width == MPU16BITS) {
        /* todo */
	} else if (fb_dev->panel_info->data_width == MPU18BITS) {
        /* todo */
    }else{
		printf("Err:no mpu sharepin parameter!\n");
		return;
	}
...
static void lcd_panel_on(struct ak_fb *fb_dev)
{
	...
	*/
	...
	if (fb_dev->power_pin >= 0) {
		debug("%s, line:%d\n", __func__, __LINE__);
		gpio_direction_output(fb_dev->power_pin, fb_dev->power_pin_active);
	}
	mdelay(1);
#endif
...
void lcd_ctrl_init(void *lcdbase)
{
	...
	gpio_direction_output(BL_GPIO_PIN, 1);
	gpio_set_pin_level(BL_GPIO_PIN, 0);
	...
	*/
...
#ifdef CONFIG_OF_CONTROL
    fb_dev->backlight_pin = fdtdec_get_valoffset_int(fb_dev->fdt_blob, node, "backlight-gpio", 1, -1);
    if (fb_dev->backlight_pin < 0) {
#ifdef CONFIG_LCD_BACKLIGHT_GPIO
        fb_dev->backlight_pin = CONFIG_LCD_BACKLIGHT_GPIO;
#else
        printf("could not get lcd device backlight pin\n");
        ret = -ENODEV;
        return;
#endif
    }
    fb_dev->backlight_pin_active = fdtdec_get_valoffset_int(fb_dev->fdt_blob, node, "backlight-gpio", 2, -1);
    if (fb_dev->backlight_pin_active < 0) {
#ifdef CONFIG_LCD_BACKLIGHT_GPIO_ACTIVE
        fb_dev->backlight_pin_active = CONFIG_LCD_BACKLIGHT_GPIO_ACTIVE;
#else
        fb_dev->backlight_pin_active = 1;
#endif
    } else {
        fb_dev->backlight_pin_active = !fb_dev->backlight_pin_active;
    }
#else
    fb_dev->backlight_pin = CONFIG_LCD_BACKLIGHT_GPIO;
#ifdef CONFIG_LCD_BACKLIGHT_GPIO_ACTIVE
    ...
    // turn off backlight and turn on power
    if (fb_dev->backlight_pin >= 0) {
        gpio_direction_output(fb_dev->backlight_pin, !fb_dev->backlight_pin_active);
    }
    mdelay(20);
	...
	mdelay(200);
	gpio_direction_output(BL_GPIO_PIN, 1);
	gpio_set_pin_level(BL_GPIO_PIN, 1);
}

...
void *RingbufVa[2] = {NULL}; /* virtual address for ring buf */
...
static void gmac_rmii_sharepin_cfg(u8 dev_num);
...
* @retval none
*/
static void gmac_ctrl_init(u8 dev_num)
{
	//unsigned long val = 0;
...
unsigned int get_eth_reset_gpio(void)
{
	...
	if (0 != ret)
	{
		printf("get_eth_reset_gpio fdt_check_header err:%d\n", ret);
		return 0;
	}

	node = fdtdec_next_compatible(s_fdt_dtb, 0, COMPAT_ANYKA_H3D_ETH);
	if(node < 0)
	{
		printf("get_eth_reset_gpio fdtdec_next_compatible err:%d\n", ret);
		return 0;
	}
	...
	ret = fdtdec_get_int_array((const void *)s_fdt_dtb, node, "reset-gpios", array, 3);
    if (ret < 0)
    {
...
unsigned int get_eth1_reset_gpio(void)
{
	unsigned int ret = 0;//, i;
	int node = -1;
	const void *s_fdt_dtb = (const void *)gd->fdt_blob;
	...
	node = fdtdec_next_compatible(s_fdt_dtb, 0, COMPAT_ANYKA_H3D_ETH1);
	if(node < 0)
	{
		printf("get_eth_reset_gpio fdtdec_next_compatible err:%d\n", ret);
		return 0;
...
static void ak_gmac_phy_software_reset(gmac_device *gmacdev)
{
	...
	gmac_read_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, PHY_CONTROL_REG, &data);
	data |= (0x1<<15);
	gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, PHY_CONTROL_REG, data);
	mdelay(25);
	do{
		status = gmac_read_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, PHY_CONTROL_REG, &value);
		if(status){
			continue;
		}
		
		if((value & Mii_reset) == 0){ 
			printf("phy software reset OK\n"); 
			break;
		}
		mdelay(5);
		tcnt++;
	...
	if (tcnt == 200){
		printf("phy sofware reset timeout!\n"); 
	}
}
...
/**
  ...
  * @retval none
  */
static void gmac_phy_gpio_reset(u8 dev_num, gmac_device *gmacdev)
{
	//unsigned long val = 0;
	unsigned int  gpio = 0;
	
	debug("GPIO Reset MAC phy!\n");
#if 0
#ifdef CONFIG_H3_D_CODE	
#ifdef CONFIG_RGB_CORE_BOARD 
	/*
	* use gpio23 share as gpio for reset phy
	*/
	gpio = CONFIG_RGB_RMII_RST;
#endif
	
#ifdef CONFIG_MIPI_CORE_BOARD 
	/*
	* use gpio80 share as gpio for reset phy
	*/
	gpio = CONFIG_MIPI_RMII_RST;
#endif	
#endif

#ifdef CONFIG_H3_B_CODE
	/*
	* use gpio81 share as gpio for reset phy
	*/
	gpio = CONFIG_RMII_RST;
#endif
#endif

	if (dev_num == 0){
#ifdef CONFIG_OF_CONTROL
		gpio = get_eth_reset_gpio();
#else
		gpio = NET_RESET_GPIO;
#endif	
		/*
		...
		*/
		if(gpio == 0){
			printf("%s, line:%d, Err, Not find phy reset-gpios,gpio_index:%d \n", __func__,__LINE__, gpio);
			ak_gmac_phy_software_reset(gmacdev);
		}else{
			gpio_set_pin_as_gpio (gpio);
			gpio_set_pin_dir(gpio, 1);
			gpio_direction_output(gpio, 1);
			mdelay(5);
			gpio_set_pin_level(gpio, 0);
			mdelay(25);
			gpio_set_pin_level(gpio, 1);
			mdelay(5);
		}
	}
...
/**
  ...
  * @retval error number on failed
  */
static s32 gmac_phy_init(u8 dev_num, gmac_device *gmacdev)
{
	s32 status;
	u16 data_ctl;
	u16 data_id1;
	...
	/* reset */
	gmac_phy_gpio_reset(dev_num, gmacdev);
	...
	if(data_id1 == 0x22) {
        ...
        debug("read reg 0x50 : 0x%x\n",data_ctl);
        data_ctl |= 0x0040;
        gmac_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase, 0x001e, 0x50);
        gmac_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase, 0x001f, data_ctl);
        debug("write reg 0x50 : 0x%x\n",data_ctl);
        gmac_write_phy_reg((u32 *)gmacdev->MacBase,gmacdev->PhyBase, 0x001e,0x00);
	} else if (data_id1 == 0x1c && data_id2 == 0xc829) { //rtl8201g
        debug("Realtek rtl8201g special init!\n");

        //set CRS/CRS_DV pin is RXDV signal
        data_ctl = RTL8201G_P0xD05;  //select page 0xd05 (RTL8201G_P0xD05)
        gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, RTL8201_PAGE_SELECT, data_ctl);
        data_ctl = 0x00;
        gmac_read_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, RTL8201G_P0xD05_R16, &data_ctl);
        data_ctl &= 0x10;
        //debug("set RTL8201G_P0xD05_R16: 0x%04x\n", data_ctl);
        gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, RTL8201G_P0xD05_R16, data_ctl);

        // set RMII Mode
        data_ctl = RTL8201G_P0xD08;  //select page 0xD08 (RTL8201G_P0xD08)
        gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, RTL8201_PAGE_SELECT, data_ctl);
        data_ctl = 0x20;    //Enable RMII
        //debug("set RTL8201G_P0xD08_R17: 0x%04x\n", data_ctl);
        gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, RTL8201G_P0xD08_R17, data_ctl);

        //set page0
        gmac_write_phy_reg((u32 *)gmacdev->MacBase, gmacdev->PhyBase, RTL8201_PAGE_SELECT, 0x00);
    } else {
...
/**
  ...
  * @retval none
  */
static u32 gmac_init_rx_desc_base(gmac_device *gmacdev)
{
	u32 rx_desc_addr;
...
#if 0
static void gmac_broadcast_disable(gmac_device *gmacdev)
{
...
static s32 gmac_mac_init(gmac_device *gmacdev)
{
    ...
    if(gmacdev->DuplexMode == FULLDUPLEX)
    {
        ...
        //gmac_broadcast_disable(gmacdev);
...
static int smc_init(struct eth_device *dev, bd_t *bd)
{
	...
	gmac_rmii_sharepin_cfg((u8)dev->index);

  	gmac_ctrl_init((u8)dev->index);
	...
	if (-1 == gmac_phy_init((u8)dev->index, gmacdev))
	{
		return -1;
	}
...
static void smc_halt(struct eth_device *dev)
{
    ...
    if (gmacdev->tx_ring[0]) {
        free(gmacdev->tx_ring[0]);
        memset(gmacdev->tx_ring, 0, TRANSMIT_DESC_SIZE * sizeof(gmacdev->tx_ring[0]));
    }
    if (gmacdev->rx_ring[0]) {
        free(gmacdev->rx_ring[0]);
        memset(gmacdev->rx_ring, 0, RECEIVE_DESC_SIZE * sizeof(gmacdev->rx_ring[0]));
    }
...
static void gmac_rmii_sharepin_cfg(u8 dev_num)
{
	...
	* h3d sharepin0(0x0800,0178), set sharepin gpio49 share as opclk
	* bit[19:18] = 01, RMII_MDC
	* bit[21:20] = 01, RMII_MDIO
	* bit[23:22] = 01, RMII_TXEN
	* bit[25:24] = 01, RMII_TXD0
	* bit[27:26] = 01, RMII_TXD1
	* bit[29:28] = 01, RMII_RXD0
	* bit[31:30] = 01, RMII_RXD1
	* h3d sharepin1(0x0800,017c)
	* bit[1:0] = 01, RMII_RXER
	* bit[3:2] = 01, RMII_RXDV
	* bit[5:4] = 01, RMII_50M
	*/
	val = REG32(SHARE_PIN_CFG0_REG);
    val &= ~((0x3<<18)|(0x3<<20)|(0x3<<22)|(0x3<<24)|(0x3<<26)|(0x3<<28)|(0x3<<30));
    val |= ((0x1<<18)|(0x1<<20)|(0x1<<22)|(0x1<<24)|(0x1<<26)|(0x1<<28)|(0x1<<30));
    REG32(SHARE_PIN_CFG0_REG) = val;

	/* add phy reset gpio */
#ifdef CONFIG_RGB_CORE_BOARD 
	printf("rmii_rgb_board!\n");
	...
	/* TODO: 
	* Need to be changed to new clock, to enable the 25MHz oscillator 
	* h3b sharepin0(0x0800,0074), set sharepin gpio47 share as opclk
	* bit[11:10] = 01, RMII_50M
	*/
	val = REG32(SHARE_PIN_CFG0_REG);
    val &= ~(0x3<<10);
    val |= (0x1<<10);
	...
	/* h3b sharepin1(0x0800,007c)
	* bit[1:0] = 01, RMII_MDC
	* bit[3:2] = 01, RMII_MDIO
	* bit[6:5] = 01, RMII_TXEN
	* bit[9:8] = 01, RMII_TXD0
	* bit[11:10] = 01, RMII_TXD1
	* bit[17:16] = 01, RMII_RXD0
	* bit[19:18] = 01, RMII_RXD1
	* bit[23:22] = 01, RMII_RXER
	* bit[25:24] = 01, RMII_RXDV
	*/
	val = REG32(SHARE_PIN_CFG2_REG);
	val &= ~((0x3<<0)|(0x3<<2)|(0x3<<5)|(0x3<<8)|(0x3<<10)|(0x3<<16)|(0x3<<18)|(0x3<<22)|(0x3<<24));
	val |= ((0x1<<0)|(0x1<<2)|(0x1<<5)|(0x1<<8)|(0x1<<10)|(0x1<<16)|(0x1<<18)|(0x1<<22)|(0x1<<24));
	REG32(SHARE_PIN_CFG2_REG) = val;
#endif
	...
	if (dev_num == 0){
		//eth0
		val = REG32(SHARE_PIN_CFG0_REG);
	    val &= ~((0x7<<0)|(0x7<<3)|(0x7<<6)|(0x7<<9)|(0x7<<12)|(0x7<<15)|(0x7<<18)|(0x7<<21)|(0x7<<24)|(0x7<<27));
	    val |= ((0x1<<0)|(0x1<<3)|(0x1<<6)|(0x1<<9)|(0x1<<12)|(0x1<<15)|(0x1<<18)|(0x1<<21)|(0x1<<24)|(0x1<<27));
	    REG32(SHARE_PIN_CFG0_REG) = val;
	...
	if (dev_num == 1){
		//eth1
		val = REG32(SHARE_PIN_CFG5_REG);
	    val &= ~((0x7<<24)|(0x7<<27));
	    val |= ((0x2<<24)|(0x2<<27));
	    REG32(SHARE_PIN_CFG5_REG) = val;

		val = REG32(SHARE_PIN_CFG6_REG);
	    val &= ~((0x7<<0)|(0x7<<3)|(0x7<<6)|(0x7<<9)|(0x7<<12)|(0x7<<15)|(0x7<<18)|(0x7<<21));
	    val |= ((0x2<<0)|(0x2<<3)|(0x2<<6)|(0x2<<9)|(0x2<<12)|(0x2<<15)|(0x2<<18)|(0x2<<21));
	    REG32(SHARE_PIN_CFG6_REG) = val;

		/*set rmii1 drv level 3 */
		val = __raw_readl(PAD_DRV_CFG3_REG);
		val &= ~((0x3<<20)|(0x3<<22)|(0x3<<24)|(0x3<<26)|(0x3<<28)|(0x3<<30));
		val |=	((0x3<<20)|(0x3<<22)|(0x3<<24)|(0x3<<26)|(0x3<<28)|(0x3<<30));
		...
		val |=	((0x1<<26)|(0x1<<27)|(0x1<<28)|(0x1<<29)|(0x1<<30)|(0x1<<31));
	...
	//printf("REG32(0x%x):0x%x\n", SHARE_PIN_CFG0_REG, REG32(SHARE_PIN_CFG0_REG));
	//printf("REG32(0x%x):0x%x\n", SHARE_PIN_CFG3_REG, REG32(SHARE_PIN_CFG3_REG));
...
/**
  ...
  * @return void.
  */
static void ak39e_gpio_disable_pupd(u8 dev_num, T_GPIO_PD_CONFG gpio_pddis_en)
{
	
	/* set chip system ctrl reg base address, 1:disable, 0:enable */ 
	if (gpio_pddis_en){
#ifdef CONFIG_H3_D_CODE
		REG32(AK_PA_SYSCTRL + 0x194) |= (0x1<<18); // cdh:disable gpio49 pull down
#endif

#ifdef CONFIG_H3_B_CODE
		REG32(PU_PD_ENABLE_CFG0_REG) |= (0x1<<8); // cdh:disable gpio47 pull down
#endif

#ifdef CONFIG_37_E_CODE
	if (dev_num == 0){
		//eth0
		REG32(PU_PD_ENABLE_CFG0_REG) &= ~(0x1<<6); // cdh:disable gpio6 pull down
	}
	
	if (dev_num == 1){
		//eth1
		REG32(PU_PD_ENABLE_CFG2_REG) &= ~(0x1<<0); // cdh:disable gpio6 pull down
	}
	
	//printf("REG32(0x%x):0x%x\n", PU_PD_ENABLE_CFG0_REG, REG32(PU_PD_ENABLE_CFG0_REG));
#endif


	}else {
#ifdef CONFIG_H3_D_CODE
		REG32(AK_PA_SYSCTRL + 0x194) &= ~(0x1<<18); // cdh:enable gpio49 pull down
#endif
	
#ifdef CONFIG_H3_B_CODE
		REG32(PU_PD_ENABLE_CFG0_REG) &= ~(0x1<<8); // cdh:enable gpio47 pull down
#endif

#ifdef CONFIG_37_E_CODE
	if (dev_num == 0){
		//eth0
		REG32(PU_PD_ENABLE_CFG0_REG) |= (0x1<<6); // cdh:enable gpio6 pull down
	}

	if (dev_num == 1){
		//eth1
		REG32(PU_PD_ENABLE_CFG2_REG) |= (0x1<<0); // cdh:enable gpio6 pull down
	}
...
static void gmac_rmii_interface_50mclk(u8 dev_num)
{
	...
	if (dev_num == 0){
		...
		REG32(AK_PA_SYSCTRL + 0x14) |=	(0x1 << 28); // cdh:mac feedback en

		REG32(AK_PA_SYSCTRL + 0x18) |=  (0x9 << 0);

		REG32(AK_PA_SYSCTRL + 0x18) |=  (0x1 << 8); // cdh:bit[8],enable generate 50m
		REG32(AK_PA_SYSCTRL + 0x18) |=  (0x1 << 9); // cdh:bit[8],enable generate 50m

		while(REG32(AK_PA_SYSCTRL + 0x18)&(0x1<<9));

		REG32(AK_PA_SYSCTRL + 0x1c) &= ~(1 << 13);		 // cdh:mac ctronller clk 
		//printf("REG32(0x%x):0x%x\n", AK_PA_SYSCTRL + 0x14, REG32(AK_PA_SYSCTRL + 0x14));
		//printf("REG32(0x%x):0x%x\n", AK_PA_SYSCTRL + 0x18, REG32(AK_PA_SYSCTRL + 0x18));
		//printf("REG32(0x%x):0x%x\n", AK_PA_SYSCTRL + 0x1c, REG32(AK_PA_SYSCTRL + 0x1c));
	}

	if (dev_num == 1){
		//eth1
		/* Rmii, select top rmii interface and 100mbps speed, and make 50M clock to phy */
		REG32(AK_PA_SYSCTRL + 0x14) &=	~(0x1 << 20); // cdh:first mac interface select Rmii
		REG32(AK_PA_SYSCTRL + 0x14) |=	(0x1 << 21); // cdh:first  mac_speed_cfg=1(100m)

		REG32(AK_PA_SYSCTRL + 0x18) |=  (0x9 << 20);

		REG32(AK_PA_SYSCTRL + 0x18) |=  (0x1 << 28); // cdh:bit[8],enable generate 50m
		REG32(AK_PA_SYSCTRL + 0x18) |=  (0x1 << 29); // cdh:bit[8],enable generate 50m

		while(REG32(AK_PA_SYSCTRL + 0x18)&(0x1<<29));

		REG32(AK_PA_SYSCTRL + 0x1c) &= ~(1 << 16);		 // cdh:mac ctronller clk 
		//printf("REG32(0x%x):0x%x\n", AK_PA_SYSCTRL + 0x14, REG32(AK_PA_SYSCTRL + 0x14));
		//printf("REG32(0x%x):0x%x\n", AK_PA_SYSCTRL + 0x18, REG32(AK_PA_SYSCTRL + 0x18));
		//printf("REG32(0x%x):0x%x\n", AK_PA_SYSCTRL + 0x1c, REG32(AK_PA_SYSCTRL + 0x1c));
	}
...
#else
	/* 
	* peri pll, 0x0800, 0014
	* bit[28] =1, mac_feedback_clk_en, in rmii mode, internel pll must set 1
	* bit[23] =1, mac_speed_cfg, 1 = to choose 100MHz
	* bit[22] =1, mac_interface_sel_cfg, 0 = to choose rmii interface
	* bit[21] =1, to enable 1/12 divider to generate 50MHz clock divider
	* bit[18] =1, mac_clk_sel, 1 = to choose 50MHz from peri pll
	* bit[13] =1, mac controller clk , 
	*/
	...
	REG32(AK_PA_SYSCTRL + 0x14) |=  ((0x1 << 28)|(0x1 << 18)); // cdh:set   bit[21],enable generate 50m,	bit[18], select 25m clock of mac from pll div
	REG32(AK_PA_SYSCTRL + 0x1c) &= ~(1 << 13);		 // cdh:mac ctronller clk 
#endif
...
/**
  ...
  * @return void none
  */
static void gmac_clk_sharepin_init(u8 dev_num)
{
#if defined(CONFIG_ETHERNET_MAC_RMII)
	debug("Configed MAC RMII interface!\n");
	
	/* set rmii interface and 50m phy clock sharepin */ 
	gmac_rmii_sharepin_cfg(dev_num);

	/* enable gpio49 pull up */ 
	ak39e_gpio_disable_pupd(dev_num, AK_PULLDOWN_ENABLE);
	
	/* open 50M phy clock */ 
	gmac_rmii_interface_50mclk(dev_num);
#else
...
int ak39_ethernet_initialize(u8 dev_num, int base_addr)
{
...
#ifndef AK_DOUBLE_ETH_SUPPORT
	if (dev_num > 0) {
		printf("only support one eth_device!\n");
		return 1;
	}
#else
	if (dev_num == 1){
		dev_addr[MAC_ADDR_LEN-1] += 0x1;
	}
#endif
	...
	*/
	...
	RingbufVa[dev_num] = malloc(sizeof (gmac_device));
	adapter_pt->gmacdev_pt = (gmac_device *)RingbufVa[dev_num];
	...
	gmac_clk_sharepin_init(dev_num);

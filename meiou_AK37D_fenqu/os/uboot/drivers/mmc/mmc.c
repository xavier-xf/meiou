...
extern enum ak_mmc_cd_mode g_mmc_plug_mod;
extern unsigned int g_mmc_plug_gpio_index;
...
static int sd_send_op_cond(struct mmc *mmc)
{
	...
	debug("cdh:%s, mmc->high_capacity=%d\n", __func__, mmc->high_capacity);
	mmc->rca = 0;
...
int mmc_getcd(struct mmc *mmc)
{
	...
	if (cd < 0)
	{
		debug("cdh:use ak mmc cd!\n");
		if (mmc->getcd)
		{
			cd = mmc->getcd(mmc);
			debug("cdh:ak getcd=%d\n", cd);
		}
		else
			cd = 1;
	}
...
retry_scr:
	...
	debug("cdh:mmc->scr[0]=0x%x, mmc->scr[1]=0x%x\n", mmc->scr[0], mmc->scr[1]);
	...
	debug("cdh:mmc->version=0x%x\n", mmc->version);

	if (mmc->scr[0] & SD_DATA_4BIT)
	{
		mmc->card_caps |= MMC_MODE_4BIT;
		debug("cdh:SD_DATA_4BIT, mmc->card_caps=0x%x\n", mmc->card_caps);
	}
	...
	while (timeout--)
	{
		...
		{
			debug("cdh:SD_SWITCH_CHECK ready\n");
			break;
		}
	}
	...
	{
		debug("cdh:SD_SWITCH_CHECK not support high-speed!\n");
		return 0;
	}
	...
	{
		debug("cdh:MMC Controller mmc->host_caps:0x%x, not support high-speed!\n", mmc->host_caps);
		return 0;
	}
	else
	{
		debug("cdh:MMC Controller mmc->host_caps:0x%x, yes support high-speed!\n", mmc->host_caps);
	}
	...
	{
		mmc->card_caps |= MMC_MODE_HS;
		debug("cdh:mmc->card_caps:0x%x, MODE HS OK!\n", mmc->card_caps);
	}
	else
	{
...
static int mmc_startup(struct mmc *mmc)
{
	...
	{
		debug("cdh:MMC_CMD_ALL_SEND_CID OK!\n");
	}
	...
	debug("cdh:mmc cid[0]=%d\n", mmc->cid[0]);
	debug("cdh:mmc cid[1]=%d\n", mmc->cid[1]);
	debug("cdh:mmc cid[2]=%d\n", mmc->cid[2]);
	debug("cdh:mmc cid[3]=%d\n", mmc->cid[3]);
	/*
	...
	debug("cmd csdlong response[0]:0x%x!\n", mmc->csd[0]);
	debug("cmd csdlong response[1]:0x%x!\n", mmc->csd[1]);
	debug("cmd csdlong response[2]:0x%x!\n", mmc->csd[2]);
	debug("cmd csdlong response[3]:0x%x!\n", mmc->csd[3]);
	...
	if (IS_SD(mmc))
	{
		...
		debug("cdh:sd card, mmc->write_bl_len:%d!\n", mmc->write_bl_len);
	}
	else
	...
	if (mmc->high_capacity)
	{
		debug("cdh:yes mmc->high_capacity!\n");
#if 0
	...
	debug("cdh:sd card, mmc->write_bl_len2:%d!\n", mmc->write_bl_len);
	...
	{ /* cmd not supported in spi */
		...
		{
			debug("cdh:MMC_CMD_SELECT_CARD OK!\n");
		}
	}
	...
	{
		debug("cdh:MMC_CMD_SET_BLOCKLEN OK!\n");
	}
	...
	{
		debug("cdh:part_num:%d, mmc_set_capacity OK!\n", mmc->part_num);
	}
	...
	if (IS_SD(mmc))
	{
		if (mmc->card_caps & MMC_MODE_4BIT)
		{
			...
			mmc_set_bus_width(mmc, 4); // cdh:�л���4�ߴ���
		}
	...
	else
	{
		...
		for (idx = 0; idx < ARRAY_SIZE(ext_csd_bits); idx++)
		{
			...
			if (!err && ext_csd[EXT_CSD_PARTITIONING_SUPPORT] == test_csd[EXT_CSD_PARTITIONING_SUPPORT] && ext_csd[EXT_CSD_ERASE_GROUP_DEF] == test_csd[EXT_CSD_ERASE_GROUP_DEF] && ext_csd[EXT_CSD_REV] == test_csd[EXT_CSD_REV] && ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] == test_csd[EXT_CSD_HC_ERASE_GRP_SIZE] && memcmp(&ext_csd[EXT_CSD_SEC_CNT], &test_csd[EXT_CSD_SEC_CNT], 4) == 0)
			{
...
#if 0
	mmc_set_clock(mmc, mmc->tran_speed); // cdh:����������MCLK
#else
	mmc->tran_speed = 25000000;
	mmc_set_clock(mmc, mmc->tran_speed); // cdh:����������MCLK
#endif
...
int mmc_initialize(bd_t *bis)
{
	...
	if (i == 3)
	{
		printf("DTB no mmc info\n");
		return 0;
	}

	if ((mmc_cd_if.mmc_plug_mod == CD_CLK_MODE) || (mmc_cd_if.mmc_plug_mod == NON_REMOVABLE_MODE))
	{
		printf("sd detect %s mode!\n", (mmc_cd_if.mmc_plug_mod == CD_CLK_MODE) ? "cd_clk" : "non_remove");
		if (ak_sdhsmmc_init(i, 0, 0, TF_DET_MCK_MODE, -1) < 0)
		{
			return -1;
		}
	}
	...
	{
		printf("sd detect gpio mode:%d!\n", mmc_cd_if.mmc_plug_gpio_index);
		if (ak_sdhsmmc_init(i, 0, 0, mmc_cd_if.mmc_plug_gpio_index, -1) < 0)
		{
			return -1;
		}
	}

...
extern void spi_sharepin_cfg(struct spi_slave *spi);
...
* @retval none
*/
static void spi_flash_addr(u32 addr, u8 *cmd, u32 addr_byte)
{
...
/**
* @brief spi flash winbond write status
...
*/
int spi_flash_winbond_cmd_write_status_volatile(struct spi_flash *flash, u32 sr)
{
	u8 cmd;
	u8 sr_tmp;
	int ret;
	...
	sr_tmp = sr & 0xff;
	ret = spi_flash_write_common_volatile(flash, &cmd, 1, &sr_tmp, 1);
	if (ret < 0) {
    ...
    if (flash->flags & SFLAG_COM_STATUS3) {
		...
		ret = spi_flash_write_common_volatile(flash, &cmd, 1, &sr_tmp, 1);
		if (ret < 0) {
			debug("SF: fail to write status3 register\n");
			return ret;
		}
    }
...
/**
* @brief spi flash write status for xmc
*
* spi flash write status
* @param[in] *flash spi_flash handle
* @param[out] sr  will to write status value
...
* @retval else other on failed
*/
int spi_flash_xmc_cmd_write_status(struct spi_flash *flash, u32 sr)
{
	u8 cmd;
	int wr_cnt;
	int ret;

    /* do not write status register3 */
...
/**
* @brief spi flash read status for xmc
*
* spi flash write status
* @param[in]  *flash spi_flash handle
* @param[out] *sr  will to read status buffer pointer
...
* @retval else other on failed
*/
int spi_flash_xmc_cmd_read_status(struct spi_flash *flash, u32 *sr)
{
	u8 cmd;
	u8 sr_tmp;
	int ret;
...
int spi_flash_quad_mode_enable(struct spi_flash *flash)
{
	...
	ret = fsr->read_sr(flash, &regval);
    if (regval & (1 << fsr->b_qe))
        return 0;

    regval |= (1<<fsr->b_qe);
    return fsr->write_sr(flash, regval);
}

/**
* @brief clean XMC EBL bit
* @param[in] *flash spi_flash handle
* @return int  0 on success, else other on failed
* @retval 0 on success
* @retval else other on failed
*/
int spi_flash_xmc_clean_ebl_bit(struct spi_flash *flash)
{
	int ret;
	u32 regval;
	struct flash_status_reg *fsr = &flash->stat_reg;
...
int spi_flash_write_common_volatile(struct spi_flash *flash, const u8 *cmd,
		size_t cmd_len, const void *buf, size_t buf_len)
{
	struct spi_slave *spi = flash->spi;
	unsigned long timeout = SPI_FLASH_PROG_TIMEOUT;
    ...
    ret = spi_flash_cmd(flash->spi, CMD_VOLATILE_SR_WRITE_ENABLE, NULL, 0);
    if (ret) {
		debug("SF: enabling write volatile failed\n");
        return ret;
    }
...
int spi_flash_cmd_erase_ops(struct spi_flash *flash, u32 offset, size_t len)
{
	...
	addr_size = flash->flags & SFLAG_ADDR_4B ? 4 : 3;
	if (len >= 8192)
		scale = (len) / 4096;
	...
	while (len) {
		...
		spi_flash_addr(offset, cmd, addr_size);

		debug("617 SF: erase %2x %2x %2x %2x (%x)\n", cmd[0], cmd[1],
		      cmd[2], cmd[3], offset);

		ret = spi_flash_write_common(flash, cmd, (addr_size + 1), NULL, 0);
		if (ret < 0) {
		...
		size_t len, const void *buf)
{
	...
	for (actual = 0; actual < len; actual += chunk_len) {
		...
		ret = spi_flash_write_common(flash, cmd, (addr_size + 1),
		...
		size_t len, void *data)
{
	...
	while (len) {
		...
		ret = spi_flash_read_common(flash, cmd, (addr_size+1),
							data, read_len);
		if (ret < 0) {
		...
		size_t len, const void *buf)
{
	...
	for (actual=0; actual<len; actual+=chunk_len) {
		...
		ret = spi_flash_write_extra(flash, cmd, (addr_size + 1),
					buf + actual, chunk_len);
		if (ret < 0) {
			debug("SF: write failed\n");
			break;

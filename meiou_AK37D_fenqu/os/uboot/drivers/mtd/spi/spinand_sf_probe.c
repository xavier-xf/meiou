...
static struct spinand_flash_info ak_spinand_param_supportlist [] = {
	...
	{ "EM73C044SNB_G", 0x11d511d5, 0, 1, 2048, 64, 128*1024, 1024, 120, 0, 12, 18, 4, 4, SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_QUAD_WRITE,},
};
...
int spinand_flash_write_inter(struct spinand_flash *flash, u32 offset, size_t len, const void *data,  const void *oob, size_t oob_len)
{
	...
	u32 total_page = flash->page_size + flash->oob_size;
	u8 *oob_buf = malloc(flash->oob_size);
	if(oob_buf == NULL)
	{
		printf("error:oob_buf malloc fail\r\n");
		return -1;
	}
	u8 *buf = malloc(total_page);
	if(buf == NULL)
	{
		free(oob_buf);
		printf("error:buf malloc fail\r\n");
		return -1;
	}
	...
	{
		//read page and oob
		if(spi_nand_read(flash, page, flash->page_size, oob_buf, flash->oob_size) == -1)
		{
			free(buf);
			free(oob_buf);
...
int spinand_flash_read_inter(struct spinand_flash *flash, u32 offset, size_t len, void *data,  void *oob, size_t oob_len)
{
	...
	u32 total_page = flash->oob_size + flash->page_size;
	u8 *buf = malloc(total_page);
	if(buf == NULL)
	{
		printf("error:buf malloc fail\r\n");
...
int spinand_flash_read_oob(struct spinand_flash *flash, u32 offset, void *oob, size_t oob_len)
{
	...
	if(spi_nand_read(flash, page, colnum, buf, flash->oob_size) == -1)
	{
		free(buf);
		return -1;
	}

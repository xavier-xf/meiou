...
{
    ak_print_normal(MODULE_ID_APP," %s -a [timeout] -b [feed_time] -c [feed_circle]\n", name);
    ak_print_normal(MODULE_ID_APP,"eg: %s -a 10 -b 9 -c 3\n", name);
}
...
#ifdef AK_RTOS
int drv_wdt_sample(int argc, char **argv)
#else
...
#endif
{
    ...
    */
    ...
    ak_print_normal(MODULE_ID_APP, "wdt test start.\n");

    /*
    *check timeout
    */
    if ((timeout < 1) || (timeout > 357)) {
        ak_print_normal(MODULE_ID_APP, "timeout error,use default value.\n");
    ...
    */
    if (feed_time <= 0) {
        ak_print_normal(MODULE_ID_APP, "feed_time error,use default value.\n");
    ...
    */
    if (feed_circle <= 0) {
        ak_print_normal(MODULE_ID_APP, "feed_circle error,use default value.\n");
        feed_circle = 5;
    }
    ...
    */
    for(int i = 0; i < feed_circle; i++){
        ak_print_normal(MODULE_ID_APP, "feed dog\n");
        ak_drv_wdt_feed();
        sleep(feed_time);
    }
    /* 
    ...
    */
    ...
    ak_sdk_exit();
    ak_print_normal(MODULE_ID_APP, "wdt test end.\n");    
    
    return 0;
}

#ifdef AK_RTOS
MSH_CMD_EXPORT_ALIAS(drv_wdt_sample, ak_drv_wdt_sample, driver watchdog sample)
#endif

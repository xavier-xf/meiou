...
#ifdef AK_RTOS
int drv_ts_sample(int argc, char **argv)
#else
...
#endif
{
    ...
    */
    ...
    ak_print_normal(MODULE_ID_APP, "ts test start.\n");
    ...
    */    
    ...
    if(ret)
    {
        ak_print_error_ex(MODULE_ID_APP, "ak_drv_ts_open fail.\n");
        return -1;
    }
    ...
    do 
    {
        ak_print_info(MODULE_ID_APP, "ak_drv_ts_get_event timeout 1000 ms.\n"); 
        ...
        */
        if(ret == 0) 
        {
            for (int i=0; i<TS_POINT_MAX_NUM; i++) 
            {
                ...
                */
                if (ts.map & (1 << i))
                    ak_print_notice(MODULE_ID_APP, "id %d, get ts:[%d , %d].\n", i, ts.info[i].x, ts.info[i].y); 
            }
        }
    ...
    */    
    ...
    ak_sdk_exit();
    ak_print_normal(MODULE_ID_APP, "ts test end.\n");    
    
    return 0;
}

#ifdef AK_RTOS
MSH_CMD_EXPORT_ALIAS(drv_ts_sample, ak_drv_ts_sample, driver touchscreen sample);
#endif

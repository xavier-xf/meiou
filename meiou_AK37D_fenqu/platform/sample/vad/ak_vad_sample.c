...
static void ai_capture_loop(int ai_handle_id, const char *path, int save_time)
{
    ...
    int ret = AK_FAILED;
    struct ak_audio_vad_status vad_status ;
    ...
    while (1)
    {
        ...
        {
            ak_ai_get_vad_status(ai_handle_id, &vad_status);
            if(vad_status.ts != trigger_ts)
            {
                ak_print_normal_ex(MODULE_ID_APP, "Voice active detected ts : %llu \n\n",vad_status.ts);
...
static int help_hint( char *pc_prog_name )
{
    ...
    printf( "\nexample: \r\n"
        "  %s -t 60 -s 8000 -c 1 -v 5 -d 0 -m 100 -p /mnt/\n", pc_prog_name);
    printf("\n\n");
    return 0;
}
...
#endif
{
    ...
    if( dev_sel != DEV_ADC ){
        ak_print_error(MODULE_ID_APP, "*** vad only support ADC ! ***\n");
        ak_sdk_exit();
        return 0;
    }

    if((sample_rate!=8000) && (sample_rate!=16000)){
        ak_print_error(MODULE_ID_APP, "*** vad only support 8K and 16K ! ***\n");
        ak_sdk_exit();
        return 0;
    }
    ...
    if(DEV_ADC == dev_sel)
    {
        ...
        vad_attr.enable = 1;				 //开启
        vad_attr.vad_thresh = AK32Q15(0.05);   //阈值0.05为较低阈值
        vad_attr.vad_level = 0;				 //0为最灵敏
        vad_attr.vad_type = AK_VAD_DET_AFTER_ADC;
        ak_ai_set_vad_attr(ai_handle_id, &vad_attr);

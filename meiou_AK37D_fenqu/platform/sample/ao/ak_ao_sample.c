...
#endif
{
    ...
    ak_print_normal(MODULE_ID_APP, "*****************************************\n");
	ak_print_normal(MODULE_ID_APP, "** ao demo version: %s **\n", ak_ao_get_version());
    ak_print_normal(MODULE_ID_APP, "*****************************************\n");
	...
	if( DEV_DAC == dev_sel ){
		if(( channel_num != 1 ) && ( channel_num != 2 )){
			printf("the channel num of dac must use 1 or 2\n");
			return -1;
		}
	}else if( DEV_I2S0_AO == dev_sel){
		if( channel_num != 2 ){
			printf("the channel num of i2s0 must use 2\n");
			return -1;
		}
	}else{
		printf("device id must use 0-dac or 1-i2s0\n");
		return -1;
	}
    ...
    if(ak_ao_open(&ao_param, &ao_handle_id)) {
        ret = -1;
		if(NULL != fp) {
			printf("--- ao open false, close file ---\n");
			fclose(fp);
			fp = NULL;
		}
        goto main_end;

...
static int read_yuv_file(char *file_name, char * data, int data_size)
{
	int read_len = 0;
...
static void *video_encode_from_sdcard_th(void *arg)
{
    ...
    char *data = (unsigned char *)ak_mem_dma_alloc(MODULE_ID_APP, data_len);
    if(data == NULL)
    {
    ...
    memset(venc_th->save_file, 0x00, sizeof(venc_th->save_file));
    if (H264_ENC_TYPE == venc_th->en_type)
        sprintf(venc_th->save_file, "%s/h264_br%d_%dkbps_chn%d_%dx%d.str", save_path, br_mode,target_kbps,chn_index,res.width, res.height);
    else
        sprintf(venc_th->save_file, "%s/h265_br%d_%dkbps_chn%d_%dx%d.str", save_path, br_mode,target_kbps,chn_index, res.width, res.height);

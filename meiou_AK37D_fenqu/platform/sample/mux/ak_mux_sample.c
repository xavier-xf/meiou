...
static int create_mux_file( struct dvr_conf *p_dvr_conf )                      //创建和开始一个mux文件
{
    ...
    switch( p_dvr_conf->mux_input_param.media_rec_type ) {                      //拼接录像暂存文件名称
        case MUX_TYPE_MP4 :
            snprintf( p_dvr_conf->ac_file_video, sizeof(p_dvr_conf->ac_file_video), "%s/%04d%02d%02d%02d%02d%02d%s", p_dvr_conf->ac_path_tmp,
                      p_dvr_conf->dvr_date_param.year,
                      p_dvr_conf->dvr_date_param.mon,
                      p_dvr_conf->dvr_date_param.day,
                      p_dvr_conf->dvr_date_param.hour,
                      p_dvr_conf->dvr_date_param.min,
                      p_dvr_conf->dvr_date_param.sec,
                      FILE_EXT_MP4 );
            break;
        case MUX_TYPE_AVI :
            snprintf( p_dvr_conf->ac_file_video, sizeof(p_dvr_conf->ac_file_video), "%s/%04d%02d%02d%02d%02d%02d%s", p_dvr_conf->ac_path_tmp,
            ...
            break;
        default :
            ak_print_error_ex(MODULE_ID_APP, "p_dvr_conf->mux_input_param.media_rec_type= %d\n", p_dvr_conf->mux_input_param.media_rec_type );
            return ret;
    }
...
#if 1                                                                           //在该录像结束前,合成下一个视频首个I帧时间戳之前的全部音频帧
                        i_aenc_times = 0;
                        for( ;; ) {
...
thread_mux_savefile :
                    ...
                    if( ( gi_record_num > 0 ) && ( i_num_file >= gi_record_num ) ) {      //合成文件个数达到指定数量则退出
                        ...
                        if( create_mux_file( p_dvr_conf ) == AK_SUCCESS ) {     //重新创建录像文件
                            i_mux_init = AK_TRUE;
                        }
                    }
                }
...
static void save_fix_file( struct dvr_conf *p_dvr_conf, char *pc_file, int i_ms, int i_type )         //保存修复的文件
{
    ...
    snprintf( ac_fullpath_file + i_len_full , sizeof(ac_fullpath_file) - i_len_full, "/%s", ac_file );        //拼接写入的文件名
    snprintf( ac_fullpath_tmp, sizeof( ac_fullpath_tmp ), "%s/%s", p_dvr_conf->ac_path_tmp, pc_file );        //拼接暂存目录和和文件名
    DEBUG_PRINT( CM_BOLD , CB_BLACK, CF_BLUE , "****** REPAIRE FILE '%s' => '%s' ******\n", ac_fullpath_tmp , ac_fullpath_file );
...
static int ak_dvr_fix( struct dvr_conf *p_dvr_conf )
{
    ...
    for( i = 0; i < i_num_scandir; i ++ ) {
        ...
        if ( strcmp( pp_dirent_list[ i ]->d_name + strlen( pp_dirent_list[ i ]->d_name ) - strlen( FILE_EXT_AVI ) , FILE_EXT_AVI ) == 0 ) {
            DEBUG_PRINT( CM_BOLD, CB_BLACK, CF_BLUE , "****** TRY REPAIR FILE: '%s' ******\n", pp_dirent_list[ i ]->d_name );
            snprintf( ac_file_name[ MUX_NODE_RECORD ], LEN_FILE, "%s/%s", p_dvr_conf->ac_path_tmp, pp_dirent_list[ i ]->d_name );
            snprintf( ac_file_name[ MUX_NODE_AVI_IDX ], LEN_FILE, "%s%s" , ac_file_name[ MUX_NODE_RECORD ], ac_mux_ext[ MUX_NODE_AVI_IDX ] );
            if ( ( ( pFILE_mux[ MUX_NODE_RECORD ] = fopen( ac_file_name[ MUX_NODE_RECORD ], "rb+" ) ) != NULL ) &&
                 ( ( pFILE_mux[ MUX_NODE_AVI_IDX ] = fopen( ac_file_name[ MUX_NODE_AVI_IDX ], "rb" ) ) != NULL ) &&
                 ( ak_mux_fix_file( pFILE_mux[ MUX_NODE_RECORD ], pFILE_mux[ MUX_NODE_AVI_IDX ], NULL, NULL, MUX_TYPE_AVI, &mux_fileinfo_param ) == AK_SUCCESS ) &&
                 ( mux_fileinfo_param.total_ms > 0 ) ) {
                save_fix_file( p_dvr_conf,  pp_dirent_list[ i ]->d_name, mux_fileinfo_param.total_ms, MUX_TYPE_AVI );
            }
        }
        else if ( strcmp( pp_dirent_list[ i ]->d_name + strlen( pp_dirent_list[ i ]->d_name ) - strlen( FILE_EXT_MP4 ) , FILE_EXT_MP4 ) == 0 ) {
            DEBUG_PRINT( CM_BOLD, CB_BLACK, CF_BLUE , "****** TRY REPAIR FILE: '%s' ******\n", pp_dirent_list[ i ]->d_name );
            snprintf( ac_file_name[ MUX_NODE_RECORD ], LEN_FILE, "%s/%s", p_dvr_conf->ac_path_tmp, pp_dirent_list[ i ]->d_name );
            snprintf( ac_file_name[ MUX_NODE_MP4_MOOV ], LEN_FILE, "%s%s" , ac_file_name[ MUX_NODE_RECORD ], ac_mux_ext[ MUX_NODE_MP4_MOOV ] );
            snprintf( ac_file_name[ MUX_NODE_MP4_STBL ], LEN_FILE, "%s%s" , ac_file_name[ MUX_NODE_RECORD ], ac_mux_ext[ MUX_NODE_MP4_STBL ] );
            if ( ( ( pFILE_mux[ MUX_NODE_RECORD ] = fopen(  ac_file_name[ MUX_NODE_RECORD ], "rb+") ) != NULL ) &&
                 ...
                 ( ak_mux_fix_file( pFILE_mux[ MUX_NODE_RECORD ], NULL, pFILE_mux[ MUX_NODE_MP4_MOOV ], pFILE_mux[ MUX_NODE_MP4_STBL ], MUX_TYPE_MP4, &mux_fileinfo_param ) == AK_SUCCESS ) &&
                 ( mux_fileinfo_param.total_ms > 0 ) ) {
        ...
        for( m = 0; m < MUX_NODE_NUM ; m ++ ) {
            if ( pFILE_mux[ m ] != NULL ) {
                ...
                DEBUG_PRINT( CM_BOLD , CB_BLACK, CF_BLUE , "****** REMOVE FILE: %s ******\n", ac_file_name[ m ] );
            }
        }
    }
    return AK_SUCCESS;
...
static int dvr_init_ai( struct dvr_conf *p_dvr_conf )
{
    ...
    ak_print_normal( MODULE_ID_APP,
                     "sample_rate= %d sample_bits=%d channel_num= %d i_ai_frame_len= %d i_ai_frame_interval= %d\n" ,

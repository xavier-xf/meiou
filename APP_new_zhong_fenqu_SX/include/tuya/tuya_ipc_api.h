...
/**
 * \fn OPERATE_RET tuya_ipc_door_bell_press_cancel
 * \brief send a doorbell pressing cancel message to tuya cloud and APP(only for ac doorbell)
 * \return OPERATE_RET
 */
...
/**
 * \fn OPERATE_RET tuya_ipc_direct_scan_qrcode_start
 * \brief start direct scan qrcode.make sure internet is work
 * \return SUCCESS -- OPERATE_RET , FAIL -- COMM ERROR
 */
OPERATE_RET tuya_ipc_direct_scan_qrcode_start();

/**
 * \fn OPERATE_RET tuya_ipc_direct_scan_qrcode_stop
 * \brief stop direct scan qrcode.
 * \return SUCCESS -- OPERATE_RET , FAIL -- COMM ERROR
 */
OPERATE_RET tuya_ipc_direct_scan_qrcode_stop();
/**
...
OPERATE_RET tuya_ipc_thing_config_start(IN CHAR_T *ssid, CHAR_T *passwd, IN BOOL_T update_token, IN UINT_T timeout);
...
/**
 * \fn OPERATE_RET tuya_ipc_register_extern_gethostbyname
 * \brief register external dns resolve function to replace system fault gethostbyname
 * \param[in] domain: url to resolve ip
 * \param[out] ip: ip result, use 4-byte format
 * \return VOID
 */
typedef INT_T (*FUNC_EX_GETHOSTBYNAME)(IN CONST CHAR_T *domain,OUT UINT_T *ip);
VOID tuya_ipc_register_extern_gethostbyname(FUNC_EX_GETHOSTBYNAME func_gethostbyname);

/**
 * \fn OPERATE_RET tuya_ipc_register_wifi_lock_channel_cb
 * \brief register wifi lock channel callback, triggered when wifi channel is locked or unlocked
 * \param[in] lock_status: TRUE when wifi channel is locked, FALSE when wifi channel is unlocked
 * \param[in] lock_channel: locked wifi channel
 * \return SUCCESS -- OPERATE_RET
 */
typedef VOID (*FUNC_WIFI_LOCK_CHANNEL_CB)(IN CONST BOOL_T lock_status,IN CONST INT_T lock_chan);
OPERATE_RET tuya_ipc_register_wifi_lock_channel_cb(FUNC_WIFI_LOCK_CHANNEL_CB func_wifi_lock_ch_cb);
/*
  *\fn OPERATE_RET tuya_ipc_wifi_switch_signal_channel_time_set
 * \brief custom wifi signal channel scan time.use it before  tuya_ipc_start_sdk
 * \param[in] timeMs:millisecond.min:100,max:2000
 * \return SUCCESS -- OPERATE_RET , FAIL -- OPRT_NOT_SUPPORTED
 */
OPERATE_RET tuya_ipc_wifi_switch_signal_channel_time_set(IN CONST UINT_T timeMs);
/**
 * \fn OPERATE_RET tuya_ipc_upgrade_progress_status
 * \brief send a upgrade status to app
 * \param[in] devId: NULL/gw  NOT NULL/sub_dev
 * \param[in] tp: tp give by tuya
 * \param[in] status: TI_UPGRD_STAT_S
 * \return SUCCESS -- OPERATE_RET , FAIL -- COMM ERROR
 */
OPERATE_RET tuya_ipc_upgrade_progress_status(IN CONST CHAR_T * devId,IN CONST BYTE_T  tp, IN BYTE_T status);
 ...
 * \return TUYA_IPC_QRCODE_CONN_STATUS
 */
OPERATE_RET tuya_ipc_network_detection_create(TUAY_IPC_NETWORK_DETECTION_START callback);

OPERATE_RET http_gw_reset(IN CONST int stat);

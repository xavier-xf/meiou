...
lv_obj_t *lv_line_create(lv_obj_t *par, const lv_obj_t *copy)
{
	...
	if (ancestor_signal == NULL)
		ancestor_signal = lv_obj_get_signal_cb(line);
	...
	if (copy == NULL)
	{
		lv_obj_set_size(line, LV_DPI, LV_DPI); /*Auto size is enables, but set default size until no points are added*/
...
static lv_res_t lv_line_signal(lv_obj_t *line, lv_signal_t sign, void *param)
{
	...
	if (sign == LV_SIGNAL_REFR_EXT_DRAW_PAD)
	{
		...
		lv_style_int_t line_width = lv_obj_get_style_line_width(line, LV_LINE_PART_MAIN);
		if (line->ext_draw_pad < line_width)
			line->ext_draw_pad = line_width;
	}

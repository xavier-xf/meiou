...
lv_obj_t *lv_btnmatrix_create(lv_obj_t *par, const lv_obj_t *copy)
{
	...
	if (ancestor_signal == NULL)
		ancestor_signal = lv_obj_get_signal_cb(btnm);
	...
	if (ancestor_design_f == NULL)
		ancestor_design_f = lv_obj_get_design_cb(btnm);
...
void lv_btnmatrix_set_map(lv_obj_t *btnm, const char *map[])
{
	...
	while (1)
	{
		...
		if (btn_cnt != 0)
		{
			...
			line_offset_x = ext->lines_offset_x[line_cnt++];

			lv_coord_t all_unit_w = max_w - ((unit_cnt - 1) * inner) - line_offset_x * 2;
			...
			for (i = 0; i < btn_cnt; i++)
			{
				...
				if (base_dir == LV_BIDI_DIR_RTL)
				{
					...
					act_x = lv_obj_get_width(btnm) - right - act_x - act_unit_w - 1 + line_offset_x * 2;
				}
				else
				{
					act_x = (unit_act_cnt * all_unit_w) / unit_cnt + unit_act_cnt * inner + left + line_offset_x * 2;
				}
		...
		if (strlen(map_p_tmp[btn_cnt]) == 0)
			break;			     /*Break on end of map*/
...
void lv_btnmatrix_set_lines_offset_map(lv_obj_t *btnm, const lv_btnmatrix_ctrl_t offset_map[])
{
	LV_ASSERT_OBJ(btnm, LV_OBJX_NAME);

	lv_btnmatrix_ext_t *ext = lv_obj_get_ext_attr(btnm);
	_lv_memcpy(ext->lines_offset_x, offset_map, sizeof(lv_btnmatrix_ctrl_t) * ext->btn_cnt);
}
/**
...
void lv_btnmatrix_set_focused_btn(lv_obj_t *btnm, uint16_t id)
{
	...
	if (id >= ext->btn_cnt && id != LV_BTNMATRIX_BTN_NONE)
		return;

	if (ext->btn_id_act == LV_BTNMATRIX_BTN_NONE)
		ext->btn_id_act = id;

	if (id == ext->btn_id_focused)
		return;
...
void lv_btnmatrix_set_align(lv_obj_t *btnm, lv_label_align_t align, lv_coord_t x_ofs, lv_coord_t y_ofs)
{
	...
	lv_btnmatrix_ext_t *ext = lv_obj_get_ext_attr(btnm);
	if ((ext->align == align) && (ext->label_x_ofs == x_ofs) && (ext->label_y_ofs == y_ofs))
		return;

	ext->align = align;
	ext->label_x_ofs = x_ofs;
	ext->label_y_ofs = y_ofs;
	lv_obj_invalidate(btnm);
}
...
static lv_design_res_t lv_btnmatrix_design(lv_obj_t *btnm, const lv_area_t *clip_area, lv_design_mode_t mode)
{
	...
	else if (mode == LV_DESIGN_DRAW_MAIN)
	{
		...
		if (align == LV_LABEL_ALIGN_CENTER)
			txt_flag |= LV_TXT_FLAG_CENTER;
		if (align == LV_LABEL_ALIGN_RIGHT)
			txt_flag |= LV_TXT_FLAG_RIGHT;
		...
		for (btn_i = 0; btn_i < ext->btn_cnt; btn_i++, txt_i++)
		{
			...
			/*Skip hidden buttons*/
			if (button_is_hidden(ext->ctrl_bits[btn_i]))
				continue;
			...
			{
				area_tmp.x1 += btn_w - txt_size.x;
				area_tmp.x2 = area_tmp.x1 + txt_size.x;
			}

			area_tmp.y1 += (btn_h - txt_size.y) / 2 + ext->label_y_ofs;

			area_tmp.x2 += ext->label_x_ofs;

			area_tmp.y2 = area_tmp.y1 + txt_size.y + ext->label_y_ofs;
...
static lv_res_t lv_btnmatrix_signal(lv_obj_t *btnm, lv_signal_t sign, void *param)
{
	/* if((sign == LV_SIGNAL_DRAG_BEGIN)||(sign == LV_SIGNAL_DRAG_THROW_BEGIN)||(sign == LV_SIGNAL_DRAG_END))
	{
	    printf("sign:%d \n",sign);
	} */
	lv_res_t res;
	if (sign == LV_SIGNAL_GET_STYLE)
	{
		...
		if (info->result != NULL)
			...
			return ancestor_signal(btnm, sign, param);
	}
	...
	if (sign == LV_SIGNAL_CLEANUP)
	{
		...
		lv_mem_free(ext->lines_offset_x);
#ifdef LV_USE_PATTERN
	...
	else if (sign == LV_SIGNAL_CONTROL)
	{
		...
		if (c == LV_KEY_RIGHT)
		{
			if (ext->btn_id_focused == LV_BTNMATRIX_BTN_NONE)
				...
				ext->btn_id_focused++;
			if (ext->btn_id_focused >= ext->btn_cnt - 1)
				ext->btn_id_focused = ext->btn_cnt - 1;
			ext->btn_id_act = ext->btn_id_focused;
			lv_obj_invalidate(btnm);
		}
		...
		else if (c == LV_KEY_DOWN)
		{
			...
			else
			{
				...
				if (area_below < ext->btn_cnt)
					ext->btn_id_focused = area_below;
			}
			ext->btn_id_act = ext->btn_id_focused;
			lv_obj_invalidate(btnm);
		}
...
static void allocate_btn_areas_and_controls(const lv_obj_t *btnm, const char **map)
{
	...
	ext->lines_offset_x = lv_mem_alloc(sizeof(lv_btnmatrix_ctrl_t) * btn_cnt);
	LV_ASSERT_MEM(ext->lines_offset_x);
	...
	if (ext->button_areas == NULL || ext->ctrl_bits == NULL || ext->pattern_p == NULL)
		btn_cnt = 0;

	_lv_memset_00(ext->ctrl_bits, sizeof(lv_btnmatrix_ctrl_t) * btn_cnt);

	_lv_memset_00(ext->lines_offset_x, sizeof(lv_btnmatrix_ctrl_t) * btn_cnt);
...
static uint16_t get_button_from_point(lv_obj_t *btnm, lv_point_t *p)
{
	...
	for (i = 0; i < ext->btn_cnt; i++)
	{
		...
		if (btn_area.x1 <= pleft)
			btn_area.x1 += btnm_cords.x1 - LV_MATH_MIN(pleft, BTN_EXTRA_CLICK_AREA_MAX);
		else
			btn_area.x1 += btnm_cords.x1 - pinner;

		if (btn_area.y1 <= ptop)
			btn_area.y1 += btnm_cords.y1 - LV_MATH_MIN(ptop, BTN_EXTRA_CLICK_AREA_MAX);
		else
			btn_area.y1 += btnm_cords.y1 - pinner;

		if (btn_area.x2 >= w - pright - 2)
			btn_area.x2 += btnm_cords.x1 + LV_MATH_MIN(pright,
								   BTN_EXTRA_CLICK_AREA_MAX); /*-2 for rounding error*/
		else
			btn_area.x2 += btnm_cords.x1 + pinner;

		if (btn_area.y2 >= h - pbottom - 2)
			btn_area.y2 += btnm_cords.y1 + LV_MATH_MIN(pbottom,
								   BTN_EXTRA_CLICK_AREA_MAX); /*-2 for rounding error*/
		else
			btn_area.y2 += btnm_cords.y1 + pinner;
...
static void make_one_button_toggled(lv_obj_t *btnm, uint16_t btn_idx)
{
	...
	if (was_toggled)
		lv_btnmatrix_set_btn_ctrl(btnm, btn_idx, LV_BTNMATRIX_CTRL_CHECK_STATE);
}

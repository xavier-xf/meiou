...
lv_obj_t *lv_page_create(lv_obj_t *par, const lv_obj_t *copy)
{
	...
	if (ancestor_signal == NULL)
		ancestor_signal = lv_obj_get_signal_cb(page);
	if (ancestor_design == NULL)
		ancestor_design = lv_obj_get_design_cb(page);
...
void lv_page_set_scrollbar_mode(lv_obj_t *page, lv_scrollbar_mode_t sb_mode)
{
	...
	else
	{
		if (ext->scrlbar.mode & LV_SCROLLBAR_MODE_HIDE)
			sb_mode |= LV_SCROLLBAR_MODE_HIDE;
...
void lv_page_set_scroll_propagation(lv_obj_t *page, bool en)
{
	...
	lv_page_ext_t *ext = lv_obj_get_ext_attr(page);
	if (en)
		lv_obj_set_drag_dir(ext->scrl, LV_DRAG_DIR_ONE);
	else
		lv_obj_set_drag_dir(ext->scrl, LV_DRAG_DIR_BOTH);
...
bool lv_page_on_edge(lv_obj_t *page, lv_page_edge_t edge)
{
	...
	if ((edge & LV_PAGE_EDGE_TOP) && scrl_coords.y1 == page_coords.y1 + top)
		return true;
	if ((edge & LV_PAGE_EDGE_BOTTOM) && scrl_coords.y2 == page_coords.y2 - bottom)
		return true;
	if ((edge & LV_PAGE_EDGE_LEFT) && scrl_coords.x1 == page_coords.x1 + left)
		return true;
	if ((edge & LV_PAGE_EDGE_RIGHT) && scrl_coords.x2 == page_coords.x2 - right)
		return true;
...
static lv_res_t lv_page_signal(lv_obj_t *page, lv_signal_t sign, void *param)
{
	...
	if (sign == LV_SIGNAL_GET_STYLE)
	{
		...
		if (info->result != NULL)
			...
			return ancestor_signal(page, sign, param);
	}
	else if (sign == LV_SIGNAL_GET_STATE_DSC)
	{
		lv_get_state_info_t *info = param;
		if (info->part == LV_PAGE_PART_SCROLLABLE)
			info->result = lv_obj_get_state(lv_page_get_scrollable(page),
							LV_CONT_PART_MAIN);
		else
			info->result = lv_obj_get_state(page, info->part);
		return LV_RES_OK;
	}
			...
			if (lv_obj_is_protected(child, LV_PROTECT_PARENT) == false)
			{
				    ...
				    base_dir != LV_BIDI_DIR_RTL)
				{
					...
					tmp->coords.x2 += scrl_left;
					 ...
					 (fit_right == LV_FIT_TIGHT || fit_right == LV_FIT_MAX) && base_dir == LV_BIDI_DIR_RTL)
				{
...
static lv_res_t lv_page_scrollable_signal(lv_obj_t *scrl, lv_signal_t sign, void *param)
{
	...
	if (res != LV_RES_OK)
		return res;
	if (sign == LV_SIGNAL_GET_TYPE)
		return lv_obj_handle_get_type_signal(param, "");
...
static void scrlbar_refresh(lv_obj_t *page)
{
	...
	if (ext->scrlbar.mode == LV_SCROLLBAR_MODE_OFF)
		return;
	...
	if (scrl_w <= obj_w - bg_left - bg_right)
	{
		...
		if (ext->scrlbar.mode == LV_SCROLLBAR_MODE_AUTO ||
		    ext->scrlbar.mode == LV_SCROLLBAR_MODE_DRAG)
			ext->scrlbar.hor_draw = 0;
	}
	...
	else
	{
		...
		if (size_tmp < LV_PAGE_SB_MIN_SIZE)
		...
		else if (ext->scrlbar.mode == LV_SCROLLBAR_MODE_DRAG)
		{
			...
			while (indev)
			{
				if (indev->driver.type == LV_INDEV_TYPE_POINTER && (indev->proc.types.pointer.drag_dir & LV_DRAG_DIR_HOR))
				{
					...
					while (drag_obj && drag_obj->drag_parent)
	...
	{
		size_tmp = (obj_h * (obj_h - (2 * sb_ver_pad))) / (scrl_h + bg_top + bg_bottom);
		if (size_tmp < LV_PAGE_SB_MIN_SIZE)
			size_tmp = LV_PAGE_SB_MIN_SIZE;
...
static void refr_ext_draw_pad(lv_obj_t *page)
{
	...
	if (page->ext_draw_pad < (-sb_right))
		page->ext_draw_pad = -sb_right;
	if (page->ext_draw_pad < (-sb_bottom))
		page->ext_draw_pad = -sb_bottom;
}
